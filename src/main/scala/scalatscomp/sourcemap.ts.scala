...
object SourceMap {
  trait SourceMapWriter {
    def initialize(filePath: String,
                   ...
                   isBundledEmit: Boolean): Unit
    def reset(): Unit
    def setSourceFile(sourceFile: SourceFile): Unit
    def emitPos(pos: Int): Unit
    def emitNodeWithSourceMap(emitContext: EmitContext,
                              node: Node,
                              emitCallback: ((EmitContext,
                                              Node) => Unit)): Unit
    def emitTokenWithSourceMap(node: Node,
                               token: SyntaxKind,
                               tokenStartPos: Int,
                               emitCallback: ((SyntaxKind, Int) => Int)): Int
    def getText(): String
    def getSourceMappingURL(): String
    def getSourceMapData(): SourceMapData
  }
  val defaultLastEncodedSourceMapSpan: SourceMapSpan = Map(
    "emittedLine" -> 1,
    "emittedColumn" -> 1,
    "sourceLine" -> 1,
    "sourceColumn" -> 1,
    "sourceIndex" -> 0)
  def createSourceMapWriter(host: EmitHost,
                            writer: EmitTextWriter): SourceMapWriter = {
    ...
    def initialize(filePath: String,
      ...
      (sourceMapData = Map(
        ...
        "jsSourceMappingURL" -> (if ((!compilerOptions.inlineSourceMap))
                                   getBaseFileName(
        ...
        "sourceMapMappings" -> "",
        "sourceMapSourcesContent" -> (if (compilerOptions.inlineSources)
                                        Array()
                                      else undefined),
        "sourceMapDecodedMappings" -> Array()))
      (sourceMapData.sourceMapSourceRoot =
        ts.normalizeSlashes(sourceMapData.sourceMapSourceRoot))
      if ((sourceMapData.sourceMapSourceRoot.length && (sourceMapData.sourceMapSourceRoot
            .charCodeAt((sourceMapData.sourceMapSourceRoot.length - 1)) !== CharacterCodes.slash))) {
      ...
      if (compilerOptions.mapRoot) {
        ...
        if ((!isBundledEmit)) {
          ...
          (sourceMapDir = getDirectoryPath(
            getSourceFilePathInNewDir(sourceFiles(0), host, sourceMapDir)))

        }
        if (((!isRootedDiskPath(sourceMapDir)) && (!isUrl(sourceMapDir)))) {
          (sourceMapDir =
            combinePaths(host.getCommonSourceDirectory(), sourceMapDir))
          (sourceMapData.jsSourceMappingURL = getRelativePathToDirectoryOrUrl(
            getDirectoryPath(normalizePath(filePath)),
            combinePaths(sourceMapDir, sourceMapData.jsSourceMappingURL),
            host.getCurrentDirectory(),
            host.getCanonicalFileName,
            true))

        } else {
          (sourceMapData.jsSourceMappingURL =
            combinePaths(sourceMapDir, sourceMapData.jsSourceMappingURL))
    ...
    def encodeLastRecordedSourceMapSpan() = {
      ...
      (sourceMapData.sourceMapMappings += base64VLQFormatEncode(
        (lastRecordedSourceMapSpan.emittedColumn - prevEncodedEmittedColumn)))
      (sourceMapData.sourceMapMappings += base64VLQFormatEncode(
        (lastRecordedSourceMapSpan.sourceIndex - lastEncodedSourceMapSpan.sourceIndex)))
      (sourceMapData.sourceMapMappings += base64VLQFormatEncode(
        (lastRecordedSourceMapSpan.sourceLine - lastEncodedSourceMapSpan.sourceLine)))
      (sourceMapData.sourceMapMappings += base64VLQFormatEncode(
        (lastRecordedSourceMapSpan.sourceColumn - lastEncodedSourceMapSpan.sourceColumn)))
      if ((lastRecordedSourceMapSpan.nameIndex >= 0)) {
        Debug.assert(
          false,
          "We do not support name index right now, Make sure to update updateLastEncodedAndRecordedSpans when we start using this")
        (sourceMapData.sourceMapMappings += base64VLQFormatEncode(
          (lastRecordedSourceMapSpan.nameIndex - lastEncodedNameIndex)))
    ...
    def emitTokenWithSourceMap(node: Node,
                               token: SyntaxKind,
                               tokenPos: Int,
                               emitCallback: ((SyntaxKind, Int) => Int)) = {
      if (disabled) {
      ...
      val emitFlags = (emitNode && emitNode.flags)
      val range = ((emitNode && emitNode.tokenSourceMapRanges) && emitNode
          .tokenSourceMapRanges(token))
      (tokenPos =
        skipTrivia(currentSourceText, (if (range) range.pos else tokenPos)))
    ...
    def setSourceFile(sourceFile: SourceFile) = {
      ...
      val sourcesDirectoryPath =
        (if (compilerOptions.sourceRoot) host.getCommonSourceDirectory()
         else sourceMapDir)
      val source = getRelativePathToDirectoryOrUrl(
    ...
    def getSourceMappingURL() = {
      ...
      if (compilerOptions.inlineSourceMap) {
        ...
        return (sourceMapData.jsSourceMappingURL =
          s"""data:application/json;base64,${base64SourceMapText}""")
