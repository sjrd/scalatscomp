...
object Checker {
  ...
  def createTypeChecker(host: TypeCheckerHost,
                        produceDiagnostics: Boolean): TypeChecker = {
    ...
    val allowSyntheticDefaultImports =
      (if ((typeof(compilerOptions.allowSyntheticDefaultImports) !== "undefined"))
         compilerOptions.allowSyntheticDefaultImports
       else (modulekind === ModuleKind.System))
    val strictNullChecks = compilerOptions.strictNullChecks
    val emitResolver = createResolver()
    val undefinedSymbol =
      createSymbol((SymbolFlags.Property | SymbolFlags.Transient), "undefined")
    (undefinedSymbol.declarations = Array())
    val argumentsSymbol =
      createSymbol((SymbolFlags.Property | SymbolFlags.Transient), "arguments")
    val checker: TypeChecker = Map(
      ...
      "getIdentifierCount" -> (() =>
                                 sum(
                                   ...
                                   "identifierCount")),
      "getSymbolCount" -> (() =>
                             (sum(host.getSourceFiles(), "symbolCount") + symbolCount)),
    ...
    val unknownSymbol =
      createSymbol((SymbolFlags.Property | SymbolFlags.Transient), "unknown")
    ...
    val undefinedWideningType =
      (if (strictNullChecks) undefinedType
       else
         createIntrinsicType(
           (TypeFlags.Undefined | TypeFlags.ContainsWideningType),
           "undefined"))
    val nullType = createIntrinsicType(TypeFlags.Null, "null")
    val nullWideningType =
      (if (strictNullChecks) nullType
       else
         createIntrinsicType(
           (TypeFlags.Null | TypeFlags.ContainsWideningType),
           "null"))
    ...
    val emptyObjectType = createAnonymousType(
      undefined,
      emptySymbols,
      emptyArray,
      emptyArray,
      undefined,
      undefined)
    val emptyGenericType = createAnonymousType(
      undefined,
      emptySymbols,
      emptyArray,
      emptyArray,
      undefined,
      undefined).asInstanceOf[ObjectType].asInstanceOf[GenericType]
    (emptyGenericType.instantiations = createMap[TypeReference]())
    val anyFunctionType = createAnonymousType(
      undefined,
      emptySymbols,
      emptyArray,
      emptyArray,
    ...
    val noConstraintType = createAnonymousType(
      undefined,
      emptySymbols,
      emptyArray,
      emptyArray,
      undefined,
      undefined)
    val anySignature = createSignature(
      undefined,
      undefined,
      undefined,
      emptyArray,
      ...
      false,
      false)
    val unknownSignature = createSignature(
      undefined,
      undefined,
      undefined,
      emptyArray,
      ...
      false,
      false)
    val resolvingSignature = createSignature(
      undefined,
      undefined,
      undefined,
      emptyArray,
      ...
      false,
      false)
    val silentNeverSignature = createSignature(
      undefined,
      undefined,
      undefined,
      emptyArray,
    ...
    val typeofTypesByName = createMap[Type](
      Map(
        "string" -> stringType,
        "number" -> numberType,
        "boolean" -> booleanType,
    ...
    def error(location: Node,
              message: DiagnosticMessage,
              arg0: (String | Int),
              arg1: (String | Int),
              arg2: (String | Int)): Unit = {
      val diagnostic =
        (if (location)
           createDiagnosticForNode(location, message, arg0, arg1, arg2)
         else createCompilerDiagnostic(message, arg0, arg1, arg2))
    ...
    def cloneSymbol(symbol: Symbol): Symbol = {
      val result =
        createSymbol((symbol.flags | SymbolFlags.Merged), symbol.name)
    ...
    def mergeSymbol(target: Symbol, source: Symbol) = {
      if ((!((target.flags & getExcludedSymbolFlags(source.flags))))) {
        ...
        forEach(source.declarations, (node => {
                                        error(
                                          (if (node.name) node.name else node),
                                          message,
    ...
    def mergeModuleAugmentation(moduleName: LiteralExpression): Unit = {
      val moduleAugmentation =
        moduleName.parent.asInstanceOf[ModuleDeclaration]
      ...
      if (isGlobalScopeAugmentation(moduleAugmentation)) {
        ...
        val moduleNotFoundError =
          (if ((!isInAmbientContext(moduleName.parent.parent)))
        ...
        if ((mainModule.flags & SymbolFlags.Namespace)) {
          (mainModule =
            (if ((mainModule.flags & SymbolFlags.Merged)) mainModule
             else cloneSymbol(mainModule)))
    ...
    def addToSymbolTable(target: SymbolTable,
                         source: SymbolTable,
                         message: DiagnosticMessage) = {
      (source).keys.foreach { fresh2 =>
        val id = zeroOfMyType = fresh2 {
          if (target(id)) {
            forEach(
              target(id).declarations,
              addDeclarationDiagnostic(id, message))
    ...
    def getObjectFlags(`type`: Type): ObjectFlags = {
      return (if ((`type`.flags & TypeFlags.Object))(`type`
    ...
    def isGlobalSourceFile(node: Node) = {
      return ((node.kind === SyntaxKind.SourceFile) && (!isExternalOrCommonJsModule(
        node.asInstanceOf[SourceFile])))

    }
    def getSymbol(symbols: SymbolTable,
                  name: String,
                  meaning: SymbolFlags): Symbol = {
      if (meaning) {
        val symbol = symbols(name)
        if (symbol) {
          Debug.assert(
            (((symbol.flags & SymbolFlags.Instantiated)) === 0),
    ...
    def isBlockScopedNameDeclaredBeforeUse(declaration: Declaration,
                                           usage: Node): Boolean = {
      ...
      if ((declarationFile !== useFile)) {
        ...
        return (indexOf(sourceFiles, declarationFile) <= indexOf(
          sourceFiles,
          useFile))

      }
      if ((declaration.pos <= usage.pos)) {
        return ((declaration.kind !== SyntaxKind.VariableDeclaration) || (!isImmediatelyUsedInInitializerOfBlockScopedVariable(
          declaration.asInstanceOf[VariableDeclaration],
          usage)))

      }
      val container = getEnclosingBlockScopeContainer(declaration)
      return isUsedInFunctionOrNonStaticProperty(usage, container)
      def isImmediatelyUsedInInitializerOfBlockScopedVariable(
          declaration: VariableDeclaration,
          usage: Node): Boolean = {
        val container = getEnclosingBlockScopeContainer(declaration)
        declaration.parent.parent.kind match {
          case SyntaxKind.VariableStatement | SyntaxKind.ForStatement |
          ...
          case SyntaxKind.ForInStatement | SyntaxKind.ForOfStatement =>
            if (isSameScopeDescendentOf(
                  ...
                  (declaration.parent.parent
                    .asInstanceOf[(ForInStatement | ForOfStatement)])
                    .expression,
                  container)) {
              return true
      ...
      def isUsedInFunctionOrNonStaticProperty(usage: Node,
                                              container: Node): Boolean = {
        var current = usage
        while (current) {
          {
            ...
            val initializerOfNonStaticProperty = (((current.parent && (current.parent.kind === SyntaxKind.PropertyDeclaration)) && (((getModifierFlags(
                current.parent) & ModifierFlags.Static)) === 0)) && ((current.parent
    ...
    def resolveName(location: (Node | undefined),
                    name: String,
                    meaning: SymbolFlags,
        ...
        while (location) {
          {
            if ((location.locals && (!isGlobalSourceFile(location)))) {
              if ((result = getSymbol(location.locals, name, meaning))) {
                var useResult = true
                if (((isFunctionLike(location) && lastLocation) && (lastLocation !== (location
                      .asInstanceOf[FunctionLikeDeclaration])
                      .body))) {
                  if ((((meaning & result.flags) & SymbolFlags.Type) && (lastLocation.kind !== SyntaxKind.JSDocComment))) {
                    (useResult =
                      (if ((result.flags & SymbolFlags.TypeParameter))
                         (((lastLocation === (location
                           .asInstanceOf[FunctionLikeDeclaration])
                           .`type`) || (lastLocation.kind === SyntaxKind.Parameter)) || (lastLocation.kind === SyntaxKind.TypeParameter))
                       else false))

                  }
                  if (((meaning & SymbolFlags.Value) && (result.flags & SymbolFlags.FunctionScopedVariable))) {
                    (useResult = ((lastLocation.kind === SyntaxKind.Parameter) || (((lastLocation === (location
                        .asInstanceOf[FunctionLikeDeclaration])
                        .`type`) && (result.valueDeclaration.kind === SyntaxKind.Parameter)))))
            ...
            location.kind match {
              case SyntaxKind.SourceFile =>
                if ((!isExternalOrCommonJsModule(
                      location.asInstanceOf[SourceFile])))
                  break()
                (isInExternalModule = true)
              case SyntaxKind.ModuleDeclaration =>
                val moduleExports = getSymbolOfNode(location).exports
                if (((location.kind === SyntaxKind.SourceFile) || isAmbientModule(
                      location))) {
                  ...
                  if (((moduleExports(name) && (moduleExports(name).flags === SymbolFlags.Alias)) && getDeclarationOfKind(
                        moduleExports(name),
              ...
              case SyntaxKind.PropertyDeclaration |
                ...
                if ((isClassLike(location.parent) && (!((getModifierFlags(
                      location) & ModifierFlags.Static))))) {
                  val ctor = findConstructorDeclaration(
                    location.parent.asInstanceOf[ClassLikeDeclaration])
                  if ((ctor && ctor.locals)) {
                    if (getSymbol(
                          ctor.locals,
                          name,
              ...
              case SyntaxKind.ClassDeclaration | SyntaxKind.ClassExpression |
                ...
                if ((result = getSymbol(
                      getSymbolOfNode(location).members,
                      name,
              ...
              case SyntaxKind.MethodDeclaration | SyntaxKind.MethodSignature |
                  SyntaxKind.Constructor | SyntaxKind.GetAccessor |
                  SyntaxKind.SetAccessor | SyntaxKind.FunctionDeclaration |
              ...
              case SyntaxKind.FunctionExpression =>
                ...
                if ((meaning & SymbolFlags.Function)) {
                  val functionName =
                    (location.asInstanceOf[FunctionExpression]).name
      ...
      if ((!result)) {
        if (nameNotFoundMessage) {
          if (((!errorLocation) || (((!checkAndReportErrorForMissingPrefix(
                errorLocation,
                name,
                nameArg)) && (!checkAndReportErrorForExtendingInterface(
                errorLocation))) && (!checkAndReportErrorForUsingTypeAsValue(
                errorLocation,
                name,
                meaning))))) {
            error(
              ...
              (if ((typeof(nameArg) === "string")) nameArg
               else declarationNameToString(nameArg)))
      ...
      if (nameNotFoundMessage) {
        if (propertyWithInvalidInitializer) {
          val propertyName = (propertyWithInvalidInitializer
            .asInstanceOf[PropertyDeclaration])
            .name
          error(
            errorLocation,
        ...
        if ((meaning & SymbolFlags.BlockScopedVariable)) {
          val exportOrLocalSymbol = getExportSymbolOfValueSymbolIfExported(
            result)
    ...
    def checkAndReportErrorForMissingPrefix(
        errorLocation: Node,
        name: String,
        nameArg: (String | Identifier)): Boolean = {
      if (((((errorLocation.kind === SyntaxKind.Identifier) && (isTypeReferenceIdentifier(
            errorLocation.asInstanceOf[Identifier]))) || isInTypeQuery(
            errorLocation)))) {
        return false
      ...
      while (location) {
        {
          if (isClassLike(location.parent)) {
            ...
            if (((location === container) && (!((getModifierFlags(location) & ModifierFlags.Static))))) {
              val instanceType = (getDeclaredTypeOfSymbol(classSymbol)
                .asInstanceOf[InterfaceType])
                .thisType
    ...
    def checkAndReportErrorForExtendingInterface(
        errorLocation: Node): Boolean = {
      val expression = getEntityNameForExtendingInterface(errorLocation)
      val isError = (!(!((expression && resolveEntityName(
        expression,
    ...
    def getEntityNameForExtendingInterface(
        node: Node): (EntityNameExpression | undefined) = {
      node.kind match {
        ...
        case SyntaxKind.ExpressionWithTypeArguments =>
          Debug.assert(
            isEntityNameExpression(
              (node.asInstanceOf[ExpressionWithTypeArguments]).expression))
          return (node
            .asInstanceOf[ExpressionWithTypeArguments])
            .expression
            .asInstanceOf[EntityNameExpression]
        case _ =>
          return undefined
      }

    }
    def checkAndReportErrorForUsingTypeAsValue(
        errorLocation: Node,
        name: String,
        meaning: SymbolFlags): Boolean = {
    ...
    def checkResolvedBlockScopedVariable(result: Symbol,
                                         errorLocation: Node): Unit = {
      Debug.assert((((result.flags & SymbolFlags.BlockScopedVariable)) !== 0))
      val declaration = forEach(
        result.declarations,
        (d => (if (isBlockOrCatchScoped(d)) d else undefined)))
      Debug.assert(
        (declaration !== undefined),
        "Block-scoped variable declaration is undefined")
      if (((!isInAmbientContext(declaration)) && (!isBlockScopedNameDeclaredBeforeUse(
    ...
    def isSameScopeDescendentOf(initial: Node,
                                parent: Node,
                                stopAt: Node): Boolean = {
      if ((!parent)) {
        return false
      ...
      {
        var current = initial
        while (((current && (current !== stopAt)) && (!isFunctionLike(
                 current)))) {
          {
            if ((current === parent)) {
              return true
    ...
    def getDeclarationOfAliasSymbol(
        symbol: Symbol): (Declaration | undefined) = {
      return forEach(
        symbol.declarations,
        (d => (if (isAliasSymbolDeclaration(d)) d else undefined)))

    }
    def getTargetOfImportEqualsDeclaration(
        node: ImportEqualsDeclaration): Symbol = {
    ...
    def getTargetOfImportClause(node: ImportClause): Symbol = {
      val moduleSymbol = resolveExternalModuleName(
        node,
        (node.parent.asInstanceOf[ImportDeclaration]).moduleSpecifier)
      if (moduleSymbol) {
        val exportDefaultSymbol =
          (if (isShorthandAmbientModuleSymbol(moduleSymbol)) moduleSymbol
           else
             (if (moduleSymbol.exports("export="))
                getPropertyOfType(
                  getTypeOfSymbol(moduleSymbol.exports("export=")),
                  "default")
              else resolveSymbol(moduleSymbol.exports("default"))))
        if (((!exportDefaultSymbol) && (!allowSyntheticDefaultImports))) {
          ...
          return (resolveExternalModuleSymbol(moduleSymbol) || resolveSymbol(
            moduleSymbol))
    ...
    def getTargetOfNamespaceImport(node: NamespaceImport): Symbol = {
      val moduleSpecifier =
        (node.parent.parent.asInstanceOf[ImportDeclaration]).moduleSpecifier
      return resolveESModuleSymbol(
        resolveExternalModuleName(node, moduleSpecifier),
        moduleSpecifier)

    }
    def combineValueAndTypeSymbols(valueSymbol: Symbol,
      ...
      val result =
        createSymbol((valueSymbol.flags | typeSymbol.flags), valueSymbol.name)
      (result.declarations =
    ...
    def getPropertyOfVariable(symbol: Symbol, name: String): Symbol = {
      if ((symbol.flags & SymbolFlags.Variable)) {
        val typeAnnotation =
          (symbol.valueDeclaration.asInstanceOf[VariableDeclaration]).`type`
        if (typeAnnotation) {
    ...
    def getExternalModuleMember(node: (ImportDeclaration | ExportDeclaration),
      ...
      if (targetSymbol) {
        ...
        if (name.text) {
          ...
          if (((moduleSymbol && moduleSymbol.exports) && moduleSymbol.exports(
                "export="))) {
            (symbolFromVariable =
              getPropertyOfType(getTypeOfSymbol(targetSymbol), name.text))

          } else {
            (symbolFromVariable =
              getPropertyOfVariable(targetSymbol, name.text))
          ...
          if ((((!symbolFromModule) && allowSyntheticDefaultImports) && (name.text === "default"))) {
            (symbolFromModule = (resolveExternalModuleSymbol(moduleSymbol) || resolveSymbol(
                moduleSymbol)))
          ...
          val symbol =
            (if ((symbolFromModule && symbolFromVariable))
             ...
             else (symbolFromModule || symbolFromVariable))
          if ((!symbol)) {
            error(
              name,
    ...
    def getTargetOfImportSpecifier(node: ImportSpecifier): Symbol = {
      return getExternalModuleMember(
        node.parent.parent.parent.asInstanceOf[ImportDeclaration],
        node)

    }
    def getTargetOfNamespaceExportDeclaration(
        node: NamespaceExportDeclaration): Symbol = {
    ...
    def getTargetOfExportSpecifier(node: ExportSpecifier): Symbol = {
      return (if ((node.parent.parent
                ...
                getExternalModuleMember(
                  node.parent.parent.asInstanceOf[ExportDeclaration],
                  node)
              else
                resolveEntityName(
    ...
    def getTargetOfExportAssignment(node: ExportAssignment): Symbol = {
      return resolveEntityName(
        node.expression.asInstanceOf[EntityNameExpression],
        ((SymbolFlags.Value | SymbolFlags.Type) | SymbolFlags.Namespace))
      ...
      node.kind match {
        ...
        case SyntaxKind.ExportAssignment =>
          return getTargetOfExportAssignment(
            node.asInstanceOf[ExportAssignment])
    ...
    def resolveSymbol(symbol: Symbol): Symbol = {
      return (if (((symbol && (symbol.flags & SymbolFlags.Alias)) && (!((symbol.flags & (((SymbolFlags.Value | SymbolFlags.Type) | SymbolFlags.Namespace)))))))
                resolveAlias(symbol)
              else symbol)
    ...
    def markExportAsReferenced(
        node: (ImportEqualsDeclaration | ExportAssignment | ExportSpecifier)) = {
      val symbol = getSymbolOfNode(node)
      val target = resolveAlias(symbol)
      if (target) {
        val markAlias = ((target === unknownSymbol) || ((((target.flags & SymbolFlags.Value)) && (!isConstEnumOrConstEnumOnlyModule(
            target)))))
        if (markAlias) {
      ...
      if ((!links.referenced)) {
        ...
        if ((node.kind === SyntaxKind.ExportAssignment)) {
          checkExpressionCached(
            (node.asInstanceOf[ExportAssignment]).expression)

        } else if ((node.kind === SyntaxKind.ExportSpecifier)) {
          checkExpressionCached(
            ((node.asInstanceOf[ExportSpecifier]).propertyName || (node
    ...
    def getSymbolOfPartOfRightHandSideOfImportEquals(
        entityName: EntityName,
        dontResolveAlias: Boolean): Symbol = {
      if (((entityName.kind === SyntaxKind.Identifier) && isRightSideOfQualifiedNameOrPropertyAccess(
            entityName))) {
    ...
    def getFullyQualifiedName(symbol: Symbol): String = {
      return (if (symbol.parent)
                ((getFullyQualifiedName(symbol.parent) + ".") + symbolToString(
                  symbol))
              else symbolToString(symbol))

    }
    def resolveEntityName(name: EntityNameOrEntityNameExpression,
                          meaning: SymbolFlags,
                          ignoreErrors: Boolean,
                          dontResolveAlias: Boolean,
                          location: Node): (Symbol | undefined) = {
      ...
      if ((name.kind === SyntaxKind.Identifier)) {
        val message =
          (if ((meaning === SymbolFlags.Namespace))
           ...
           else Diagnostics.Cannot_find_name_0)
        (symbol = resolveName(
          (location || name),
          (name.asInstanceOf[Identifier]).text,
          meaning,
          (if (ignoreErrors) undefined else message),
          name.asInstanceOf[Identifier]))
        if ((!symbol)) {
          return undefined
        ...
        val left =
          (if ((name.kind === SyntaxKind.QualifiedName))(name
             .asInstanceOf[QualifiedName])
             .left
           else
             (name
               .asInstanceOf[PropertyAccessEntityNameExpression])
               .expression)
        val right =
          (if ((name.kind === SyntaxKind.QualifiedName))(name
             .asInstanceOf[QualifiedName])
             .right
           else (name.asInstanceOf[PropertyAccessExpression]).name)
        val namespace = resolveEntityName(
          left,
          SymbolFlags.Namespace,
          ignoreErrors,
          false,
      ...
      Debug.assert(
        (((symbol.flags & SymbolFlags.Instantiated)) === 0),
        "Should never get an instantiated symbol here.")
      return (if ((((symbol.flags & meaning)) || dontResolveAlias)) symbol
              else resolveAlias(symbol))

    }
    def resolveExternalModuleName(
        location: Node,
        moduleReferenceExpression: Expression): Symbol = {
    ...
    def resolveExternalModuleNameWorker(
        location: Node,
        moduleReferenceExpression: Expression,
        moduleNotFoundError: DiagnosticMessage): Symbol = {
      if ((moduleReferenceExpression.kind !== SyntaxKind.StringLiteral)) {
        return

      }
      val moduleReferenceLiteral =
        moduleReferenceExpression.asInstanceOf[LiteralExpression]
      return resolveExternalModule(
        location,
        moduleReferenceLiteral.text,
        moduleNotFoundError,
        moduleReferenceLiteral)

    }
    def resolveExternalModule(location: Node,
                              moduleReference: String,
                              moduleNotFoundError: DiagnosticMessage,
                              errorNode: Node): Symbol = {
      ...
      val resolvedModule =
        getResolvedModule(getSourceFileOfNode(location), moduleReference)
      val sourceFile = (resolvedModule && host.getSourceFile(
      ...
      if (patternAmbientModules) {
        val pattern = findBestPatternMatch(
          patternAmbientModules,
          (_underscore_ => _underscore_.pattern),
          moduleName)
        if (pattern) {
      ...
      if (moduleNotFoundError) {
        ...
        if (tsExtension) {
          val diag =
            Diagnostics.An_import_path_cannot_end_with_a_0_extension_Consider_importing_1_instead
          error(
            errorNode,
            diag,
            tsExtension,
            removeExtension(moduleName, tsExtension))
    ...
    def resolveExternalModuleSymbol(moduleSymbol: Symbol): Symbol = {
      return ((moduleSymbol && getMergedSymbol(
        resolveSymbol(moduleSymbol.exports("export=")))) || moduleSymbol)

    }
    def resolveESModuleSymbol(
        moduleSymbol: Symbol,
        moduleReferenceExpression: Expression): Symbol = {
      ...
      if ((symbol && (!((symbol.flags & ((SymbolFlags.Module | SymbolFlags.Variable))))))) {
        error(
          moduleReferenceExpression,
          Diagnostics.Module_0_resolves_to_a_non_module_entity_and_cannot_be_imported_using_this_construct,
          symbolToString(moduleSymbol))
        (symbol = undefined)
    ...
    def getExportsOfSymbol(symbol: Symbol): SymbolTable = {
      return (if ((symbol.flags & SymbolFlags.Module))
                getExportsOfModule(symbol)
              else (symbol.exports || emptySymbols))

    }
    def getExportsOfModule(moduleSymbol: Symbol): SymbolTable = {
      val links = getSymbolLinks(moduleSymbol)
      return (links.resolvedExports || ((links.resolvedExports =
        getExportsForModule(moduleSymbol))))
    ...
    def extendExportSymbols(target: SymbolTable,
                            source: SymbolTable,
                            lookupTable: Map[ExportCollisionTracker],
                            exportNode: ExportDeclaration) = {
      (source).keys.foreach { fresh3 =>
        val id = zeroOfMyType = fresh3 {
          if (((id !== "default") && (!target(id)))) {
            (target(id) = source(id))
            if ((lookupTable && exportNode)) {
              (lookupTable(id) = Map(
                "specifierText" -> getTextOfNode(exportNode.moduleSpecifier))
                .asInstanceOf[ExportCollisionTracker])

            }

          } else if (((((lookupTable && exportNode) && (id !== "default")) && target(
                       id)) && (resolveSymbol(target(id)) !== resolveSymbol(
                       source(id))))) {
    ...
    def getExportsForModule(moduleSymbol: Symbol): SymbolTable = {
      ...
      def visit(symbol: Symbol): SymbolTable = {
        if ((!(((symbol && (symbol.flags & SymbolFlags.HasExports)) && (!contains(
              visitedSymbols,
              symbol)))))) {
          return
        ...
        if (exportStars) {
          ...
          (exportStars.declarations).foreach { fresh4 =>
            val node = zeroOfMyType = fresh4 {
              val resolvedModule = resolveExternalModuleName(
                node,
          ...
          (lookupTable).keys.foreach { fresh5 =>
            val id = zeroOfMyType = fresh5 {
              const fresh6 = lookupTable(id)
              val exportsWithDuplicate = fresh6.exportsWithDuplicate
              if ((((id === "export=") || (!((exportsWithDuplicate && exportsWithDuplicate.length)))) || symbols(
                    id))) {
                continue

              }
              (exportsWithDuplicate).foreach { fresh7 =>
                val node = zeroOfMyType = fresh7 {
                  diagnostics.add(
    ...
    def getMergedSymbol(symbol: Symbol): Symbol = {
      ...
      return (if (((symbol && symbol.mergeId) && ((merged =
                    mergedSymbols(symbol.mergeId))))) merged
              else symbol)
    ...
    def getExportSymbolOfValueSymbolIfExported(symbol: Symbol): Symbol = {
      return (if ((symbol && (((symbol.flags & SymbolFlags.ExportValue)) !== 0)))
    ...
    def findConstructorDeclaration(
        node: ClassLikeDeclaration): ConstructorDeclaration = {
      val members = node.members
      (members).foreach { fresh8 =>
        val member = zeroOfMyType = fresh8 {
          if (((member.kind === SyntaxKind.Constructor) && nodeIsPresent(
                (member.asInstanceOf[ConstructorDeclaration]).body))) {
    ...
    def createIntrinsicType(kind: TypeFlags,
                            intrinsicName: String): IntrinsicType = {
      val `type` = createType(kind).asInstanceOf[IntrinsicType]
      (`type`.intrinsicName = intrinsicName)
      return `type`

    }
    def createBooleanType(
        trueFalseTypes: Array[Type]): (IntrinsicType with UnionType) = {
      val `type` = getUnionType(trueFalseTypes)
        .asInstanceOf[(IntrinsicType with UnionType)]
      (`type`.flags |= TypeFlags.Boolean)
      (`type`.intrinsicName = "boolean")
      return `type`

    }
    def createObjectType(objectFlags: ObjectFlags,
                         symbol: Symbol): ObjectType = {
    ...
    def isReservedMemberName(name: String) = {
      return ((((name.charCodeAt(0) === CharacterCodes._underscore_) && (name
        .charCodeAt(1) === CharacterCodes._underscore_)) && (name.charCodeAt(2) !== CharacterCodes._underscore_)) && (name
        .charCodeAt(2) !== CharacterCodes.at))

    }
    def getNamedMembers(members: SymbolTable): Array[Symbol] = {
      var result: Array[Symbol] = zeroOfMyType
      (members).keys.foreach { fresh9 =>
        val id = zeroOfMyType = fresh9 {
    ...
    def createAnonymousType(symbol: Symbol,
                            members: SymbolTable,
                            callSignatures: Array[Signature],
                            constructSignatures: Array[Signature],
                            stringIndexInfo: IndexInfo,
    ...
    def forEachSymbolTableInScope[T](enclosingDeclaration: Node,
                                     callback: ((SymbolTable) => T)): T = {
    ...
    def getQualifiedLeftMeaning(rightMeaning: SymbolFlags) = {
      return (if ((rightMeaning === SymbolFlags.Value)) SymbolFlags.Value
              else SymbolFlags.Namespace)

    }
    def getAccessibleSymbolChain(
        symbol: Symbol,
        enclosingDeclaration: Node,
        meaning: SymbolFlags,
        useOnlyExternalAliasing: Boolean): Array[Symbol] = {
      def getAccessibleSymbolChainFromSymbolTable(
          symbols: SymbolTable): Array[Symbol] = {
        def canQualifySymbol(symbolFromSymbolTable: Symbol,
                             meaning: SymbolFlags) = {
          ...
          val accessibleParent = getAccessibleSymbolChain(
        ...
        def isAccessible(symbolFromSymbolTable: Symbol,
                         resolvedAliasSymbol: Symbol) = {
          if ((symbol === ((resolvedAliasSymbol || symbolFromSymbolTable)))) {
            return ((!forEach(
              symbolFromSymbolTable.declarations,
              hasExternalModuleSymbol)) && canQualifySymbol(
              symbolFromSymbolTable,
              meaning))
        ...
        return forEachProperty(
          symbols,
          (symbolFromSymbolTable => {
             if ((((symbolFromSymbolTable.flags & SymbolFlags.Alias) && (symbolFromSymbolTable.name !== "export=")) && (!getDeclarationOfKind(
                   symbolFromSymbolTable,
                   SyntaxKind.ExportSpecifier)))) {
               if (((!useOnlyExternalAliasing) || ts.forEach(
                     symbolFromSymbolTable.declarations,
                     isExternalModuleImportEqualsDeclaration))) {
                 val resolvedImportedSymbol =
                 ...
                 val accessibleSymbolsFromExports =
                   (if (resolvedImportedSymbol.exports)
                      getAccessibleSymbolChainFromSymbolTable(
                        resolvedImportedSymbol.exports)
                    else undefined)
                 if ((accessibleSymbolsFromExports && canQualifySymbol(
                       symbolFromSymbolTable,
                       getQualifiedLeftMeaning(meaning)))) {
                   return Array(symbolFromSymbolTable).concat(
                     accessibleSymbolsFromExports)
    ...
    def needsQualification(symbol: Symbol,
                           enclosingDeclaration: Node,
                           meaning: SymbolFlags) = {
      var qualify = false
      forEachSymbolTableInScope(
        ...
        (symbolTable => {
           ...
           (symbolFromSymbolTable =
             (if ((((symbolFromSymbolTable.flags & SymbolFlags.Alias) && (!getDeclarationOfKind(
                    symbolFromSymbolTable,
                    SyntaxKind.ExportSpecifier)))))
                resolveAlias(symbolFromSymbolTable)
              else symbolFromSymbolTable))
    ...
    def isPropertyOrMethodDeclarationSymbol(symbol: Symbol) = {
      if ((symbol.declarations && symbol.declarations.length)) {
        (symbol.declarations).foreach { fresh10 =>
          val declaration = zeroOfMyType = fresh10 {
            declaration.kind match {
              case SyntaxKind.PropertyDeclaration |
                  SyntaxKind.MethodDeclaration | SyntaxKind.GetAccessor |
    ...
    def isSymbolAccessible(symbol: Symbol,
                           enclosingDeclaration: Node,
                           meaning: SymbolFlags,
      ...
      if (((symbol && enclosingDeclaration) && (!((symbol.flags & SymbolFlags.TypeParameter))))) {
        ...
        while (symbol) {
          {
            val accessibleSymbolChain = getAccessibleSymbolChain(
              symbol,
              enclosingDeclaration,
              meaningToLook,
              false)
            if (accessibleSymbolChain) {
              val hasAccessibleDeclarations = hasVisibleDeclarations(
              ...
              if ((!hasAccessibleDeclarations)) {
                return Map(
                  ...
                  "errorSymbolName" -> symbolToString(
                    ...
                    enclosingDeclaration,
                    meaning),
                  "errorModuleName" -> (if ((symbol !== initialSymbol))
                                          symbolToString(
                                            symbol,
                                            enclosingDeclaration,
                                            SymbolFlags.Namespace)
                                        else undefined))
                  .asInstanceOf[SymbolAccessibilityResult]
        ...
        val symbolExternalModule =
          forEach(initialSymbol.declarations, getExternalModuleContainer)
        if (symbolExternalModule) {
          val enclosingExternalModule = getExternalModuleContainer(
            enclosingDeclaration)
    ...
    def hasExternalModuleSymbol(declaration: Node) = {
      return (isAmbientModule(declaration) || (((declaration.kind === SyntaxKind.SourceFile) && isExternalOrCommonJsModule(
        declaration.asInstanceOf[SourceFile]))))

    }
    def hasVisibleDeclarations(
        symbol: Symbol,
        shouldComputeAliasToMakeVisible: Boolean): SymbolVisibilityResult = {
      var aliasesToMakeVisible: Array[AnyImportSyntax] = zeroOfMyType
      if (forEach(
            symbol.declarations,
            (declaration => (!getIsDeclarationVisible(declaration))))) {
        return undefined
      ...
      def getIsDeclarationVisible(declaration: Declaration) = {
        if ((!isDeclarationVisible(declaration))) {
          ...
          if (((anyImportSyntax && (!((getModifierFlags(anyImportSyntax) & ModifierFlags.Export)))) && isDeclarationVisible(
                anyImportSyntax.parent.asInstanceOf[Declaration]))) {
    ...
    def isEntityNameVisible(
      ...
      if (((entityName.parent.kind === SyntaxKind.TypeQuery) || isExpressionWithTypeArgumentsInClassExtendsClause(
            entityName.parent))) {
      ...
      val symbol = resolveName(
        enclosingDeclaration,
        (firstIdentifier.asInstanceOf[Identifier]).text,
        meaning,
        undefined,
        undefined)
      return (((symbol && hasVisibleDeclarations(symbol, true))) || Map(
        "accessibility" -> SymbolAccessibility.NotAccessible,
    ...
    def symbolToString(symbol: Symbol,
      ...
      getSymbolDisplayBuilder()
        .buildSymbolDisplay(symbol, writer, enclosingDeclaration, meaning)
      val result = writer.string()
      releaseStringWriter(writer)
      return result

    }
    def signatureToString(signature: Signature,
                          enclosingDeclaration: Node,
                          flags: TypeFormatFlags,
                          kind: SignatureKind): String = {
      val writer = getSingleLineStringWriter()
      getSymbolDisplayBuilder().buildSignatureDisplay(
        signature,
        writer,
        enclosingDeclaration,
        flags,
    ...
    def typeToString(`type`: Type,
      ...
      releaseStringWriter(writer)
      val maxLength =
        (if ((compilerOptions.noErrorTruncation || (flags & TypeFormatFlags.NoTruncation)))
           undefined
         else 100)
    ...
    def typePredicateToString(typePredicate: TypePredicate,
                              enclosingDeclaration: Declaration,
                              flags: TypeFormatFlags): String = {
      val writer = getSingleLineStringWriter()
      getSymbolDisplayBuilder().buildTypePredicateDisplay(
        typePredicate,
        writer,
        enclosingDeclaration,
        flags)
        ...
        while ((i < types.length)) {
          {
            ...
            if ((!((t.flags & TypeFlags.Nullable)))) {
              if ((t.flags & ((TypeFlags.BooleanLiteral | TypeFlags.EnumLiteral)))) {
                val baseType =
                  (if ((t.flags & TypeFlags.BooleanLiteral)) booleanType
                   else (t.asInstanceOf[EnumLiteralType]).baseType)
                val count = baseType.types.length
                if ((((i + count) <= types.length) && (types(((i + count) - 1)) === baseType
                      .types((count - 1))))) {
    ...
    def literalTypeToString(`type`: LiteralType) = {
      return (if ((`type`.flags & TypeFlags.StringLiteral)) s"""\"${escapeString(
                (`type`.asInstanceOf[LiteralType]).text)}\""""
              else (`type`.asInstanceOf[LiteralType]).text)
    ...
    def getSymbolDisplayBuilder(): SymbolDisplayBuilder = {
      ...
      def buildSymbolDisplay(symbol: Symbol,
                             ...
                             typeFlags: TypeFormatFlags): Unit = {
            ...
            if ((flags & SymbolFormatFlags.WriteTypeParametersOrArguments)) {
              if ((symbol.flags & SymbolFlags.Instantiated)) {
                buildDisplayForTypeArgumentsAndDelimiters(
                  ...
                  (symbol.asInstanceOf[TransientSymbol]).mapper,
                  writer,
                  enclosingDeclaration)
        ...
        def walkSymbol(symbol: Symbol,
                       meaning: SymbolFlags,
                       endOfChain: Boolean): Unit = {
          val accessibleSymbolChain = getAccessibleSymbolChain(
            symbol,
            enclosingDeclaration,
            meaning,
            (!(!((flags & SymbolFormatFlags.UseOnlyExternalAliasing)))))
          if (((!accessibleSymbolChain) || needsQualification(
                accessibleSymbolChain(0),
                enclosingDeclaration,
                (if ((accessibleSymbolChain.length === 1)) meaning
                 else getQualifiedLeftMeaning(meaning))))) {
            val parent = getParentOfSymbol(
              (if (accessibleSymbolChain) accessibleSymbolChain(0)
               else symbol))
            if (parent) {
          ...
          if (accessibleSymbolChain) {
            (accessibleSymbolChain).foreach { fresh11 =>
              val accessibleSymbol = zeroOfMyType = fresh11 {
                appendParentTypeArgumentsAndSymbolName(accessibleSymbol)

              }
            }

          } else if ((endOfChain || ((!(((!parentSymbol) && ts.forEach(
                       symbol.declarations,
                       hasExternalModuleSymbol)))) && (!((symbol.flags & ((SymbolFlags.TypeLiteral | SymbolFlags.ObjectLiteral)))))))) {
      ...
      def buildTypeDisplay(`type`: Type,
        ...
        def writeType(`type`: Type, flags: TypeFormatFlags) = {
          ...
          if ((`type`.flags & TypeFlags.Intrinsic)) {
            writer.writeKeyword(
              (if (((!((globalFlags & TypeFormatFlags.WriteOwnNameForAnyLike))) && isTypeAny(
                     `type`))) "any"
               else (`type`.asInstanceOf[IntrinsicType]).intrinsicName))

          } else if (((`type`.flags & TypeFlags.TypeParameter) && (`type`
                       .asInstanceOf[TypeParameter])
                       .isThisType)) {
          ...
          } else if (((((!((flags & TypeFormatFlags.InTypeAlias))) && ((((getObjectFlags(
                       `type`) & ObjectFlags.Anonymous) && (!(`type`
                       .asInstanceOf[AnonymousType])
                       .target)) || (`type`.flags & TypeFlags.UnionOrIntersection)))) && `type`.aliasSymbol) && (isSymbolAccessible(
                       `type`.aliasSymbol,
                       enclosingDeclaration,
                       SymbolFlags.Type,
                       false).accessibility === SymbolAccessibility.Accessible))) {
            val typeArguments =
              (`type`.asInstanceOf[AnonymousType]).aliasTypeArguments
            writeSymbolTypeReference(
              `type`.aliasSymbol,
              typeArguments,
              0,
              (if (typeArguments) typeArguments.length else 0),
              nextFlags)

          } else if ((`type`.flags & TypeFlags.UnionOrIntersection)) {
            writeUnionOrIntersectionType(
              `type`.asInstanceOf[UnionOrIntersectionType],
              nextFlags)
            ...
            writer.writeStringLiteral(
              literalTypeToString(`type`.asInstanceOf[LiteralType]))
            ...
            while ((i < types.length)) {
              {
                ...
                writeType(
                  ...
                  (if ((delimiter === SyntaxKind.CommaToken))
                   ...
                   else TypeFormatFlags.InElementType))
        ...
        def writeSymbolTypeReference(symbol: Symbol,
                                     typeArguments: Array[Type],
                                     pos: Int,
                                     end: Int,
                                     flags: TypeFormatFlags) = {
          if (((symbol.flags & SymbolFlags.Class) || (!isReservedMemberName(
                symbol.name)))) {
            buildSymbolDisplay(
              symbol,
              writer,
        ...
        def writeTypeReference(`type`: TypeReference, flags: TypeFormatFlags) = {
          ...
          if (((`type`.target === globalArrayType) && (!((flags & TypeFormatFlags.WriteArrayAsGenericType))))) {
            ...
            writePunctuation(writer, SyntaxKind.OpenBracketToken)
            writeTypeList(
              `type`.typeArguments.slice(0, getTypeReferenceArity(`type`)),
            ...
            if (outerTypeParameters) {
              ...
              while ((i < length)) {
                {
                  val start = i
                  val parent = getParentSymbolOfTypeParameter(
                  ...
                  } while (((i < length) && (getParentSymbolOfTypeParameter(
                    outerTypeParameters(i)) === parent)))
                  if ((!rangeEquals(
                        outerTypeParameters,
                        typeArguments,
                        start,
            ...
            val typeParameterCount =
              ((`type`.target.typeParameters || emptyArray)).length
            writeSymbolTypeReference(
              `type`.symbol,
              typeArguments,
              i,
        ...
        def writeUnionOrIntersectionType(`type`: UnionOrIntersectionType,
                                         flags: TypeFormatFlags) = {
        ...
        def writeAnonymousType(`type`: ObjectType, flags: TypeFormatFlags) = {
          ...
          def shouldWriteTypeOfFunctionSymbol() = {
            val isStaticMethodSymbol =
              (!(!(((symbol.flags & SymbolFlags.Method) && forEach(
                symbol.declarations,
                (declaration =>
                   (getModifierFlags(declaration) & ModifierFlags.Static)))))))
            val isNonLocalFunctionSymbol = ((!(!((symbol.flags & SymbolFlags.Function)))) && ((symbol.parent || forEach(
                symbol.declarations,
                (declaration =>
                   ((declaration.parent.kind === SyntaxKind.SourceFile) || (declaration.parent.kind === SyntaxKind.ModuleBlock)))))))
            if ((isStaticMethodSymbol || isNonLocalFunctionSymbol)) {
              return ((!(!((flags & TypeFormatFlags.UseTypeOfFunction)))) || (contains(
                symbolStack,
                symbol)))
        ...
        def writeTypeOfSymbol(`type`: ObjectType,
        ...
        def shouldAddParenthesisAroundFunctionType(callSignature: Signature,
                                                   flags: TypeFormatFlags) = {
          if ((flags & TypeFormatFlags.InElementType)) {
            return true
            ...
            val typeParameters =
              (if ((callSignature.target && ((flags & TypeFormatFlags.WriteTypeArgumentsOfSignature))))
                 callSignature.target.typeParameters
               else callSignature.typeParameters)
        ...
        def writeLiteralType(`type`: ObjectType, flags: TypeFormatFlags) = {
          ...
          if ((((!resolved.properties.length) && (!resolved.stringIndexInfo)) && (!resolved.numberIndexInfo))) {
            ...
            if (((resolved.callSignatures.length === 1) && (!resolved.constructSignatures.length))) {
              val parenthesizeSignature =
                shouldAddParenthesisAroundFunctionType(
                  resolved.callSignatures(0),
                  flags)
            ...
            if (((resolved.constructSignatures.length === 1) && (!resolved.callSignatures.length))) {
              ...
              writeSpace(writer)
              buildSignatureDisplay(
                resolved.constructSignatures(0),
                writer,
                enclosingDeclaration,
                (globalFlagsToPass | TypeFormatFlags.WriteArrowStyleSignature),
                undefined,
          ...
          (resolved.callSignatures).foreach { fresh12 =>
            val signature = zeroOfMyType = fresh12 {
              buildSignatureDisplay(
                signature,
                writer,
                enclosingDeclaration,
          ...
          (resolved.constructSignatures).foreach { fresh13 =>
            val signature = zeroOfMyType = fresh13 {
              buildSignatureDisplay(
                signature,
                writer,
                enclosingDeclaration,
          ...
          (resolved.properties).foreach { fresh14 =>
            val p = zeroOfMyType = fresh14 {
              val t = getTypeOfSymbol(p)
              if (((p.flags & ((SymbolFlags.Function | SymbolFlags.Method))) && (!getPropertiesOfObjectType(
                    t).length))) {
                val signatures = getSignaturesOfType(t, SignatureKind.Call)
                (signatures).foreach { fresh15 =>
                  val signature = zeroOfMyType = fresh15 {
      ...
      def buildParameterDisplay(p: Symbol,
                                writer: SymbolWriter,
                                enclosingDeclaration: Node,
                                flags: TypeFormatFlags,
                                symbolStack: Array[Symbol]) = {
        val parameterNode =
          p.valueDeclaration.asInstanceOf[ParameterDeclaration]
        if (isRestParameter(parameterNode)) {
          writePunctuation(writer, SyntaxKind.DotDotDotToken)

        }
        if (isBindingPattern(parameterNode.name)) {
          buildBindingPatternDisplay(
            parameterNode.name.asInstanceOf[BindingPattern],
            writer,
            enclosingDeclaration,
            flags,
            symbolStack)
      ...
      def buildBindingPatternDisplay(bindingPattern: BindingPattern,
                                     writer: SymbolWriter,
                                     enclosingDeclaration: Node,
                                     flags: TypeFormatFlags,
                                     symbolStack: Array[Symbol]) = {
        if ((bindingPattern.kind === SyntaxKind.ObjectBindingPattern)) {
          writePunctuation(writer, SyntaxKind.OpenBraceToken)
          buildDisplayForCommaSeparatedList(
            bindingPattern.elements,
            writer,
            (e =>
               buildBindingElementDisplay(
                 e,
                 writer,
                 enclosingDeclaration,
                 flags,
      ...
      def buildBindingElementDisplay(bindingElement: ArrayBindingElement,
                                     writer: SymbolWriter,
                                     enclosingDeclaration: Node,
                                     flags: TypeFormatFlags,
                                     symbolStack: Array[Symbol]) = {
        ...
        if (bindingElement.propertyName) {
          writer.writeSymbol(
            getTextOfNode(bindingElement.propertyName),
      ...
      def buildDisplayForCommaSeparatedList[T](list: Array[T],
                                               writer: SymbolWriter,
                                               action: ((T) => Unit)) = {
        {
          var i = 0
      ...
      def buildDisplayForTypeArgumentsAndDelimiters(
          typeParameters: Array[TypeParameter],
          mapper: TypeMapper,
          writer: SymbolWriter,
          enclosingDeclaration: Node) = {
      ...
      def buildDisplayForParametersAndDelimiters(
      ...
      def buildSignatureDisplay(signature: Signature,
                                writer: SymbolWriter,
                                enclosingDeclaration: Node,
                                flags: TypeFormatFlags,
                                kind: SignatureKind,
      ...
      return (_displayBuilder || ((_displayBuilder = Map(
        ...
        node.kind match {
          case SyntaxKind.BindingElement =>
            return isDeclarationVisible(
              node.parent.parent.asInstanceOf[Declaration])
          case SyntaxKind.VariableDeclaration =>
            if ((isBindingPattern(node.name) && (!(node.name
                  .asInstanceOf[BindingPattern])
                  .elements
                  .length))) {
              return false

            }
          case SyntaxKind.ModuleDeclaration | SyntaxKind.ClassDeclaration |
              SyntaxKind.InterfaceDeclaration |
              SyntaxKind.TypeAliasDeclaration |
              SyntaxKind.FunctionDeclaration | SyntaxKind.EnumDeclaration |
            ...
            if (((!((getCombinedModifierFlags(node) & ModifierFlags.Export))) && (!((((node.kind !== SyntaxKind.ImportEqualsDeclaration) && (parent.kind !== SyntaxKind.SourceFile)) && isInAmbientContext(
                  parent)))))) {
          ...
          case SyntaxKind.Constructor | SyntaxKind.ConstructSignature |
              SyntaxKind.CallSignature | SyntaxKind.IndexSignature |
              SyntaxKind.Parameter | SyntaxKind.ModuleBlock |
          ...
          case SyntaxKind.ImportClause | SyntaxKind.NamespaceImport |
              SyntaxKind.ImportSpecifier =>
            return false
          case SyntaxKind.TypeParameter | SyntaxKind.SourceFile |
    ...
    def collectLinkedAliases(node: Identifier): Array[Node] = {
      ...
      if ((node.parent && (node.parent.kind === SyntaxKind.ExportAssignment))) {
        ...
        (exportSymbol =
          (if ((exportSpecifier.parent.parent
                 .asInstanceOf[ExportDeclaration])
                 .moduleSpecifier)
             getExternalModuleMember(
               exportSpecifier.parent.parent.asInstanceOf[ExportDeclaration],
               exportSpecifier)
           else
             resolveEntityName(
        ...
        forEach(
          ...
          (declaration => {
             ...
             if (isInternalModuleImportEqualsDeclaration(declaration)) {
               val internalModuleReference = (declaration
                 .asInstanceOf[ImportEqualsDeclaration])
                 .moduleReference
                 .asInstanceOf[(Identifier | QualifiedName)]
               val firstIdentifier =
                 getFirstIdentifier(internalModuleReference)
               val importSymbol = resolveName(
                 declaration,
    ...
    def pushTypeResolution(target: TypeSystemEntity,
                           propertyName: TypeSystemPropertyName): Boolean = {
      val resolutionCycleStartIndex =
        findResolutionCycleStartIndex(target, propertyName)
    ...
    def findResolutionCycleStartIndex(
        target: TypeSystemEntity,
        propertyName: TypeSystemPropertyName): Int = {
      {
        ...
        while ((i >= 0)) {
          {
            ...
            if (((resolutionTargets(i) === target) && (resolutionPropertyNames(
                  i) === propertyName))) {
              return i
    ...
    def hasType(target: TypeSystemEntity,
                propertyName: TypeSystemPropertyName): Type = {
      ...
      while (node) {
        {
          node.kind match {
            case SyntaxKind.VariableDeclaration |
                ...
                SyntaxKind.ImportSpecifier | SyntaxKind.NamedImports |
                SyntaxKind.NamespaceImport | SyntaxKind.ImportClause =>
              (node = node.parent)
            case _ =>
              return node.parent
          }
    ...
    def getTypeOfPrototypeProperty(prototype: Symbol): Type = {
      val classType = getDeclaredTypeOfSymbol(getParentOfSymbol(prototype))
        .asInstanceOf[InterfaceType]
      return (if (classType.typeParameters)
                createTypeReference(
                  classType.asInstanceOf[GenericType],
                  map(classType.typeParameters, (_underscore_ => anyType)))
              else classType)
    ...
    def getTextOfPropertyName(name: PropertyName): String = {
      name.kind match {
        ...
        case SyntaxKind.ComputedPropertyName =>
          if (isStringOrNumericLiteral(
                (name.asInstanceOf[ComputedPropertyName]).expression.kind)) {
            return ((name
    ...
    def isComputedNonLiteralName(name: PropertyName): Boolean = {
      return ((name.kind === SyntaxKind.ComputedPropertyName) && (!isStringOrNumericLiteral(
        (name.asInstanceOf[ComputedPropertyName]).expression.kind)))

    }
    def getTypeForBindingElement(declaration: BindingElement): Type = {
      val pattern = declaration.parent.asInstanceOf[BindingPattern]
      val parentType = getTypeForBindingElementParent(
        pattern.parent.asInstanceOf[VariableLikeDeclaration])
      ...
      if ((pattern.kind === SyntaxKind.ObjectBindingPattern)) {
        ...
        (`type` = ((getTypeOfPropertyOfType(parentType, text) || (isNumericLiteralName(
            text) && getIndexTypeOfType(parentType, IndexKind.Number))) || getIndexTypeOfType(
            parentType,
            IndexKind.String)))
        if ((!`type`)) {
          error(
            name,
            Diagnostics.Type_0_has_no_property_1_and_no_string_index_signature,
        ...
        val elementType =
          checkIteratedTypeOrElementType(parentType, pattern, false)
        if ((!declaration.dotDotDotToken)) {
          val propName = ("" + indexOf(pattern.elements, declaration))
          (`type` =
            (if (isTupleLikeType(parentType))
          ...
          if ((!`type`)) {
            if (isTupleType(parentType)) {
              error(
                declaration,
                Diagnostics.Tuple_type_0_with_length_1_cannot_be_assigned_to_tuple_with_length_2,
                typeToString(parentType),
                getTypeReferenceArity(parentType.asInstanceOf[TypeReference]),
                pattern.elements.length)

            } else {
              error(
                declaration,
                Diagnostics.Type_0_has_no_property_1,
                typeToString(parentType),
                propName)
      ...
      if (((strictNullChecks && declaration.initializer) && (!((getFalsyFlags(checkExpressionCached(
            declaration.initializer)) & TypeFlags.Undefined))))) {
        (`type` = getTypeWithFacts(`type`, TypeFacts.NEUndefined))

      }
      return (if (declaration.initializer)
                getUnionType(
                  Array(
                    `type`,
                    checkExpressionCached(declaration.initializer)),
                  true)
              else `type`)

    }
    def getTypeForVariableLikeDeclarationFromJSDocComment(
        declaration: VariableLikeDeclaration) = {
      val jsDocType = getJSDocTypeForVariableLikeDeclarationFromJSDocComment(
        declaration)
      if (jsDocType) {
    ...
    def getJSDocTypeForVariableLikeDeclarationFromJSDocComment(
        declaration: VariableLikeDeclaration): JSDocType = {
      ...
      if ((((declaration.kind === SyntaxKind.VariableDeclaration) && (declaration.parent.kind === SyntaxKind.VariableDeclarationList)) && (declaration.parent.parent.kind === SyntaxKind.VariableStatement))) {
        ...
        val paramTag = getCorrespondingJSDocParameterTag(
          declaration.asInstanceOf[ParameterDeclaration])
    ...
    def isNullOrUndefined(node: Expression) = {
      ...
      return ((expr.kind === SyntaxKind.NullKeyword) || ((expr.kind === SyntaxKind.Identifier) && (getResolvedSymbol(
        expr.asInstanceOf[Identifier]) === undefinedSymbol)))

    }
    def isEmptyArrayLiteral(node: Expression) = {
      val expr = skipParentheses(node)
      return ((expr.kind === SyntaxKind.ArrayLiteralExpression) && ((expr
    ...
    def addOptionality(`type`: Type, optional: Boolean): Type = {
      return (if ((strictNullChecks && optional))
                includeFalsyTypes(`type`, TypeFlags.Undefined)
              else `type`)

    }
    def getTypeForVariableLikeDeclaration(
        declaration: VariableLikeDeclaration,
        includeOptionality: Boolean): Type = {
      if ((declaration.flags & NodeFlags.JavaScriptFile)) {
        val `type` = getTypeForVariableLikeDeclarationFromJSDocComment(
          declaration)
      ...
      if (isBindingPattern(declaration.parent)) {
        return getTypeForBindingElement(
          declaration.asInstanceOf[BindingElement])
      ...
      if (((((declaration.kind === SyntaxKind.VariableDeclaration) && (!isBindingPattern(
            declaration.name))) && (!((getCombinedModifierFlags(declaration) & ModifierFlags.Export)))) && (!isInAmbientContext(
            declaration)))) {
        if (((!((getCombinedNodeFlags(declaration) & NodeFlags.Const))) && (((!declaration.initializer) || isNullOrUndefined(
              declaration.initializer))))) {
          return autoType

        }
        if ((declaration.initializer && isEmptyArrayLiteral(
              declaration.initializer))) {
      ...
      if ((declaration.kind === SyntaxKind.Parameter)) {
        ...
        if (((func.kind === SyntaxKind.SetAccessor) && (!hasDynamicName(
              func)))) {
          val getter = getDeclarationOfKind(
            declaration.parent.symbol,
            SyntaxKind.GetAccessor).asInstanceOf[AccessorDeclaration]
          if (getter) {
            val getterSignature = getSignatureFromDeclaration(getter)
            val thisParameter = getAccessorThisParameter(
              func.asInstanceOf[AccessorDeclaration])
        ...
        if ((declaration.symbol.name === "this")) {
          ...
          (`type` = getContextuallyTypedParameterType(
            declaration.asInstanceOf[ParameterDeclaration]))
      ...
      if (isBindingPattern(declaration.name)) {
        return getTypeFromBindingPattern(
          declaration.name.asInstanceOf[BindingPattern],
          false,
          true)
    ...
    def getTypeFromBindingElement(element: BindingElement,
      ...
      if (isBindingPattern(element.name)) {
        return getTypeFromBindingPattern(
          element.name.asInstanceOf[BindingPattern],
          includePatternInType,
          reportErrors)

      }
      if (((reportErrors && compilerOptions.noImplicitAny) && (!declarationBelongsToPrivateAmbientMember(
            element)))) {
    ...
    def getTypeFromObjectBindingPattern(pattern: ObjectBindingPattern,
                                        includePatternInType: Boolean,
                                        reportErrors: Boolean): Type = {
      ...
      forEach(
        ...
        (e => {
           ...
           val text = getTextOfPropertyName(name)
           val flags = ((SymbolFlags.Property | SymbolFlags.Transient) | ((if (e.initializer)
      ...
      val result = createAnonymousType(
        undefined,
        members,
        emptyArray,
        emptyArray,
    ...
    def getTypeFromArrayBindingPattern(pattern: BindingPattern,
                                       includePatternInType: Boolean,
                                       reportErrors: Boolean): Type = {
      ...
      val elementTypes = map(
        elements,
        (e =>
           (if (isOmittedExpression(e)) anyType
            else
    ...
    def getTypeFromBindingPattern(pattern: BindingPattern,
                                  includePatternInType: Boolean,
                                  reportErrors: Boolean): Type = {
      return (if ((pattern.kind === SyntaxKind.ObjectBindingPattern))
                getTypeFromObjectBindingPattern(
                  pattern.asInstanceOf[ObjectBindingPattern],
                  includePatternInType,
                  reportErrors)
              else
                getTypeFromArrayBindingPattern(
                  pattern.asInstanceOf[ArrayBindingPattern],
                  includePatternInType,
                  reportErrors))

    }
    def getWidenedTypeForVariableLikeDeclaration(
        declaration: VariableLikeDeclaration,
        reportErrors: Boolean): Type = {
    ...
    def declarationBelongsToPrivateAmbientMember(
        declaration: VariableLikeDeclaration) = {
      val root = getRootDeclaration(declaration)
      val memberDeclaration =
        (if ((root.kind === SyntaxKind.Parameter)) root.parent else root)
    ...
    def getTypeOfVariableOrParameterOrProperty(symbol: Symbol): Type = {
      ...
      if ((!links.`type`)) {
        ...
        if ((declaration.kind === SyntaxKind.ExportAssignment)) {
          return (links.`type` = checkExpression(
            (declaration.asInstanceOf[ExportAssignment]).expression))

        }
        if ((((declaration.flags & NodeFlags.JavaScriptFile) && (declaration.kind === SyntaxKind.JSDocPropertyTag)) && (declaration
              .asInstanceOf[JSDocPropertyTag])
              .typeExpression)) {
          return (links.`type` = getTypeFromTypeNode(
            (declaration
              .asInstanceOf[JSDocPropertyTag])
              .typeExpression
              .`type`))
        ...
        if (((declaration.kind === SyntaxKind.BinaryExpression) || ((declaration.kind === SyntaxKind.PropertyAccessExpression) && (declaration.parent.kind === SyntaxKind.BinaryExpression)))) {
          ...
          val declaredTypes = map(
            symbol.declarations,
            (decl =>
               (if ((decl.kind === SyntaxKind.BinaryExpression))
                  checkExpressionCached(
                    (decl.asInstanceOf[BinaryExpression]).right)
                else
                  checkExpressionCached(
                    (decl.parent.asInstanceOf[BinaryExpression]).right))))
          (`type` = getUnionType(declaredTypes, true))

        } else {
          (`type` = getWidenedTypeForVariableLikeDeclaration(
            declaration.asInstanceOf[VariableLikeDeclaration],
            true))

        }
        if ((!popTypeResolution())) {
          if ((symbol.valueDeclaration
                .asInstanceOf[VariableLikeDeclaration])
                .`type`) {
            (`type` = unknownType)
            error(
    ...
    def getAnnotatedAccessorType(accessor: AccessorDeclaration): Type = {
      if (accessor) {
        if ((accessor.kind === SyntaxKind.GetAccessor)) {
          ...
          return (setterTypeAnnotation && getTypeFromTypeNode(
    ...
    def getAnnotatedAccessorThisParameter(
        accessor: AccessorDeclaration): (Symbol | undefined) = {
      val parameter = getAccessorThisParameter(accessor)
      return (parameter && parameter.symbol)

    }
    def getThisTypeOfDeclaration(
        declaration: SignatureDeclaration): (Type | undefined) = {
    ...
    def getTypeOfAccessors(symbol: Symbol): Type = {
      val links = getSymbolLinks(symbol)
      if ((!links.`type`)) {
        val getter = getDeclarationOfKind(symbol, SyntaxKind.GetAccessor)
          .asInstanceOf[AccessorDeclaration]
        val setter = getDeclarationOfKind(symbol, SyntaxKind.SetAccessor)
          .asInstanceOf[AccessorDeclaration]
        if ((getter && (getter.flags & NodeFlags.JavaScriptFile))) {
          val jsDocType = getTypeForVariableLikeDeclarationFromJSDocComment(
            getter)
          if (jsDocType) {
            ...
            if ((getter && getter.body)) {
              ...
              if (compilerOptions.noImplicitAny) {
                if (setter) {
                  ...
                  Debug.assert(
                    (!(!getter)),
                    "there must existed getter as we are current checking either setter or getter in this function")
                  error(
                    getter,
    ...
    def getTypeOfFuncClassEnumModule(symbol: Symbol): Type = {
      val links = getSymbolLinks(symbol)
      if ((!links.`type`)) {
        if (((symbol.valueDeclaration.kind === SyntaxKind.ModuleDeclaration) && isShorthandAmbientModuleSymbol(
              symbol))) {
          (links.`type` = anyType)
          ...
          (links.`type` =
            (if ((strictNullChecks && (symbol.flags & SymbolFlags.Optional)))
               includeFalsyTypes(`type`, TypeFlags.Undefined)
             else `type`))
    ...
    def getTypeOfInstantiatedSymbol(symbol: Symbol): Type = {
      ...
      if ((!links.`type`)) {
        (links.`type` =
          instantiateType(getTypeOfSymbol(links.target), links.mapper))
    ...
    def getTargetType(`type`: Type): Type = {
      return (if ((getObjectFlags(`type`) & ObjectFlags.Reference))(`type`
                .asInstanceOf[TypeReference])
                .target
              else `type`)
    ...
    def appendTypeParameters(typeParameters: Array[TypeParameter],
                             declarations: Array[TypeParameterDeclaration])
      : Array[TypeParameter] = {
      (declarations).foreach { fresh17 =>
        val declaration = zeroOfMyType = fresh17 {
    ...
    def appendOuterTypeParameters(typeParameters: Array[TypeParameter],
                                  node: Node): Array[TypeParameter] = {
      while (true) {
        {
          (node = node.parent)
          if ((!node)) {
          ...
          if (((((((node.kind === SyntaxKind.ClassDeclaration) || (node.kind === SyntaxKind.ClassExpression)) || (node.kind === SyntaxKind.FunctionDeclaration)) || (node.kind === SyntaxKind.FunctionExpression)) || (node.kind === SyntaxKind.MethodDeclaration)) || (node.kind === SyntaxKind.ArrowFunction))) {
            val declarations = (node
              .asInstanceOf[(ClassLikeDeclaration | FunctionLikeDeclaration)])
              .typeParameters
            if (declarations) {
              return appendTypeParameters(
                appendOuterTypeParameters(typeParameters, node),
                declarations)
    ...
    def getOuterTypeParametersOfClassOrInterface(
        symbol: Symbol): Array[TypeParameter] = {
      val declaration =
        (if ((symbol.flags & SymbolFlags.Class)) symbol.valueDeclaration
         else getDeclarationOfKind(symbol, SyntaxKind.InterfaceDeclaration))
      return appendOuterTypeParameters(undefined, declaration)

    }
    def getLocalTypeParametersOfClassOrInterfaceOrTypeAlias(
        symbol: Symbol): Array[TypeParameter] = {
      var result: Array[TypeParameter] = zeroOfMyType
      (symbol.declarations).foreach { fresh18 =>
        val node = zeroOfMyType = fresh18 {
          if (((((node.kind === SyntaxKind.InterfaceDeclaration) || (node.kind === SyntaxKind.ClassDeclaration)) || (node.kind === SyntaxKind.ClassExpression)) || (node.kind === SyntaxKind.TypeAliasDeclaration))) {
            val declaration =
              node.asInstanceOf[(InterfaceDeclaration | TypeAliasDeclaration)]
            if (declaration.typeParameters) {
              (result =
                appendTypeParameters(result, declaration.typeParameters))
    ...
    def getTypeParametersOfClassOrInterface(
        symbol: Symbol): Array[TypeParameter] = {
      return concatenate(
    ...
    def isConstructorType(`type`: Type): Boolean = {
      return ((`type`.flags & TypeFlags.Object) && (getSignaturesOfType(
        `type`,
        SignatureKind.Construct).length > 0))

    }
    def getBaseTypeNodeOfClass(
        `type`: InterfaceType): ExpressionWithTypeArguments = {
      return getClassExtendsHeritageClauseElement(
        `type`.symbol.valueDeclaration.asInstanceOf[ClassLikeDeclaration])

    }
    def getConstructorsForTypeArguments(
        `type`: Type,
        typeArgumentNodes: Array[TypeNode]): Array[Signature] = {
      val typeArgCount =
        (if (typeArgumentNodes) typeArgumentNodes.length else 0)
      return filter(
        getSignaturesOfType(`type`, SignatureKind.Construct),
        (sig =>
           (((if (sig.typeParameters) sig.typeParameters.length else 0) ) === typeArgCount)))

    }
    def getInstantiatedConstructorsForTypeArguments(
        `type`: Type,
        typeArgumentNodes: Array[TypeNode]): Array[Signature] = {
      var signatures =
        getConstructorsForTypeArguments(`type`, typeArgumentNodes)
      if (typeArgumentNodes) {
        val typeArguments = map(typeArgumentNodes, getTypeFromTypeNodeNoAlias)
        (signatures = map(
          signatures,
          (sig => getSignatureInstantiation(sig, typeArguments))))
      ...
      if ((!`type`.resolvedBaseConstructorType)) {
        ...
        if ((baseConstructorType.flags & TypeFlags.Object)) {
          resolveStructuredTypeMembers(
            baseConstructorType.asInstanceOf[ObjectType])
        ...
        if ((((baseConstructorType !== unknownType) && (baseConstructorType !== nullWideningType)) && (!isConstructorType(
              baseConstructorType)))) {
          error(
    ...
    def getBaseTypes(`type`: InterfaceType): Array[ObjectType] = {
      if ((!`type`.resolvedBaseTypes)) {
        if ((`type`.objectFlags & ObjectFlags.Tuple)) {
          (`type`.resolvedBaseTypes = Array(
            createArrayType(getUnionType(`type`.typeParameters))))
    ...
    def resolveBaseTypesOfClass(`type`: InterfaceType): Unit = {
      ...
      val originalBaseType =
        (if ((baseConstructorType && baseConstructorType.symbol))
           getDeclaredTypeOfSymbol(baseConstructorType.symbol)
         else undefined)
      if (((baseConstructorType.symbol && (baseConstructorType.symbol.flags & SymbolFlags.Class)) && areAllOuterTypeParametersApplied(
            originalBaseType))) {
        ...
        val constructors = getInstantiatedConstructorsForTypeArguments(
          baseConstructorType,
      ...
      if ((!((getObjectFlags(getTargetType(baseType)) & ObjectFlags.ClassOrInterface)))) {
        error(
          baseTypeNode.expression,
          Diagnostics.Base_constructor_return_type_0_is_not_a_class_or_interface_type,
          typeToString(baseType))
        return

      }
      if (((`type` === baseType) || hasBaseType(
            baseType.asInstanceOf[InterfaceType],
            `type`))) {
        error(
          `type`.symbol.valueDeclaration,
    ...
    def areAllOuterTypeParametersApplied(`type`: Type): Boolean = {
      val outerTypeParameters =
        (`type`.asInstanceOf[InterfaceType]).outerTypeParameters
      if (outerTypeParameters) {
    ...
    def resolveBaseTypesOfInterface(`type`: InterfaceType): Unit = {
      ...
      (`type`.symbol.declarations).foreach { fresh19 =>
        val declaration = zeroOfMyType = fresh19 {
          if (((declaration.kind === SyntaxKind.InterfaceDeclaration) && getInterfaceBaseTypeNodes(
                declaration.asInstanceOf[InterfaceDeclaration]))) {
            (getInterfaceBaseTypeNodes(
              declaration.asInstanceOf[InterfaceDeclaration])).foreach {
              fresh20 =>
                val node = zeroOfMyType = fresh20 {
                  ...
                  if ((baseType !== unknownType)) {
                    if ((getObjectFlags(getTargetType(baseType)) & ObjectFlags.ClassOrInterface)) {
                      if (((`type` !== baseType) && (!hasBaseType(
                        ...
                        if ((`type`.resolvedBaseTypes === emptyArray)) {
                          (`type`.resolvedBaseTypes =
                            Array(baseType.asInstanceOf[ObjectType]))

                        } else {
                          `type`.resolvedBaseTypes.push(
                            baseType.asInstanceOf[ObjectType])
                      ...
                      error(
                        node,
                        Diagnostics.An_interface_may_only_extend_a_class_or_another_interface)
    ...
    def isIndependentInterface(symbol: Symbol): Boolean = {
      (symbol.declarations).foreach { fresh21 =>
        val declaration = zeroOfMyType = fresh21 {
          if ((declaration.kind === SyntaxKind.InterfaceDeclaration)) {
            if ((declaration.flags & NodeFlags.ContainsThis)) {
              return false

            }
            val baseTypeNodes = getInterfaceBaseTypeNodes(
              declaration.asInstanceOf[InterfaceDeclaration])
            if (baseTypeNodes) {
              (baseTypeNodes).foreach { fresh22 =>
                val node = zeroOfMyType = fresh22 {
                  if (isEntityNameExpression(node.expression)) {
                    val baseSymbol = resolveEntityName(
                      node.expression,
                      SymbolFlags.Type,
                      true)
                    if ((((!baseSymbol) || (!((baseSymbol.flags & SymbolFlags.Interface)))) || getDeclaredTypeOfClassOrInterface(
                          baseSymbol).thisType)) {
                      return false
    ...
    def getDeclaredTypeOfClassOrInterface(symbol: Symbol): InterfaceType = {
      ...
      if ((!links.declaredType)) {
        val kind =
          (if ((symbol.flags & SymbolFlags.Class)) ObjectFlags.Class
           else ObjectFlags.Interface)
        val `type` = (links.declaredType =
          createObjectType(kind, symbol).asInstanceOf[InterfaceType])
        val outerTypeParameters = getOuterTypeParametersOfClassOrInterface(
          symbol)
        val localTypeParameters =
          getLocalTypeParametersOfClassOrInterfaceOrTypeAlias(symbol)
        if ((((outerTypeParameters || localTypeParameters) || (kind === ObjectFlags.Class)) || (!isIndependentInterface(
              symbol)))) {
          (`type`.objectFlags |= ObjectFlags.Reference)
          (`type`.typeParameters =
            concatenate(outerTypeParameters, localTypeParameters))
          (`type`.outerTypeParameters = outerTypeParameters)
          (`type`.localTypeParameters = localTypeParameters)
          ((`type`.asInstanceOf[GenericType]).instantiations =
            createMap[TypeReference]())
          ((`type`
            .asInstanceOf[GenericType])
            .instantiations(getTypeListId(`type`.typeParameters)) =
              `type`.asInstanceOf[GenericType])
          ((`type`.asInstanceOf[GenericType]).target =
            `type`.asInstanceOf[GenericType])
          ((`type`.asInstanceOf[GenericType]).typeArguments =
            `type`.typeParameters)
    ...
    def getDeclaredTypeOfTypeAlias(symbol: Symbol): Type = {
      ...
      if ((!links.declaredType)) {
        ...
        val typeParameters =
          getLocalTypeParametersOfClassOrInterfaceOrTypeAlias(symbol)
        var declaration: (JSDocTypedefTag | TypeAliasDeclaration) =
          getDeclarationOfKind(symbol, SyntaxKind.JSDocTypedefTag)
        ...
        if (declaration) {
          ...
          (declaration =
            getDeclarationOfKind(symbol, SyntaxKind.TypeAliasDeclaration)
              .asInstanceOf[TypeAliasDeclaration])
          (`type` =
            getTypeFromTypeNode(declaration.`type`, symbol, typeParameters))
    ...
    def isLiteralEnumMember(symbol: Symbol, member: EnumMember) = {
      ...
      return (((expr.kind === SyntaxKind.NumericLiteral) || (((expr.kind === SyntaxKind.PrefixUnaryExpression) && ((expr
        .asInstanceOf[PrefixUnaryExpression])
        .operator === SyntaxKind.MinusToken)) && ((expr
        .asInstanceOf[PrefixUnaryExpression])
        .operand
        .kind === SyntaxKind.NumericLiteral))) || ((expr.kind === SyntaxKind.Identifier) && (!(!symbol
        .exports((expr.asInstanceOf[Identifier]).text)))))

    }
    def enumHasLiteralMembers(symbol: Symbol) = {
      (symbol.declarations).foreach { fresh23 =>
        val declaration = zeroOfMyType = fresh23 {
          if ((declaration.kind === SyntaxKind.EnumDeclaration)) {
            ((declaration.asInstanceOf[EnumDeclaration]).members).foreach {
              fresh24 =>
                val member = zeroOfMyType = fresh24 {
    ...
    def createEnumLiteralType(symbol: Symbol,
                              baseType: EnumType,
                              text: String) = {
      val `type` = createType(TypeFlags.EnumLiteral)
        .asInstanceOf[EnumLiteralType]
      (`type`.symbol = symbol)
    ...
    def getDeclaredTypeOfEnum(symbol: Symbol): Type = {
      val links = getSymbolLinks(symbol)
      if ((!links.declaredType)) {
        val enumType = (links.declaredType = createType(TypeFlags.Enum)
          .asInstanceOf[EnumType])
        ...
        if (enumHasLiteralMembers(symbol)) {
          ...
          (enumType.symbol.declarations).foreach { fresh25 =>
            val declaration = zeroOfMyType = fresh25 {
              if ((declaration.kind === SyntaxKind.EnumDeclaration)) {
                computeEnumMemberValues(
                  declaration.asInstanceOf[EnumDeclaration])
                ((declaration.asInstanceOf[EnumDeclaration]).members).foreach {
                  fresh26 =>
                    val member = zeroOfMyType = fresh26 {
                      val memberSymbol = getSymbolOfNode(member)
                      val value = getEnumMemberValue(member)
                      if ((!memberTypes(value))) {
                        val memberType =
                          (memberTypes(value) = createEnumLiteralType(
                            memberSymbol,
                            enumType,
                            ("" + value)))
          ...
          if ((memberTypeList.length > 1)) {
            ...
            ((enumType.asInstanceOf[(EnumType with UnionType)]).types =
              memberTypeList)
            (unionTypes(getTypeListId(memberTypeList)) =
              enumType.asInstanceOf[(EnumType with UnionType)])
    ...
    def getDeclaredTypeOfEnumMember(symbol: Symbol): Type = {
      val links = getSymbolLinks(symbol)
      if ((!links.declaredType)) {
        val enumType = getDeclaredTypeOfEnum(getParentOfSymbol(symbol))
          .asInstanceOf[EnumType]
        (links.declaredType =
          (if ((enumType.flags & TypeFlags.Union))
             enumType.memberTypes(
               getEnumMemberValue(
                 symbol.valueDeclaration.asInstanceOf[EnumMember]))
           else enumType))
    ...
    def getDeclaredTypeOfTypeParameter(symbol: Symbol): TypeParameter = {
      val links = getSymbolLinks(symbol)
      if ((!links.declaredType)) {
        val `type` = createType(TypeFlags.TypeParameter)
          .asInstanceOf[TypeParameter]
        (`type`.symbol = symbol)
        if ((!(getDeclarationOfKind(symbol, SyntaxKind.TypeParameter)
              .asInstanceOf[TypeParameterDeclaration])
              .constraint)) {
    ...
    def isIndependentTypeReference(node: TypeReferenceNode): Boolean = {
      if (node.typeArguments) {
        (node.typeArguments).foreach { fresh27 =>
          val typeNode = zeroOfMyType = fresh27 {
    ...
    def isIndependentType(node: TypeNode): Boolean = {
      node.kind match {
        case SyntaxKind.AnyKeyword | SyntaxKind.StringKeyword |
            ...
            SyntaxKind.NeverKeyword | SyntaxKind.LiteralType =>
          return true
        case SyntaxKind.ArrayType =>
          return isIndependentType(
            (node.asInstanceOf[ArrayTypeNode]).elementType)
        case SyntaxKind.TypeReference =>
          return isIndependentTypeReference(
    ...
    def isIndependentVariableLikeDeclaration(
        node: VariableLikeDeclaration): Boolean = {
      return ((node.`type` && isIndependentType(node.`type`)) || ((!node.`type`) && (!node.initializer)))

    }
    def isIndependentFunctionLikeDeclaration(
        node: FunctionLikeDeclaration): Boolean = {
      if (((node.kind !== SyntaxKind.Constructor) && (((!node.`type`) || (!isIndependentType(
            node.`type`)))))) {
        return false

      }
      (node.parameters).foreach { fresh28 =>
        val parameter = zeroOfMyType = fresh28 {
      ...
      if ((symbol.declarations && (symbol.declarations.length === 1))) {
        ...
        if (declaration) {
          declaration.kind match {
            case SyntaxKind.PropertyDeclaration |
                SyntaxKind.PropertySignature =>
              return isIndependentVariableLikeDeclaration(
                declaration.asInstanceOf[VariableLikeDeclaration])
            case SyntaxKind.MethodDeclaration | SyntaxKind.MethodSignature |
                SyntaxKind.Constructor =>
              return isIndependentFunctionLikeDeclaration(
                declaration.asInstanceOf[FunctionLikeDeclaration])
            case _ =>
          }
    ...
    def createSymbolTable(symbols: Array[Symbol]): SymbolTable = {
      ...
      (symbols).foreach { fresh29 =>
        val symbol = zeroOfMyType = fresh29 {
    ...
    def createInstantiatedSymbolTable(
        symbols: Array[Symbol],
        mapper: TypeMapper,
        mappingThisOnly: Boolean): SymbolTable = {
      val result = createMap[Symbol]()
      (symbols).foreach { fresh30 =>
        val symbol = zeroOfMyType = fresh30 {
          (result(symbol.name) =
            (if ((mappingThisOnly && isIndependentMember(symbol))) symbol
             else instantiateSymbol(symbol, mapper)))
    ...
    def resolveDeclaredMembers(
        `type`: InterfaceType): InterfaceTypeWithDeclaredMembers = {
      if ((!(`type`
            .asInstanceOf[InterfaceTypeWithDeclaredMembers])
            .declaredProperties)) {
        val symbol = `type`.symbol
        ((`type`
          .asInstanceOf[InterfaceTypeWithDeclaredMembers])
        ...
        ((`type`
          ...
          getIndexInfoOfSymbol(symbol, IndexKind.Number))
    ...
    def getTypeWithThisArgument(`type`: Type, thisArgument: Type) = {
      if ((getObjectFlags(`type`) & ObjectFlags.Reference)) {
        return createTypeReference(
          (`type`.asInstanceOf[TypeReference]).target,
          concatenate(
            (`type`.asInstanceOf[TypeReference]).typeArguments,
            Array(
              (thisArgument || (`type`
                .asInstanceOf[TypeReference])
                .target
    ...
    def resolveObjectTypeMembers(`type`: ObjectType,
      ...
      if (rangeEquals(typeParameters, typeArguments, 0, typeParameters.length)) {
        ...
        (members =
          (if (source.symbol) source.symbol.members
           else createSymbolTable(source.declaredProperties)))
        ...
        (callSignatures = instantiateList(
          source.declaredCallSignatures,
          mapper,
          instantiateSignature))
        (constructSignatures = instantiateList(
          source.declaredConstructSignatures,
          mapper,
          instantiateSignature))
        (stringIndexInfo =
          instantiateIndexInfo(source.declaredStringIndexInfo, mapper))
        (numberIndexInfo =
          instantiateIndexInfo(source.declaredNumberIndexInfo, mapper))
      ...
      if (baseTypes.length) {
        ...
        (baseTypes).foreach { fresh32 =>
          val baseType = zeroOfMyType = fresh32 {
            val instantiatedBaseType =
              (if (thisArgument)
                 getTypeWithThisArgument(
                   instantiateType(baseType, mapper).asInstanceOf[ObjectType],
                   thisArgument)
               else baseType)
            addInheritedMembers(
              members,
              getPropertiesOfObjectType(instantiatedBaseType))
            ...
            (constructSignatures = concatenate(
              ...
              getSignaturesOfType(
                ...
                SignatureKind.Construct)))
            (stringIndexInfo = (stringIndexInfo || getIndexInfoOfType(
                instantiatedBaseType,
                IndexKind.String)))
    ...
    def resolveTypeReferenceMembers(`type`: TypeReference): Unit = {
      ...
      val typeParameters =
        concatenate(source.typeParameters, Array(source.thisType))
      val typeArguments =
        (if ((`type`.typeArguments && (`type`.typeArguments.length === typeParameters.length)))
           `type`.typeArguments
         else concatenate(`type`.typeArguments, Array(`type`)))
      resolveObjectTypeMembers(`type`, source, typeParameters, typeArguments)

    }
    def createSignature(declaration: SignatureDeclaration,
                        typeParameters: Array[TypeParameter],
                        thisParameter: (Symbol | undefined),
                        parameters: Array[Symbol],
    ...
    def getDefaultConstructSignatures(
        classType: InterfaceType): Array[Signature] = {
      ...
      val typeArguments =
        map(baseTypeNode.typeArguments, getTypeFromTypeNodeNoAlias)
      ...
      (baseSignatures).foreach { fresh33 =>
        val baseSig = zeroOfMyType = fresh33 {
          val typeParamCount =
            (if (baseSig.typeParameters) baseSig.typeParameters.length else 0)
          if ((typeParamCount === typeArgCount)) {
            val sig =
              (if (typeParamCount)
                 getSignatureInstantiation(baseSig, typeArguments)
               else cloneSignature(baseSig))
    ...
    def findMatchingSignature(signatureList: Array[Signature],
                              signature: Signature,
                              partialMatch: Boolean,
                              ignoreThisTypes: Boolean,
                              ignoreReturnTypes: Boolean): Signature = {
      (signatureList).foreach { fresh34 =>
        val s = zeroOfMyType = fresh34 {
    ...
    def findMatchingSignatures(signatureLists: Array[Array[Signature]],
                               signature: Signature,
                               listIndex: Int): Array[Signature] = {
    ...
    def getUnionSignatures(types: Array[Type],
        ...
        while ((i < signatureLists.length)) {
          {
            (signatureLists(i)).foreach { fresh35 =>
              val signature = zeroOfMyType = fresh35 {
                if (((!result) || (!findMatchingSignature(
                      ...
                      true)))) {
                  val unionSignatures =
                    findMatchingSignatures(signatureLists, signature, i)
                  if (unionSignatures) {
                    var s = signature
                    if ((unionSignatures.length > 1)) {
                      (s = cloneSignature(signature))
                      if (forEach(unionSignatures, (sig => sig.thisParameter))) {
                        val thisType = getUnionType(
                          map(
                            unionSignatures,
                            (sig =>
                               (getTypeOfSymbol(sig.thisParameter) || anyType))),
                          true)
                        (s.thisParameter = createTransientSymbol(
                          signature.thisParameter,
                          thisType))
    ...
    def resolveUnionTypeMembers(`type`: UnionType) = {
      ...
      val constructSignatures =
        getUnionSignatures(`type`.types, SignatureKind.Construct)
    ...
    def intersectTypes(type1: Type, type2: Type): Type = {
      return (if ((!type1)) type2
                    ...
                    else getIntersectionType(Array(type1, type2))))

    }
    def intersectIndexInfos(info1: IndexInfo, info2: IndexInfo): IndexInfo = {
      return (if ((!info1)) info2
              else (if ((!info2)) info1
                    else createIndexInfo(getIntersectionType(Array(info1.`type`, info2.`type`)), (info1.isReadonly && info2.isReadonly))))
    ...
    def resolveIntersectionTypeMembers(`type`: IntersectionType) = {
      ...
      (`type`.types).foreach { fresh37 =>
        val t = zeroOfMyType = fresh37 {
          (callSignatures = concatenate(
            callSignatures,
            getSignaturesOfType(t, SignatureKind.Call)))
          (constructSignatures = concatenate(
            constructSignatures,
            getSignaturesOfType(t, SignatureKind.Construct)))
    ...
    def resolveAnonymousTypeMembers(`type`: AnonymousType) = {
      val symbol = `type`.symbol
      if (`type`.target) {
        val members = createInstantiatedSymbolTable(
          getPropertiesOfObjectType(`type`.target),
          `type`.mapper,
          false)
        val callSignatures = instantiateList(
          getSignaturesOfType(`type`.target, SignatureKind.Call),
          `type`.mapper,
          instantiateSignature)
        val constructSignatures = instantiateList(
          getSignaturesOfType(`type`.target, SignatureKind.Construct),
          `type`.mapper,
          instantiateSignature)
        val stringIndexInfo = instantiateIndexInfo(
          getIndexInfoOfType(`type`.target, IndexKind.String),
          `type`.mapper)
        val numberIndexInfo = instantiateIndexInfo(
          getIndexInfoOfType(`type`.target, IndexKind.Number),
          `type`.mapper)
        setStructuredTypeMembers(
          `type`,
          members,
        ...
        if ((symbol.flags & SymbolFlags.Class)) {
          ...
          (constructSignatures = getSignaturesOfSymbol(
            symbol.members("___constructor")))
          ...
          if ((baseConstructorType.flags & TypeFlags.Object)) {
            ...
            addInheritedMembers(
              members,
              getPropertiesOfObjectType(baseConstructorType))

          }

        }
        val numberIndexInfo =
          (if ((symbol.flags & SymbolFlags.Enum)) enumNumberIndexInfo
           else undefined)
        setStructuredTypeMembers(
          `type`,
          members,
        ...
        if ((symbol.flags & ((SymbolFlags.Function | SymbolFlags.Method)))) {
          ((`type`.asInstanceOf[ResolvedType]).callSignatures =
    ...
    def getPropertyOfObjectType(`type`: Type, name: String): Symbol = {
      if ((`type`.flags & TypeFlags.Object)) {
        val resolved = resolveStructuredTypeMembers(
          `type`.asInstanceOf[ObjectType])
    ...
    def getPropertiesOfUnionOrIntersectionType(
        `type`: UnionOrIntersectionType): Array[Symbol] = {
      (`type`.types).foreach { fresh38 =>
        val current = zeroOfMyType = fresh38 {
          (getPropertiesOfType(current)).foreach { fresh39 =>
            val prop = zeroOfMyType = fresh39 {
      ...
      if (props) {
        ...
        (props).keys.foreach { fresh40 =>
          val key = zeroOfMyType = fresh40 {
            val prop = props(key)
            if ((!(((prop.flags & SymbolFlags.SyntheticProperty) && (prop
                  .asInstanceOf[TransientSymbol])
                  .isPartial)))) {
              result.push(prop)
    ...
    def getPropertiesOfType(`type`: Type): Array[Symbol] = {
      ...
      return (if ((`type`.flags & TypeFlags.UnionOrIntersection))
                getPropertiesOfUnionOrIntersectionType(
                  `type`.asInstanceOf[UnionType])
              else getPropertiesOfObjectType(`type`))
    ...
    def getApparentTypeOfTypeParameter(`type`: TypeParameter) = {
      if ((!`type`.resolvedApparentType)) {
        ...
        while ((constraintType && (constraintType.flags & TypeFlags.TypeParameter))) {
          {
            (constraintType = getConstraintOfTypeParameter(
              constraintType.asInstanceOf[TypeParameter]))
    ...
    def getApparentType(`type`: Type): Type = {
      if ((`type`.flags & TypeFlags.TypeParameter)) {
        (`type` = getApparentTypeOfTypeParameter(
          `type`.asInstanceOf[TypeParameter]))
    ...
    def createUnionOrIntersectionProperty(
      ...
      val result = createSymbol(
        (((SymbolFlags.Property | SymbolFlags.Transient) | SymbolFlags.SyntheticProperty) | commonFlags),
        name).asInstanceOf[TransientSymbol]
      ...
      (result.`type` =
        (if ((containingType.flags & TypeFlags.Union)) getUnionType(propTypes)
         else getIntersectionType(propTypes)))
      return result

    }
    def getUnionOrIntersectionProperty(`type`: UnionOrIntersectionType,
                                       name: String): Symbol = {
    ...
    def getPropertyOfUnionOrIntersectionType(`type`: UnionOrIntersectionType,
                                             name: String): Symbol = {
      val property = getUnionOrIntersectionProperty(`type`, name)
      return (if ((property && (!(((property.flags & SymbolFlags.SyntheticProperty) && (property
                    .asInstanceOf[TransientSymbol])
                    .isPartial))))) property
              else undefined)
    ...
    def getSignaturesOfStructuredType(
        `type`: Type,
        kind: SignatureKind): Array[Signature] = {
      if ((`type`.flags & TypeFlags.StructuredType)) {
        val resolved = resolveStructuredTypeMembers(
          `type`.asInstanceOf[ObjectType])
        return (if ((kind === SignatureKind.Call)) resolved.callSignatures
                else resolved.constructSignatures)
    ...
    def getIndexInfoOfStructuredType(`type`: Type,
                                     kind: IndexKind): IndexInfo = {
      if ((`type`.flags & TypeFlags.StructuredType)) {
        val resolved = resolveStructuredTypeMembers(
          `type`.asInstanceOf[ObjectType])
        return (if ((kind === IndexKind.String)) resolved.stringIndexInfo
                else resolved.numberIndexInfo)
    ...
    def getImplicitIndexTypeOfType(`type`: Type, kind: IndexKind): Type = {
      if (isObjectLiteralType(`type`)) {
        ...
        (getPropertiesOfType(`type`)).foreach { fresh43 =>
          val prop = zeroOfMyType = fresh43 {
            if (((kind === IndexKind.String) || isNumericLiteralName(
                  prop.name))) {
    ...
    def getTypeParametersFromJSDocTemplate(
        declaration: SignatureDeclaration): Array[TypeParameter] = {
      if ((declaration.flags & NodeFlags.JavaScriptFile)) {
    ...
    def getTypeParametersFromDeclaration(
        typeParameterDeclarations: Array[TypeParameterDeclaration])
      : Array[TypeParameter] = {
    ...
    def isOptionalParameter(node: ParameterDeclaration) = {
      ...
      if (node.initializer) {
        val signatureDeclaration =
          node.parent.asInstanceOf[SignatureDeclaration]
    ...
    def createTypePredicateFromTypePredicateNode(node: TypePredicateNode)
      : (IdentifierTypePredicate | ThisTypePredicate) = {
      if ((node.parameterName.kind === SyntaxKind.Identifier)) {
        val parameterName = node.parameterName.asInstanceOf[Identifier]
        return Map(
          "kind" -> TypePredicateKind.Identifier,
          "parameterName" -> (if (parameterName) parameterName.text
                              else undefined),
          "parameterIndex" -> (if (parameterName)
                                 getTypePredicateParameterIndex(
                                   (node.parent
                                     .asInstanceOf[SignatureDeclaration])
                                     .parameters,
                                   parameterName)
                               else undefined),
          "type" -> getTypeFromTypeNode(node.`type`))
          .asInstanceOf[IdentifierTypePredicate]

      } else {
        return Map(
          "kind" -> TypePredicateKind.This,
          "type" -> getTypeFromTypeNode(node.`type`))
          .asInstanceOf[ThisTypePredicate]
    ...
    def getSignatureFromDeclaration(
      ...
      if ((!links.resolvedSignature)) {
          ...
          while ((i < n)) {
            {
              ...
              if (((paramSymbol && (!(!((paramSymbol.flags & SymbolFlags.Property))))) && (!isBindingPattern(
                    param.name)))) {
                val resolvedSymbol = resolveName(
                  param,
                  paramSymbol.name,
                  SymbolFlags.Value,
                  undefined,
              ...
              if ((((param.initializer || param.questionToken) || param.dotDotDotToken) || isJSDocOptionalParameter(
                    param))) {
                if ((minArgumentCount < 0)) {
                  (minArgumentCount = (i - ((if (hasThisParameter) 1
                                             else 0) )))
        ...
        if ((((((declaration.kind === SyntaxKind.GetAccessor) || (declaration.kind === SyntaxKind.SetAccessor))) && (!hasDynamicName(
              declaration))) && (((!hasThisParameter) || (!thisParameter))))) {
          val otherKind =
            (if ((declaration.kind === SyntaxKind.GetAccessor))
               SyntaxKind.SetAccessor
             else SyntaxKind.GetAccessor)
          val other = getDeclarationOfKind(declaration.symbol, otherKind)
            .asInstanceOf[AccessorDeclaration]
          if (other) {
        ...
        if ((minArgumentCount < 0)) {
          (minArgumentCount = (declaration.parameters.length - ((if (hasThisParameter)
                                                                   1
                                                                 else 0) )))
        ...
        val classType =
          (if ((declaration.kind === SyntaxKind.Constructor))
             getDeclaredTypeOfClassOrInterface(
               getMergedSymbol(
                 (declaration.parent.asInstanceOf[ClassDeclaration]).symbol))
           else undefined)
        val typeParameters =
          (if (classType) classType.localTypeParameters
           else
             (if (declaration.typeParameters)
                getTypeParametersFromDeclaration(declaration.typeParameters)
              else getTypeParametersFromJSDocTemplate(declaration)))
        val returnType = getSignatureReturnTypeFromDeclaration(
          declaration,
          isJSConstructSignature,
          classType)
        val typePredicate =
          (if ((declaration.`type` && (declaration.`type`.kind === SyntaxKind.TypePredicate)))
             createTypePredicateFromTypePredicateNode(
               declaration.`type`.asInstanceOf[TypePredicateNode])
           else undefined)
    ...
    def getSignatureReturnTypeFromDeclaration(
        declaration: SignatureDeclaration,
        isJSConstructSignature: Boolean,
        classType: Type) = {
      if (isJSConstructSignature) {
      ...
      if (((declaration.kind === SyntaxKind.GetAccessor) && (!hasDynamicName(
            declaration)))) {
        val setter =
          getDeclarationOfKind(declaration.symbol, SyntaxKind.SetAccessor)
            .asInstanceOf[AccessorDeclaration]
        return getAnnotatedAccessorType(setter)

      }
      if (nodeIsMissing(
            (declaration.asInstanceOf[FunctionLikeDeclaration]).body)) {
        return anyType
        ...
        while ((i < len)) {
          {
            ...
            node.kind match {
              case SyntaxKind.FunctionType | SyntaxKind.ConstructorType |
                  ...
                  SyntaxKind.MethodDeclaration | SyntaxKind.MethodSignature |
                  SyntaxKind.Constructor | SyntaxKind.CallSignature |
    ...
    def getReturnTypeOfSignature(signature: Signature): Type = {
      if ((!signature.resolvedReturnType)) {
        ...
        if (signature.target) {
          ...
          (`type` = getUnionType(
            map(signature.unionSignatures, getReturnTypeOfSignature),
            true))

        } else {
          (`type` = getReturnTypeFromBody(
            signature.declaration.asInstanceOf[FunctionLikeDeclaration]))
    ...
    def getRestTypeOfSignature(signature: Signature): Type = {
      if (signature.hasRestParameter) {
        ...
        if (((getObjectFlags(`type`) & ObjectFlags.Reference) && ((`type`
              .asInstanceOf[TypeReference])
              .target === globalArrayType))) {
    ...
    def getSignatureInstantiation(signature: Signature,
                                  typeArguments: Array[Type]): Signature = {
    ...
    def getErasedSignature(signature: Signature): Signature = {
      ...
      if ((!signature.erasedSignatureCache)) {
        (signature.erasedSignatureCache = instantiateSignature(
          signature,
    ...
    def getOrCreateTypeFromSignature(signature: Signature): ObjectType = {
      if ((!signature.isolatedSignatureType)) {
        ...
        val `type` = createObjectType(ObjectFlags.Anonymous)
          .asInstanceOf[ResolvedType]
        (`type`.members = emptySymbols)
        (`type`.properties = emptyArray)
        (`type`.callSignatures =
          (if ((!isConstructor)) Array(signature) else emptyArray))
    ...
    def getIndexDeclarationOfSymbol(symbol: Symbol,
                                    kind: IndexKind): SignatureDeclaration = {
      val syntaxKind =
        (if ((kind === IndexKind.Number)) SyntaxKind.NumberKeyword
         else SyntaxKind.StringKeyword)
      val indexSymbol = getIndexSymbol(symbol)
      if (indexSymbol) {
        (indexSymbol.declarations).foreach { fresh45 =>
          val decl = zeroOfMyType = fresh45 {
    ...
    def createIndexInfo(`type`: Type,
                        isReadonly: Boolean,
                        declaration: SignatureDeclaration): IndexInfo = {
      return Map(
        "type" -> `type`,
    ...
    def getIndexInfoOfSymbol(symbol: Symbol, kind: IndexKind): IndexInfo = {
      ...
      if (declaration) {
        return createIndexInfo(
          (if (declaration.`type`) getTypeFromTypeNode(declaration.`type`)
           else anyType),
          (((getModifierFlags(declaration) & ModifierFlags.Readonly)) !== 0),
          declaration)
    ...
    def getConstraintDeclaration(`type`: TypeParameter) = {
      return (getDeclarationOfKind(`type`.symbol, SyntaxKind.TypeParameter)
        .asInstanceOf[TypeParameterDeclaration])
        .constraint

    }
    def hasConstraintReferenceTo(`type`: Type,
                                 target: TypeParameter): Boolean = {
      var checked: Array[Type] = zeroOfMyType
      while ((((`type` && (`type`.flags & TypeFlags.TypeParameter)) && (!((`type`
               .asInstanceOf[TypeParameter])
               .isThisType))) && (!contains(checked, `type`)))) {
        {
          if ((`type` === target)) {
            return true
          ...
          val constraintDeclaration = getConstraintDeclaration(
            `type`.asInstanceOf[TypeParameter])
          (`type` = (constraintDeclaration && getTypeFromTypeNode(
    ...
    def getConstraintOfTypeParameter(typeParameter: TypeParameter): Type = {
      if ((!typeParameter.constraint)) {
        if (typeParameter.target) {
          val targetConstraint = getConstraintOfTypeParameter(
            typeParameter.target)
          (typeParameter.constraint =
            (if (targetConstraint)
               instantiateType(targetConstraint, typeParameter.mapper)
             else noConstraintType))
      ...
      return (if ((typeParameter.constraint === noConstraintType)) undefined
              else typeParameter.constraint)

    }
    def getParentSymbolOfTypeParameter(typeParameter: TypeParameter): Symbol = {
      return getSymbolOfNode(getDeclarationOfKind(
        typeParameter.symbol,
        SyntaxKind.TypeParameter).parent)
    ...
    def getPropagatingFlagsOfTypes(types: Array[Type],
                                   excludeKinds: TypeFlags): TypeFlags = {
      var result: TypeFlags = 0
      (types).foreach { fresh46 =>
        val `type` = zeroOfMyType = fresh46 {
    ...
    def createTypeReference(target: GenericType,
                            typeArguments: Array[Type]): TypeReference = {
      val id = getTypeListId(typeArguments)
      var `type` = target.instantiations(id)
      if ((!`type`)) {
        (`type` = (target.instantiations(id) =
          createObjectType(ObjectFlags.Reference, target.symbol)
            .asInstanceOf[TypeReference]))
        (`type`.flags |= (if (typeArguments)
                            getPropagatingFlagsOfTypes(typeArguments, 0)
                          else 0))
        (`type`.target = target)
    ...
    def getTypeReferenceArity(`type`: TypeReference): Int = {
      return (if (`type`.target.typeParameters)
                `type`.target.typeParameters.length
              else 0)

    }
    def getTypeFromClassOrInterfaceReference(
        node: (TypeReferenceNode | ExpressionWithTypeArguments | JSDocTypeReference),
        symbol: Symbol): Type = {
      val `type` = getDeclaredTypeOfSymbol(getMergedSymbol(symbol))
        .asInstanceOf[InterfaceType]
      ...
      if (typeParameters) {
        ...
        return createTypeReference(
          ...
          concatenate(
            ...
            map(node.typeArguments, getTypeFromTypeNodeNoAlias)))
    ...
    def getTypeFromTypeAliasReference(
        node: (TypeReferenceNode | ExpressionWithTypeArguments | JSDocTypeReference),
        symbol: Symbol): Type = {
      ...
      if (typeParameters) {
        ...
        val id = getTypeListId(typeArguments)
        return (links.instantiations(id) || ((
          links.instantiations(id) = instantiateType(
            `type`,
    ...
    def getTypeFromNonGenericTypeReference(
        node: (TypeReferenceNode | ExpressionWithTypeArguments | JSDocTypeReference),
        symbol: Symbol): Type = {
      if (node.typeArguments) {
        error(node, Diagnostics.Type_0_is_not_generic, symbolToString(symbol))
    ...
    def getTypeFromTypeReference(
      ...
      if ((!links.resolvedType)) {
        ...
        if ((node.kind === SyntaxKind.JSDocTypeReference)) {
          ...
          val typeNameOrExpression: EntityNameOrEntityNameExpression =
            (if ((node.kind === SyntaxKind.TypeReference))(node
          ...
          (symbol = ((typeNameOrExpression && resolveEntityName(
              typeNameOrExpression,
              SymbolFlags.Type)) || unknownSymbol))
          (`type` =
            (if ((symbol === unknownSymbol)) unknownType
             else
               (if ((symbol.flags & ((SymbolFlags.Class | SymbolFlags.Interface))))
                  ...
                  (if ((symbol.flags & SymbolFlags.TypeAlias))
                   ...
                   else getTypeFromNonGenericTypeReference(node, symbol)))))
    ...
    def getTypeOfGlobalSymbol(symbol: Symbol, arity: Int): ObjectType = {
      def getTypeDeclaration(symbol: Symbol): Declaration = {
        ...
        (declarations).foreach { fresh47 =>
          val declaration = zeroOfMyType = fresh47 {
            declaration.kind match {
      ...
      if ((((if ((`type`.asInstanceOf[InterfaceType]).typeParameters)
               (`type`.asInstanceOf[InterfaceType]).typeParameters.length
             else 0) ) !== arity)) {
        error(
          getTypeDeclaration(symbol),
    ...
    def getGlobalSymbol(name: String,
                        meaning: SymbolFlags,
                        diagnostic: DiagnosticMessage): Symbol = {
    ...
    def getExportedTypeFromNamespace(namespace: String, name: String): Type = {
      val namespaceSymbol =
        getGlobalSymbol(namespace, SymbolFlags.Namespace, undefined)
      val typeSymbol = (namespaceSymbol && getSymbol(
    ...
    def createTypedPropertyDescriptorType(propertyType: Type): Type = {
      val globalTypedPropertyDescriptorType =
        getGlobalTypedPropertyDescriptorType()
      return (if ((globalTypedPropertyDescriptorType !== emptyGenericType))
                createTypeReference(
                  globalTypedPropertyDescriptorType.asInstanceOf[GenericType],
                  Array(propertyType))
              else emptyObjectType)

    }
    def createTypeFromGenericGlobalType(
        genericGlobalType: GenericType,
        typeArguments: Array[Type]): ObjectType = {
      return (if ((genericGlobalType !== emptyGenericType))
    ...
    def getTypeFromArrayTypeNode(node: ArrayTypeNode): Type = {
      val links = getNodeLinks(node)
      if ((!links.resolvedType)) {
        (links.resolvedType = createArrayType(
    ...
    def createTupleTypeOfArity(arity: Int): GenericType = {
      ...
      val properties: Array[Symbol] = Array() {
        ...
        while ((i < arity)) {
          {
            val typeParameter =
              createType(TypeFlags.TypeParameter).asInstanceOf[TypeParameter]
            typeParameters.push(typeParameter)
            val property = createSymbol(
              (SymbolFlags.Property | SymbolFlags.Transient),
              ("" + i)).asInstanceOf[TransientSymbol]
      ...
      val `type` = createObjectType(
        (ObjectFlags.Tuple | ObjectFlags.Reference))
        .asInstanceOf[(GenericType with InterfaceTypeWithDeclaredMembers)]
      (`type`.typeParameters = typeParameters)
      (`type`.outerTypeParameters = undefined)
      (`type`.localTypeParameters = typeParameters)
      (`type`.instantiations = createMap[TypeReference]())
      (`type`.instantiations(getTypeListId(`type`.typeParameters)) =
        `type`.asInstanceOf[GenericType])
    ...
    def getTupleTypeOfArity(arity: Int): GenericType = {
      return (tupleTypes(arity) || ((
        tupleTypes(arity) = createTupleTypeOfArity(arity))))

    }
    def createTupleType(elementTypes: Array[Type]) = {
      return createTypeReference(
        getTupleTypeOfArity(elementTypes.length),
        elementTypes)
    ...
    def getTypeFromTupleTypeNode(node: TupleTypeNode): Type = {
      val links = getNodeLinks(node)
      if ((!links.resolvedType)) {
        (links.resolvedType = createTupleType(
          map(node.elementTypes, getTypeFromTypeNodeNoAlias)))
    ...
    def addTypeToUnion(typeSet: TypeSet, `type`: Type) = {
      ...
      if ((flags & TypeFlags.Union)) {
        ...
        val index =
          (if ((len && (`type`.id > typeSet((len - 1)).id))) (~len)
           else binarySearchTypes(typeSet, `type`))
        if ((index < 0)) {
          if ((!((((((flags & TypeFlags.Object) && ((`type`
                .asInstanceOf[ObjectType])
                .objectFlags & ObjectFlags.Anonymous)) && `type`.symbol) && (`type`.symbol.flags & ((SymbolFlags.Function | SymbolFlags.Method)))) && containsIdenticalType(
                typeSet,
                `type`))))) {
    ...
    def isSubtypeOfAny(candidate: Type, types: Array[Type]): Boolean = {
      {
        ...
        while ((i < len)) {
          {
            if (((candidate !== types(i)) && isTypeSubtypeOf(
                  candidate,
                  types(i)))) {
              return true
      ...
      if ((first.flags & TypeFlags.EnumLiteral)) {
        val firstEnum = getParentOfSymbol(first.symbol) {
          ...
          while ((i < types.length)) {
            {
              val other = types(i)
              if (((!((other.flags & TypeFlags.EnumLiteral))) || ((firstEnum !== getParentOfSymbol(
                    other.symbol))))) {
                return false
      ...
      while ((i > 0)) {
        {
          (i -= 1)
          val t = types(i)
          val remove = ((((t.flags & TypeFlags.StringLiteral) && types.containsString) || ((t.flags & TypeFlags.NumberLiteral) && types.containsNumber)) || (((t.flags & TypeFlags.StringOrNumberLiteral) && (t.flags & TypeFlags.FreshLiteral)) && containsType(
              types,
              (t.asInstanceOf[LiteralType]).regularType)))
          if (remove) {
            orderedRemoveItemAt(types, i)
    ...
    def getUnionType(types: Array[Type],
                     subtypeReduction: Boolean,
                     aliasSymbol: Symbol,
                     aliasTypeArguments: Array[Type]): Type = {
      if ((types.length === 0)) {
        return neverType
      ...
      if ((typeSet.length === 0)) {
        return (if (typeSet.containsNull)
                  (if (typeSet.containsNonWideningType) nullType
                   else nullWideningType)
                else
                  (if (typeSet.containsUndefined)
                     (if (typeSet.containsNonWideningType) undefinedType
                      else undefinedWideningType)
                   else neverType))
    ...
    def getUnionTypeFromSortedList(types: Array[Type],
                                   aliasSymbol: Symbol,
                                   aliasTypeArguments: Array[Type]): Type = {
      if ((types.length === 0)) {
        return neverType
      ...
      if ((!`type`)) {
        val propagatedFlags =
          getPropagatingFlagsOfTypes(types, TypeFlags.Nullable)
        (`type` = (unionTypes(id) = createType(
          (TypeFlags.Union | propagatedFlags)).asInstanceOf[UnionType]))
        (`type`.types = types)
    ...
    def getTypeFromUnionTypeNode(node: UnionTypeNode,
                                 aliasSymbol: Symbol,
                                 aliasTypeArguments: Array[Type]): Type = {
      val links = getNodeLinks(node)
      if ((!links.resolvedType)) {
        (links.resolvedType = getUnionType(
          map(node.types, getTypeFromTypeNodeNoAlias),
          false,
          aliasSymbol,
          aliasTypeArguments))
    ...
    def addTypeToIntersection(typeSet: TypeSet, `type`: Type) = {
      if ((`type`.flags & TypeFlags.Intersection)) {
        addTypesToIntersection(
          typeSet,
          (`type`.asInstanceOf[IntersectionType]).types)

      } else if ((`type`.flags & TypeFlags.Any)) {
        (typeSet.containsAny = true)

      } else if ((((!((`type`.flags & TypeFlags.Never))) && ((strictNullChecks || (!((`type`.flags & TypeFlags.Nullable)))))) && (!contains(
                   typeSet,
                   `type`)))) {
        typeSet.push(`type`)
    ...
    def getIntersectionType(types: Array[Type],
                            aliasSymbol: Symbol,
                            aliasTypeArguments: Array[Type]): Type = {
      if ((types.length === 0)) {
        ...
        while ((i < types.length)) {
          {
            ...
            if ((`type`.flags & TypeFlags.Union)) {
              return getUnionType(
                map(
                  (`type`.asInstanceOf[UnionType]).types,
                  (t => getIntersectionType(replaceElement(types, i, t)))),
                false,
                aliasSymbol,
                aliasTypeArguments)
      ...
      if ((!`type`)) {
        val propagatedFlags =
          getPropagatingFlagsOfTypes(typeSet, TypeFlags.Nullable)
        (`type` = (intersectionTypes(id) = createType(
          (TypeFlags.Intersection | propagatedFlags))
          .asInstanceOf[IntersectionType]))
        (`type`.types = typeSet)
        (`type`.aliasSymbol = aliasSymbol)
        (`type`.aliasTypeArguments = aliasTypeArguments)
    ...
    def getTypeFromIntersectionTypeNode(
    ...
    def getTypeFromTypeLiteralOrFunctionOrConstructorTypeNode(
        node: Node,
        aliasSymbol: Symbol,
        aliasTypeArguments: Array[Type]): Type = {
      val links = getNodeLinks(node)
    ...
    def getFreshTypeOfLiteralType(`type`: Type) = {
      if (((`type`.flags & TypeFlags.StringOrNumberLiteral) && (!((`type`.flags & TypeFlags.FreshLiteral))))) {
        if ((!(`type`.asInstanceOf[LiteralType]).freshType)) {
          val freshType = createLiteralType(
            (`type`.flags | TypeFlags.FreshLiteral),
            (`type`.asInstanceOf[LiteralType]).text).asInstanceOf[LiteralType]
    ...
    def getRegularTypeOfLiteralType(`type`: Type) = {
      return (if (((`type`.flags & TypeFlags.StringOrNumberLiteral) && (`type`.flags & TypeFlags.FreshLiteral)))(`type`
                .asInstanceOf[LiteralType])
                .regularType
              else `type`)
    ...
    def getLiteralTypeForText(flags: TypeFlags, text: String) = {
      val map =
        (if ((flags & TypeFlags.StringLiteral)) stringLiteralTypes
         else numericLiteralTypes)
    ...
    def getTypeFromJSDocVariadicType(node: JSDocVariadicType): Type = {
      ...
      if ((!links.resolvedType)) {
        ...
        (links.resolvedType =
          (if (`type`) createArrayType(`type`) else unknownType))
    ...
    def getThisType(node: Node): Type = {
      ...
      if ((parent && ((isClassLike(parent) || (parent.kind === SyntaxKind.InterfaceDeclaration))))) {
        if (((!((getModifierFlags(container) & ModifierFlags.Static))) && (((container.kind !== SyntaxKind.Constructor) || isNodeDescendantOf(
              node,
              (container.asInstanceOf[ConstructorDeclaration]).body))))) {
    ...
    def getTypeFromTypeNode(node: TypeNode,
                            aliasSymbol: Symbol,
                            aliasTypeArguments: Array[Type]): Type = {
      node.kind match {
        case SyntaxKind.AnyKeyword | SyntaxKind.JSDocAllType |
        ...
        case SyntaxKind.JSDocLiteralType =>
          return getTypeFromLiteralTypeNode(
            (node.asInstanceOf[JSDocLiteralType]).literal)
        ...
        case SyntaxKind.UnionType | SyntaxKind.JSDocUnionType =>
          return getTypeFromUnionTypeNode(
            node.asInstanceOf[UnionTypeNode],
            aliasSymbol,
            aliasTypeArguments)
        case SyntaxKind.IntersectionType =>
          return getTypeFromIntersectionTypeNode(
            ...
            aliasSymbol,
            aliasTypeArguments)
        case SyntaxKind.ParenthesizedType | SyntaxKind.JSDocNullableType |
            SyntaxKind.JSDocNonNullableType | SyntaxKind.JSDocConstructorType |
            SyntaxKind.JSDocThisType | SyntaxKind.JSDocOptionalType =>
          return getTypeFromTypeNode((node
            .asInstanceOf[(ParenthesizedTypeNode | JSDocTypeReferencingNode)])
            .`type`)
        case SyntaxKind.JSDocRecordType =>
          return getTypeFromTypeNode(
            (node.asInstanceOf[JSDocRecordType]).literal)
        case SyntaxKind.FunctionType | SyntaxKind.ConstructorType |
            SyntaxKind.TypeLiteral | SyntaxKind.JSDocTypeLiteral |
            SyntaxKind.JSDocFunctionType =>
          return getTypeFromTypeLiteralOrFunctionOrConstructorTypeNode(
            node,
            aliasSymbol,
            aliasTypeArguments)
        ...
        case SyntaxKind.JSDocVariadicType =>
          return getTypeFromJSDocVariadicType(
            node.asInstanceOf[JSDocVariadicType])
        case _ =>
          return unknownType
      }

    }
    def instantiateList[T](items: Array[T],
                           mapper: TypeMapper,
                           instantiator: ((T, TypeMapper) => T)): Array[T] = {
      if ((items && items.length)) {
        val result: Array[T] = Array()
        (items).foreach { fresh51 =>
          val v = zeroOfMyType = fresh51 {
    ...
    def createBinaryTypeMapper(source1: Type,
                               target1: Type,
                               source2: Type,
                               target2: Type): TypeMapper = {
      return (t =>
                (if ((t === source1)) target1
                 else (if ((t === source2)) target2
                       else t)))

    }
    def createArrayTypeMapper(sources: Array[Type],
                              targets: Array[Type]): TypeMapper = {
      return (t => {
                {
                  var i = 0
    ...
    def createTypeMapper(sources: Array[Type],
                         targets: Array[Type]): TypeMapper = {
      ...
      val mapper: TypeMapper =
        (if ((count == 1))
           createUnaryTypeMapper(
             sources(0),
             (if (targets) targets(0) else anyType))
         else
           (if ((count == 2))
              createBinaryTypeMapper(
                ...
                (if (targets) targets(0) else anyType),
                sources(1),
                (if (targets) targets(1) else anyType))
            else createArrayTypeMapper(sources, targets)))
    ...
    def getInferenceMapper(context: InferenceContext): TypeMapper = {
      if ((!context.mapper)) {
        val mapper: TypeMapper = (t => {
                                    val typeParameters =
                                      context.signature.typeParameters {
                                        var i = 0
                                        while ((i < typeParameters.length)) {
                                          {
                                            if ((t === typeParameters(i))) {
                                              (context.inferences(i).isFixed =
                                                true)
    ...
    def combineTypeMappers(mapper1: TypeMapper,
                           mapper2: TypeMapper): TypeMapper = {
    ...
    def cloneTypeParameter(typeParameter: TypeParameter): TypeParameter = {
      val result = createType(TypeFlags.TypeParameter)
        .asInstanceOf[TypeParameter]
    ...
    def cloneTypePredicate(
        predicate: TypePredicate,
        mapper: TypeMapper): (ThisTypePredicate | IdentifierTypePredicate) = {
      if (isIdentifierTypePredicate(predicate)) {
        return Map(
          "kind" -> TypePredicateKind.Identifier,
    ...
    def instantiateSignature(signature: Signature,
                             mapper: TypeMapper,
                             eraseTypeParameters: Boolean): Signature = {
      var freshTypeParameters: Array[TypeParameter] = zeroOfMyType
      var freshTypePredicate: TypePredicate = zeroOfMyType
      if ((signature.typeParameters && (!eraseTypeParameters))) {
        (freshTypeParameters =
          map(signature.typeParameters, cloneTypeParameter))
      ...
      if (signature.typePredicate) {
        (freshTypePredicate =
          cloneTypePredicate(signature.typePredicate, mapper))

      }
      val result = createSignature(
    ...
    def instantiateSymbol(symbol: Symbol, mapper: TypeMapper): Symbol = {
      ...
      val result = createSymbol(
        ...
        symbol.name).asInstanceOf[TransientSymbol]
    ...
    def instantiateAnonymousType(`type`: AnonymousType,
                                 mapper: TypeMapper): ObjectType = {
      if (mapper.instantiations) {
        val cachedType =
          mapper.instantiations(`type`.id).asInstanceOf[ObjectType]
        if (cachedType) {
          return cachedType
      ...
      val result = createObjectType(
        (ObjectFlags.Anonymous | ObjectFlags.Instantiated),
        `type`.symbol).asInstanceOf[AnonymousType]
      (result.target = `type`)
      (result.mapper = mapper)
    ...
    def isSymbolInScopeOfMappedTypeParameter(symbol: Symbol,
                                             mapper: TypeMapper) = {
      ...
      while (node) {
        {
          node.kind match {
            case SyntaxKind.FunctionType | SyntaxKind.ConstructorType |
                ...
                SyntaxKind.IndexSignature | SyntaxKind.GetAccessor |
                SyntaxKind.SetAccessor | SyntaxKind.FunctionExpression |
                SyntaxKind.ArrowFunction | SyntaxKind.ClassDeclaration |
                SyntaxKind.ClassExpression | SyntaxKind.InterfaceDeclaration |
                SyntaxKind.TypeAliasDeclaration =>
              val declaration =
                node.asInstanceOf[DeclarationWithTypeParameters]
              if (declaration.typeParameters) {
                (declaration.typeParameters).foreach { fresh53 =>
                  val d = zeroOfMyType = fresh53 {
                    if (contains(
                          mappedTypes,
                          getDeclaredTypeOfTypeParameter(getSymbolOfNode(d)))) {
                      return true
              ...
              if ((isClassLike(node) || (node.kind === SyntaxKind.InterfaceDeclaration))) {
                val thisType = getDeclaredTypeOfClassOrInterface(
                  getSymbolOfNode(node)).thisType
      ...
      if ((`type` && (mapper !== identityMapper))) {
        ...
        if ((`type`.flags & TypeFlags.Object)) {
          if (((`type`
                .asInstanceOf[ObjectType])
                .objectFlags & ObjectFlags.Anonymous)) {
            return (if (((`type`.symbol && (`type`.symbol.flags & (((((SymbolFlags.Function | SymbolFlags.Method) | SymbolFlags.Class) | SymbolFlags.TypeLiteral) | SymbolFlags.ObjectLiteral)))) && ((((`type`
                          .asInstanceOf[ObjectType])
                          .objectFlags & ObjectFlags.Instantiated) || isSymbolInScopeOfMappedTypeParameter(
                          `type`.symbol,
                          mapper)))))
                      instantiateAnonymousType(
                        `type`.asInstanceOf[AnonymousType],
                        mapper)
                    else `type`)
    ...
    def instantiateIndexInfo(info: IndexInfo, mapper: TypeMapper): IndexInfo = {
      return (info && createIndexInfo(
        instantiateType(info.`type`, mapper),
        info.isReadonly,
        info.declaration))

    }
    def isContextSensitive(
      ...
      node.kind match {
        ...
        case SyntaxKind.ObjectLiteralExpression =>
          return forEach(
            (node.asInstanceOf[ObjectLiteralExpression]).properties,
            isContextSensitive)
        case SyntaxKind.ArrayLiteralExpression =>
          return forEach(
            (node.asInstanceOf[ArrayLiteralExpression]).elements,
            isContextSensitive)
        case SyntaxKind.ConditionalExpression =>
          return (isContextSensitive(
            (node
              .asInstanceOf[ConditionalExpression])
              .whenTrue) || isContextSensitive(
            (node.asInstanceOf[ConditionalExpression]).whenFalse))
        case SyntaxKind.BinaryExpression =>
          return (((node
            .asInstanceOf[BinaryExpression])
            .operatorToken
            .kind === SyntaxKind.BarBarToken) && ((isContextSensitive(
            (node.asInstanceOf[BinaryExpression]).left) || isContextSensitive(
            (node.asInstanceOf[BinaryExpression]).right))))
        case SyntaxKind.PropertyAssignment =>
          return isContextSensitive(
            (node.asInstanceOf[PropertyAssignment]).initializer)
        case SyntaxKind.MethodDeclaration | SyntaxKind.MethodSignature =>
          return isContextSensitiveFunctionLikeDeclaration(
            node.asInstanceOf[MethodDeclaration])
        case SyntaxKind.ParenthesizedExpression =>
          return isContextSensitive(
            (node.asInstanceOf[ParenthesizedExpression]).expression)
        case _ =>
      }
      return false

    }
    def isContextSensitiveFunctionLikeDeclaration(
    ...
    def isContextSensitiveFunctionOrObjectLiteralMethod(func: Node): Boolean = {
      return (((isFunctionExpressionOrArrowFunction(func) || isObjectLiteralMethod(
        func))) && isContextSensitiveFunctionLikeDeclaration(func))
    ...
    def getTypeWithoutSignatures(`type`: Type): Type = {
      if ((`type`.flags & TypeFlags.Object)) {
        ...
        if (resolved.constructSignatures.length) {
          val result = createObjectType(ObjectFlags.Anonymous, `type`.symbol)
            .asInstanceOf[ResolvedType]
    ...
    def compareTypesIdentical(source: Type, target: Type): Ternary = {
      return (if (isTypeRelatedTo(source, target, identityRelation))
                Ternary.True
              else Ternary.False)

    }
    def compareTypesAssignable(source: Type, target: Type): Ternary = {
      return (if (isTypeRelatedTo(source, target, assignableRelation))
                Ternary.True
              else Ternary.False)
    ...
    def isTypeInstanceOf(source: Type, target: Type): Boolean = {
      return ((source === target) || (isTypeSubtypeOf(source, target) && (!isTypeIdenticalTo(
        source,
        target))))
    ...
    def isSignatureAssignableTo(source: Signature,
      ...
      return (compareSignaturesRelated(
        ...
        false,
        undefined,
        compareTypesAssignable) !== Ternary.False)

    }
    type ErrorReporter = ((DiagnosticMessage, String, String) => Unit)
    def compareSignaturesRelated(
        source: Signature,
        target: Signature,
        ignoreReturnTypes: Boolean,
        reportErrors: Boolean,
        errorReporter: ErrorReporter,
        compareTypes: ((Type, Type, Boolean) => Ternary)): Ternary = {
      if ((source === target)) {
        return Ternary.True
      ...
      if ((sourceThisType && (sourceThisType !== voidType))) {
        ...
        if (targetThisType) {
          val related = (compareTypes(sourceThisType, targetThisType, false) || compareTypes(
              targetThisType,
              sourceThisType,
              reportErrors))
          if ((!related)) {
      ...
      val checkCount = getNumParametersToCheckForSignatureRelatability(
        source,
        sourceMax,
        target,
        targetMax)
      ...
      val targetParams = target.parameters {
        ...
        while ((i < checkCount)) {
          {
            val s =
              (if ((i < sourceMax)) getTypeOfParameter(sourceParams(i))
               else getRestTypeOfSignature(source))
            val t =
              (if ((i < targetMax)) getTypeOfParameter(targetParams(i))
               else getRestTypeOfSignature(target))
            val related = (compareTypes(s, t, false) || compareTypes(
                t,
                s,
                reportErrors))
            if ((!related)) {
              if (reportErrors) {
                errorReporter(
                  ...
                  sourceParams((if ((i < sourceMax)) i else sourceMax)).name,
                  targetParams((if ((i < targetMax)) i else targetMax)).name)
      ...
      if ((!ignoreReturnTypes)) {
        ...
        if (target.typePredicate) {
          if (source.typePredicate) {
            ...
            if (reportErrors) {
              errorReporter(
                Diagnostics.Signature_0_must_have_a_type_predicate,
    ...
    def compareTypePredicateRelatedTo(
        source: TypePredicate,
        target: TypePredicate,
        reportErrors: Boolean,
        errorReporter: ErrorReporter,
      ...
      if ((source.kind === TypePredicateKind.Identifier)) {
        val sourceIdentifierPredicate =
          source.asInstanceOf[IdentifierTypePredicate]
        val targetIdentifierPredicate =
          target.asInstanceOf[IdentifierTypePredicate]
    ...
    def isImplementationCompatibleWithOverload(
        implementation: Signature,
        overload: Signature): Boolean = {
      ...
      if ((((targetReturnType === voidType) || isTypeRelatedTo(
            targetReturnType,
            sourceReturnType,
            assignableRelation)) || isTypeRelatedTo(
            sourceReturnType,
            targetReturnType,
            assignableRelation))) {
    ...
    def getNumParametersToCheckForSignatureRelatability(
        source: Signature,
        sourceNonRestParamCount: Int,
        target: Signature,
        targetNonRestParamCount: Int) = {
      if ((source.hasRestParameter === target.hasRestParameter)) {
        if (source.hasRestParameter) {
          return (Math
            .max(sourceNonRestParamCount, targetNonRestParamCount) + 1)
        ...
        return (if (source.hasRestParameter) targetNonRestParamCount
                else sourceNonRestParamCount)

      }

    }
    def isEnumTypeRelatedTo(source: EnumType,
                            target: EnumType,
                            errorReporter: ErrorReporter) = {
      if ((source === target)) {
        return true
      ...
      (getPropertiesOfType(getTypeOfSymbol(source.symbol))).foreach {
        fresh54 =>
          val property = zeroOfMyType = fresh54 {
            if ((property.flags & SymbolFlags.EnumMember)) {
              val targetProperty =
                getPropertyOfType(targetEnumType, property.name)
    ...
    def isSimpleTypeRelatedTo(source: Type,
                              target: Type,
      ...
      if (((source.flags & TypeFlags.BooleanLike) && (target.flags & TypeFlags.Boolean)))
        return true
      if ((((source.flags & TypeFlags.EnumLiteral) && (target.flags & TypeFlags.Enum)) && ((source
            .asInstanceOf[EnumLiteralType])
            .baseType === target)))
        return true
      if ((((source.flags & TypeFlags.Enum) && (target.flags & TypeFlags.Enum)) && isEnumTypeRelatedTo(
            source.asInstanceOf[EnumType],
            target.asInstanceOf[EnumType],
            errorReporter)))
        return true
      ...
      if (((relation === assignableRelation) || (relation === comparableRelation))) {
        ...
        if (((((source.flags & TypeFlags.Number) | (source.flags & TypeFlags.NumberLiteral))) && (target.flags & TypeFlags.EnumLike)))
          return true
        if (((((source.flags & TypeFlags.EnumLiteral) && (target.flags & TypeFlags.EnumLiteral)) && ((source
              .asInstanceOf[EnumLiteralType])
              .text === (target
              .asInstanceOf[EnumLiteralType])
              .text)) && isEnumTypeRelatedTo(
              (source.asInstanceOf[EnumLiteralType]).baseType,
              (target.asInstanceOf[EnumLiteralType]).baseType,
              errorReporter))) {
          return true

        }
        if ((((source.flags & TypeFlags.EnumLiteral) && (target.flags & TypeFlags.Enum)) && isEnumTypeRelatedTo(
              target.asInstanceOf[EnumType],
              (source.asInstanceOf[EnumLiteralType]).baseType,
              errorReporter))) {
          return true
    ...
    def isTypeRelatedTo(source: Type,
                        target: Type,
                        relation: Map[RelationComparisonResult]) = {
      ...
      if (((source === target) || ((relation !== identityRelation) && isSimpleTypeRelatedTo(
            source,
            target,
            relation)))) {
        return true
    ...
    def checkTypeRelatedTo(
      ...
      if (overflow) {
        ...
        diagnostics.add(
          createDiagnosticForNodeFromMessageChain(errorNode, errorInfo))

      }
      return (result !== Ternary.False)
      def reportError(message: DiagnosticMessage,
                      arg0: String,
                      arg1: String,
                      arg2: String): Unit = {
        Debug.assert((!(!errorNode)))
        (errorInfo =
          chainDiagnosticMessages(errorInfo, message, arg0, arg1, arg2))

      }
      def reportRelationError(message: DiagnosticMessage,
                              source: Type,
                              target: Type) = {
        ...
        if ((!message)) {
          (message =
            (if ((relation === comparableRelation))
             ...
             else Diagnostics.Type_0_is_not_assignable_to_type_1))

        }
        reportError(message, sourceType, targetType)

      }
      def tryElaborateErrorsForPrimitivesAndObjects(source: Type,
                                                    target: Type) = {
      ...
      def isRelatedTo(source: Type,
                      target: Type,
                      reportErrors: Boolean,
                      headMessage: DiagnosticMessage): Ternary = {
        ...
        if (isSimpleTypeRelatedTo(
              source,
              target,
              relation,
              (if (reportErrors) reportError else undefined)))
          return Ternary.True
        if (((getObjectFlags(source) & ObjectFlags.ObjectLiteral) && (source.flags & TypeFlags.FreshLiteral))) {
          if (hasExcessProperties(
                source.asInstanceOf[FreshObjectLiteralType],
                target,
                reportErrors)) {
            if (reportErrors) {
        ...
        if ((source.flags & TypeFlags.Union)) {
          if ((relation === comparableRelation)) {
            (result = someTypeRelatedToType(
              source.asInstanceOf[UnionType],
              target,
              (reportErrors && (!((source.flags & TypeFlags.Primitive))))))

          } else {
            (result = eachTypeRelatedToType(
              source.asInstanceOf[UnionType],
              target,
              (reportErrors && (!((source.flags & TypeFlags.Primitive))))))
          ...
          if ((result = typeRelatedToSomeType(
                source,
                target.asInstanceOf[UnionType],
          ...
          if ((result = typeRelatedToEachType(
                source,
                target.asInstanceOf[IntersectionType],
                reportErrors))) {
            return result
          ...
          if ((result = someTypeRelatedToType(
                source.asInstanceOf[IntersectionType],
                target,
                false))) {
            return result
        ...
        if ((source.flags & TypeFlags.TypeParameter)) {
          var constraint = getConstraintOfTypeParameter(
            source.asInstanceOf[TypeParameter])
          ...
          if ((((getObjectFlags(source) & ObjectFlags.Reference) && (getObjectFlags(
                target) & ObjectFlags.Reference)) && ((source
                .asInstanceOf[TypeReference])
                .target === (target.asInstanceOf[TypeReference]).target))) {
            if ((result = typeArgumentsRelatedTo(
                  source.asInstanceOf[TypeReference],
                  target.asInstanceOf[TypeReference],
                  reportErrors))) {
              return result
      ...
      def isIdenticalTo(source: Type, target: Type): Ternary = {
        ...
        if ((((source.flags & TypeFlags.Union) && (target.flags & TypeFlags.Union)) || ((source.flags & TypeFlags.Intersection) && (target.flags & TypeFlags.Intersection)))) {
          if ((result = eachTypeRelatedToSomeType(
                source.asInstanceOf[UnionOrIntersectionType],
                target.asInstanceOf[UnionOrIntersectionType]))) {
            if ((result &= eachTypeRelatedToSomeType(
                  target.asInstanceOf[UnionOrIntersectionType],
                  source.asInstanceOf[UnionOrIntersectionType]))) {
              return result
      ...
      def isKnownProperty(`type`: Type, name: String): Boolean = {
        if ((`type`.flags & TypeFlags.Object)) {
          val resolved = resolveStructuredTypeMembers(
            `type`.asInstanceOf[ObjectType])
          if ((((((((relation === assignableRelation) || (relation === comparableRelation))) && (((`type` === globalObjectType) || isEmptyObjectType(
                resolved)))) || resolved.stringIndexInfo) || ((resolved.numberIndexInfo && isNumericLiteralName(
                name)))) || getPropertyOfType(`type`, name))) {
            return true

          }

        } else if ((`type`.flags & TypeFlags.UnionOrIntersection)) {
          ((`type`.asInstanceOf[UnionOrIntersectionType]).types).foreach {
            fresh55 =>
              val t = zeroOfMyType = fresh55 {
      ...
      def hasExcessProperties(source: FreshObjectLiteralType,
                              target: Type,
                              reportErrors: Boolean): Boolean = {
        if ((maybeTypeOfKind(target, TypeFlags.Object) && (!((getObjectFlags(
              target) & ObjectFlags.ObjectLiteralPatternWithComputedProperties))))) {
          (getPropertiesOfObjectType(source)).foreach { fresh56 =>
            val prop = zeroOfMyType = fresh56 {
      ...
      def eachTypeRelatedToSomeType(
          source: UnionOrIntersectionType,
          target: UnionOrIntersectionType): Ternary = {
        var result = Ternary.True
        val sourceTypes = source.types
        (sourceTypes).foreach { fresh57 =>
          val sourceType = zeroOfMyType = fresh57 {
      ...
      def typeRelatedToEachType(source: Type,
                                target: UnionOrIntersectionType,
                                reportErrors: Boolean): Ternary = {
        var result = Ternary.True
        val targetTypes = target.types
        (targetTypes).foreach { fresh58 =>
          val targetType = zeroOfMyType = fresh58 {
      ...
      def someTypeRelatedToType(source: UnionOrIntersectionType,
                                target: Type,
                                reportErrors: Boolean): Ternary = {
        val sourceTypes = source.types
        if (((source.flags & TypeFlags.Union) && containsType(
              sourceTypes,
              target))) {
          return Ternary.True
      ...
      def eachTypeRelatedToType(source: UnionOrIntersectionType,
                                target: Type,
                                reportErrors: Boolean): Ternary = {
        var result = Ternary.True
        val sourceTypes = source.types
        (sourceTypes).foreach { fresh59 =>
          val sourceType = zeroOfMyType = fresh59 {
      ...
      def typeArgumentsRelatedTo(source: TypeReference,
      ...
      def objectTypeRelatedTo(source: Type,
                              originalSource: Type,
                              target: Type,
                              reportErrors: Boolean): Ternary = {
        if (overflow) {
        ...
        if ((related !== undefined)) {
          if ((reportErrors && (related === RelationComparisonResult.Failed))) {
            ...
            return (if ((related === RelationComparisonResult.Succeeded))
                      Ternary.True
                    else Ternary.False)
        ...
        if (((!((expandingFlags & 1))) && isDeeplyNestedGeneric(
              source,
              sourceStack,
              depth)))
          (expandingFlags |= 1)
        if (((!((expandingFlags & 2))) && isDeeplyNestedGeneric(
              target,
              targetStack,
              depth)))
          (expandingFlags |= 2)
        ...
        if (result) {
          ...
          val destinationCache =
            (if ((((result === Ternary.True) || (depth === 0)))) relation
             else maybeStack((depth - 1)))
          copyProperties(maybeCache, destinationCache)

        } else {
          (relation(id) =
            (if (reportErrors) RelationComparisonResult.FailedAndReported
             else RelationComparisonResult.Failed))

        }
        return result

      }
      def propertiesRelatedTo(source: Type,
                              target: Type,
                              reportErrors: Boolean): Ternary = {
        if ((relation === identityRelation)) {
        ...
        val requireOptionalProperties = ((relation === subtypeRelation) && (!((getObjectFlags(
            source) & ObjectFlags.ObjectLiteral))))
        (properties).foreach { fresh60 =>
          val targetProp = zeroOfMyType = fresh60 {
            ...
            if ((sourceProp !== targetProp)) {
              if ((!sourceProp)) {
                ...
                if (((sourcePropFlags & ModifierFlags.Private) || (targetPropFlags & ModifierFlags.Private))) {
                  ...
                  val sourceClass =
                    (if (sourceDeclaredInClass)
                       getDeclaredTypeOfSymbol(getParentOfSymbol(sourceProp))
                         .asInstanceOf[InterfaceType]
                     else undefined)
                  val targetClass = getDeclaredTypeOfSymbol(
                    getParentOfSymbol(targetProp)).asInstanceOf[InterfaceType]
                  if (((!sourceClass) || (!hasBaseType(
                        sourceClass,
                        targetClass)))) {
                    if (reportErrors) {
                      reportError(
                        ...
                        symbolToString(targetProp),
                        typeToString((sourceClass || source)),
      ...
      def propertiesIdenticalTo(source: Type, target: Type): Ternary = {
        ...
        (sourceProperties).foreach { fresh61 =>
          val sourceProp = zeroOfMyType = fresh61 {
            ...
            val related =
              compareProperties(sourceProp, targetProp, isRelatedTo)
            if ((!related)) {
              return Ternary.False
      ...
      def signaturesRelatedTo(source: Type,
                              target: Type,
                              kind: SignatureKind,
                              reportErrors: Boolean): Ternary = {
        if ((relation === identityRelation)) {
        ...
        if ((((kind === SignatureKind.Construct) && sourceSignatures.length) && targetSignatures.length)) {
          if ((isAbstractConstructorType(source) && (!isAbstractConstructorType(
                target)))) {
            if (reportErrors) {
              reportError(
        ...
        outer.breakable {
          (targetSignatures).foreach { fresh62 =>
            val t = zeroOfMyType = fresh62 {
              var shouldElaborateErrors = reportErrors
              (sourceSignatures).foreach { fresh63 =>
                val s = zeroOfMyType = fresh63 {
              ...
              if (shouldElaborateErrors) {
                reportError(
                  ...
                  typeToString(source),
                  signatureToString(t, undefined, undefined, kind))
      ...
      def signatureRelatedTo(source: Signature,
                             target: Signature,
                             reportErrors: Boolean): Ternary = {
        return compareSignaturesRelated(
          source,
      ...
      def signaturesIdenticalTo(source: Type,
                                target: Type,
                                kind: SignatureKind): Ternary = {
        val sourceSignatures = getSignaturesOfType(source, kind)
        val targetSignatures = getSignaturesOfType(target, kind)
        ...
        var result = Ternary.True {
          ...
          while ((i < len)) {
            {
              val related = compareSignaturesIdentical(
                sourceSignatures(i),
                targetSignatures(i),
                false,
                false,
      ...
      def eachPropertyRelatedTo(source: Type,
                                target: Type,
                                kind: IndexKind,
                                reportErrors: Boolean): Ternary = {
        var result = Ternary.True
        (getPropertiesOfObjectType(source)).foreach { fresh64 =>
          val prop = zeroOfMyType = fresh64 {
            if (((kind === IndexKind.String) || isNumericLiteralName(
                  prop.name))) {
              val related =
                isRelatedTo(getTypeOfSymbol(prop), target, reportErrors)
              if ((!related)) {
                if (reportErrors) {
                  reportError(
      ...
      def indexInfoRelatedTo(sourceInfo: IndexInfo,
                             targetInfo: IndexInfo,
                             reportErrors: Boolean) = {
        val related =
          isRelatedTo(sourceInfo.`type`, targetInfo.`type`, reportErrors)
      ...
      def indexTypesRelatedTo(source: Type,
                              originalSource: Type,
                              target: Type,
                              kind: IndexKind,
                              reportErrors: Boolean) = {
        ...
        val sourceInfo = (getIndexInfoOfType(source, kind) || ((kind === IndexKind.Number) && getIndexInfoOfType(
            source,
            IndexKind.String)))
        if (sourceInfo) {
        ...
        if (isObjectLiteralType(source)) {
          ...
          if ((kind === IndexKind.String)) {
            ...
            if (sourceNumberInfo) {
              (related =
                indexInfoRelatedTo(sourceNumberInfo, targetInfo, reportErrors))
      ...
      def indexTypesIdenticalTo(source: Type,
                                target: Type,
                                indexKind: IndexKind): Ternary = {
      ...
      def constructorVisibilitiesAreCompatible(sourceSignature: Signature,
        ...
        val sourceAccessibility = (getModifierFlags(
            sourceSignature.declaration) & ModifierFlags.NonPublicAccessibilityModifier)
        val targetAccessibility = (getModifierFlags(
            targetSignature.declaration) & ModifierFlags.NonPublicAccessibilityModifier)
    ...
    def isDeeplyNestedGeneric(`type`: Type,
                              stack: Array[Type],
                              depth: Int): Boolean = {
    ...
    def isPropertyIdenticalTo(sourceProp: Symbol,
                              targetProp: Symbol): Boolean = {
      return (compareProperties(sourceProp, targetProp, compareTypesIdentical) !== Ternary.False)

    }
    def compareProperties(sourceProp: Symbol,
                          targetProp: Symbol,
                          compareTypes: ((Type, Type) => Ternary)): Ternary = {
      if ((sourceProp === targetProp)) {
        return Ternary.True

      }
      val sourcePropAccessibility = (getDeclarationModifierFlagsFromSymbol(
          sourceProp) & ModifierFlags.NonPublicAccessibilityModifier)
      val targetPropAccessibility = (getDeclarationModifierFlagsFromSymbol(
          targetProp) & ModifierFlags.NonPublicAccessibilityModifier)
    ...
    def isMatchingSignature(source: Signature,
                            target: Signature,
                            partialMatch: Boolean) = {
    ...
    def compareSignaturesIdentical(
        source: Signature,
        target: Signature,
        partialMatch: Boolean,
        ignoreThisTypes: Boolean,
      ...
      if ((((if (source.typeParameters) source.typeParameters.length else 0) ) !== ((if (target.typeParameters)
      ...
      val targetLen = target.parameters.length {
        ...
        while ((i < targetLen)) {
          {
            val s =
              (if (isRestParameterIndex(source, i))
                 getRestTypeOfSignature(source)
               else getTypeOfParameter(source.parameters(i)))
            val t =
              (if (isRestParameterIndex(target, i))
                 getRestTypeOfSignature(target)
               else getTypeOfParameter(target.parameters(i)))
    ...
    def getSupertypeOrUnion(types: Array[Type]): Type = {
      return (if (literalTypesWithSameBaseType(types)) getUnionType(types)
              else
                forEach(
                  types,
                  (t => (if (isSupertypeOfEach(t, types)) t else undefined))))
    ...
    def getCommonSupertype(types: Array[Type]): Type = {
      ...
      val primaryTypes =
        filter(types, (t => (!((t.flags & TypeFlags.Nullable)))))
      if ((!primaryTypes.length)) {
        return getUnionType(types, true)

      }
      val supertype = getSupertypeOrUnion(primaryTypes)
      return (supertype && includeFalsyTypes(
        supertype,
        (getFalsyFlagsOfTypes(types) & TypeFlags.Nullable)))

    }
    def reportNoCommonSupertypeError(
        types: Array[Type],
        errorLocation: Node,
        errorMessageChainHead: DiagnosticMessageChain): Unit = {
        ...
        while ((i < types.length)) {
          {
            ...
            Debug.assert(
              (!(!downfallType)),
              "If there is no common supertype, each type should have a downfallType")
    ...
    def isArrayType(`type`: Type): Boolean = {
      return ((getObjectFlags(`type`) & ObjectFlags.Reference) && ((`type`
        .asInstanceOf[TypeReference])
        .target === globalArrayType))
    ...
    def isArrayLikeType(`type`: Type): Boolean = {
      return (((getObjectFlags(`type`) & ObjectFlags.Reference) && ((((`type`
        .asInstanceOf[TypeReference])
        .target === globalArrayType) || ((`type`
        .asInstanceOf[TypeReference])
        .target === globalReadonlyArrayType)))) || ((!((`type`.flags & TypeFlags.Nullable))) && isTypeAssignableTo(
        `type`,
    ...
    def isLiteralType(`type`: Type): Boolean = {
      return (if ((`type`.flags & TypeFlags.Boolean)) true
              else
                (if ((`type`.flags & TypeFlags.Union))
                   (if ((`type`.flags & TypeFlags.Enum)) true
                    else
                      (!forEach(
                        (`type`.asInstanceOf[UnionType]).types,
                        (t => (!isUnitType(t))))))
                 else isUnitType(`type`)))

    }
    def getBaseTypeOfLiteralType(`type`: Type): Type = {
      return (if ((`type`.flags & TypeFlags.StringLiteral)) stringType
              else (if ((`type`.flags & TypeFlags.NumberLiteral)) numberType
                    else (if ((`type`.flags & TypeFlags.BooleanLiteral)) booleanType
                          else (if ((`type`.flags & TypeFlags.EnumLiteral))(`type`.asInstanceOf[EnumLiteralType]).baseType
                                else (if (((`type`.flags & TypeFlags.Union) && (!((`type`.flags & TypeFlags.Enum))))) getUnionType(sameMap((`type`.asInstanceOf[UnionType]).types, getBaseTypeOfLiteralType))
                                      else `type`)))))

    }
    def getWidenedLiteralType(`type`: Type): Type = {
      return (if (((`type`.flags & TypeFlags.StringLiteral) && (`type`.flags & TypeFlags.FreshLiteral)))
                stringType
              else
                (if (((`type`.flags & TypeFlags.NumberLiteral) && (`type`.flags & TypeFlags.FreshLiteral)))
                   numberType
                 else
                   (if ((`type`.flags & TypeFlags.BooleanLiteral)) booleanType
                    else
                      (if ((`type`.flags & TypeFlags.EnumLiteral))(`type`
                         .asInstanceOf[EnumLiteralType])
                         .baseType
                       else
    ...
    def getFalsyFlags(`type`: Type): TypeFlags = {
      return (if ((`type`.flags & TypeFlags.Union))
                getFalsyFlagsOfTypes((`type`.asInstanceOf[UnionType]).types)
              else
                (if ((`type`.flags & TypeFlags.StringLiteral))
                   (if (((`type`.asInstanceOf[LiteralType]).text === ""))
                      TypeFlags.StringLiteral
                    else 0)
                 else
                   (if ((`type`.flags & TypeFlags.NumberLiteral))
                      (if (((`type`.asInstanceOf[LiteralType]).text === "0"))
                      ...
                      (if ((`type`.flags & TypeFlags.BooleanLiteral))
                       ...
                       else (`type`.flags & TypeFlags.PossiblyFalsy)))))
    ...
    def removeDefinitelyFalsyTypes(`type`: Type): Type = {
      return (if ((getFalsyFlags(`type`) & TypeFlags.DefinitelyFalsy))
                filterType(
                  `type`,
    ...
    def getNonNullableType(`type`: Type): Type = {
      return (if (strictNullChecks)
                getTypeWithFacts(`type`, TypeFacts.NEUndefinedOrNull)
              else `type`)

    }
    def isObjectLiteralType(`type`: Type) = {
      return (((`type`.symbol && (((`type`.symbol.flags & ((SymbolFlags.ObjectLiteral | SymbolFlags.TypeLiteral)))) !== 0)) && (getSignaturesOfType(
        `type`,
        SignatureKind.Call).length === 0)) && (getSignaturesOfType(
        `type`,
        SignatureKind.Construct).length === 0))
    ...
    def createTransientSymbol(source: Symbol, `type`: Type) = {
      val symbol =
        createSymbol((source.flags | SymbolFlags.Transient), source.name)
          .asInstanceOf[TransientSymbol]
    ...
    def transformTypeOfMembers(`type`: Type, f: ((Type) => Type)) = {
      ...
      (getPropertiesOfObjectType(`type`)).foreach { fresh68 =>
        val property = zeroOfMyType = fresh68 {
          val original = getTypeOfSymbol(property)
          val updated = f(original)
          (members(property.name) =
            (if ((updated === original)) property
             else createTransientSymbol(property, updated)))
    ...
    def getRegularTypeOfObjectLiteral(`type`: Type): Type = {
      ...
      val regularType =
        (`type`.asInstanceOf[FreshObjectLiteralType]).regularType
      if (regularType) {
        return regularType
      ...
      val members =
        transformTypeOfMembers(`type`, getRegularTypeOfObjectLiteral)
      val regularNew = createAnonymousType(
        resolved.symbol,
        members,
    ...
    def getWidenedTypeOfObjectLiteral(`type`: Type): Type = {
      val members =
        transformTypeOfMembers(`type`, (prop => {
                                          val widened = getWidenedType(prop)
                                          return (if ((prop === widened)) prop
                                                  else widened)
    ...
    def getWidenedConstituentType(`type`: Type): Type = {
      return (if ((`type`.flags & TypeFlags.Nullable)) `type`
              else getWidenedType(`type`))
    ...
    def reportWideningErrorsInType(`type`: Type): Boolean = {
      ...
      if ((isArrayType(`type`) || isTupleType(`type`))) {
        ((`type`.asInstanceOf[TypeReference]).typeArguments).foreach {
          fresh70 =>
            val t = zeroOfMyType = fresh70 {
              if (reportWideningErrorsInType(t)) {
                (errorReported = true)
        ...
        case SyntaxKind.Parameter =>
          (diagnostic =
            (if ((declaration
             ...
             else Diagnostics.Parameter_0_implicitly_has_an_1_type))
        case SyntaxKind.BindingElement =>
          (diagnostic = Diagnostics.Binding_element_0_implicitly_has_an_1_type)
        case SyntaxKind.FunctionDeclaration | SyntaxKind.MethodDeclaration |
    ...
    def forEachMatchingParameterType(source: Signature,
                                     target: Signature,
                                     callback: ((Type, Type) => Unit)) = {
    ...
    def createInferenceContext(signature: Signature,
                               inferUnionTypes: Boolean): InferenceContext = {
      val inferences =
        map(signature.typeParameters, createTypeInferencesObject)
      return Map(
        "signature" -> signature,
    ...
    def couldContainTypeParameters(`type`: Type): Boolean = {
      return (!(!(((((`type`.flags & TypeFlags.TypeParameter) || ((getObjectFlags(
        `type`) & ObjectFlags.Reference) && forEach((`type`.asInstanceOf[TypeReference]).typeArguments, couldContainTypeParameters))) || (((getObjectFlags(
        `type`) & ObjectFlags.Anonymous) && `type`.symbol) && (`type`.symbol.flags & (((SymbolFlags.Method | SymbolFlags.TypeLiteral) | SymbolFlags.Class))))) || ((`type`.flags & TypeFlags.UnionOrIntersection) && couldUnionOrIntersectionContainTypeParameters(
        `type`.asInstanceOf[UnionOrIntersectionType]))))))

    }
    def couldUnionOrIntersectionContainTypeParameters(
        `type`: UnionOrIntersectionType): Boolean = {
      if ((`type`.couldContainTypeParameters === undefined)) {
        (`type`.couldContainTypeParameters =
          forEach(`type`.types, couldContainTypeParameters))

      }
      return `type`.couldContainTypeParameters

    }
    def isTypeParameterAtTopLevel(`type`: Type,
                                  typeParameter: TypeParameter): Boolean = {
      return ((`type` === typeParameter) || ((`type`.flags & TypeFlags.UnionOrIntersection) && forEach(
        (`type`.asInstanceOf[UnionOrIntersectionType]).types,
        (t => isTypeParameterAtTopLevel(t, typeParameter)))))

    }
    def inferTypes(context: InferenceContext,
                   originalSource: Type,
                   originalTarget: Type) = {
      ...
      def inferFromTypes(source: Type, target: Type) = {
        ...
        if (((((source.flags & TypeFlags.Union) && (target.flags & TypeFlags.Union)) && (!(((source.flags & TypeFlags.Enum) && (target.flags & TypeFlags.Enum))))) || ((source.flags & TypeFlags.Intersection) && (target.flags & TypeFlags.Intersection)))) {
          if ((source === target)) {
            ((source.asInstanceOf[UnionOrIntersectionType]).types).foreach {
              fresh72 =>
                val t = zeroOfMyType = fresh72 {
                  inferFromTypes(t, t)
          ...
          ((source.asInstanceOf[UnionOrIntersectionType]).types).foreach {
            fresh73 =>
              val t = zeroOfMyType = fresh73 {
                if (typeIdenticalToSomeType(
                      t,
                      (target.asInstanceOf[UnionOrIntersectionType]).types)) {
                  ...
                  if (typeIdenticalToSomeType(
                        b,
                        (target
                          .asInstanceOf[UnionOrIntersectionType])
                          .types)) {
          ...
          if (matchingTypes) {
            (source = removeTypesFromUnionOrIntersection(
              source.asInstanceOf[UnionOrIntersectionType],
              matchingTypes))
            (target = removeTypesFromUnionOrIntersection(
              target.asInstanceOf[UnionOrIntersectionType],
              matchingTypes))
        ...
        if ((target.flags & TypeFlags.TypeParameter)) {
                  ...
                  if ((!inferences.isFixed)) {
                    val candidates =
                      (if (inferiority)
                         ...
                         (inferences.primary || ((inferences.primary = Array()))))
                    if ((!contains(candidates, source))) {
                      candidates.push(source)

                    }
                    if ((!isTypeParameterAtTopLevel(
                          originalTarget,
                          target.asInstanceOf[TypeParameter]))) {
        ...
        } else if ((((getObjectFlags(source) & ObjectFlags.Reference) && (getObjectFlags(
                     target) & ObjectFlags.Reference)) && ((source
                     .asInstanceOf[TypeReference])
                     .target === (target
                     .asInstanceOf[TypeReference])
                     .target))) {
          val sourceTypes = ((source
              .asInstanceOf[TypeReference])
              .typeArguments || emptyArray)
          val targetTypes = ((target
              .asInstanceOf[TypeReference])
              .typeArguments || emptyArray)
          val count =
            (if ((sourceTypes.length < targetTypes.length)) sourceTypes.length
             else targetTypes.length) {
              var i = 0
              while ((i < count)) {
                {
          ...
          (targetTypes).foreach { fresh74 =>
            val t = zeroOfMyType = fresh74 {
              if (((t.flags & TypeFlags.TypeParameter) && contains(
                    typeParameters,
                    t))) {
          ...
          val sourceTypes =
            (source.asInstanceOf[UnionOrIntersectionType]).types
          (sourceTypes).foreach { fresh75 =>
            val sourceType = zeroOfMyType = fresh75 {
          ...
          if ((source.flags & TypeFlags.Object)) {
            ...
            if ((isDeeplyNestedGeneric(source, sourceStack, depth) && isDeeplyNestedGeneric(
                  target,
                  targetStack,
                  depth))) {
              return
      ...
      def inferFromProperties(source: Type, target: Type) = {
        ...
        (properties).foreach { fresh76 =>
          val targetProp = zeroOfMyType = fresh76 {
      ...
      def inferFromSignatures(source: Type,
                              target: Type,
                              kind: SignatureKind) = {
        val sourceSignatures = getSignaturesOfType(source, kind)
        val targetSignatures = getSignaturesOfType(target, kind)
        ...
        val len = (if ((sourceLen < targetLen)) sourceLen else targetLen) {
          ...
          while ((i < len)) {
            {
              inferFromSignature(
                getErasedSignature(sourceSignatures(((sourceLen - len) + i))),
      ...
      def inferFromSignature(source: Signature, target: Signature) = {
        ...
        if (((source.typePredicate && target.typePredicate) && (source.typePredicate.kind === target.typePredicate.kind))) {
          inferFromTypes(
            source.typePredicate.`type`,
            target.typePredicate.`type`)
      ...
      def inferFromIndexTypes(source: Type, target: Type) = {
        val targetStringIndexType =
          getIndexTypeOfType(target, IndexKind.String)
        if (targetStringIndexType) {
          val sourceIndexType = (getIndexTypeOfType(source, IndexKind.String) || getImplicitIndexTypeOfType(
              source,
              IndexKind.String))
          if (sourceIndexType) {
        ...
        val targetNumberIndexType =
          getIndexTypeOfType(target, IndexKind.Number)
        if (targetNumberIndexType) {
          val sourceIndexType = ((getIndexTypeOfType(source, IndexKind.Number) || getIndexTypeOfType(
              source,
              IndexKind.String)) || getImplicitIndexTypeOfType(
              source,
              IndexKind.Number))
          if (sourceIndexType) {
    ...
    def removeTypesFromUnionOrIntersection(`type`: UnionOrIntersectionType,
                                           typesToRemove: Array[Type]) = {
      val reducedTypes: Array[Type] = Array()
      (`type`.types).foreach { fresh78 =>
        val t = zeroOfMyType = fresh78 {
      ...
      return (if ((`type`.flags & TypeFlags.Union)) getUnionType(reducedTypes)
              else getIntersectionType(reducedTypes))

    }
    def getInferenceCandidates(context: InferenceContext,
    ...
    def getInferredType(context: InferenceContext, index: Int): Type = {
      ...
      if ((!inferredType)) {
        ...
        if (inferences.length) {
          ...
          val widenLiteralTypes = ((context
              .inferences(index)
              .topLevel && (!hasPrimitiveConstraint(
              signature.typeParameters(index)))) && ((context
              .inferences(index)
              .isFixed || (!isTypeParameterAtTopLevel(
              getReturnTypeOfSignature(signature),
              signature.typeParameters(index))))))
          val baseInferences =
            (if (widenLiteralTypes) sameMap(inferences, getWidenedLiteralType)
             else inferences)
          val unionOrSuperType =
            (if (context.inferUnionTypes) getUnionType(baseInferences, true)
             else getCommonSupertype(baseInferences))
          (inferredType =
            (if (unionOrSuperType) getWidenedType(unionOrSuperType)
             else unknownType))
        ...
        if (inferenceSucceeded) {
          val constraint = getConstraintOfTypeParameter(
            context.signature.typeParameters(index))
          if (constraint) {
            val instantiatedConstraint =
              instantiateType(constraint, getInferenceMapper(context))
            if ((!isTypeAssignableTo(
                  inferredType,
                  getTypeWithThisArgument(
                    instantiatedConstraint,
                    inferredType)))) {
              (context.inferredTypes(index) =
    ...
    def getFlowCacheKey(node: Node): String = {
      if ((node.kind === SyntaxKind.Identifier)) {
        ...
        return (if ((symbol !== unknownSymbol)) ("" + getSymbolId(symbol))
                else undefined)
      ...
      if ((node.kind === SyntaxKind.PropertyAccessExpression)) {
        val key = getFlowCacheKey(
          (node.asInstanceOf[PropertyAccessExpression]).expression)
        return (key && ((key + ".") + (node
    ...
    def getLeftmostIdentifierOrThis(node: Node): Node = {
      node.kind match {
        ...
        case SyntaxKind.PropertyAccessExpression =>
          return getLeftmostIdentifierOrThis(
            (node.asInstanceOf[PropertyAccessExpression]).expression)
        case _ =>
      }
      return undefined
    ...
    def isMatchingReference(source: Node, target: Node): Boolean = {
      source.kind match {
        case SyntaxKind.Identifier =>
          return (((target.kind === SyntaxKind.Identifier) && (getResolvedSymbol(
            source.asInstanceOf[Identifier]) === getResolvedSymbol(target
            .asInstanceOf[Identifier]))) || ((((target.kind === SyntaxKind.VariableDeclaration) || (target.kind === SyntaxKind.BindingElement))) && (getExportSymbolOfValueSymbolIfExported(
            getResolvedSymbol(source.asInstanceOf[Identifier])) === getSymbolOfNode(
            target))))
        case SyntaxKind.ThisKeyword =>
          return (target.kind === SyntaxKind.ThisKeyword)
        case SyntaxKind.PropertyAccessExpression =>
          return (((target.kind === SyntaxKind.PropertyAccessExpression) && ((source
            .asInstanceOf[PropertyAccessExpression])
            .name
            .text === (target
            .asInstanceOf[PropertyAccessExpression])
            .name
            .text)) && isMatchingReference(
            (source.asInstanceOf[PropertyAccessExpression]).expression,
            (target.asInstanceOf[PropertyAccessExpression]).expression))
        case _ =>
      }
      return false
    ...
    def containsMatchingReferenceDiscriminant(source: Node, target: Node) = {
      return (((target.kind === SyntaxKind.PropertyAccessExpression) && containsMatchingReference(
        source,
        (target
          .asInstanceOf[PropertyAccessExpression])
          .expression)) && isDiscriminantProperty(
        getDeclaredTypeOfReference(
          (target.asInstanceOf[PropertyAccessExpression]).expression),
        (target.asInstanceOf[PropertyAccessExpression]).name.text))

    }
    def getDeclaredTypeOfReference(expr: Node): Type = {
      if ((expr.kind === SyntaxKind.Identifier)) {
        return getTypeOfSymbol(
          getResolvedSymbol(expr.asInstanceOf[Identifier]))

      }
      if ((expr.kind === SyntaxKind.PropertyAccessExpression)) {
        val `type` = getDeclaredTypeOfReference(
          (expr.asInstanceOf[PropertyAccessExpression]).expression)
        return (`type` && getTypeOfPropertyOfType(
          `type`,
          (expr.asInstanceOf[PropertyAccessExpression]).name.text))
    ...
    def isDiscriminantProperty(`type`: Type, name: String) = {
      if ((`type` && (`type`.flags & TypeFlags.Union))) {
        val prop =
          getUnionOrIntersectionProperty(`type`.asInstanceOf[UnionType], name)
        if ((prop && (prop.flags & SymbolFlags.SyntheticProperty))) {
          if (((prop
                .asInstanceOf[TransientSymbol])
                .isDiscriminantProperty === undefined)) {
            ((prop
              .asInstanceOf[TransientSymbol])
              .isDiscriminantProperty = ((prop
                .asInstanceOf[TransientSymbol])
                .hasNonUniformType && isLiteralType(getTypeOfSymbol(prop))))
    ...
    def hasMatchingArgument(callExpression: CallExpression, reference: Node) = {
      if (callExpression.arguments) {
        (callExpression.arguments).foreach { fresh79 =>
          val argument = zeroOfMyType = fresh79 {
      ...
      if (((callExpression.expression.kind === SyntaxKind.PropertyAccessExpression) && isOrContainsMatchingReference(
            reference,
    ...
    def isFunctionObjectType(`type`: ObjectType): Boolean = {
      val resolved = resolveStructuredTypeMembers(`type`)
      return (!(!(((resolved.callSignatures.length || resolved.constructSignatures.length) || (resolved
        .members("bind") && isTypeSubtypeOf(`type`, globalFunctionType))))))

    }
    def getTypeFacts(`type`: Type): TypeFacts = {
      val flags = `type`.flags
      if ((flags & TypeFlags.String)) {
        return (if (strictNullChecks) TypeFacts.StringStrictFacts
                else TypeFacts.StringFacts)
      ...
      if ((flags & TypeFlags.StringLiteral)) {
        return (if (strictNullChecks)
                  (if (((`type`.asInstanceOf[LiteralType]).text === ""))
                   ...
                   else TypeFacts.NonEmptyStringStrictFacts)
                else
                  (if (((`type`.asInstanceOf[LiteralType]).text === ""))
                     TypeFacts.EmptyStringFacts
                   else TypeFacts.NonEmptyStringFacts))

      }
      if ((flags & ((TypeFlags.Number | TypeFlags.Enum)))) {
        return (if (strictNullChecks) TypeFacts.NumberStrictFacts
                else TypeFacts.NumberFacts)

      }
      if ((flags & ((TypeFlags.NumberLiteral | TypeFlags.EnumLiteral)))) {
        val isZero = ((`type`.asInstanceOf[LiteralType]).text === "0")
        return (if (strictNullChecks)
                  (if (isZero) TypeFacts.ZeroStrictFacts
                   else TypeFacts.NonZeroStrictFacts)
                else (if (isZero) TypeFacts.ZeroFacts
                      else TypeFacts.NonZeroFacts))

      }
      if ((flags & TypeFlags.Boolean)) {
        return (if (strictNullChecks) TypeFacts.BooleanStrictFacts
                else TypeFacts.BooleanFacts)

      }
      if ((flags & TypeFlags.BooleanLike)) {
        return (if (strictNullChecks)
                  (if ((`type` === falseType)) TypeFacts.FalseStrictFacts
                   else TypeFacts.TrueStrictFacts)
                else (if ((`type` === falseType)) TypeFacts.FalseFacts
                      else TypeFacts.TrueFacts))

      }
      if ((flags & TypeFlags.Object)) {
        return (if (isFunctionObjectType(`type`.asInstanceOf[ObjectType]))
                  (if (strictNullChecks) TypeFacts.FunctionStrictFacts
                   else TypeFacts.FunctionFacts)
                else (if (strictNullChecks) TypeFacts.ObjectStrictFacts
                      else TypeFacts.ObjectFacts))
      ...
      if ((flags & TypeFlags.ESSymbol)) {
        return (if (strictNullChecks) TypeFacts.SymbolStrictFacts
                else TypeFacts.SymbolFacts)
    ...
    def getTypeWithDefault(`type`: Type, defaultExpression: Expression) = {
      if (defaultExpression) {
        ...
        return getUnionType(
          Array(getTypeWithFacts(`type`, TypeFacts.NEUndefined), defaultType))

      }
      return `type`

    }
    def getTypeOfDestructuredProperty(
        `type`: Type,
        name: (Identifier | LiteralExpression | ComputedPropertyName)) = {
      val text = getTextOfPropertyName(name)
      return (((getTypeOfPropertyOfType(`type`, text) || (isNumericLiteralName(
        text) && getIndexTypeOfType(`type`, IndexKind.Number))) || getIndexTypeOfType(
        `type`,
        IndexKind.String)) || unknownType)

    }
    def getTypeOfDestructuredArrayElement(`type`: Type, index: Int) = {
      return (((isTupleLikeType(`type`) && getTypeOfPropertyOfType(
        `type`,
        ("" + index))) || checkIteratedTypeOrElementType(
        `type`,
        undefined,
        false)) || unknownType)
    ...
    def getAssignedTypeOfBinaryExpression(node: BinaryExpression): Type = {
      return (if (((node.parent.kind === SyntaxKind.ArrayLiteralExpression) || (node.parent.kind === SyntaxKind.PropertyAssignment)))
                getTypeWithDefault(getAssignedType(node), node.right)
              else checkExpression(node.right))

    }
    def getAssignedTypeOfArrayLiteralElement(node: ArrayLiteralExpression,
                                             element: Expression): Type = {
      return getTypeOfDestructuredArrayElement(
        getAssignedType(node),
        indexOf(node.elements, element))

    }
    def getAssignedTypeOfSpreadElement(node: SpreadElementExpression): Type = {
      return getTypeOfDestructuredSpreadElement(
        getAssignedType(node.parent.asInstanceOf[ArrayLiteralExpression]))

    }
    def getAssignedTypeOfPropertyAssignment(
        node: (PropertyAssignment | ShorthandPropertyAssignment)): Type = {
      return getTypeOfDestructuredProperty(
        getAssignedType(node.parent.asInstanceOf[ObjectLiteralExpression]),
        node.name)

    }
    def getAssignedTypeOfShorthandPropertyAssignment(
        node: ShorthandPropertyAssignment): Type = {
    ...
    def getAssignedType(node: Expression): Type = {
      ...
      parent.kind match {
        ...
        case SyntaxKind.ForOfStatement =>
          return (checkRightHandSideOfForOf(
            (parent.asInstanceOf[ForOfStatement]).expression) || unknownType)
        case SyntaxKind.BinaryExpression =>
          return getAssignedTypeOfBinaryExpression(
            parent.asInstanceOf[BinaryExpression])
        case SyntaxKind.DeleteExpression =>
          return undefinedType
        case SyntaxKind.ArrayLiteralExpression =>
          return getAssignedTypeOfArrayLiteralElement(
            parent.asInstanceOf[ArrayLiteralExpression],
            node)
        case SyntaxKind.SpreadElementExpression =>
          return getAssignedTypeOfSpreadElement(
            parent.asInstanceOf[SpreadElementExpression])
        case SyntaxKind.PropertyAssignment =>
          return getAssignedTypeOfPropertyAssignment(
            parent.asInstanceOf[PropertyAssignment])
        case SyntaxKind.ShorthandPropertyAssignment =>
          return getAssignedTypeOfShorthandPropertyAssignment(
            parent.asInstanceOf[ShorthandPropertyAssignment])
        case _ =>
      }
      return unknownType
    ...
    def getInitialTypeOfBindingElement(node: BindingElement): Type = {
      ...
      val parentType = getInitialType(
        pattern.parent.asInstanceOf[(VariableDeclaration | BindingElement)])
      val `type` =
        (if ((pattern.kind === SyntaxKind.ObjectBindingPattern))
           getTypeOfDestructuredProperty(
             parentType,
             (node.propertyName || node.name.asInstanceOf[Identifier]))
         else
           (if ((!node.dotDotDotToken))
              getTypeOfDestructuredArrayElement(
                parentType,
                indexOf(pattern.elements, node))
            else getTypeOfDestructuredSpreadElement(parentType)))
    ...
    def getInitialType(node: (VariableDeclaration | BindingElement)) = {
      return (if ((node.kind === SyntaxKind.VariableDeclaration))
                getInitialTypeOfVariableDeclaration(
                  node.asInstanceOf[VariableDeclaration])
              else
                getInitialTypeOfBindingElement(
                  node.asInstanceOf[BindingElement]))

    }
    def getInitialOrAssignedType(
        node: (VariableDeclaration | BindingElement | Expression)) = {
      return (if (((node.kind === SyntaxKind.VariableDeclaration) || (node.kind === SyntaxKind.BindingElement)))
                getInitialType(
                  node.asInstanceOf[(VariableDeclaration | BindingElement)])
              else getAssignedType(node.asInstanceOf[Expression]))

    }
    def isEmptyArrayAssignment(
        node: (VariableDeclaration | BindingElement | Expression)) = {
      return ((((node.kind === SyntaxKind.VariableDeclaration) && (node
        .asInstanceOf[VariableDeclaration])
        .initializer) && isEmptyArrayLiteral((node
        .asInstanceOf[VariableDeclaration])
        .initializer)) || (((node.kind !== SyntaxKind.BindingElement) && (node.parent.kind === SyntaxKind.BinaryExpression)) && isEmptyArrayLiteral(
    ...
    def getReferenceRoot(node: Node): Node = {
      val parent = node.parent
      return (if ((((parent.kind === SyntaxKind.ParenthesizedExpression) || (((parent.kind === SyntaxKind.BinaryExpression) && ((parent
                    .asInstanceOf[BinaryExpression])
                    .operatorToken
                    .kind === SyntaxKind.EqualsToken)) && ((parent
                    .asInstanceOf[BinaryExpression])
                    .left === node))) || (((parent.kind === SyntaxKind.BinaryExpression) && ((parent
                    .asInstanceOf[BinaryExpression])
                    .operatorToken
                    .kind === SyntaxKind.CommaToken)) && ((parent
                    .asInstanceOf[BinaryExpression])
                    .right === node)))) getReferenceRoot(parent)
              else node)

    }
    def getTypeOfSwitchClause(clause: (CaseClause | DefaultClause)) = {
      if ((clause.kind === SyntaxKind.CaseClause)) {
        val caseType = getRegularTypeOfLiteralType(
          checkExpression((clause.asInstanceOf[CaseClause]).expression))
    ...
    def getSwitchClauseTypes(switchStatement: SwitchStatement): Array[Type] = {
      ...
      if ((!links.switchTypes)) {
        val types =
          map(switchStatement.caseBlock.clauses, getTypeOfSwitchClause)
        (links.switchTypes =
          (if ((!contains(types, undefined))) types else emptyArray))
    ...
    def eachTypeContainedIn(source: Type, types: Array[Type]) = {
      return (if ((source.flags & TypeFlags.Union))
                (!forEach(
                  (source.asInstanceOf[UnionType]).types,
                  (t => (!contains(types, t)))))
              else contains(types, source))
    ...
    def filterType(`type`: Type, f: ((Type) => Boolean)): Type = {
      if ((`type`.flags & TypeFlags.Union)) {
        ...
        return (if ((filtered === types)) `type`
                else getUnionTypeFromSortedList(filtered))
    ...
    def mapType(`type`: Type, f: ((Type) => Type)): Type = {
      return (if ((`type`.flags & TypeFlags.Union))
                getUnionType(map((`type`.asInstanceOf[UnionType]).types, f))
              else f(`type`))
    ...
    def replacePrimitivesWithLiterals(typeWithPrimitives: Type,
                                      typeWithLiterals: Type) = {
      if (((isTypeSubsetOf(stringType, typeWithPrimitives) && maybeTypeOfKind(
            typeWithLiterals,
            TypeFlags.StringLiteral)) || (isTypeSubsetOf(
            numberType,
            typeWithPrimitives) && maybeTypeOfKind(
            typeWithLiterals,
            TypeFlags.NumberLiteral)))) {
        return mapType(
          typeWithPrimitives,
    ...
    def getTypeFromFlowType(flowType: FlowType) = {
      return (if ((flowType.flags === 0))(flowType
                .asInstanceOf[IncompleteType])
                .`type`
              else flowType.asInstanceOf[Type])

    }
    def createFlowType(`type`: Type, incomplete: Boolean): FlowType = {
      return (if (incomplete) Map("flags" -> 0, "type" -> `type`) else `type`)

    }
    def createEvolvingArrayType(elementType: Type): EvolvingArrayType = {
      val result = createObjectType(ObjectFlags.EvolvingArray)
        .asInstanceOf[EvolvingArrayType]
    ...
    def getEvolvingArrayType(elementType: Type): EvolvingArrayType = {
      return (evolvingArrayTypes(elementType.id) || ((
        evolvingArrayTypes(elementType.id) =
          createEvolvingArrayType(elementType))))

    }
    def addEvolvingArrayElementType(evolvingArrayType: EvolvingArrayType,
                                    node: Expression): EvolvingArrayType = {
      val elementType = getBaseTypeOfLiteralType(checkExpression(node))
      return (if (isTypeSubsetOf(elementType, evolvingArrayType.elementType))
                evolvingArrayType
              else
                getEvolvingArrayType(
                  getUnionType(
                    Array(evolvingArrayType.elementType, elementType))))

    }
    def createFinalArrayType(elementType: Type) = {
      return (if ((elementType.flags & TypeFlags.Never)) autoArrayType
              else
                createArrayType(
                  (if ((elementType.flags & TypeFlags.Union))
                     getUnionType(
                       (elementType.asInstanceOf[UnionType]).types,
                       true)
                   else elementType)))

    }
    def getFinalArrayType(evolvingArrayType: EvolvingArrayType): Type = {
      return (evolvingArrayType.finalArrayType || ((evolvingArrayType.finalArrayType =
        createFinalArrayType(evolvingArrayType.elementType))))

    }
    def finalizeEvolvingArrayType(`type`: Type): Type = {
      return (if ((getObjectFlags(`type`) & ObjectFlags.EvolvingArray))
                getFinalArrayType(`type`.asInstanceOf[EvolvingArrayType])
              else `type`)

    }
    def getElementTypeOfEvolvingArrayType(`type`: Type) = {
      return (if ((getObjectFlags(`type`) & ObjectFlags.EvolvingArray))(`type`
                .asInstanceOf[EvolvingArrayType])
                .elementType
              else neverType)
    ...
    def getUnionOrEvolvingArrayType(types: Array[Type],
                                    subtypeReduction: Boolean) = {
      return (if (isEvolvingArrayTypeList(types))
                getEvolvingArrayType(
    ...
    def isEvolvingArrayOperationTarget(node: Node) = {
      ...
      val isLengthPushOrUnshift = ((parent.kind === SyntaxKind.PropertyAccessExpression) && ((((parent
          .asInstanceOf[PropertyAccessExpression])
          .name
          .text === "length") || ((parent.parent.kind === SyntaxKind.CallExpression) && isPushOrUnshiftIdentifier(
          (parent.asInstanceOf[PropertyAccessExpression]).name)))))
      val isElementAssignment = (((((((parent.kind === SyntaxKind.ElementAccessExpression) && ((parent
          .asInstanceOf[ElementAccessExpression])
          .expression === root)) && (parent.parent.kind === SyntaxKind.BinaryExpression)) && ((parent.parent
          .asInstanceOf[BinaryExpression])
          .operatorToken
          .kind === SyntaxKind.EqualsToken)) && ((parent.parent
          .asInstanceOf[BinaryExpression])
          .left === parent)) && (!isAssignmentTarget(parent.parent))) && isTypeAnyOrAllConstituentTypesHaveKind(
          checkExpression(
    ...
    def getFlowTypeOfReference(reference: Node,
                               declaredType: Type,
      ...
      val initialType =
        (if (assumeInitialized) declaredType
         else
           (if (((declaredType === autoType) || (declaredType === autoArrayType)))
              undefinedType
            else includeFalsyTypes(declaredType, TypeFlags.Undefined)))
      val visitedFlowStart = visitedFlowCount
      val evolvedType = getTypeFromFlowType(
        getTypeAtFlowNode(reference.flowNode))
      (visitedFlowCount = visitedFlowStart)
      val resultType =
        (if (((getObjectFlags(evolvedType) & ObjectFlags.EvolvingArray) && isEvolvingArrayOperationTarget(
               reference))) anyArrayType
         else finalizeEvolvingArrayType(evolvedType))
      if (((reference.parent.kind === SyntaxKind.NonNullExpression) && (getTypeWithFacts(
            resultType,
        ...
        while (true) {
          {
            ...
            if ((flow.flags & FlowFlags.Assignment)) {
              (`type` = getTypeAtFlowAssignment(
                flow.asInstanceOf[FlowAssignment]))
              if ((!`type`)) {
              ...
              (`type` = getTypeAtFlowCondition(
                flow.asInstanceOf[FlowCondition]))

            } else if ((flow.flags & FlowFlags.SwitchClause)) {
              (`type` = getTypeAtSwitchClause(
                flow.asInstanceOf[FlowSwitchClause]))
              ...
              (`type` =
                (if ((flow.flags & FlowFlags.BranchLabel))
                   getTypeAtFlowBranchLabel(flow.asInstanceOf[FlowLabel])
                 else getTypeAtFlowLoopLabel(flow.asInstanceOf[FlowLabel])))

            } else if ((flow.flags & FlowFlags.ArrayMutation)) {
              (`type` = getTypeAtFlowArrayMutation(
                flow.asInstanceOf[FlowArrayMutation]))
              if ((!`type`)) {
      ...
      def getTypeAtFlowAssignment(flow: FlowAssignment) = {
        ...
        if (isMatchingReference(reference, node)) {
          ...
          if (((declaredType === autoType) || (declaredType === autoArrayType))) {
            ...
            val assignedType = getBaseTypeOfLiteralType(
              getInitialOrAssignedType(node))
            return (if (isTypeAssignableTo(assignedType, declaredType))
                      assignedType
                    else anyArrayType)
          ...
          if ((declaredType.flags & TypeFlags.Union)) {
            return getAssignmentReducedType(
              declaredType.asInstanceOf[UnionType],
              getInitialOrAssignedType(node))
      ...
      def getTypeAtFlowArrayMutation(flow: FlowArrayMutation): FlowType = {
        val node = flow.node
        val expr =
          (if ((node.kind === SyntaxKind.CallExpression))((node
        ...
        if (isMatchingReference(reference, getReferenceCandidate(expr))) {
          ...
          if ((getObjectFlags(`type`) & ObjectFlags.EvolvingArray)) {
            ...
            if ((node.kind === SyntaxKind.CallExpression)) {
              ((node.asInstanceOf[CallExpression]).arguments).foreach {
                fresh84 =>
                  val arg = zeroOfMyType = fresh84 {
                    (evolvedType =
                      addEvolvingArrayElementType(evolvedType, arg))

                  }
              }

            } else {
              val indexType = checkExpression(
                ((node
                  .asInstanceOf[BinaryExpression])
                  .left
                  .asInstanceOf[ElementAccessExpression])
                  .argumentExpression)
              if (isTypeAnyOrAllConstituentTypesHaveKind(
                    indexType,
                    (TypeFlags.NumberLike | TypeFlags.Undefined))) {
            ...
            return (if ((evolvedType === `type`)) flowType
                    else createFlowType(evolvedType, isIncomplete(flowType)))
      ...
      def getTypeAtFlowCondition(flow: FlowCondition): FlowType = {
        ...
        val narrowedType =
          narrowType(nonEvolvingType, flow.expression, assumeTrue)
        ...
        val resultType =
          (if ((incomplete && (narrowedType.flags & TypeFlags.Never)))
             silentNeverType
           else narrowedType)
      ...
      def getTypeAtSwitchClause(flow: FlowSwitchClause): FlowType = {
        ...
        if (isMatchingReference(reference, expr)) {
          ...
          (`type` = narrowTypeByDiscriminant(
            `type`,
            expr.asInstanceOf[PropertyAccessExpression],
            (t =>
      ...
      def getTypeAtFlowBranchLabel(flow: FlowLabel): FlowType = {
        ...
        (flow.antecedents).foreach { fresh85 =>
          val antecedent = zeroOfMyType = fresh85 {
        ...
        return createFlowType(
          getUnionOrEvolvingArrayType(antecedentTypes, subtypeReduction),
          seenIncomplete)

      }
      def getTypeAtFlowLoopLabel(flow: FlowLabel): FlowType = {
        val id = getFlowNodeId(flow)
        val cache = (flowLoopCaches(id) || ((
        ...
        }
        {
          ...
          while ((i < flowLoopCount)) {
            {
              if ((((flowLoopNodes(i) === flow) && (flowLoopKeys(i) === key)) && flowLoopTypes(
                    i).length)) {
                return createFlowType(
                  getUnionOrEvolvingArrayType(flowLoopTypes(i), false),
                  true)
        ...
        (flow.antecedents).foreach { fresh86 =>
          val antecedent = zeroOfMyType = fresh86 {
            (flowLoopCount += 1)
            val flowType = getTypeAtFlowNode(antecedent)
            (flowLoopCount -= 1)
      ...
      def isMatchingReferenceDiscriminant(expr: Expression) = {
        return ((((expr.kind === SyntaxKind.PropertyAccessExpression) && (declaredType.flags & TypeFlags.Union)) && isMatchingReference(
          reference,
          (expr
            .asInstanceOf[PropertyAccessExpression])
            .expression)) && isDiscriminantProperty(
      ...
      def narrowTypeByDiscriminant(`type`: Type,
                                   propAccess: PropertyAccessExpression,
                                   narrowType: ((Type) => Type)): Type = {
        ...
        return (if ((propType === narrowedPropType)) `type`
                else
                  filterType(
                    `type`,
                    (t =>
                       isTypeComparableTo(
                         getTypeOfPropertyOfType(t, propName),
                         narrowedPropType))))

      }
      def narrowTypeByTruthiness(`type`: Type,
                                 expr: Expression,
                                 assumeTrue: Boolean): Type = {
        if (isMatchingReference(reference, expr)) {
          return getTypeWithFacts(
            `type`,
            (if (assumeTrue) TypeFacts.Truthy else TypeFacts.Falsy))
      ...
      def narrowTypeByBinaryExpression(`type`: Type,
          ...
          case SyntaxKind.EqualsEqualsToken |
            ...
            if (((left.kind === SyntaxKind.TypeOfExpression) && (right.kind === SyntaxKind.StringLiteral))) {
              return narrowTypeByTypeof(
                `type`,
                left.asInstanceOf[TypeOfExpression],
                operator,
                right.asInstanceOf[LiteralExpression],
                assumeTrue)
            ...
            if (((right.kind === SyntaxKind.TypeOfExpression) && (left.kind === SyntaxKind.StringLiteral))) {
              return narrowTypeByTypeof(
                `type`,
                right.asInstanceOf[TypeOfExpression],
                operator,
                left.asInstanceOf[LiteralExpression],
                assumeTrue)
            ...
            if (isMatchingReferenceDiscriminant(left)) {
              return narrowTypeByDiscriminant(
                ...
                (t => narrowTypeByEquality(t, operator, right, assumeTrue)))

            }
            if (isMatchingReferenceDiscriminant(right)) {
              return narrowTypeByDiscriminant(
                `type`,
                right.asInstanceOf[PropertyAccessExpression],
                (t => narrowTypeByEquality(t, operator, left, assumeTrue)))

            }
            if ((containsMatchingReferenceDiscriminant(reference, left) || containsMatchingReferenceDiscriminant(
                  reference,
                  right))) {
              return declaredType
      ...
      def narrowTypeByEquality(`type`: Type,
                               operator: SyntaxKind,
                               value: Expression,
                               assumeTrue: Boolean): Type = {
        ...
        if ((valueType.flags & TypeFlags.Nullable)) {
          ...
          val facts =
            (if (doubleEquals)
               (if (assumeTrue) TypeFacts.EQUndefinedOrNull
                else TypeFacts.NEUndefinedOrNull)
             else
               (if ((value.kind === SyntaxKind.NullKeyword))
                  (if (assumeTrue) TypeFacts.EQNull else TypeFacts.NENull)
                else (if (assumeTrue) TypeFacts.EQUndefined
                      else TypeFacts.NEUndefined)))
        ...
        if (assumeTrue) {
          val narrowedType =
            filterType(`type`, (t => areTypesComparable(t, valueType)))
          return (if ((narrowedType.flags & TypeFlags.Never)) `type`
                  else replacePrimitivesWithLiterals(narrowedType, valueType))
      ...
      def narrowTypeByTypeof(`type`: Type,
        ...
        val facts =
          (if (assumeTrue)
           ...
           else (typeofNEFacts(literal.text) || TypeFacts.TypeofNEHostObject))
        return getTypeWithFacts(`type`, facts)

      }
      def narrowTypeBySwitchOnDiscriminant(`type`: Type,
        ...
        val hasDefaultClause = ((clauseStart === clauseEnd) || contains(
            clauseTypes,
            neverType))
        val discriminantType = getUnionType(clauseTypes)
        val caseType =
          (if ((discriminantType.flags & TypeFlags.Never)) neverType
           else
             replacePrimitivesWithLiterals(
               filterType(
                 `type`,
                 (t => isTypeComparableTo(discriminantType, t))),
               discriminantType))
        if ((!hasDefaultClause)) {
          return caseType

        }
        val defaultType = filterType(
          `type`,
          (t =>
             (!((isUnitType(t) && contains(
               switchTypes,
               getRegularTypeOfLiteralType(t)))))))
        return (if ((caseType.flags & TypeFlags.Never)) defaultType
                else getUnionType(Array(caseType, defaultType)))

      }
      def narrowTypeByInstanceof(`type`: Type,
                                 expr: BinaryExpression,
                                 assumeTrue: Boolean): Type = {
        val left = getReferenceCandidate(expr.left)
        if ((!isMatchingReference(reference, left))) {
        ...
        if ((!targetType)) {
          ...
          if ((getObjectFlags(rightType) & ObjectFlags.Interface)) {
            (constructSignatures = resolveDeclaredMembers(
              rightType
                .asInstanceOf[InterfaceType]).declaredConstructSignatures)

          } else if ((getObjectFlags(rightType) & ObjectFlags.Anonymous)) {
            (constructSignatures =
              getSignaturesOfType(rightType, SignatureKind.Construct))

          }
          if ((constructSignatures && constructSignatures.length)) {
            (targetType = getUnionType(
              map(
                constructSignatures,
                (signature =>
                   getReturnTypeOfSignature(getErasedSignature(signature))))))
      ...
      def getNarrowedType(`type`: Type, candidate: Type, assumeTrue: Boolean) = {
        ...
        if ((`type`.flags & TypeFlags.Union)) {
          val assignableType =
            filterType(`type`, (t => isTypeInstanceOf(t, candidate)))
        ...
        return (if (isTypeSubtypeOf(candidate, `type`)) candidate
                else (if (isTypeAssignableTo(`type`, candidate)) `type`
                      else (if (isTypeAssignableTo(candidate, targetType)) candidate
                            else getIntersectionType(Array(`type`, candidate)))))

      }
      def narrowTypeByTypePredicate(`type`: Type,
        ...
        if (isIdentifierTypePredicate(predicate)) {
          val predicateArgument =
            callExpression.arguments(predicate.parameterIndex)
        ...
        expr.kind match {
          ...
          case SyntaxKind.CallExpression =>
            return narrowTypeByTypePredicate(
              `type`,
              expr.asInstanceOf[CallExpression],
              assumeTrue)
          case SyntaxKind.ParenthesizedExpression =>
            return narrowType(
              `type`,
      ...
      if ((location.kind === SyntaxKind.Identifier)) {
        ...
        if ((isPartOfExpression(location) && (!isAssignmentTarget(location)))) {
          ...
          if ((getExportSymbolOfValueSymbolIfExported(
                getNodeLinks(location).resolvedSymbol) === symbol)) {
            return `type`
    ...
    def skipParenthesizedNodes(expression: Expression): Expression = {
      while ((expression.kind === SyntaxKind.ParenthesizedExpression)) {
        {
          (expression =
            (expression.asInstanceOf[ParenthesizedExpression]).expression)
    ...
    def getControlFlowContainer(node: Node): Node = {
      while (true) {
        {
          (node = node.parent)
          if (((((isFunctionLike(node) && (!getImmediatelyInvokedFunctionExpression(
                node))) || (node.kind === SyntaxKind.ModuleBlock)) || (node.kind === SyntaxKind.SourceFile)) || (node.kind === SyntaxKind.PropertyDeclaration))) {
            return node
    ...
    def isParameterAssigned(symbol: Symbol) = {
      val func = getRootDeclaration(symbol.valueDeclaration).parent
        .asInstanceOf[FunctionLikeDeclaration]
    ...
    def markParameterAssignments(node: Node) = {
      if ((node.kind === SyntaxKind.Identifier)) {
        if (isAssignmentTarget(node)) {
          val symbol = getResolvedSymbol(node.asInstanceOf[Identifier])
          if ((symbol.valueDeclaration && (getRootDeclaration(
    ...
    def isConstVariable(symbol: Symbol) = {
      return (((symbol.flags & SymbolFlags.Variable) && (((getDeclarationNodeFlagsFromSymbol(
        symbol) & NodeFlags.Const)) !== 0)) && (getTypeOfSymbol(symbol) !== autoArrayType))
    ...
    def checkIdentifier(node: Identifier): Type = {
      ...
      if ((((symbol.flags & SymbolFlags.Alias) && (!isInTypeQuery(node))) && (!isConstEnumOrConstEnumOnlyModule(
            resolveAlias(symbol))))) {
        markAliasSymbolAsReferenced(symbol)
      ...
      if ((localOrExportSymbol.flags & SymbolFlags.Class)) {
        ...
        if ((((languageVersion === ScriptTarget.ES2015) && (declaration.kind === SyntaxKind.ClassDeclaration)) && nodeIsDecorated(
              declaration))) {
          ...
          while ((container !== undefined)) {
            {
              if ((container.parent === declaration)) {
                if (((container.kind === SyntaxKind.PropertyDeclaration) && hasModifier(
                      container,
      ...
      val declaration = localOrExportSymbol.valueDeclaration
      if ((((!((localOrExportSymbol.flags & SymbolFlags.Variable))) || isAssignmentTarget(
            node)) || (!declaration))) {
        return `type`
      ...
      while ((((flowContainer !== declarationContainer) && ((((flowContainer.kind === SyntaxKind.FunctionExpression) || (flowContainer.kind === SyntaxKind.ArrowFunction)) || isObjectLiteralOrClassExpressionMethod(
               flowContainer)))) && ((isConstVariable(localOrExportSymbol) || (isParameter && (!isParameterAssigned(
               localOrExportSymbol))))))) {
        {
      ...
      val assumeInitialized = (((isParameter || isOuterVariable) || (((`type` !== autoType) && (`type` !== autoArrayType)) && (((!strictNullChecks) || (((`type`.flags & TypeFlags.Any)) !== 0))))) || isInAmbientContext(
          declaration))
      val flowType =
        getFlowTypeOfReference(node, `type`, assumeInitialized, flowContainer)
      ...
      } else if ((((!assumeInitialized) && (!((getFalsyFlags(`type`) & TypeFlags.Undefined)))) && (getFalsyFlags(
                   flowType) & TypeFlags.Undefined))) {
        error(
          node,
    ...
    def checkNestedBlockScopedBinding(node: Identifier, symbol: Symbol): Unit = {
      ...
      if (containedInIterationStatement) {
        ...
        if ((((container.kind === SyntaxKind.ForStatement) && (getAncestor(
              symbol.valueDeclaration,
              SyntaxKind.VariableDeclarationList).parent === container)) && isAssignedInBodyOfForStatement(
              node,
              container.asInstanceOf[ForStatement]))) {
    ...
    def isAssignedInBodyOfForStatement(node: Identifier,
                                       container: ForStatement): Boolean = {
    ...
    def getSuperCallInConstructor(
        `constructor`: ConstructorDeclaration): ExpressionStatement = {
    ...
    def classDeclarationExtendsNull(classDecl: ClassDeclaration): Boolean = {
      ...
      val classInstanceType = getDeclaredTypeOfSymbol(classSymbol)
        .asInstanceOf[InterfaceType]
      val baseConstructorType = getBaseConstructorTypeOfClass(
        classInstanceType)
    ...
    def checkThisExpression(node: Node): Type = {
      ...
      if ((container.kind === SyntaxKind.Constructor)) {
        val containingClassDecl =
          container.parent.asInstanceOf[ClassDeclaration]
        val baseTypeNode = getClassExtendsHeritageClauseElement(
          containingClassDecl)
        if ((baseTypeNode && (!classDeclarationExtendsNull(
              containingClassDecl)))) {
          val superCall = getSuperCallInConstructor(
            container.asInstanceOf[ConstructorDeclaration])
      ...
      container.kind match {
        ...
        case SyntaxKind.PropertyDeclaration | SyntaxKind.PropertySignature =>
          if ((getModifierFlags(container) & ModifierFlags.Static)) {
            error(
              node,
              Diagnostics.this_cannot_be_referenced_in_a_static_property_initializer)
      ...
      if ((isFunctionLike(container) && (((!isInParameterInitializerBeforeContainingFunction(
            node)) || getThisParameter(container))))) {
        if ((((container.kind === SyntaxKind.FunctionExpression) && isInJavaScriptFile(
              container.parent)) && (getSpecialPropertyAssignmentKind(
              container.parent) === SpecialPropertyAssignmentKind.PrototypeProperty))) {
          val className = (((container.parent
            .asInstanceOf[BinaryExpression])
            .left
            .asInstanceOf[PropertyAccessExpression])
            .expression
        ...
        val thisType = (getThisTypeOfDeclaration(container) || getContextualThisParameterType(
            container))
        if (thisType) {
          return thisType
    ...
    def getTypeForThisExpressionFromJSDoc(node: Node) = {
      ...
      if ((((typeTag && typeTag.typeExpression) && typeTag.typeExpression.`type`) && (typeTag.typeExpression.`type`.kind === SyntaxKind.JSDocFunctionType))) {
        val jsDocFunctionType =
          typeTag.typeExpression.`type`.asInstanceOf[JSDocFunctionType]
    ...
    def isInConstructorArgumentInitializer(node: Node,
                                           constructorDecl: Node): Boolean = {
      {
        var n = node
    ...
    def checkSuperExpression(node: Node): Type = {
      val isCallExpression = ((node.parent.kind === SyntaxKind.CallExpression) && ((node.parent
          .asInstanceOf[CallExpression])
          .expression === node))
      ...
      if ((!canUseSuperExpression)) {
        ...
        } else if ((((!container) || (!container.parent)) || (!((isClassLike(
                     container.parent) || (container.parent.kind === SyntaxKind.ObjectLiteralExpression)))))) {
          error(
            node,
      ...
      if (((container.kind === SyntaxKind.MethodDeclaration) && (getModifierFlags(
            container) & ModifierFlags.Async))) {
      ...
      val classLikeDeclaration =
        container.parent.asInstanceOf[ClassLikeDeclaration]
      val classType = getDeclaredTypeOfSymbol(
        getSymbolOfNode(classLikeDeclaration)).asInstanceOf[InterfaceType]
      ...
      if (((container.kind === SyntaxKind.Constructor) && isInConstructorArgumentInitializer(
            node,
            container))) {
        error(
          node,
      ...
      return (if ((nodeCheckFlag === NodeCheckFlags.SuperStatic))
              ...
              else getTypeWithThisArgument(baseClassType, classType.thisType))
    ...
    def getContextuallyTypedParameterType(
        parameter: ParameterDeclaration): Type = {
      ...
      if (isContextSensitiveFunctionOrObjectLiteralMethod(func)) {
        ...
        if (iife) {
          ...
          if ((iife.arguments && (indexOfParameter < iife.arguments.length))) {
                ...
                while ((i < iife.arguments.length)) {
                  {
                    restTypes.push(
                      getWidenedLiteralType(
                        checkExpression(iife.arguments(i))))
            ...
            val `type` = getWidenedLiteralType(
              checkExpression(iife.arguments(indexOfParameter)))
        ...
        if (contextualSignature) {
          ...
          val len = (func.parameters.length - ((if (funcHasRestParameters) 1
                                                else 0) ))
          val indexOfParameter = indexOf(func.parameters, parameter)
          ...
          if (((funcHasRestParameters && (indexOfParameter === ((func.parameters.length - 1)))) && isRestParameterIndex(
            ...
            return getTypeOfSymbol(
              lastOrUndefined(contextualSignature.parameters))
    ...
    def getContextualTypeForInitializerExpression(node: Expression): Type = {
      ...
      if ((node === declaration.initializer)) {
        ...
        if ((declaration.kind === SyntaxKind.Parameter)) {
          val `type` = getContextuallyTypedParameterType(
            declaration.asInstanceOf[ParameterDeclaration])
          if (`type`) {
            return `type`
        ...
        if (isBindingPattern(declaration.parent)) {
          ...
          val name = (declaration.propertyName || declaration.name)
          if (((isVariableLike(parentDeclaration) && parentDeclaration.`type`) && (!isBindingPattern(
                name)))) {
            val text = getTextOfPropertyName(name)
            if (text) {
              return getTypeOfPropertyOfType(
                getTypeFromTypeNode(parentDeclaration.`type`),
                text)
    ...
    def getContextualTypeForYieldOperand(node: YieldExpression): Type = {
      ...
      if (func) {
        val contextualReturnType = getContextualReturnType(func)
        if (contextualReturnType) {
          return (if (node.asteriskToken) contextualReturnType
                  else getElementTypeOfIterableIterator(contextualReturnType))
    ...
    def isInParameterInitializerBeforeContainingFunction(node: Node) = {
      while ((node.parent && (!isFunctionLike(node.parent)))) {
        {
          if (((node.parent.kind === SyntaxKind.Parameter) && ((node.parent
                .asInstanceOf[ParameterDeclaration])
                .initializer === node))) {
            return true
    ...
    def getContextualReturnType(functionDecl: FunctionLikeDeclaration): Type = {
      if (((functionDecl.`type` || (functionDecl.kind === SyntaxKind.Constructor)) || ((functionDecl.kind === SyntaxKind.GetAccessor) && getSetAccessorTypeAnnotationNode(
            getDeclarationOfKind(functionDecl.symbol, SyntaxKind.SetAccessor)
              .asInstanceOf[SetAccessorDeclaration])))) {
        return getReturnTypeOfSignature(
          getSignatureFromDeclaration(functionDecl))

      }
      val signature = getContextualSignatureForFunctionLikeDeclaration(
        functionDecl.asInstanceOf[FunctionExpression])
      if (signature) {
    ...
    def getContextualTypeForArgument(callTarget: CallLikeExpression,
                                     arg: Expression): Type = {
    ...
    def getContextualTypeForSubstitutionExpression(
      ...
      if ((template.parent.kind === SyntaxKind.TaggedTemplateExpression)) {
        return getContextualTypeForArgument(
          template.parent.asInstanceOf[TaggedTemplateExpression],
    ...
    def applyToContextualType(`type`: Type, mapper: ((Type) => Type)): Type = {
      ...
      (types).foreach { fresh87 =>
        val current = zeroOfMyType = fresh87 {
    ...
    def getTypeOfPropertyOfContextualType(`type`: Type, name: String) = {
      return applyToContextualType(
        `type`,
        (t => {
           val prop =
             (if ((t.flags & TypeFlags.StructuredType))
                getPropertyOfType(t, name)
              else undefined)
           return (if (prop) getTypeOfSymbol(prop) else undefined)
    ...
    def getIndexTypeOfContextualType(`type`: Type, kind: IndexKind) = {
      return applyToContextualType(
        `type`,
        (t => getIndexTypeOfStructuredType(t, kind)))
    ...
    def contextualTypeIsTupleLikeType(`type`: Type): Boolean = {
      return (!(!((if ((`type`.flags & TypeFlags.Union))
                     forEach(
                       (`type`.asInstanceOf[UnionType]).types,
                       isTupleLikeType)
                   else isTupleLikeType(`type`)) )))

    }
    def getContextualTypeForObjectLiteralMethod(
        node: MethodDeclaration): Type = {
    ...
    def getContextualTypeForObjectLiteralElement(
      ...
      if (`type`) {
        if ((!hasDynamicName(element))) {
          ...
          val propertyType =
            getTypeOfPropertyOfContextualType(`type`, symbolName)
          if (propertyType) {
            return propertyType
        ...
        return ((isNumericName(element.name) && getIndexTypeOfContextualType(
          `type`,
          IndexKind.Number)) || getIndexTypeOfContextualType(
          `type`,
          IndexKind.String))
    ...
    def getContextualTypeForElementExpression(node: Expression): Type = {
      ...
      if (`type`) {
        ...
        return ((getTypeOfPropertyOfContextualType(`type`, ("" + index)) || getIndexTypeOfContextualType(
          `type`,
          IndexKind.Number)) || ((if ((languageVersion >= ScriptTarget.ES2015))
                                    getElementTypeOfIterable(`type`, undefined)
                                  else undefined) ))
    ...
    def getContextualTypeForConditionalOperand(node: Expression): Type = {
      ...
      return (if (((node === conditional.whenTrue) || (node === conditional.whenFalse)))
                getContextualType(conditional)
              else undefined)

    }
    def getContextualTypeForJsxAttribute(
      ...
      if ((attribute.kind === SyntaxKind.JsxAttribute)) {
        ...
        return getTypeOfPropertyOfType(
          attrsType,
          (attribute.asInstanceOf[JsxAttribute]).name.text)
      ...
      Debug.fail(
        s"""Expected JsxAttribute or JsxSpreadAttribute, got ts.SyntaxKind[${kind}]""")
    ...
    def getContextualType(node: Expression): Type = {
      ...
      val parent = node.parent
      parent.kind match {
        case SyntaxKind.VariableDeclaration | SyntaxKind.Parameter |
        ...
        case SyntaxKind.YieldExpression =>
          return getContextualTypeForYieldOperand(
            parent.asInstanceOf[YieldExpression])
        case SyntaxKind.CallExpression | SyntaxKind.NewExpression =>
          return getContextualTypeForArgument(
            parent.asInstanceOf[CallExpression],
            node)
        case SyntaxKind.TypeAssertionExpression | SyntaxKind.AsExpression =>
          return getTypeFromTypeNode(
            (parent.asInstanceOf[AssertionExpression]).`type`)
        case SyntaxKind.BinaryExpression =>
          return getContextualTypeForBinaryOperand(node)
        case SyntaxKind.PropertyAssignment |
            SyntaxKind.ShorthandPropertyAssignment =>
          return getContextualTypeForObjectLiteralElement(
            parent.asInstanceOf[ObjectLiteralElementLike])
        case SyntaxKind.ArrayLiteralExpression =>
        ...
        case SyntaxKind.TemplateSpan =>
          ...
          return getContextualTypeForSubstitutionExpression(
            parent.parent.asInstanceOf[TemplateExpression],
            node)
        case SyntaxKind.ParenthesizedExpression =>
          return getContextualType(
            parent.asInstanceOf[ParenthesizedExpression])
        ...
        case SyntaxKind.JsxAttribute | SyntaxKind.JsxSpreadAttribute =>
          return getContextualTypeForJsxAttribute(
            parent.asInstanceOf[(JsxAttribute | JsxSpreadAttribute)])
        case _ =>
      }
      return undefined

    }
    def getNonGenericSignature(
        `type`: Type,
        node: (FunctionExpression | ArrowFunction | MethodDeclaration))
      : Signature = {
      val signatures =
        getSignaturesOfStructuredType(`type`, SignatureKind.Call)
      if ((signatures.length === 1)) {
        val signature = signatures(0)
        if (((!signature.typeParameters) && (!isAritySmaller(
              signature,
              node)))) {
          return signature
    ...
    def isAritySmaller(
        signature: Signature,
        target: (FunctionExpression | ArrowFunction | MethodDeclaration)) = {
      ...
      if ((target.parameters.length && parameterIsThisKeyword(
            target.parameters(0)))) {
        (targetParameterCount -= 1)

      }
      val sourceLength =
        (if (signature.hasRestParameter) Number.MAX_VALUE
         else signature.parameters.length)
    ...
    def getContextualSignatureForFunctionLikeDeclaration(
        node: FunctionLikeDeclaration): Signature = {
      return (if ((isFunctionExpressionOrArrowFunction(node) || isObjectLiteralMethod(
                    node)))
                getContextualSignature(node.asInstanceOf[FunctionExpression])
              else undefined)

    }
    def getContextualTypeForFunctionLikeDeclaration(
        node: (FunctionExpression | ArrowFunction | MethodDeclaration)) = {
      return (if (isObjectLiteralMethod(node))
                getContextualTypeForObjectLiteralMethod(node)
              else getApparentTypeOfContextualType(node))

    }
    def getContextualSignature(
        node: (FunctionExpression | ArrowFunction | MethodDeclaration))
      : Signature = {
      Debug.assert(
        ((node.kind !== SyntaxKind.MethodDeclaration) || isObjectLiteralMethod(
      ...
      (types).foreach { fresh88 =>
        val current = zeroOfMyType = fresh88 {
    ...
    def checkSpreadElementExpression(node: SpreadElementExpression,
                                     contextualMapper: TypeMapper): Type = {
      val arrayOrIterableType =
        checkExpressionCached(node.expression, contextualMapper)
    ...
    def hasDefaultValue(node: (BindingElement | Expression)): Boolean = {
      return ((((node.kind === SyntaxKind.BindingElement) && (!(!(node
        .asInstanceOf[BindingElement])
        .initializer)))) || (((node.kind === SyntaxKind.BinaryExpression) && ((node
        .asInstanceOf[BinaryExpression])
        .operatorToken
        .kind === SyntaxKind.EqualsToken))))

    }
    def checkArrayLiteral(node: ArrayLiteralExpression,
                          contextualMapper: TypeMapper): Type = {
      ...
      if ((!hasSpreadElement)) {
        ...
        if ((contextualType && contextualTypeIsTupleLikeType(contextualType))) {
          ...
          if ((pattern && (((pattern.kind === SyntaxKind.ArrayBindingPattern) || (pattern.kind === SyntaxKind.ArrayLiteralExpression))))) {
            val patternElements = (pattern
              .asInstanceOf[(BindingPattern | ArrayLiteralExpression)])
              .elements {
      ...
      return createArrayType(
        (if (elementTypes.length) getUnionType(elementTypes, true)
         else (if (strictNullChecks) neverType
               else undefinedWideningType)))

    }
    def isNumericName(name: DeclarationName): Boolean = {
      return (if ((name.kind === SyntaxKind.ComputedPropertyName))
                isNumericComputedName(name.asInstanceOf[ComputedPropertyName])
              else isNumericLiteralName((name.asInstanceOf[Identifier]).text))
    ...
    def isTypeAnyOrAllConstituentTypesHaveKind(`type`: Type,
                                               kind: TypeFlags): Boolean = {
    ...
    def checkComputedPropertyName(node: ComputedPropertyName): Type = {
      ...
      if ((!links.resolvedType)) {
        ...
        if ((!isTypeAnyOrAllConstituentTypesHaveKind(
              links.resolvedType,
              ((TypeFlags.NumberLike | TypeFlags.StringLike) | TypeFlags.ESSymbol)))) {
          error(
            node,
    ...
    def getObjectLiteralIndexInfo(node: ObjectLiteralExpression,
      ...
      val propTypes: Array[Type] = Array() {
        ...
        while ((i < properties.length)) {
          {
            if (((kind === IndexKind.String) || isNumericName(
      ...
      val unionType =
        (if (propTypes.length) getUnionType(propTypes, true)
         else undefinedType)
      return createIndexInfo(unionType, false)

    }
    def checkObjectLiteral(node: ObjectLiteralExpression,
                           contextualMapper: TypeMapper): Type = {
      val inDestructuringPattern = isAssignmentTarget(node)
      ...
      (node.properties).foreach { fresh90 =>
        val memberDecl = zeroOfMyType = fresh90 {
          var member = memberDecl.symbol
          if ((((memberDecl.kind === SyntaxKind.PropertyAssignment) || (memberDecl.kind === SyntaxKind.ShorthandPropertyAssignment)) || isObjectLiteralMethod(
                memberDecl))) {
            var `type`: Type = zeroOfMyType
            if ((memberDecl.kind === SyntaxKind.PropertyAssignment)) {
              (`type` = checkPropertyAssignment(
                memberDecl.asInstanceOf[PropertyAssignment],
                contextualMapper))

            } else if ((memberDecl.kind === SyntaxKind.MethodDeclaration)) {
              (`type` = checkObjectLiteralMethod(
                memberDecl.asInstanceOf[MethodDeclaration],
                contextualMapper))

            } else {
              Debug.assert(
                (memberDecl.kind === SyntaxKind.ShorthandPropertyAssignment))
              (`type` = checkExpressionForMutableLocation(
                (memberDecl.asInstanceOf[ShorthandPropertyAssignment]).name,
                contextualMapper))

            }
            (typeFlags |= `type`.flags)
            val prop = createSymbol(
              ((SymbolFlags.Property | SymbolFlags.Transient) | member.flags),
              member.name).asInstanceOf[TransientSymbol]
            if (inDestructuringPattern) {
              val isOptional = ((((memberDecl.kind === SyntaxKind.PropertyAssignment) && hasDefaultValue(
                  (memberDecl
                    .asInstanceOf[PropertyAssignment])
                    .initializer))) || (((memberDecl.kind === SyntaxKind.ShorthandPropertyAssignment) && (memberDecl
            ...
            } else if ((contextualTypeHasPattern && (!((getObjectFlags(
                         contextualType) & ObjectFlags.ObjectLiteralPatternWithComputedProperties))))) {
            ...
            checkAccessorDeclaration(
              memberDecl.asInstanceOf[AccessorDeclaration])
      ...
      val result = createAnonymousType(
        ...
        stringIndexInfo,
        numberIndexInfo)
      val freshObjectLiteralFlag =
        (if (compilerOptions.suppressExcessPropertyErrors) 0
         else TypeFlags.FreshLiteral)
    ...
    def checkJsxElement(node: JsxElement) = {
      ...
      (node.children).foreach { fresh92 =>
        val child = zeroOfMyType = fresh92 {
          child.kind match {
            case SyntaxKind.JsxExpression =>
            ...
            case SyntaxKind.JsxSelfClosingElement =>
              checkJsxSelfClosingElement(
                child.asInstanceOf[JsxSelfClosingElement])
            case _ =>
          }
    ...
    def checkJsxAttribute(node: JsxAttribute,
                          elementAttributesType: Type,
                          nameTable: Map[Boolean]) = {
      ...
      if (((elementAttributesType === emptyObjectType) && isUnhyphenatedJsxName(
        ...
        val correspondingPropSymbol =
          getPropertyOfType(elementAttributesType, node.name.text)
        (correspondingPropType = (correspondingPropSymbol && getTypeOfSymbol(
            correspondingPropSymbol)))
        if (isUnhyphenatedJsxName(node.name.text)) {
          val attributeType = (getTypeOfPropertyOfType(
              elementAttributesType,
              getTextOfPropertyName(node.name)) || getIndexTypeOfType(
              elementAttributesType,
              IndexKind.String))
          if (attributeType) {
            ...
            if ((!correspondingPropType)) {
              error(
                ...
                node.name.text,
                typeToString(elementAttributesType))
              return unknownType
    ...
    def getJsxType(name: String) = {
      if ((jsxTypes(name) === undefined)) {
        return (jsxTypes(name) = (getExportedTypeFromNamespace(
            JsxNames.JSX,
            name) || unknownType))
    ...
    def getIntrinsicTagSymbol(
        node: (JsxOpeningLikeElement | JsxClosingElement)): Symbol = {
      val links = getNodeLinks(node)
      if ((!links.resolvedSymbol)) {
        val intrinsicElementsType = getJsxType(JsxNames.IntrinsicElements)
        if ((intrinsicElementsType !== unknownType)) {
          val intrinsicProp = getPropertyOfType(
            intrinsicElementsType,
            (node.tagName.asInstanceOf[Identifier]).text)
          if (intrinsicProp) {
          ...
          val indexSignatureType =
            getIndexTypeOfType(intrinsicElementsType, IndexKind.String)
    ...
    def getJsxElementPropertiesName() = {
      val jsxNamespace =
        getGlobalSymbol(JsxNames.JSX, SymbolFlags.Namespace, undefined)
      val attribsPropTypeSym = (jsxNamespace && getSymbol(
          jsxNamespace.exports,
          JsxNames.ElementAttributesPropertyNameContainer,
          SymbolFlags.Type))
      val attribPropType = (attribsPropTypeSym && getDeclaredTypeOfSymbol(
          attribsPropTypeSym))
      val attribProperties = (attribPropType && getPropertiesOfType(
          attribPropType))
      if (attribProperties) {
        if ((attribProperties.length === 0)) {
          ...
          error(
            attribsPropTypeSym.declarations(0),
    ...
    def getResolvedJsxType(node: JsxOpeningLikeElement,
                           elemType: Type,
                           elemClassType: Type): Type = {
      if ((!elemType)) {
      ...
      if (((!elemClassType) || (!isTypeAssignableTo(
            elemInstanceType,
            elemClassType)))) {
        if (jsxElementType) {
          val callSignatures = (elemType && getSignaturesOfType(
              elemType,
              SignatureKind.Call))
          val callSignature = ((callSignatures && (callSignatures.length > 0)) && callSignatures(
              0))
          val callReturnType = (callSignature && getReturnTypeOfSignature(
              callSignature))
          var paramType = (callReturnType && ((if ((callSignature.parameters.length === 0))
                                                 emptyObjectType
                                               else
                                                 getTypeOfSymbol(callSignature
                                                   .parameters(0))) ))
          if ((callReturnType && isTypeAssignableTo(
                callReturnType,
                jsxElementType))) {
      ...
      if (elemClassType) {
        checkTypeRelatedTo(
          ...
          assignableRelation,
          node,
          Diagnostics.JSX_element_type_0_is_not_a_constructor_function_for_JSX_elements)
      ...
      if ((propsName === undefined)) {
        ...
        val attributesType =
          getTypeOfPropertyOfType(elemInstanceType, propsName)
        if ((!attributesType)) {
          return emptyObjectType
          ...
          error(
            node.tagName,
            Diagnostics.JSX_element_attributes_type_0_may_not_be_a_union_type,
            typeToString(attributesType))
          return anyType

        } else {
          var apparentAttributesType = attributesType
          val intrinsicClassAttribs = getJsxType(
            ...
            if (typeParams) {
              if ((typeParams.length === 1)) {
                (apparentAttributesType = intersectTypes(
                  createTypeReference(
                    intrinsicClassAttribs.asInstanceOf[GenericType],
                    Array(elemInstanceType)),
              ...
              (apparentAttributesType =
                intersectTypes(attributesType, intrinsicClassAttribs))
          ...
          if ((intrinsicAttribs !== unknownType)) {
            (apparentAttributesType =
              intersectTypes(intrinsicAttribs, apparentAttributesType))
        ...
        if (isJsxIntrinsicIdentifier(node.tagName)) {
          ...
          if ((links.jsxFlags & JsxFlags.IntrinsicNamedElement)) {
            ...
            return (links.resolvedJsxType =
              getIndexInfoOfSymbol(symbol, IndexKind.String).`type`)
          ...
          return (links.resolvedJsxType =
            getResolvedJsxType(node, undefined, elemClassType))
    ...
    def getJsxAttributePropertySymbol(attrib: JsxAttribute): Symbol = {
      val attributesType = getJsxElementAttributesType(
        attrib.parent.asInstanceOf[JsxOpeningElement])
    ...
    def getJsxGlobalElementClassType(): Type = {
      if ((!jsxElementClassType)) {
        (jsxElementClassType =
          getExportedTypeFromNamespace(JsxNames.JSX, JsxNames.ElementClass))
    ...
    def checkJsxOpeningLikeElement(node: JsxOpeningLikeElement) = {
      ...
      val reactNamespace =
        (if (compilerOptions.reactNamespace) compilerOptions.reactNamespace
         else "React")
      val reactSym = resolveName(
        node.tagName,
        reactNamespace,
        SymbolFlags.Value,
        reactRefErr,
      ...
      if (reactSym) {
        ...
        if (((reactSym.flags & SymbolFlags.Alias) && (!isConstEnumOrConstEnumOnlyModule(
              resolveAlias(reactSym))))) {
        ...
        while ((i >= 0)) {
          {
            if ((node.attributes(i).kind === SyntaxKind.JsxAttribute)) {
              checkJsxAttribute(
                (node.attributes(i)).asInstanceOf[JsxAttribute],
                targetAttributesType,
                nameTable)

            } else {
              Debug.assert(
                (node.attributes(i).kind === SyntaxKind.JsxSpreadAttribute))
              val spreadType = checkJsxSpreadAttribute(
                (node.attributes(i)).asInstanceOf[JsxSpreadAttribute],
                targetAttributesType,
                nameTable)
      ...
      if ((targetAttributesType && (!sawSpreadedAny))) {
        val targetProperties = getPropertiesOfType(targetAttributesType) {
          ...
          while ((i < targetProperties.length)) {
            {
              if (((!((targetProperties(i).flags & SymbolFlags.Optional))) && (!nameTable(
                    targetProperties(i).name)))) {
                error(
                  node,
                  Diagnostics.Property_0_is_missing_in_type_1,
                  targetProperties(i).name,
                  typeToString(targetAttributesType))
    ...
    def getDeclarationKindFromSymbol(s: Symbol) = {
      return (if (s.valueDeclaration) s.valueDeclaration.kind
              else SyntaxKind.PropertyDeclaration)

    }
    def getDeclarationModifierFlagsFromSymbol(s: Symbol): ModifierFlags = {
      return (if (s.valueDeclaration)
                getCombinedModifierFlags(s.valueDeclaration)
              else
                (if ((s.flags & SymbolFlags.Prototype))
                   (ModifierFlags.Public | ModifierFlags.Static)
                 else 0))

    }
    def getDeclarationNodeFlagsFromSymbol(s: Symbol): NodeFlags = {
      return (if (s.valueDeclaration) getCombinedNodeFlags(s.valueDeclaration)
              else 0)

    }
    def checkClassPropertyAccess(
        node: (PropertyAccessExpression | QualifiedName | VariableLikeDeclaration),
        left: (Expression | QualifiedName),
        `type`: Type,
        prop: Symbol): Boolean = {
      val flags = getDeclarationModifierFlagsFromSymbol(prop)
      val declaringClass = getDeclaredTypeOfSymbol(getParentOfSymbol(prop))
        .asInstanceOf[InterfaceType]
      val errorNode =
        (if (((node.kind === SyntaxKind.PropertyAccessExpression) || (node.kind === SyntaxKind.VariableDeclaration)))(node
           .asInstanceOf[(PropertyAccessExpression | VariableDeclaration)])
           .name
         else (node.asInstanceOf[QualifiedName]).right)
      if ((left.kind === SyntaxKind.SuperKeyword)) {
        if (((languageVersion < ScriptTarget.ES2015) && (getDeclarationKindFromSymbol(
              prop) !== SyntaxKind.MethodDeclaration))) {
          error(
            errorNode,
      ...
      if ((flags & ModifierFlags.Private)) {
        val declaringClassDeclaration = getClassLikeDeclarationOfSymbol(
          getParentOfSymbol(prop)).asInstanceOf[ClassLikeDeclaration]
      ...
      val enclosingClass = forEachEnclosingClass(
        node,
        (enclosingDeclaration => {
           val enclosingClass = getDeclaredTypeOfSymbol(
             getSymbolOfNode(enclosingDeclaration)).asInstanceOf[InterfaceType]
           return (if (hasBaseType(enclosingClass, declaringClass))
                     enclosingClass
                   else undefined)
      ...
      if (((`type`.flags & TypeFlags.TypeParameter) && (`type`
            .asInstanceOf[TypeParameter])
            .isThisType)) {
        (`type` = getConstraintOfTypeParameter(
          `type`.asInstanceOf[TypeParameter]))

      }
      if ((!(((getObjectFlags(getTargetType(`type`)) & ObjectFlags.ClassOrInterface) && hasBaseType(
            `type`.asInstanceOf[InterfaceType],
            enclosingClass))))) {
        error(
          errorNode,
          Diagnostics.Property_0_is_protected_and_only_accessible_through_an_instance_of_class_1,
          symbolToString(prop),
          ...
          error(
            ...
            (if ((kind & TypeFlags.Undefined))
               (if ((kind & TypeFlags.Null))
                ...
                else Diagnostics.Object_is_possibly_undefined)
             else Diagnostics.Object_is_possibly_null))
    ...
    def checkPropertyAccessExpressionOrQualifiedName(
      ...
      if (((apparentType === unknownType) || (((`type`.flags & TypeFlags.TypeParameter) && isTypeAny(
            apparentType))))) {
        return apparentType
      ...
      if ((((noUnusedIdentifiers && ((prop.flags & SymbolFlags.ClassMember))) && prop.valueDeclaration) && ((getModifierFlags(
            prop.valueDeclaration) & ModifierFlags.Private)))) {
      ...
      if ((((node.kind !== SyntaxKind.PropertyAccessExpression) || isAssignmentTarget(
            node)) || ((!((prop.flags & (((SymbolFlags.Variable | SymbolFlags.Property) | SymbolFlags.Accessor))))) && (!(((prop.flags & SymbolFlags.Method) && (propType.flags & TypeFlags.Union))))))) {
        return propType

      }
      return getFlowTypeOfReference(node, propType, true, undefined)
      def reportNonexistentProperty(propNode: Identifier,
                                    containingType: Type) = {
        var errorInfo: DiagnosticMessageChain = zeroOfMyType
        if (((containingType.flags & TypeFlags.Union) && (!((containingType.flags & TypeFlags.Primitive))))) {
          ((containingType.asInstanceOf[UnionType]).types).foreach { fresh94 =>
            val subtype = zeroOfMyType = fresh94 {
        ...
        diagnostics.add(
          createDiagnosticForNodeFromMessageChain(propNode, errorInfo))
    ...
    def isValidPropertyAccess(node: (PropertyAccessExpression | QualifiedName),
      ...
      val left =
        (if ((node.kind === SyntaxKind.PropertyAccessExpression))(node
           .asInstanceOf[PropertyAccessExpression])
           .expression
         else (node.asInstanceOf[QualifiedName]).left)
    ...
    def getForInVariableSymbol(node: ForInStatement): Symbol = {
      ...
      if ((initializer.kind === SyntaxKind.VariableDeclarationList)) {
        val variable =
          (initializer.asInstanceOf[VariableDeclarationList]).declarations(0)
    ...
    def hasNumericPropertyNames(`type`: Type) = {
      return (getIndexTypeOfType(`type`, IndexKind.Number) && (!getIndexTypeOfType(
        `type`,
        IndexKind.String)))
    ...
    def isForInVariableForNumericPropertyNames(expr: Expression) = {
      ...
      if ((e.kind === SyntaxKind.Identifier)) {
        ...
        if ((symbol.flags & SymbolFlags.Variable)) {
          ...
          while (node) {
            {
              if (((((node.kind === SyntaxKind.ForInStatement) && (child === (node
                    .asInstanceOf[ForInStatement])
                    .statement)) && (getForInVariableSymbol(node.asInstanceOf[
                    ForInStatement]) === symbol)) && hasNumericPropertyNames(
                    checkExpression(
                      (node.asInstanceOf[ForInStatement]).expression)))) {
                return true
    ...
    def checkIndexedAccess(node: ElementAccessExpression): Type = {
      if ((!node.argumentExpression)) {
        ...
        if (((node.parent.kind === SyntaxKind.NewExpression) && ((node.parent
      ...
      val indexType =
        (if (node.argumentExpression) checkExpression(node.argumentExpression)
         else unknownType)
      ...
      if (node.argumentExpression) {
        val name =
          getPropertyNameForIndexedAccess(node.argumentExpression, indexType)
        if ((name !== undefined)) {
          ...
          if (prop) {
            ...
            error(
              node.argumentExpression,
              Diagnostics.Property_0_does_not_exist_on_const_enum_1,
              name,
      ...
      val allowedNullableFlags =
        (if (strictNullChecks) 0 else TypeFlags.Nullable)
      if (isTypeAnyOrAllConstituentTypesHaveKind(
            indexType,
            (((TypeFlags.StringLike | TypeFlags.NumberLike) | TypeFlags.ESSymbol) | allowedNullableFlags))) {
        if ((isTypeAnyOrAllConstituentTypesHaveKind(
              indexType,
              (TypeFlags.NumberLike | allowedNullableFlags)) || isForInVariableForNumericPropertyNames(
              node.argumentExpression))) {
          val numberIndexInfo =
        ...
        if (((compilerOptions.noImplicitAny && (!compilerOptions.suppressImplicitAnyIndexErrors)) && (!isTypeAny(
              objectType)))) {
          error(
            node,
      ...
      error(
        node,
        Diagnostics.An_index_expression_argument_must_be_of_type_string_number_symbol_or_any)
      return unknownType

    }
    def getPropertyNameForIndexedAccess(indexArgumentExpression: Expression,
                                        indexArgumentType: Type): String = {
      ...
      if (((indexArgumentExpression.kind === SyntaxKind.ElementAccessExpression) || (indexArgumentExpression.kind === SyntaxKind.PropertyAccessExpression))) {
        val value = getConstantValue(indexArgumentExpression
          .asInstanceOf[(ElementAccessExpression | PropertyAccessExpression)])
        if ((value !== undefined)) {
      ...
      if (checkThatExpressionIsProperSymbolReference(
            indexArgumentExpression,
            indexArgumentType,
            false)) {
        val rightHandSideName = ((indexArgumentExpression
          .asInstanceOf[PropertyAccessExpression])
          .name
          .asInstanceOf[Identifier])
          .text
    ...
    def checkThatExpressionIsProperSymbolReference(
      ...
      if ((((expressionType.flags & TypeFlags.ESSymbol)) === 0)) {
        if (reportError) {
          error(
            expression,
            Diagnostics.A_computed_property_name_of_the_form_0_must_be_of_type_symbol,
            getTextOfNode(expression))
      ...
      if ((leftHandSideSymbol !== globalESSymbol)) {
        if (reportError) {
          error(
            leftHandSide,
            Diagnostics.Symbol_reference_does_not_refer_to_the_global_Symbol_constructor_object)
    ...
    def resolveUntypedCall(node: CallLikeExpression): Signature = {
      if ((node.kind === SyntaxKind.TaggedTemplateExpression)) {
        ...
        forEach(
          (node.asInstanceOf[CallExpression]).arguments,
          (argument => {
    ...
    def reorderCandidates(signatures: Array[Signature],
                          result: Array[Signature]): Unit = {
      ...
      (signatures).foreach { fresh95 =>
        val signature = zeroOfMyType = fresh95 {
          val symbol = (signature.declaration && getSymbolOfNode(
              signature.declaration))
    ...
    def hasCorrectArity(node: CallLikeExpression,
                        args: Array[Expression],
                        signature: Signature,
                        signatureHelpTrailingComma: Nothing = false) = {
      ...
      if ((node.kind === SyntaxKind.TaggedTemplateExpression)) {
        ...
        if ((tagExpression.template.kind === SyntaxKind.TemplateExpression)) {
          val templateExpression =
            tagExpression.template.asInstanceOf[TemplateExpression]
          ...
          val templateLiteral =
            tagExpression.template.asInstanceOf[LiteralExpression]
          Debug.assert(
        ...
        val callExpression =
          node.asInstanceOf[(CallExpression | NewExpression)]
        if ((!callExpression.arguments)) {
        ...
        (argCount =
          (if (signatureHelpTrailingComma) (args.length + 1) else args.length))
    ...
    def instantiateSignatureInContextOf(
        signature: Signature,
      ...
      forEachMatchingParameterType(
        ...
        ((source, target) => {
           inferTypes(
             context,
             instantiateType(source, contextualMapper),
             target)
    ...
    def inferTypeArguments(node: CallLikeExpression,
                           signature: Signature,
                           args: Array[Expression],
                           excludeArgument: Array[Boolean],
                           context: InferenceContext): Unit = {
      ...
      if (((context.failedTypeParameterIndex !== undefined) && (!context
            .inferences(context.failedTypeParameterIndex)
            .isFixed))) {
      ...
      if (thisType) {
        ...
        val thisArgumentType =
          (if (thisArgumentNode) checkExpression(thisArgumentNode)
           else voidType)
              ...
              if ((argType === undefined)) {
                val mapper =
                  (if ((excludeArgument && (excludeArgument(i) !== undefined)))
                     identityMapper
                   else inferenceMapper)
                (argType =
                  checkExpressionWithContextualType(arg, paramType, mapper))
    ...
    def checkTypeArguments(signature: Signature,
                           typeArgumentNodes: Array[TypeNode],
                           typeArgumentTypes: Array[Type],
                           reportErrors: Boolean,
                           headMessage: DiagnosticMessage): Boolean = {
      val typeParameters = signature.typeParameters
              ...
              if (constraint) {
                ...
                (typeArgumentsAreAssignable = checkTypeAssignableTo(
                  ...
                  getTypeWithThisArgument(
                    ...
                    typeArgument),
                  (if (reportErrors) typeArgumentNodes(i) else undefined),
                  typeArgumentHeadMessage,
                  errorInfo))
    ...
    def checkApplicableSignature(node: CallLikeExpression,
                                 args: Array[Expression],
                                 signature: Signature,
                                 relation: Map[RelationComparisonResult],
                                 excludeArgument: Array[Boolean],
      ...
      if (((thisType && (thisType !== voidType)) && (node.kind !== SyntaxKind.NewExpression))) {
        ...
        val errorNode =
          (if (reportErrors) ((thisArgumentNode || node)) else undefined)
        val headMessage =
        ...
        while ((i < argCount)) {
          {
            ...
            if (((arg === undefined) || (arg.kind !== SyntaxKind.OmittedExpression))) {
              ...
              var argType = getEffectiveArgumentType(node, i)
              if ((argType === undefined)) {
                (argType = checkExpressionWithContextualType(
                  arg,
                  paramType,
                  (if ((excludeArgument && excludeArgument(i))) identityMapper
                   else undefined)))

              }
              val errorNode =
                (if (reportErrors) getEffectiveArgumentErrorNode(node, i, arg)
                 else undefined)
              if ((!checkTypeRelatedTo(
                    argType,
                    paramType,
    ...
    def getThisArgumentOfCall(
        node: CallLikeExpression): LeftHandSideExpression = {
      if ((node.kind === SyntaxKind.CallExpression)) {
    ...
    def getEffectiveCallArguments(
        node: CallLikeExpression): Array[Expression] = {
      ...
      if ((node.kind === SyntaxKind.TaggedTemplateExpression)) {
        ...
        if ((template.kind === SyntaxKind.TemplateExpression)) {
          forEach(
            (template.asInstanceOf[TemplateExpression]).templateSpans,
            (span => {
    ...
    def getEffectiveArgumentCount(node: CallLikeExpression,
                                  args: Array[Expression],
                                  signature: Signature) = {
      if ((node.kind === SyntaxKind.Decorator)) {
        node.parent.kind match {
          ...
          case SyntaxKind.PropertyDeclaration =>
            return 2
          case SyntaxKind.MethodDeclaration | SyntaxKind.GetAccessor |
              SyntaxKind.SetAccessor =>
    ...
    def getEffectiveDecoratorSecondArgumentType(node: Node) = {
      ...
      if (((((node.kind === SyntaxKind.PropertyDeclaration) || (node.kind === SyntaxKind.MethodDeclaration)) || (node.kind === SyntaxKind.GetAccessor)) || (node.kind === SyntaxKind.SetAccessor))) {
        ...
        element.name.kind match {
          case SyntaxKind.Identifier | SyntaxKind.NumericLiteral |
            ...
            return getLiteralTypeForText(
              ...
              (element.name
                ...
                .text)
          case SyntaxKind.ComputedPropertyName =>
            val nameType = checkComputedPropertyName(
              element.name.asInstanceOf[ComputedPropertyName])
    ...
    def getEffectiveDecoratorArgumentType(node: Decorator,
                                          argIndex: Int): Type = {
      if ((argIndex === 0)) {
    ...
    def getEffectiveArgumentType(node: CallLikeExpression,
                                 argIndex: Int): Type = {
      if ((node.kind === SyntaxKind.Decorator)) {
        return getEffectiveDecoratorArgumentType(
          node.asInstanceOf[Decorator],
          argIndex)
    ...
    def getEffectiveArgument(node: CallLikeExpression,
                             args: Array[Expression],
                             argIndex: Int) = {
    ...
    def getEffectiveArgumentErrorNode(node: CallLikeExpression,
                                      argIndex: Int,
                                      arg: Expression) = {
      if ((node.kind === SyntaxKind.Decorator)) {
    ...
    def resolveCall(node: CallLikeExpression,
                    signatures: Array[Signature],
                    candidatesOutArray: Array[Signature],
                    headMessage: DiagnosticMessage): Signature = {
      ...
      var result: Signature = zeroOfMyType
      val signatureHelpTrailingComma = ((candidatesOutArray && (node.kind === SyntaxKind.CallExpression)) && (node
          .asInstanceOf[CallExpression])
          .arguments
          .hasTrailingComma)
      ...
      if (candidateForArgumentError) {
        ...
        if ((((!isTaggedTemplate) && (!isDecorator)) && typeArguments)) {
          ...
          checkTypeArguments(
            ...
            typeArguments,
            map(typeArguments, getTypeFromTypeNodeNoAlias),
            true,
            headMessage)

        } else {
          Debug.assert((resultOfFailedInference.failedTypeParameterIndex >= 0))
          val failedTypeParameter =
            candidateForTypeArgumentError.typeParameters(
              resultOfFailedInference.failedTypeParameterIndex)
          val inferenceCandidates = getInferenceCandidates(
          ...
          if (headMessage) {
            (diagnosticChainHead =
              chainDiagnosticMessages(diagnosticChainHead, headMessage))

          }
          reportNoCommonSupertypeError(
            inferenceCandidates,
            ((node.asInstanceOf[CallExpression]).expression || (node
      ...
      if ((!produceDiagnostics)) {
        (candidates).foreach { fresh96 =>
          var candidate = zeroOfMyType = fresh96 {
      ...
      def reportError(message: DiagnosticMessage,
        ...
        diagnostics.add(
          createDiagnosticForNodeFromMessageChain(node, errorInfo))

      }
      def chooseOverload(candidates: Array[Signature],
                         relation: Map[RelationComparisonResult],
                         signatureHelpTrailingComma: Nothing = false) = {
        (candidates).foreach { fresh97 =>
          val originalCandidate = zeroOfMyType = fresh97 {
            if ((!hasCorrectArity(
                  node,
                  args,
                  originalCandidate,
            ...
            while (true) {
              {
                ...
                if (candidate.typeParameters) {
                  ...
                  (candidate =
                    getSignatureInstantiation(candidate, typeArgumentTypes))
                ...
                val index =
                  (if (excludeArgument) indexOf(excludeArgument, true)
                   else (-1))
                if ((index < 0)) {
                  return candidate
    ...
    def resolveCallExpression(
      ...
      if ((node.expression.kind === SyntaxKind.SuperKeyword)) {
        ...
        if ((superType !== unknownType)) {
          ...
          if (baseTypeNode) {
            val baseConstructors = getInstantiatedConstructorsForTypeArguments(
              superType,
              baseTypeNode.typeArguments)
    ...
    def isUntypedFunctionCall(funcType: Type,
                              apparentFuncType: Type,
                              numCallSignatures: Int,
                              numConstructSignatures: Int) = {
    ...
    def resolveNewExpression(
      ...
      val valueDecl = (expressionType.symbol && getClassLikeDeclarationOfSymbol(
          expressionType.symbol))
      if ((valueDecl && (getModifierFlags(valueDecl) & ModifierFlags.Abstract))) {
        error(
          node,
          Diagnostics.Cannot_create_an_instance_of_the_abstract_class_0,
    ...
    def isConstructorAccessible(node: NewExpression, signature: Signature) = {
      ...
      val declaringClassDeclaration = getClassLikeDeclarationOfSymbol(
        declaration.parent.symbol).asInstanceOf[ClassLikeDeclaration]
      val declaringClass = getDeclaredTypeOfSymbol(declaration.parent.symbol)
        .asInstanceOf[InterfaceType]
      if ((!isNodeWithinClass(node, declaringClassDeclaration))) {
        val containingClass = getContainingClass(node)
        if (containingClass) {
          val containingType = getTypeOfNode(containingClass)
          val baseTypes = getBaseTypes(
            containingType.asInstanceOf[InterfaceType])
          if (baseTypes.length) {
    ...
    def resolveTaggedTemplateExpression(
    ...
    def resolveDecorator(node: Decorator,
                         candidatesOutArray: Array[Signature]): Signature = {
    ...
    def resolveSignature(node: CallLikeExpression,
                         candidatesOutArray: Array[Signature]): Signature = {
      node.kind match {
        case SyntaxKind.CallExpression =>
    ...
    def getResolvedSignature(
      ...
      (links.resolvedSignature =
        (if ((flowLoopStart === flowLoopCount)) result else cached))
      return result

    }
    def getResolvedOrAnySignature(node: CallLikeExpression) = {
      return (if ((getNodeLinks(node).resolvedSignature === resolvingSignature))
                resolvingSignature
              else getResolvedSignature(node))

    }
    def getInferredClassType(symbol: Symbol) = {
      val links = getSymbolLinks(symbol)
      if ((!links.inferredClassType)) {
        (links.inferredClassType = createAnonymousType(
          symbol,
          symbol.members,
          emptyArray,
          emptyArray,
    ...
    def checkCallExpression(node: (CallExpression | NewExpression)): Type = {
      (checkGrammarTypeArguments(node, node.typeArguments) || checkGrammarArguments(
        node,
        node.arguments))
      ...
      if ((node.kind === SyntaxKind.NewExpression)) {
        val declaration = signature.declaration
        if (((((declaration && (declaration.kind !== SyntaxKind.Constructor)) && (declaration.kind !== SyntaxKind.ConstructSignature)) && (declaration.kind !== SyntaxKind.ConstructorType)) && (!isJSDocConstructSignature(
              declaration)))) {
          val funcSymbol =
            (if ((node.expression.kind === SyntaxKind.Identifier))
               getResolvedSymbol(node.expression.asInstanceOf[Identifier])
             else checkExpression(node.expression).symbol)
          if (((funcSymbol && funcSymbol.members) && (((funcSymbol.flags & SymbolFlags.Function) || isDeclarationOfFunctionExpression(
                funcSymbol))))) {
      ...
      if ((isInJavaScriptFile(node) && isCommonJsRequire(node))) {
        return resolveExternalModuleTypeByLiteral(
          node.arguments(0).asInstanceOf[StringLiteral])
    ...
    def isCommonJsRequire(node: Node) = {
      ...
      val resolvedRequire = resolveName(
        node.expression,
        (node.expression.asInstanceOf[Identifier]).text,
        SymbolFlags.Value,
        undefined,
        undefined)
      if ((!resolvedRequire)) {
      ...
      val targetDeclarationKind =
        (if ((resolvedRequire.flags & SymbolFlags.Function))
           SyntaxKind.FunctionDeclaration
         else
           (if ((resolvedRequire.flags & SymbolFlags.Variable))
    ...
    def checkAssertion(node: AssertionExpression) = {
      val exprType = getRegularTypeOfObjectLiteral(
        getBaseTypeOfLiteralType(checkExpression(node.expression)))
      ...
      if ((produceDiagnostics && (targetType !== unknownType))) {
        ...
        if ((!isTypeComparableTo(targetType, widenedType))) {
          checkTypeComparableTo(
            ...
            targetType,
            node,
            Diagnostics.Type_0_cannot_be_converted_to_type_1)
    ...
    def getTypeAtPosition(signature: Signature, pos: Int): Type = {
      return (if (signature.hasRestParameter)
                (if ((pos < (signature.parameters.length - 1)))
                   getTypeOfParameter(signature.parameters(pos))
                 else getRestTypeOfSignature(signature))
              else
                (if ((pos < signature.parameters.length))
                   getTypeOfParameter(signature.parameters(pos))
                 else anyType))

    }
    def assignContextualParameterTypes(signature: Signature,
                                       context: Signature,
                                       mapper: TypeMapper) = {
      val len = (signature.parameters.length - ((if (signature.hasRestParameter)
                                                   1
                                                 else 0) ))
      ...
      if (context.thisParameter) {
        ...
        if (((!parameter) || (parameter.valueDeclaration && (!(parameter.valueDeclaration
              .asInstanceOf[ParameterDeclaration])
              .`type`)))) {
          if ((!parameter)) {
            (signature.thisParameter =
              createTransientSymbol(context.thisParameter, undefined))
      ...
      if ((signature.hasRestParameter && isRestParameterIndex(
            context,
        ...
        if ((!(parameter.valueDeclaration
              .asInstanceOf[ParameterDeclaration])
              .`type`)) {
          val contextualParameterType = getTypeOfSymbol(
            lastOrUndefined(context.parameters))
          assignTypeToParameterAndFixTypeParameters(
            parameter,
            contextualParameterType,
            mapper)
    ...
    def assignBindingElementTypes(node: VariableLikeDeclaration) = {
      if (isBindingPattern(node.name)) {
        ((node.name.asInstanceOf[BindingPattern]).elements).foreach {
          fresh98 =>
            val element = zeroOfMyType = fresh98 {
              if ((!isOmittedExpression(element))) {
                if ((element.name.kind === SyntaxKind.Identifier)) {
                  (getSymbolLinks(getSymbolOfNode(element)).`type` =
                    getTypeForBindingElement(element))
    ...
    def assignTypeToParameterAndFixTypeParameters(parameter: Symbol,
                                                  contextualType: Type,
                                                  mapper: TypeMapper) = {
      ...
      if ((!links.`type`)) {
        ...
        if (((links.`type` === emptyObjectType) && (((parameter.valueDeclaration.name.kind === SyntaxKind.ObjectBindingPattern) || (parameter.valueDeclaration.name.kind === SyntaxKind.ArrayBindingPattern))))) {
          (links.`type` = getTypeFromBindingPattern(
            parameter.valueDeclaration.name.asInstanceOf[BindingPattern]))

        }
        assignBindingElementTypes(
          parameter.valueDeclaration.asInstanceOf[ParameterDeclaration])
    ...
    def getReturnTypeFromJSDocComment(
        func: (SignatureDeclaration | FunctionDeclaration)): Type = {
    ...
    def createPromiseType(promisedType: Type): Type = {
      ...
      if ((globalPromiseType !== emptyGenericType)) {
        ...
        return createTypeReference(
          globalPromiseType.asInstanceOf[GenericType],
          Array(promisedType))
    ...
    def createPromiseReturnType(func: FunctionLikeDeclaration,
                                promisedType: Type) = {
    ...
    def getReturnTypeFromBody(func: FunctionLikeDeclaration,
                              contextualMapper: TypeMapper): Type = {
      val contextualSignature =
        getContextualSignatureForFunctionLikeDeclaration(func)
      if ((!func.body)) {
        return unknownType
      ...
      if ((func.body.kind !== SyntaxKind.Block)) {
        (`type` = checkExpressionCached(
          func.body.asInstanceOf[Expression],
          contextualMapper))
        if (isAsync) {
        ...
        if (funcIsGenerator) {
          ...
          if ((types.length === 0)) {
            ...
            if (compilerOptions.noImplicitAny) {
              error(
                func.asteriskToken,
                Diagnostics.Generator_implicitly_has_type_0_because_it_does_not_yield_any_values_Consider_supplying_a_return_type,
          ...
          (types =
            checkAndAggregateReturnExpressionTypes(func, contextualMapper))
          if ((!types)) {
            return (if (isAsync) createPromiseReturnType(func, neverType)
                    else neverType)

          }
          if ((types.length === 0)) {
            return (if (isAsync) createPromiseReturnType(func, voidType)
                    else voidType)
      ...
      if ((isUnitType(`type`) && (!((contextualSignature && isLiteralContextualType(
            (if ((contextualSignature === getSignatureFromDeclaration(func)))
               `type`
             else getReturnTypeOfSignature(contextualSignature)))))))) {
        (`type` = getWidenedLiteralType(`type`))

      }
      val widenedType = getWidenedType(`type`)
      return (if (isAsync) createPromiseReturnType(func, widenedType)
              else widenedType)

    }
    def checkAndAggregateYieldOperandTypes(
        func: FunctionLikeDeclaration,
        contextualMapper: TypeMapper): Array[Type] = {
      val aggregatedTypes: Array[Type] = Array()
      forEachYieldExpression(
        func.body.asInstanceOf[Block],
        (yieldExpression => {
           val expr = yieldExpression.expression
           if (expr) {
             var `type` = checkExpressionCached(expr, contextualMapper)
             if (yieldExpression.asteriskToken) {
               (`type` = checkElementTypeOfIterable(
                 `type`,
    ...
    def functionHasImplicitReturn(func: FunctionLikeDeclaration) = {
      ...
      val lastStatement = lastOrUndefined(
        (func.body.asInstanceOf[Block]).statements)
      if (((lastStatement && (lastStatement.kind === SyntaxKind.SwitchStatement)) && isExhaustiveSwitchStatement(
            lastStatement.asInstanceOf[SwitchStatement]))) {
        return false
    ...
    def checkAndAggregateReturnExpressionTypes(
        func: FunctionLikeDeclaration,
        contextualMapper: TypeMapper): Array[Type] = {
      val isAsync = isAsyncFunctionLike(func)
      val aggregatedTypes: Array[Type] = Array()
    ...
    def checkAllCodePathsInNonVoidFunctionReturnOrThrow(
        func: FunctionLikeDeclaration,
      ...
      if ((returnType && maybeTypeOfKind(
            returnType,
            (TypeFlags.Any | TypeFlags.Void)))) {
        return

      }
      if (((nodeIsMissing(func.body) || (func.body.kind !== SyntaxKind.Block)) || (!functionHasImplicitReturn(
            func)))) {
        return
      ...
      if ((returnType && (returnType.flags & TypeFlags.Never))) {
      ...
      } else if (((returnType && strictNullChecks) && (!isTypeAssignableTo(
                   undefinedType,
                   returnType)))) {
        error(
          func.`type`,
        ...
        if ((!returnType)) {
          ...
          val inferredReturnType = getReturnTypeOfSignature(
    ...
    def checkFunctionExpressionOrObjectLiteralMethod(
      ...
      val mightFixTypeParameters = (contextSensitive && isInferentialContext(
          contextualMapper))
          ...
          if (contextualSignature) {
            ...
            if (contextSensitive) {
              assignContextualParameterTypes(
                signature,
                contextualSignature,
                (contextualMapper || identityMapper))
      ...
      if ((produceDiagnostics && (node.kind !== SyntaxKind.MethodDeclaration))) {
        checkCollisionWithCapturedSuperVariable(
          node,
    ...
    def checkFunctionExpressionOrObjectLiteralMethodDeferred(
        node: (ArrowFunction | FunctionExpression | MethodDeclaration)) = {
      Debug.assert(
        ((node.kind !== SyntaxKind.MethodDeclaration) || isObjectLiteralMethod(
          node)))
      val isAsync = isAsyncFunctionLike(node)
      val returnOrPromisedType = (node.`type` && ((if (isAsync)
      ...
      if (node.body) {
        ...
        if ((node.body.kind === SyntaxKind.Block)) {
          ...
          if (returnOrPromisedType) {
            if (isAsync) {
              val awaitedType = checkAwaitedType(
                exprType,
                node.body,
                Diagnostics.Expression_body_for_async_arrow_function_does_not_have_a_valid_callable_then_member)
              checkTypeAssignableTo(
                awaitedType,
                returnOrPromisedType,
                node.body)
    ...
    def checkArithmeticOperandType(operand: Node,
                                   `type`: Type,
                                   diagnostic: DiagnosticMessage): Boolean = {
      if ((!isTypeAnyOrAllConstituentTypesHaveKind(
            `type`,
            TypeFlags.NumberLike))) {
    ...
    def isReadonlySymbol(symbol: Symbol): Boolean = {
      return ((((symbol.isReadonly || ((symbol.flags & SymbolFlags.Property) && (((getDeclarationModifierFlagsFromSymbol(
        symbol) & ModifierFlags.Readonly)) !== 0))) || ((symbol.flags & SymbolFlags.Variable) && (((getDeclarationNodeFlagsFromSymbol(
        symbol) & NodeFlags.Const)) !== 0))) || ((symbol.flags & SymbolFlags.Accessor) && (!((symbol.flags & SymbolFlags.SetAccessor))))) || (((symbol.flags & SymbolFlags.EnumMember)) !== 0))

    }
    def isReferenceToReadonlyEntity(expr: Expression,
                                    symbol: Symbol): Boolean = {
      if (isReadonlySymbol(symbol)) {
        if ((((symbol.flags & SymbolFlags.Property) && (((expr.kind === SyntaxKind.PropertyAccessExpression) || (expr.kind === SyntaxKind.ElementAccessExpression)))) && ((expr
              .asInstanceOf[
    ...
    def checkReferenceExpression(
        expr: Expression,
        invalidReferenceMessage: DiagnosticMessage,
        constantVariableMessage: DiagnosticMessage): Boolean = {
      val node = skipParenthesizedNodes(expr)
      ...
      if (symbol) {
        if (((symbol !== unknownSymbol) && (symbol !== argumentsSymbol))) {
          ...
          if ((isReferenceToReadonlyEntity(node, symbol) || isReferenceThroughNamespaceImport(
                node))) {
      ...
      if (produceDiagnostics) {
        if ((!((node.flags & NodeFlags.AwaitContext)))) {
          grammarErrorOnFirstToken(
            node,
            Diagnostics.await_expression_is_only_allowed_within_an_async_function)
    ...
    def checkPrefixUnaryExpression(node: PrefixUnaryExpression): Type = {
      ...
      if (((node.operator === SyntaxKind.MinusToken) && (node.operand.kind === SyntaxKind.NumericLiteral))) {
        return getFreshTypeOfLiteralType(
          getLiteralTypeForText(
            TypeFlags.NumberLiteral,
            ("" + (-(node.operand.asInstanceOf[LiteralExpression]).text))))
      ...
      node.operator match {
        ...
        case SyntaxKind.ExclamationToken =>
          ...
          return (if ((facts === TypeFacts.Truthy)) falseType
                  else (if ((facts === TypeFacts.Falsy)) trueType
                        else booleanType))
        case SyntaxKind.PlusPlusToken | SyntaxKind.MinusMinusToken =>
          val ok = checkArithmeticOperandType(
            node.operand,
            getNonNullableType(operandType),
    ...
    def checkInstanceOfExpression(left: Expression,
                                  right: Expression,
                                  leftType: Type,
                                  rightType: Type): Type = {
      ...
      if ((!((isTypeAny(rightType) || isTypeSubtypeOf(
            rightType,
            globalFunctionType))))) {
        error(
          right,
    ...
    def checkInExpression(left: Expression,
                          right: Expression,
                          leftType: Type,
                          rightType: Type): Type = {
      if (((leftType === silentNeverType) || (rightType === silentNeverType))) {
      ...
      if ((!isTypeAnyOrAllConstituentTypesHaveKind(
            leftType,
            ((TypeFlags.StringLike | TypeFlags.NumberLike) | TypeFlags.ESSymbol)))) {
        error(
          left,
      ...
      if ((!isTypeAnyOrAllConstituentTypesHaveKind(
            rightType,
            (TypeFlags.Object | TypeFlags.TypeParameter)))) {
        error(
          right,
    ...
    def checkObjectLiteralAssignment(node: ObjectLiteralExpression,
                                     sourceType: Type): Type = {
      val properties = node.properties
      (properties).foreach { fresh102 =>
        val p = zeroOfMyType = fresh102 {
    ...
    def checkObjectLiteralDestructuringPropertyAssignment(
      ...
      if (((property.kind === SyntaxKind.PropertyAssignment) || (property.kind === SyntaxKind.ShorthandPropertyAssignment))) {
        ...
        val text = getTextOfPropertyName(name)
        val `type` =
          (if (isTypeAny(objectLiteralType)) objectLiteralType
           else
             ((getTypeOfPropertyOfType(objectLiteralType, text) || (isNumericLiteralName(
               text) && getIndexTypeOfType(
               objectLiteralType,
               IndexKind.Number))) || getIndexTypeOfType(
               objectLiteralType,
               IndexKind.String)))
        if (`type`) {
          if ((property.kind === SyntaxKind.ShorthandPropertyAssignment)) {
            ...
            return checkDestructuringAssignment(
              (property.asInstanceOf[PropertyAssignment]).initializer,
              `type`)
          ...
          error(
            name,
            Diagnostics.Type_0_has_no_property_1_and_no_string_index_signature,
            typeToString(objectLiteralType),
            declarationNameToString(name))
    ...
    def checkArrayLiteralAssignment(node: ArrayLiteralExpression,
                                    sourceType: Type,
                                    contextualMapper: TypeMapper): Type = {
      val elementType = (checkIteratedTypeOrElementType(
          sourceType,
          node,
          false) || unknownType)
      val elements = node.elements {
    ...
    def checkArrayLiteralDestructuringElementAssignment(
      ...
      if ((element.kind !== SyntaxKind.OmittedExpression)) {
        if ((element.kind !== SyntaxKind.SpreadElementExpression)) {
          ...
          val `type` =
            (if (isTypeAny(sourceType)) sourceType
             else
               (if (isTupleLikeType(sourceType))
          ...
          if (`type`) {
            ...
            if (isTupleType(sourceType)) {
              error(
                ...
                getTypeReferenceArity(sourceType.asInstanceOf[TypeReference]),
                elements.length)
          ...
          if ((elementIndex < (elements.length - 1))) {
            error(
              element,
              Diagnostics.A_rest_element_must_be_last_in_an_array_destructuring_pattern)

          } else {
            val restExpression =
              (element.asInstanceOf[SpreadElementExpression]).expression
            if (((restExpression.kind === SyntaxKind.BinaryExpression) && ((restExpression
                  .asInstanceOf[BinaryExpression])
                  .operatorToken
                  .kind === SyntaxKind.EqualsToken))) {
              error(
                (restExpression.asInstanceOf[BinaryExpression]).operatorToken,
    ...
    def checkDestructuringAssignment(
      ...
      if ((exprOrAssignment.kind === SyntaxKind.ShorthandPropertyAssignment)) {
        ...
        if (prop.objectAssignmentInitializer) {
          if ((strictNullChecks && (!((getFalsyFlags(checkExpression(
                prop.objectAssignmentInitializer)) & TypeFlags.Undefined))))) {
        ...
        (target =
          (exprOrAssignment.asInstanceOf[ShorthandPropertyAssignment]).name)

      } else {
        (target = exprOrAssignment.asInstanceOf[Expression])

      }
      if (((target.kind === SyntaxKind.BinaryExpression) && ((target
            .asInstanceOf[BinaryExpression])
            .operatorToken
            .kind === SyntaxKind.EqualsToken))) {
        checkBinaryExpression(
          target.asInstanceOf[BinaryExpression],
          contextualMapper)
    ...
    def checkReferenceAssignment(target: Expression,
                                 sourceType: Type,
                                 contextualMapper: TypeMapper): Type = {
    ...
    def isSideEffectFree(node: Node): Boolean = {
      ...
      node.kind match {
        case SyntaxKind.Identifier | SyntaxKind.StringLiteral |
            SyntaxKind.RegularExpressionLiteral |
            SyntaxKind.TaggedTemplateExpression |
            SyntaxKind.TemplateExpression |
            SyntaxKind.NoSubstitutionTemplateLiteral |
            SyntaxKind.NumericLiteral | SyntaxKind.TrueKeyword |
            SyntaxKind.FalseKeyword | SyntaxKind.NullKeyword |
            SyntaxKind.UndefinedKeyword | SyntaxKind.FunctionExpression |
            SyntaxKind.ClassExpression | SyntaxKind.ArrowFunction |
            SyntaxKind.ArrayLiteralExpression |
            SyntaxKind.ObjectLiteralExpression | SyntaxKind.TypeOfExpression |
            SyntaxKind.NonNullExpression | SyntaxKind.JsxSelfClosingElement |
        ...
        case SyntaxKind.ConditionalExpression =>
          return (isSideEffectFree(
            (node
              .asInstanceOf[ConditionalExpression])
              .whenTrue) && isSideEffectFree(
            (node.asInstanceOf[ConditionalExpression]).whenFalse))
        case SyntaxKind.BinaryExpression =>
          if (isAssignmentOperator(
                (node.asInstanceOf[BinaryExpression]).operatorToken.kind)) {
            return false

          }
          return (isSideEffectFree((node.asInstanceOf[BinaryExpression]).left) && isSideEffectFree(
            (node.asInstanceOf[BinaryExpression]).right))
        case SyntaxKind.PrefixUnaryExpression |
            SyntaxKind.PostfixUnaryExpression =>
          (node.asInstanceOf[PrefixUnaryExpression]).operator match {
            case SyntaxKind.ExclamationToken | SyntaxKind.PlusToken |
                SyntaxKind.MinusToken | SyntaxKind.TildeToken =>
              return true
            case _ =>
          }
          return false
    ...
    def getBestChoiceType(type1: Type, type2: Type): Type = {
      ...
      return (if ((secondAssignableToFirst && (!firstAssignableToSecond)))
                type1
              else
                (if ((firstAssignableToSecond && (!secondAssignableToFirst)))
                   type2
                 else getUnionType(Array(type1, type2), true)))

    }
    def checkBinaryExpression(node: BinaryExpression,
                              contextualMapper: TypeMapper) = {
    ...
    def checkBinaryLikeExpression(left: Expression,
                                  operatorToken: Node,
                                  right: Expression,
                                  contextualMapper: TypeMapper,
                                  errorNode: Node) = {
      ...
      operator match {
        case SyntaxKind.AsteriskToken | SyntaxKind.AsteriskAsteriskToken |
            SyntaxKind.AsteriskEqualsToken |
            SyntaxKind.AsteriskAsteriskEqualsToken | SyntaxKind.SlashToken |
            SyntaxKind.SlashEqualsToken | SyntaxKind.PercentToken |
            SyntaxKind.PercentEqualsToken | SyntaxKind.MinusToken |
            SyntaxKind.MinusEqualsToken | SyntaxKind.LessThanLessThanToken |
          ...
          if (((((leftType.flags & TypeFlags.BooleanLike)) && ((rightType.flags & TypeFlags.BooleanLike))) && (((suggestedOperator = getSuggestedBooleanOperator(
                operatorToken.kind))) !== undefined))) {
            error(
            ...
            val leftOk = checkArithmeticOperandType(
              left,
              leftType,
              Diagnostics.The_left_hand_side_of_an_arithmetic_operation_must_be_of_type_any_number_or_an_enum_type)
            val rightOk = checkArithmeticOperandType(
              right,
              rightType,
              Diagnostics.The_right_hand_side_of_an_arithmetic_operation_must_be_of_type_any_number_or_an_enum_type)
            if ((leftOk && rightOk)) {
        ...
        case SyntaxKind.PlusToken | SyntaxKind.PlusEqualsToken =>
          ...
          if ((isTypeOfKind(leftType, TypeFlags.NumberLike) && isTypeOfKind(
                rightType,
                TypeFlags.NumberLike))) {
            (resultType = numberType)

          } else {
            if ((isTypeOfKind(leftType, TypeFlags.StringLike) || isTypeOfKind(
                  rightType,
        ...
        case SyntaxKind.LessThanToken | SyntaxKind.GreaterThanToken |
          ...
          if (checkForDisallowedESSymbolOperand(operator)) {
            if (((!isTypeComparableTo(leftType, rightType)) && (!isTypeComparableTo(
                  rightType,
                  leftType)))) {
              reportOperatorError()
        ...
        case SyntaxKind.EqualsEqualsToken | SyntaxKind.ExclamationEqualsToken |
          ...
          if (((!leftIsLiteral) || (!rightIsLiteral))) {
            (leftType =
              (if (leftIsLiteral) getBaseTypeOfLiteralType(leftType)
               else leftType))
            (rightType =
              (if (rightIsLiteral) getBaseTypeOfLiteralType(rightType)
               else rightType))

          }
          if (((!isTypeEqualityComparableTo(leftType, rightType)) && (!isTypeEqualityComparableTo(
                rightType,
                leftType)))) {
            reportOperatorError()
        ...
        case SyntaxKind.AmpersandAmpersandToken =>
          return (if ((getTypeFacts(leftType) & TypeFacts.Truthy))
                    includeFalsyTypes(
                      rightType,
                      getFalsyFlags(
                        (if (strictNullChecks) leftType
                         else getBaseTypeOfLiteralType(rightType))))
                  else leftType)
        case SyntaxKind.BarBarToken =>
          return (if ((getTypeFacts(leftType) & TypeFacts.Falsy))
                    getBestChoiceType(
                      removeDefinitelyFalsyTypes(leftType),
                      rightType)
                  else leftType)
        case SyntaxKind.EqualsToken =>
        ...
        case SyntaxKind.CommaToken =>
          if (((!compilerOptions.allowUnreachableCode) && isSideEffectFree(
                left))) {
            error(
              left,
      ...
      def checkForDisallowedESSymbolOperand(operator: SyntaxKind): Boolean = {
        val offendingSymbolOperand =
          (if (maybeTypeOfKind(leftType, TypeFlags.ESSymbol)) left
           else (if (maybeTypeOfKind(rightType, TypeFlags.ESSymbol)) right
                 else undefined))
      ...
      def checkAssignmentOperator(valueType: Type): Unit = {
        if (((produceDiagnostics && (operator >= SyntaxKind.FirstAssignment)) && (operator <= SyntaxKind.LastAssignment))) {
          val ok = checkReferenceExpression(
            left,
            Diagnostics.Invalid_left_hand_side_of_assignment_expression,
            Diagnostics.Left_hand_side_of_assignment_expression_cannot_be_a_constant_or_a_read_only_property)
          if (ok) {
    ...
    def isYieldExpressionInClass(node: YieldExpression): Boolean = {
      ...
      while (parent) {
        {
          if ((isFunctionLike(parent) && (current === (parent
                .asInstanceOf[FunctionLikeDeclaration])
                .body))) {
            return false
    ...
    def checkYieldExpression(node: YieldExpression): Type = {
      if (produceDiagnostics) {
        if (((!((node.flags & NodeFlags.YieldContext))) || isYieldExpressionInClass(
              node))) {
          grammarErrorOnFirstToken(
            node,
      ...
      if (node.expression) {
        ...
        if ((func && func.asteriskToken)) {
          val expressionType =
            checkExpressionCached(node.expression, undefined)
          ...
          if (func.`type`) {
            val signatureElementType = (getElementTypeOfIterableIterator(
                getTypeFromTypeNode(func.`type`)) || anyType)
            if (nodeIsYieldStar) {
              checkTypeAssignableTo(
                expressionElementType,
    ...
    def checkConditionalExpression(node: ConditionalExpression,
                                   contextualMapper: TypeMapper): Type = {
      ...
      node.kind match {
        case SyntaxKind.StringLiteral =>
          return getFreshTypeOfLiteralType(
            getLiteralTypeForText(
              TypeFlags.StringLiteral,
              (node.asInstanceOf[LiteralExpression]).text))
    ...
    def checkTemplateExpression(node: TemplateExpression): Type = {
      forEach(
        (node.asInstanceOf[TemplateExpression]).templateSpans,
        (templateSpan => {
    ...
    def checkExpressionWithContextualType(
        node: Expression,
        contextualType: Type,
        contextualMapper: TypeMapper): Type = {
    ...
    def checkExpressionCached(node: Expression,
                              contextualMapper: TypeMapper): Type = {
      val links = getNodeLinks(node)
      if ((!links.resolvedType)) {
    ...
    def checkDeclarationInitializer(declaration: VariableLikeDeclaration) = {
      ...
      return (if ((((getCombinedNodeFlags(declaration) & NodeFlags.Const) || (getCombinedModifierFlags(
                    declaration) & ModifierFlags.Readonly)) || isTypeAssertion(
                    declaration.initializer))) `type`
              else getWidenedLiteralType(`type`))

    }
    def isLiteralContextualType(contextualType: Type) = {
      if (contextualType) {
        if ((contextualType.flags & TypeFlags.TypeParameter)) {
          val apparentType = getApparentTypeOfTypeParameter(
            contextualType.asInstanceOf[TypeParameter])
    ...
    def checkExpressionForMutableLocation(
        node: Expression,
        contextualMapper: TypeMapper): Type = {
      val `type` = checkExpression(node, contextualMapper)
      return (if ((isTypeAssertion(node) || isLiteralContextualType(
                    getContextualType(node)))) `type`
              else getWidenedLiteralType(`type`))

    }
    def checkPropertyAssignment(node: PropertyAssignment,
                                contextualMapper: TypeMapper): Type = {
    ...
    def checkObjectLiteralMethod(node: MethodDeclaration,
                                 contextualMapper: TypeMapper): Type = {
      ...
      val uninstantiatedType =
        checkFunctionExpressionOrObjectLiteralMethod(node, contextualMapper)
    ...
    def instantiateTypeWithSingleGenericCallSignature(
      ...
      if (isInferentialContext(contextualMapper)) {
        ...
        if ((signature && signature.typeParameters)) {
          val contextualType = getApparentTypeOfContextualType(
    ...
    def checkExpression(node: (Expression | QualifiedName),
                        contextualMapper: TypeMapper): Type = {
      var `type`: Type = zeroOfMyType
      if ((node.kind === SyntaxKind.QualifiedName)) {
        ...
        val uninstantiatedType = checkExpressionWorker(
          node.asInstanceOf[Expression],
          contextualMapper)
      ...
      if (isConstEnumObjectType(`type`)) {
        val ok = (((((node.parent.kind === SyntaxKind.PropertyAccessExpression) && ((node.parent
            .asInstanceOf[PropertyAccessExpression])
            .expression === node))) || (((node.parent.kind === SyntaxKind.ElementAccessExpression) && ((node.parent
            .asInstanceOf[ElementAccessExpression])
            .expression === node)))) || (((((node.kind === SyntaxKind.Identifier) || (node.kind === SyntaxKind.QualifiedName))) && isInRightSideOfImportOrExportAssignment(
            node.asInstanceOf[Identifier]))))
        if ((!ok)) {
          error(
            node,
            Diagnostics.const_enums_can_only_be_used_in_property_or_index_access_expressions_or_the_right_hand_side_of_an_import_declaration_or_export_assignment)
    ...
    def checkExpressionWorker(node: Expression,
                              contextualMapper: TypeMapper): Type = {
      node.kind match {
        case SyntaxKind.Identifier =>
    ...
    def checkParameter(node: ParameterDeclaration) = {
      ...
      if (((node.dotDotDotToken && (!isBindingPattern(node.name))) && (!isArrayType(
    ...
    def getTypePredicateParameterIndex(
        parameterList: NodeArray[ParameterDeclaration],
          ...
          while ((i < parameterList.length)) {
            {
              ...
              if (((param.name.kind === SyntaxKind.Identifier) && ((param.name
                    .asInstanceOf[Identifier])
                    .text === parameter.text))) {
                return i
      ...
      if (isThisTypePredicate(typePredicate)) {
        ...
        if ((typePredicate.parameterIndex >= 0)) {
          if (parent.parameters(typePredicate.parameterIndex).dotDotDotToken) {
            ...
            val leadingError = chainDiagnosticMessages(
              undefined,
          ...
          (parent.parameters).foreach { fresh104 =>
            ...
            val name = fresh105.name = fresh104 {
              if ((isBindingPattern(name) && checkIfTypePredicateVariableIsDeclaredInBindingPattern(
                    name,
                    parameterName,
    ...
    def getTypePredicateParent(node: Node): SignatureDeclaration = {
      node.parent.kind match {
        case SyntaxKind.ArrowFunction | SyntaxKind.CallSignature |
            SyntaxKind.FunctionDeclaration | SyntaxKind.FunctionExpression |
            SyntaxKind.FunctionType | SyntaxKind.MethodDeclaration |
    ...
    def checkIfTypePredicateVariableIsDeclaredInBindingPattern(
      ...
      (pattern.elements).foreach { fresh106 =>
        val element = zeroOfMyType = fresh106 {
          if (isOmittedExpression(element)) {
            continue

          }
          val name = element.name
          if (((name.kind === SyntaxKind.Identifier) && ((name
                .asInstanceOf[Identifier])
                .text === predicateVariableName))) {
            error(
            ...
            if (checkIfTypePredicateVariableIsDeclaredInBindingPattern(
                  name.asInstanceOf[BindingPattern],
                  predicateVariableNode,
                  predicateVariableName)) {
              return true
    ...
    def checkSignatureDeclaration(node: SignatureDeclaration) = {
      ...
      if (produceDiagnostics) {
        ...
        if (node.`type`) {
          if (((languageVersion >= ScriptTarget.ES2015) && isSyntacticallyValidGenerator(
                node))) {
            ...
            if ((returnType === voidType)) {
              ...
              val generatorElementType = (getElementTypeOfIterableIterator(
                  returnType) || anyType)
              val iterableIteratorInstantiation = createIterableIteratorType(
    ...
    def checkClassForDuplicateDeclarations(node: ClassLikeDeclaration) = {
      ...
      (node.members).foreach { fresh107 =>
        val member = zeroOfMyType = fresh107 {
          if ((member.kind === SyntaxKind.Constructor)) {
            ((member
              .asInstanceOf[ConstructorDeclaration])
              .parameters)
              .foreach { fresh108 =>
                val param = zeroOfMyType = fresh108 {
                  if (isParameterPropertyDeclaration(param)) {
                    addName(
                      instanceNames,
                      param.name,
                      (param.name.asInstanceOf[Identifier]).text,
                      Accessor.Property)
      ...
      def addName(names: Map[Accessor],
                  location: Node,
                  name: String,
                  meaning: Accessor) = {
        val prev = names(name)
      ...
      (node.members).foreach { fresh109 =>
        val member = zeroOfMyType = fresh109 {
          if ((member.kind == SyntaxKind.PropertySignature)) {
            ...
            member.name.kind match {
              case SyntaxKind.StringLiteral | SyntaxKind.NumericLiteral |
                ...
                (memberName = (member.name
                  .asInstanceOf[(LiteralExpression | Identifier)])
                  .text)
              case _ =>
                continue
            }
            if (names(memberName)) {
              error(
                member.symbol.valueDeclaration.name,
                Diagnostics.Duplicate_identifier_0,
                memberName)
              error(
                member.name,
    ...
    def checkTypeForDuplicateIndexSignatures(node: Node) = {
      if ((node.kind === SyntaxKind.InterfaceDeclaration)) {
        ...
        if (((nodeSymbol.declarations.length > 0) && (nodeSymbol.declarations(
              0) !== node))) {
          return
      ...
      if (indexSymbol) {
        ...
        (indexSymbol.declarations).foreach { fresh110 =>
          val decl = zeroOfMyType = fresh110 {
            val declaration = decl.asInstanceOf[SignatureDeclaration]
            if (((declaration.parameters.length === 1) && declaration
                  .parameters(0)
                  .`type`)) {
              declaration.parameters(0).`type`.kind match {
                case SyntaxKind.StringKeyword =>
                  if ((!seenStringIndexer)) {
                    ...
                    error(
                      declaration,
                      Diagnostics.Duplicate_string_index_signature)
                ...
                case SyntaxKind.NumberKeyword =>
                  if ((!seenNumericIndexer)) {
                    ...
                    error(
                      declaration,
                      Diagnostics.Duplicate_number_index_signature)
    ...
    def checkPropertyDeclaration(node: PropertyDeclaration) = {
      (((checkGrammarDecorators(node) || checkGrammarModifiers(node)) || checkGrammarProperty(
        node)) || checkGrammarComputedPropertyName(node.name))
      checkVariableLikeDeclaration(node)
    ...
    def checkConstructorDeclaration(node: ConstructorDeclaration) = {
      checkSignatureDeclaration(node)
      (checkGrammarConstructorTypeParameters(node) || checkGrammarConstructorTypeAnnotation(
        node))
      checkSourceElement(node.body)
      registerForUnusedIdentifiersCheck(node)
      val symbol = getSymbolOfNode(node)
      ...
      def containsSuperCall(n: Node): Boolean = {
        if (isSuperCall(n)) {
          ...
          return forEach(
            (n.asInstanceOf[ClassLikeDeclaration]).members,
      ...
      def isInstancePropertyWithInitializer(n: Node): Boolean = {
        return (((n.kind === SyntaxKind.PropertyDeclaration) && (!((getModifierFlags(
          n) & ModifierFlags.Static)))) && (!(!(n
          .asInstanceOf[PropertyDeclaration])
          .initializer)))
      ...
      if (getClassExtendsHeritageClauseElement(containingClassDecl)) {
        ...
        if (superCall) {
          ...
          val superCallShouldBeFirst = (forEach(
              (node.parent.asInstanceOf[ClassDeclaration]).members,
              isInstancePropertyWithInitializer) || forEach(
              node.parameters,
              (p =>
                 (getModifierFlags(p) & ModifierFlags.ParameterPropertyModifier))))
          if (superCallShouldBeFirst) {
            ...
            (statements).foreach { fresh111 =>
              val statement = zeroOfMyType = fresh111 {
            ...
            if ((!superCallStatement)) {
              error(
                node,
                Diagnostics.A_super_call_must_be_the_first_statement_in_the_constructor_when_a_class_contains_initialized_properties_or_has_parameter_properties)
      ...
      if (produceDiagnostics) {
        ...
        if ((node.name.kind === SyntaxKind.ComputedPropertyName)) {
          checkComputedPropertyName(
            node.name.asInstanceOf[ComputedPropertyName])

        }
        if ((!hasDynamicName(node))) {
          val otherKind =
            (if ((node.kind === SyntaxKind.GetAccessor)) SyntaxKind.SetAccessor
             else SyntaxKind.GetAccessor)
          val otherAccessor = getDeclarationOfKind(node.symbol, otherKind)
            .asInstanceOf[AccessorDeclaration]
          if (otherAccessor) {
            if ((((getModifierFlags(node) & ModifierFlags.AccessibilityModifier)) !== ((getModifierFlags(
                  otherAccessor) & ModifierFlags.AccessibilityModifier)))) {
              error(
                node.name,
                Diagnostics.Getter_and_setter_accessors_do_not_agree_in_visibility)

            }
            if ((hasModifier(node, ModifierFlags.Abstract) !== hasModifier(
                  otherAccessor,
            ...
            checkAccessorDeclarationTypesIdentical(
              ...
              Diagnostics.get_and_set_accessor_must_have_the_same_type)
            checkAccessorDeclarationTypesIdentical(
              node,
              otherAccessor,
              getThisTypeOfDeclaration,
              Diagnostics.get_and_set_accessor_must_have_the_same_this_type)
    ...
    def checkTypeArgumentConstraints(
        typeParameters: Array[TypeParameter],
        typeArgumentNodes: Array[TypeNode]): Boolean = {
        ...
        while ((i < typeParameters.length)) {
          {
            ...
            if (constraint) {
              if ((!typeArguments)) {
                (typeArguments =
                  map(typeArgumentNodes, getTypeFromTypeNodeNoAlias))
                (mapper = createTypeMapper(typeParameters, typeArguments))
    ...
    def checkTypeReferenceNode(
      ...
      if ((`type` !== unknownType)) {
        ...
        if ((((`type`.flags & TypeFlags.Enum) && (!(`type`
              .asInstanceOf[EnumType])
              .memberTypes)) && (getNodeLinks(node).resolvedSymbol.flags & SymbolFlags.EnumMember))) {
          error(
            node,
    ...
    def checkTypeLiteral(node: TypeLiteralNode) = {
      ...
      if (produceDiagnostics) {
        val `type` = getTypeFromTypeLiteralOrFunctionOrConstructorTypeNode(
          node)
    ...
    def checkTupleType(node: TupleTypeNode) = {
      val hasErrorFromDisallowedTrailingComma =
        checkGrammarForDisallowedTrailingComma(node.elementTypes)
      if (((!hasErrorFromDisallowedTrailingComma) && (node.elementTypes.length === 0))) {
        grammarErrorOnNode(
          node,
          Diagnostics.A_tuple_type_element_list_cannot_be_empty)
    ...
    def isPrivateWithinAmbient(node: Node): Boolean = {
      return (((getModifierFlags(node) & ModifierFlags.Private)) && isInAmbientContext(
        node))

    }
    def getEffectiveDeclarationFlags(
        n: Node,
        flagsToCheck: ModifierFlags): ModifierFlags = {
      var flags = getCombinedModifierFlags(n)
      if (((((n.parent.kind !== SyntaxKind.InterfaceDeclaration) && (n.parent.kind !== SyntaxKind.ClassDeclaration)) && (n.parent.kind !== SyntaxKind.ClassExpression)) && isInAmbientContext(
            n))) {
    ...
    def checkFunctionOrConstructorSymbol(symbol: Symbol): Unit = {
      ...
      def getCanonicalOverload(overloads: Array[Declaration],
                               implementation: FunctionLikeDeclaration) = {
        val implementationSharesContainerWithFirstOverload = ((implementation !== undefined) && (implementation.parent === overloads(
            0).parent))
        return (if (implementationSharesContainerWithFirstOverload)
                  implementation
                else overloads(0))

      }
      def checkFlagAgreementBetweenOverloads(
          overloads: Array[Declaration],
          implementation: FunctionLikeDeclaration,
          flagsToCheck: ModifierFlags,
          someOverloadFlags: ModifierFlags,
          allOverloadFlags: ModifierFlags): Unit = {
        val someButNotAllOverloadFlags = (someOverloadFlags ^ allOverloadFlags)
        if ((someButNotAllOverloadFlags !== 0)) {
          val canonicalFlags = getEffectiveDeclarationFlags(
      ...
      def checkQuestionTokenAgreementBetweenOverloads(
          overloads: Array[Declaration],
          implementation: FunctionLikeDeclaration,
          someHaveQuestionToken: Boolean,
          allHaveQuestionToken: Boolean): Unit = {
        if ((someHaveQuestionToken !== allHaveQuestionToken)) {
          val canonicalHasQuestionToken = hasQuestionToken(
            getCanonicalOverload(overloads, implementation))
          forEach(
            overloads,
            (o => {
      ...
      def reportImplementationExpectedError(
        ...
        if ((subsequentNode && (subsequentNode.pos === node.end))) {
          if ((subsequentNode.kind === node.kind)) {
            val errorNode: Node = ((subsequentNode
                .asInstanceOf[FunctionLikeDeclaration])
                .name || subsequentNode)
            if (((node.name && (subsequentNode
                  .asInstanceOf[FunctionLikeDeclaration])
                  .name) && ((node.name
                  .asInstanceOf[Identifier])
                  .text === ((subsequentNode
                  ...
                  .asInstanceOf[Identifier])
                  .text))) {
              val reportError = ((((node.kind === SyntaxKind.MethodDeclaration) || (node.kind === SyntaxKind.MethodSignature))) && (((getModifierFlags(
                  node) & ModifierFlags.Static)) !== ((getModifierFlags(
                  subsequentNode) & ModifierFlags.Static))))
              if (reportError) {
                val diagnostic =
                  (if ((getModifierFlags(node) & ModifierFlags.Static))
                     Diagnostics.Function_overload_must_be_static
                   else Diagnostics.Function_overload_must_not_be_static)
                error(errorNode, diagnostic)

              }
              return

            } else if (nodeIsPresent(
                         (subsequentNode
                           .asInstanceOf[FunctionLikeDeclaration])
                           .body)) {
              error(
      ...
      (declarations).foreach { fresh112 =>
        val current = zeroOfMyType = fresh112 {
      ...
      if ((((lastSeenNonAmbientDeclaration && (!lastSeenNonAmbientDeclaration.body)) && (!((getModifierFlags(
            lastSeenNonAmbientDeclaration) & ModifierFlags.Abstract)))) && (!lastSeenNonAmbientDeclaration.questionToken))) {
      ...
      if (hasOverloads) {
        ...
        if (bodyDeclaration) {
          ...
          (signatures).foreach { fresh113 =>
            val signature = zeroOfMyType = fresh113 {
    ...
    def checkExportsOnMergedDeclarations(node: Node): Unit = {
      ...
      (symbol.declarations).foreach { fresh114 =>
        val d = zeroOfMyType = fresh114 {
          val declarationSpaces = getDeclarationSpaces(d)
          val effectiveDeclarationFlags = getEffectiveDeclarationFlags(
            d,
      ...
      def getDeclarationSpaces(d: Declaration): SymbolFlags = {
        d.kind match {
          ...
          case SyntaxKind.ModuleDeclaration =>
            return (if ((isAmbientModule(d) || (getModuleInstanceState(d) !== ModuleInstanceState.NonInstantiated)))
                      (SymbolFlags.ExportNamespace | SymbolFlags.ExportValue)
                    else SymbolFlags.ExportNamespace)
    ...
    def checkNonThenableType(`type`: Type,
                             location: Node,
                             message: DiagnosticMessage) = {
      (`type` = getWidenedType(`type`))
      if ((((!isTypeAny(`type`)) && (!isTypeNever(`type`))) && isTypeAssignableTo(
            `type`,
            getGlobalThenableType()))) {
        if (location) {
          if ((!message)) {
            (message =
              Diagnostics.Operand_for_await_does_not_have_a_valid_callable_then_member)
    ...
    def getPromisedType(promise: Type): Type = {
      ...
      if ((getObjectFlags(promise) & ObjectFlags.Reference)) {
        if ((((promise
              .asInstanceOf[GenericType])
              .target === tryGetGlobalPromiseType()) || ((promise
              .asInstanceOf[GenericType])
              .target === getGlobalPromiseLikeType()))) {
      ...
      if (((globalPromiseLikeType === emptyObjectType) || (!isTypeAssignableTo(
            promise,
      ...
      val onfulfilledParameterType = getTypeWithFacts(
      ...
      val onfulfilledParameterSignatures =
    ...
    def getTypeOfFirstParameterOfSignature(signature: Signature) = {
      return (if ((signature.parameters.length > 0))
    ...
    def checkAwaitedType(`type`: Type,
                         location: Node,
                         message: DiagnosticMessage) = {
      ...
      def checkAwaitedTypeWorker(`type`: Type): Type = {
        if ((`type`.flags & TypeFlags.Union)) {
          ...
          ((`type`.asInstanceOf[UnionType]).types).foreach { fresh116 =>
            val constituentType = zeroOfMyType = fresh116 {
          ...
          if ((promisedType === undefined)) {
            ...
            if (((`type`.id === promisedType.id) || (indexOf(
                  awaitedTypeStack,
                  promisedType.id) >= 0))) {
              if (location) {
                error(
    ...
    def checkCorrectPromiseType(returnType: Type,
                                location: Node,
      ...
      val globalPromiseType = getGlobalPromiseType()
      if (((globalPromiseType === emptyGenericType) || (globalPromiseType === getTargetType(
            returnType)))) {
        return checkAwaitedType(
          returnType,
          location,
          Diagnostics.An_async_function_or_method_must_have_a_valid_awaitable_return_type)
    ...
    def checkAsyncFunctionReturnType(node: FunctionLikeDeclaration): Type = {
      ...
      if (((!promiseConstructor) || (!symbolIsValue(promiseConstructor)))) {
        val typeName =
          (if (promiseConstructor) symbolToString(promiseConstructor)
           else typeToString(promiseType))
        return checkCorrectPromiseType(
          promiseType,
          node.`type`,
          Diagnostics.Type_0_is_not_a_valid_async_function_return_type,
          typeName)
      ...
      node.parent.kind match {
        case SyntaxKind.ClassDeclaration =>
          ...
          (expectedReturnType = getUnionType(
            Array(classConstructorType, voidType)))
        case SyntaxKind.Parameter =>
    ...
    def checkTypeNodeAsExpression(node: TypeNode) = {
      if ((node && (node.kind === SyntaxKind.TypeReference))) {
        val root = getFirstIdentifier(
          (node.asInstanceOf[TypeReferenceNode]).typeName)
        val meaning =
          (if ((root.parent.kind === SyntaxKind.TypeReference))
             SymbolFlags.Type
           else SymbolFlags.Namespace)
        val rootSymbol = resolveName(
          root,
          root.text,
        ...
        if ((rootSymbol && (rootSymbol.flags & SymbolFlags.Alias))) {
          ...
          if (((aliasTarget.flags & SymbolFlags.Value) && (!isConstEnumOrConstEnumOnlyModule(
                resolveAlias(rootSymbol))))) {
    ...
    def checkTypeAnnotationAsExpression(node: VariableLikeDeclaration) = {
      checkTypeNodeAsExpression(
        (node.asInstanceOf[PropertyDeclaration]).`type`)
    ...
    def checkParameterTypeAnnotationsAsExpressions(
        node: FunctionLikeDeclaration) = {
      (node.parameters).foreach { fresh117 =>
        val parameter = zeroOfMyType = fresh117 {
    ...
    def checkDecorators(node: Node): Unit = {
      ...
      if (compilerOptions.emitDecoratorMetadata) {
        node.kind match {
          case SyntaxKind.ClassDeclaration =>
            val `constructor` = getFirstConstructorWithBody(
          ...
          case SyntaxKind.MethodDeclaration | SyntaxKind.GetAccessor |
              SyntaxKind.SetAccessor =>
            checkParameterTypeAnnotationsAsExpressions(
              node.asInstanceOf[FunctionLikeDeclaration])
            checkReturnTypeAnnotationAsExpression(
              node.asInstanceOf[FunctionLikeDeclaration])
          case SyntaxKind.PropertyDeclaration | SyntaxKind.Parameter =>
            checkTypeAnnotationAsExpression(
              node.asInstanceOf[(PropertyDeclaration | ParameterDeclaration)])
          case _ =>
        }
    ...
    def checkFunctionDeclaration(node: FunctionDeclaration): Unit = {
      if (produceDiagnostics) {
        (checkFunctionOrMethodDeclaration(node) || checkGrammarForGenerator(
          node))
    ...
    def checkFunctionOrMethodDeclaration(
        node: (FunctionDeclaration | MethodDeclaration)): Unit = {
      checkDecorators(node)
      checkSignatureDeclaration(node)
      val isAsync = isAsyncFunctionLike(node)
      ...
      if ((!hasDynamicName(node))) {
        ...
        val firstDeclaration = forEach(
          localSymbol.declarations,
          (declaration =>
             (if (((declaration.kind === node.kind) && (!isSourceFileJavaScript(
      ...
      if ((produceDiagnostics && (!node.`type`))) {
        if (((compilerOptions.noImplicitAny && nodeIsMissing(node.body)) && (!isPrivateWithinAmbient(
              node)))) {
          ...
          val node = zeroOfMyType = fresh118 {
            node.kind match {
              ...
              case SyntaxKind.Constructor | SyntaxKind.FunctionExpression |
                ...
                checkUnusedTypeParameters(
                  node.asInstanceOf[FunctionLikeDeclaration])
              case SyntaxKind.MethodSignature | SyntaxKind.CallSignature |
                  SyntaxKind.ConstructSignature | SyntaxKind.IndexSignature |
                  SyntaxKind.FunctionType | SyntaxKind.ConstructorType =>
                checkUnusedTypeParameters(
                  node.asInstanceOf[FunctionLikeDeclaration])
    ...
    def checkUnusedLocalsAndParameters(node: Node): Unit = {
      if ((((node.parent.kind !== SyntaxKind.InterfaceDeclaration) && noUnusedIdentifiers) && (!isInAmbientContext(
            node)))) {
        (node.locals).keys.foreach { fresh119 =>
          val key = zeroOfMyType = fresh119 {
            val local = node.locals(key)
            if ((!local.isReferenced)) {
              if ((local.valueDeclaration && (getRootDeclaration(
                    local.valueDeclaration).kind === SyntaxKind.Parameter))) {
                val parameter = getRootDeclaration(local.valueDeclaration)
                  .asInstanceOf[ParameterDeclaration]
                if ((((compilerOptions.noUnusedParameters && (!isParameterPropertyDeclaration(
                      parameter))) && (!parameterIsThisKeyword(parameter))) && (!parameterNameStartsWithUnderscore(
                      local.valueDeclaration.name)))) {
                  error(
                    local.valueDeclaration.name,
                ...
                forEach(
                  local.declarations,
                  (d => errorUnusedLocal((d.name || d), local.name)))
    ...
    def parameterNameStartsWithUnderscore(parameterName: DeclarationName) = {
      return (parameterName && isIdentifierThatStartsWithUnderScore(
        parameterName))
    ...
    def isIdentifierThatStartsWithUnderScore(node: Node) = {
      return ((node.kind === SyntaxKind.Identifier) && ((node
        .asInstanceOf[Identifier])
        .text
        .charCodeAt(0) === CharacterCodes._underscore_))

    }
    def checkUnusedClassMembers(
        node: (ClassDeclaration | ClassExpression)): Unit = {
      if ((compilerOptions.noUnusedLocals && (!isInAmbientContext(node)))) {
        if (node.members) {
          (node.members).foreach { fresh120 =>
            val member = zeroOfMyType = fresh120 {
              if (((member.kind === SyntaxKind.MethodDeclaration) || (member.kind === SyntaxKind.PropertyDeclaration))) {
                ...
                ((member
                  ...
                  .parameters)
                  .foreach { fresh121 =>
                    val parameter = zeroOfMyType = fresh121 {
                      if (((!parameter.symbol.isReferenced) && (getModifierFlags(
                            parameter) & ModifierFlags.Private))) {
                        error(
                          parameter.name,
    ...
    def checkUnusedTypeParameters(
        node: (ClassDeclaration | ClassExpression | FunctionDeclaration | MethodDeclaration | FunctionExpression | ArrowFunction | ConstructorDeclaration | SignatureDeclaration | InterfaceDeclaration)) = {
      if ((compilerOptions.noUnusedLocals && (!isInAmbientContext(node)))) {
        if (node.typeParameters) {
          val symbol = getSymbolOfNode(node)
          val lastDeclaration = ((symbol && symbol.declarations) && lastOrUndefined(
              symbol.declarations))
          ...
          (node.typeParameters).foreach { fresh122 =>
            val typeParameter = zeroOfMyType = fresh122 {
    ...
    def checkUnusedModuleMembers(
        node: (ModuleDeclaration | SourceFile)): Unit = {
      if ((compilerOptions.noUnusedLocals && (!isInAmbientContext(node)))) {
        (node.locals).keys.foreach { fresh123 =>
          val key = zeroOfMyType = fresh123 {
            val local = node.locals(key)
            if (((!local.isReferenced) && (!local.exportSymbol))) {
              (local.declarations).foreach { fresh124 =>
                val declaration = zeroOfMyType = fresh124 {
    ...
    def checkCollisionWithArgumentsInGeneratedCode(
        node: SignatureDeclaration) = {
      if ((((!hasDeclaredRestParameter(node)) || isInAmbientContext(node)) || nodeIsMissing(
      ...
      forEach(
        node.parameters,
        (p => {
           if (((p.name && (!isBindingPattern(p.name))) && ((p.name
                 .asInstanceOf[Identifier])
                 .text === argumentsSymbol.name))) {
             error(
               p,
    ...
    def needCollisionCheckForIdentifier(node: Node,
      ...
      if (((root.kind === SyntaxKind.Parameter) && nodeIsMissing(
            (root.parent.asInstanceOf[FunctionLikeDeclaration]).body))) {
        return false
    ...
    def checkCollisionWithCapturedThisVariable(node: Node,
      ...
      while (current) {
        {
          if ((getNodeCheckFlags(current) & NodeCheckFlags.CaptureThis)) {
            ...
            if (isDeclaration) {
              error(
                (node.asInstanceOf[Declaration]).name,
    ...
    def checkCollisionWithCapturedSuperVariable(node: Node, name: Identifier) = {
      ...
      if (getClassExtendsHeritageClauseElement(enclosingClass)) {
        ...
        if (isDeclaration) {
          error(
            node,
            Diagnostics.Duplicate_identifier_super_Compiler_uses_super_to_capture_base_class_reference)

        } else {
          error(
            node,
            Diagnostics.Expression_resolves_to_super_that_compiler_uses_to_capture_base_class_reference)
    ...
    def checkCollisionWithRequireExportsInGeneratedCode(node: Node,
                                                        name: Identifier) = {
      ...
      if (((node.kind === SyntaxKind.ModuleDeclaration) && (getModuleInstanceState(
            node) !== ModuleInstanceState.Instantiated))) {
        return

      }
      val parent = getDeclarationContainer(node)
      if (((parent.kind === SyntaxKind.SourceFile) && isExternalOrCommonJsModule(
            parent.asInstanceOf[SourceFile]))) {
        error(
          name,
    ...
    def checkCollisionWithGlobalPromiseInGeneratedCode(
        node: Node,
        name: Identifier): Unit = {
      ...
      val parent = getDeclarationContainer(node)
      if ((((parent.kind === SyntaxKind.SourceFile) && isExternalOrCommonJsModule(parent.asInstanceOf[
            SourceFile])) && (parent.flags & NodeFlags.HasAsyncFunctions))) {
        error(
          name,
    ...
    def checkVarDeclaredNamesNotShadowed(
        node: (VariableDeclaration | BindingElement)) = {
      if (((((getCombinedNodeFlags(node) & NodeFlags.BlockScoped)) !== 0) || isParameterDeclaration(
            node))) {
        return
      ...
      if ((symbol.flags & SymbolFlags.FunctionScopedVariable)) {
        val localDeclarationSymbol = resolveName(
          node,
          (node.name.asInstanceOf[Identifier]).text,
        ...
        if (((localDeclarationSymbol && (localDeclarationSymbol !== symbol)) && (localDeclarationSymbol.flags & SymbolFlags.BlockScopedVariable))) {
          if ((getDeclarationNodeFlagsFromSymbol(localDeclarationSymbol) & NodeFlags.BlockScoped)) {
            val varDeclList = getAncestor(
              localDeclarationSymbol.valueDeclaration,
              SyntaxKind.VariableDeclarationList)
            val container =
              (if (((varDeclList.parent.kind === SyntaxKind.VariableStatement) && varDeclList.parent.parent))
                 varDeclList.parent.parent
               else undefined)
            val namesShareScope = (container && ((((((container.kind === SyntaxKind.Block) && isFunctionLike(
                container.parent)) || (container.kind === SyntaxKind.ModuleBlock)) || (container.kind === SyntaxKind.ModuleDeclaration)) || (container.kind === SyntaxKind.SourceFile))))
            if ((!namesShareScope)) {
    ...
    def checkParameterInitializer(node: VariableLikeDeclaration): Unit = {
      ...
      def visit(n: Node): Unit = {
        ...
        if ((n.kind === SyntaxKind.PropertyAccessExpression)) {
          ...
          val symbol = resolveName(
            n,
            (n.asInstanceOf[Identifier]).text,
          ...
          if ((symbol.valueDeclaration === node)) {
            error(
              n,
              Diagnostics.Parameter_0_cannot_be_referenced_in_its_initializer,
              declarationNameToString(node.name))
            return

          }
          val enclosingContainer = getEnclosingBlockScopeContainer(
            symbol.valueDeclaration)
          if ((enclosingContainer === func)) {
              ...
              while ((current !== node.initializer)) {
                {
                  ...
                  if ((((current.parent.kind === SyntaxKind.PropertyDeclaration) && (!(hasModifier(
                        current.parent,
                        ModifierFlags.Static)))) && isClassLike(
            ...
            error(
              n,
              Diagnostics.Initializer_of_parameter_0_cannot_reference_identifier_1_declared_after_it,
              declarationNameToString(node.name),
              declarationNameToString(n.asInstanceOf[Identifier]))
    ...
    def convertAutoToAny(`type`: Type) = {
      return (if ((`type` === autoType)) anyType
              else (if ((`type` === autoArrayType)) anyArrayType
                    else `type`))
    ...
    def checkVariableLikeDeclaration(node: VariableLikeDeclaration) = {
      ...
      if ((node.kind === SyntaxKind.BindingElement)) {
        if ((node.propertyName && (node.propertyName.kind === SyntaxKind.ComputedPropertyName))) {
          checkComputedPropertyName(
            node.propertyName.asInstanceOf[ComputedPropertyName])
        ...
        val property =
          getPropertyOfType(parentType, getTextOfPropertyName(name))
      ...
      if (isBindingPattern(node.name)) {
        forEach(
          (node.name.asInstanceOf[BindingPattern]).elements,
          checkSourceElement)

      }
      if (((node.initializer && (getRootDeclaration(node).kind === SyntaxKind.Parameter)) && nodeIsMissing(
      ...
      if ((node === symbol.valueDeclaration)) {
        ...
        val declarationType = convertAutoToAny(
          getWidenedTypeForVariableLikeDeclaration(node))
        if ((((`type` !== unknownType) && (declarationType !== unknownType)) && (!isTypeIdenticalTo(
              `type`,
              declarationType)))) {
          error(
            node.name,
    ...
    def areDeclarationFlagsIdentical(left: Declaration, right: Declaration) = {
      ...
      return (((getModifierFlags(left) & interestingFlags)) === ((getModifierFlags(
        right) & interestingFlags)))
    ...
    def checkVariableStatement(node: VariableStatement) = {
      (((checkGrammarDecorators(node) || checkGrammarModifiers(node)) || checkGrammarVariableDeclarationList(
        node.declarationList)) || checkGrammarForDisallowedLetOrConstStatement(
        node))
      forEach(node.declarationList.declarations, checkSourceElement)

    }
    def checkGrammarDisallowedModifiersOnObjectLiteralExpressionMethod(
        node: Node) = {
      ...
      if ((!checkGrammarStatementInAmbientContext(node))) {
        if ((node.initializer && (node.initializer.kind === SyntaxKind.VariableDeclarationList))) {
          checkGrammarVariableDeclarationList(
            node.initializer.asInstanceOf[VariableDeclarationList])
    ...
    def checkForInStatement(node: ForInStatement) = {
      ...
      if ((node.initializer.kind === SyntaxKind.VariableDeclarationList)) {
        ...
        if (((varExpr.kind === SyntaxKind.ArrayLiteralExpression) || (varExpr.kind === SyntaxKind.ObjectLiteralExpression))) {
        ...
        } else if ((!isTypeAnyOrAllConstituentTypesHaveKind(
                     leftType,
                     TypeFlags.StringLike))) {
          error(
            varExpr,
          ...
          checkReferenceExpression(
            varExpr,
            Diagnostics.Invalid_left_hand_side_in_for_in_statement,
      ...
      if ((!isTypeAnyOrAllConstituentTypesHaveKind(
        ...
        error(
          node.expression,
          Diagnostics.The_right_hand_side_of_a_for_in_statement_must_be_of_type_any_an_object_type_or_a_type_parameter)
    ...
    def checkForInOrForOfVariableDeclaration(
        iterationStatement: (ForInStatement | ForOfStatement)): Unit = {
      val variableDeclarationList =
        iterationStatement.initializer.asInstanceOf[VariableDeclarationList]
    ...
    def checkIteratedTypeOrElementType(inputType: Type,
                                       errorNode: Node,
                                       allowStringInput: Boolean): Type = {
    ...
    def checkElementTypeOfIterable(iterable: Type, errorNode: Node): Type = {
      ...
      if ((errorNode && elementType)) {
        checkTypeAssignableTo(
          iterable,
          createIterableType(elementType),
          errorNode)
    ...
    def getElementTypeOfIterable(`type`: Type, errorNode: Node): Type = {
      ...
      if ((!typeAsIterable.iterableElementType)) {
        if ((((getObjectFlags(`type`) & ObjectFlags.Reference)) && ((`type`
              .asInstanceOf[GenericType])
              .target === getGlobalIterableType()))) {
          (typeAsIterable.iterableElementType =
            (`type`.asInstanceOf[GenericType]).typeArguments(0))

        } else {
          val iteratorFunction = getTypeOfPropertyOfType(
            `type`,
          ...
          val iteratorFunctionSignatures =
            (if (iteratorFunction)
               getSignaturesOfType(iteratorFunction, SignatureKind.Call)
             else emptyArray)
          ...
          (typeAsIterable.iterableElementType = getElementTypeOfIterator(
            getUnionType(
              map(iteratorFunctionSignatures, getReturnTypeOfSignature),
              true),
            errorNode))
    ...
    def getElementTypeOfIterator(`type`: Type, errorNode: Node): Type = {
      ...
      if ((!typeAsIterator.iteratorElementType)) {
        if ((((getObjectFlags(`type`) & ObjectFlags.Reference)) && ((`type`
          ...
          val iteratorNextFunctionSignatures =
            (if (iteratorNextFunction)
               getSignaturesOfType(iteratorNextFunction, SignatureKind.Call)
             else emptyArray)
          ...
          val iteratorNextResult = getUnionType(
            map(iteratorNextFunctionSignatures, getReturnTypeOfSignature),
            true)
          if (isTypeAny(iteratorNextResult)) {
            return undefined

          }
          val iteratorNextValue =
            getTypeOfPropertyOfType(iteratorNextResult, "value")
    ...
    def getElementTypeOfIterableIterator(`type`: Type): Type = {
      ...
      if ((((getObjectFlags(`type`) & ObjectFlags.Reference)) && ((`type`
            .asInstanceOf[GenericType])
            .target === getGlobalIterableIteratorType()))) {
        return (`type`.asInstanceOf[GenericType]).typeArguments(0)

      }
      return (getElementTypeOfIterable(`type`, undefined) || getElementTypeOfIterator(
        `type`,
        undefined))

    }
    def checkElementTypeOfArrayOrString(arrayOrStringType: Type,
                                        errorNode: Node): Type = {
      ...
      if ((arrayOrStringType.flags & TypeFlags.Union)) {
        (arrayType = getUnionType(
          filter(
            (arrayOrStringType.asInstanceOf[UnionType]).types,
      ...
      if ((!isArrayLikeType(arrayType))) {
        if ((!reportedError)) {
          val diagnostic =
            (if (hasStringConstituent) Diagnostics.Type_0_is_not_an_array_type
             else Diagnostics.Type_0_is_not_an_array_type_or_a_string_type)
    ...
    def checkBreakOrContinueStatement(node: BreakOrContinueStatement) = {
      (checkGrammarStatementInAmbientContext(node) || checkGrammarBreakOrContinueStatement(
        node))

    }
    def isGetAccessorWithAnnotatedSetAccessor(node: FunctionLikeDeclaration) = {
      return (!(!(((node.kind === SyntaxKind.GetAccessor) && getSetAccessorTypeAnnotationNode(
        getDeclarationOfKind(node.symbol, SyntaxKind.SetAccessor)
          .asInstanceOf[SetAccessorDeclaration])))))

    }
    def isUnwrappedReturnTypeVoidOrAny(func: FunctionLikeDeclaration,
                                       returnType: Type): Boolean = {
      val unwrappedReturnType =
        (if (isAsyncFunctionLike(func)) getPromisedType(returnType)
         else returnType)
      return (unwrappedReturnType && maybeTypeOfKind(
        unwrappedReturnType,
        (TypeFlags.Void | TypeFlags.Any)))
    ...
    def checkReturnStatement(node: ReturnStatement) = {
      ...
      if (func) {
        ...
        if (((strictNullChecks || node.expression) || (returnType.flags & TypeFlags.Never))) {
          val exprType =
            (if (node.expression) checkExpressionCached(node.expression)
             else undefinedType)
          ...
          } else if ((func.`type` || isGetAccessorWithAnnotatedSetAccessor(
                       func))) {
            if (isAsyncFunctionLike(func)) {
        ...
        (clause => {
           if (((clause.kind === SyntaxKind.DefaultClause) && (!hasDuplicateDefaultClause))) {
             if ((firstDefaultClause === undefined)) {
               ...
               val end =
                 (if ((clause.statements.length > 0)) clause.statements(0).pos
                  else clause.end)
               grammarErrorAtPos(
                 sourceFile,
                 start,
        ...
        while (current) {
          {
            ...
            if (((current.kind === SyntaxKind.LabeledStatement) && ((current
        ...
        if (catchClause.variableDeclaration) {
          if (catchClause.variableDeclaration.`type`) {
            grammarErrorOnFirstToken(
              ...
              Diagnostics.Catch_clause_variable_cannot_have_a_type_annotation)
            ...
            if (blockLocals) {
              (catchClause.locals).keys.foreach { fresh125 =>
                val caughtName = zeroOfMyType = fresh125 {
    ...
    def checkIndexConstraints(`type`: Type) = {
      val declaredNumberIndexer =
        getIndexDeclarationOfSymbol(`type`.symbol, IndexKind.Number)
      val declaredStringIndexer =
        getIndexDeclarationOfSymbol(`type`.symbol, IndexKind.String)
      ...
      if ((stringIndexType || numberIndexType)) {
        ...
        if (((getObjectFlags(`type`) & ObjectFlags.Class) && isClassLike(
              `type`.symbol.valueDeclaration))) {
          val classDeclaration =
            `type`.symbol.valueDeclaration.asInstanceOf[ClassLikeDeclaration]
          (classDeclaration.members).foreach { fresh126 =>
            val member = zeroOfMyType = fresh126 {
              if (((!((getModifierFlags(member) & ModifierFlags.Static))) && hasDynamicName(
                    member))) {
      ...
      if ((stringIndexType && numberIndexType)) {
        ...
        if (((!errorNode) && ((getObjectFlags(`type`) & ObjectFlags.Interface)))) {
          val someBaseTypeHasBothIndexers = forEach(
            getBaseTypes(`type`.asInstanceOf[InterfaceType]),
            (base =>
               (getIndexTypeOfType(base, IndexKind.String) && getIndexTypeOfType(
                 base,
                 IndexKind.Number))))
          (errorNode =
            (if (someBaseTypeHasBothIndexers) undefined
             else `type`.symbol.declarations(0)))

        }

      }
      if ((errorNode && (!isTypeAssignableTo(
            numberIndexType,
            stringIndexType)))) {
        error(
          errorNode,
          Diagnostics.Numeric_index_type_0_is_not_assignable_to_string_index_type_1,
          typeToString(numberIndexType),
          typeToString(stringIndexType))

      }
      def checkIndexConstraintForProperty(prop: Symbol,
                                          propertyType: Type,
                                          containingType: Type,
        ...
        if (((indexKind === IndexKind.Number) && (!isNumericName(
        ...
        if (((prop.valueDeclaration.name.kind === SyntaxKind.ComputedPropertyName) || (prop.parent === containingType.symbol))) {
          ...
          val someBaseClassHasBothPropertyAndIndexer = forEach(
            getBaseTypes(containingType.asInstanceOf[InterfaceType]),
            (base =>
               (getPropertyOfObjectType(base, prop.name) && getIndexTypeOfType(
                 base,
                 indexKind))))
          (errorNode =
            (if (someBaseClassHasBothPropertyAndIndexer) undefined
             else containingType.symbol.declarations(0)))

        }
        if ((errorNode && (!isTypeAssignableTo(propertyType, indexType)))) {
          val errorMessage =
            (if ((indexKind === IndexKind.String))
    ...
    def checkTypeNameIsReserved(name: DeclarationName,
                                message: DiagnosticMessage): Unit = {
    ...
    def checkTypeParameterListsIdentical(
      ...
      var firstDecl: (ClassLikeDeclaration | InterfaceDeclaration) =
        zeroOfMyType
      (symbol.declarations).foreach { fresh127 =>
        val declaration = zeroOfMyType = fresh127 {
          if (((declaration.kind === SyntaxKind.ClassDeclaration) || (declaration.kind === SyntaxKind.InterfaceDeclaration))) {
            if ((!firstDecl)) {
              (firstDecl = declaration
                .asInstanceOf[(ClassLikeDeclaration | InterfaceDeclaration)])

            } else if ((!areTypeParametersIdentical(
                         firstDecl.typeParameters,
                         node.typeParameters))) {
              error(
                node.name,
    ...
    def checkClassLikeDeclaration(node: ClassLikeDeclaration) = {
        ...
        if ((baseTypes.length && produceDiagnostics)) {
          ...
          if (baseTypeNode.typeArguments) {
            ...
            (getConstructorsForTypeArguments(
              staticBaseType,
              baseTypeNode.typeArguments)).foreach { fresh128 =>
              val `constructor` = zeroOfMyType = fresh128 {
          ...
          if (((baseType.symbol.valueDeclaration && (!isInAmbientContext(
                baseType.symbol.valueDeclaration))) && (baseType.symbol.valueDeclaration.kind === SyntaxKind.ClassDeclaration))) {
            if ((!isBlockScopedNameDeclaredBeforeUse(
                  baseType.symbol.valueDeclaration,
                  node))) {
              error(
                baseTypeNode,
                Diagnostics.A_class_must_be_declared_after_its_base_class)
          ...
          if ((!((staticBaseType.symbol && (staticBaseType.symbol.flags & SymbolFlags.Class))))) {
            ...
            if (forEach(
                  constructors,
                  (sig => (getReturnTypeOfSignature(sig) !== baseType)))) {
              error(
                baseTypeNode.expression,
      ...
      if (implementedTypeNodes) {
        (implementedTypeNodes).foreach { fresh129 =>
          val typeRefNode = zeroOfMyType = fresh129 {
            ...
            if (produceDiagnostics) {
              ...
              if ((t !== unknownType)) {
                val declaredType =
                  (if ((getObjectFlags(t) & ObjectFlags.Reference))(t
                     .asInstanceOf[TypeReference])
                     .target
                   else t)
                if ((getObjectFlags(declaredType) & ObjectFlags.ClassOrInterface)) {
                  ...
                  error(
                    typeRefNode,
                    Diagnostics.A_class_may_only_implement_another_class_or_interface)
    ...
    def checkBaseTypeAccessibility(`type`: Type,
      ...
      if (signatures.length) {
        ...
        if ((declaration && (getModifierFlags(declaration) & ModifierFlags.Private))) {
          val typeClassDeclaration = getClassLikeDeclarationOfSymbol(
            `type`.symbol).asInstanceOf[ClassLikeDeclaration]
    ...
    def getTargetSymbol(s: Symbol) = {
      return (if ((s.flags & SymbolFlags.Instantiated))
                getSymbolLinks(s).target
              else s)
    ...
    def getClassLikeDeclarationOfSymbol(symbol: Symbol): Declaration = {
      return forEach(
        symbol.declarations,
        (d => (if (isClassLike(d)) d else undefined)))

    }
    def checkKindsOfPropertyMemberOverrides(`type`: InterfaceType,
                                            baseType: ObjectType): Unit = {
      val baseProperties = getPropertiesOfObjectType(baseType)
      (baseProperties).foreach { fresh130 =>
        val baseProperty = zeroOfMyType = fresh130 {
          ...
          val derived =
            getTargetSymbol(getPropertyOfObjectType(`type`, base.name))
          val baseDeclarationFlags =
            getDeclarationModifierFlagsFromSymbol(base)
          Debug.assert(
            (!(!derived)),
            "derived should point to something, even if it is the base class' declaration.")
          if (derived) {
            if ((derived === base)) {
              val derivedClassDecl =
                getClassLikeDeclarationOfSymbol(`type`.symbol)
              if (((baseDeclarationFlags & ModifierFlags.Abstract) && (((!derivedClassDecl) || (!((getModifierFlags(
                    derivedClassDecl) & ModifierFlags.Abstract))))))) {
              ...
              if ((base.flags & SymbolFlags.Method)) {
                if ((derived.flags & SymbolFlags.Accessor)) {
                  ...
                  Debug.assert(
                    (((derived.flags & SymbolFlags.Property)) !== 0))
                  (errorMessage =
                    Diagnostics.Class_0_defines_instance_member_function_1_but_extended_class_2_defines_it_as_instance_member_property)
    ...
    def areTypeParametersIdentical(list1: Array[TypeParameterDeclaration],
                                   list2: Array[TypeParameterDeclaration]) = {
      if (((!list1) && (!list2))) {
        return true
        ...
        while ((i < len)) {
          {
            ...
            if ((!isTypeIdenticalTo(
                  getTypeFromTypeNode(tp1.constraint),
                  getTypeFromTypeNode(tp2.constraint)))) {
              return false
    ...
    def checkInheritedPropertiesAreIdentical(`type`: InterfaceType,
                                             typeNode: Node): Boolean = {
      val baseTypes = getBaseTypes(`type`)
      ...
      forEach(
        ...
        (p => {
           (seen(p.name) = Map("prop" -> p, "containingType" -> `type`))

         }))
      var ok = true
      (baseTypes).foreach { fresh131 =>
        val base = zeroOfMyType = fresh131 {
          val properties = getPropertiesOfObjectType(
            getTypeWithThisArgument(base, `type`.thisType))
          (properties).foreach { fresh132 =>
            val prop = zeroOfMyType = fresh132 {
              val existing = seen(prop.name)
              if ((!existing)) {
                (seen(prop.name) =
                  Map("prop" -> prop, "containingType" -> base))

              } else {
                val isInheritedProperty = (existing.containingType !== `type`)
                if ((isInheritedProperty && (!isPropertyIdenticalTo(
                      existing.prop,
                      prop)))) {
                  (ok = false)
    ...
    def checkInterfaceDeclaration(node: InterfaceDeclaration) = {
      ((checkGrammarDecorators(node) || checkGrammarModifiers(node)) || checkGrammarInterfaceDeclaration(
        node))
      checkTypeParameters(node.typeParameters)
      if (produceDiagnostics) {
        checkTypeNameIsReserved(
        ...
        if ((node === firstInterfaceDecl)) {
          ...
          if (checkInheritedPropertiesAreIdentical(`type`, node.name)) {
            (getBaseTypes(`type`)).foreach { fresh133 =>
              val baseType = zeroOfMyType = fresh133 {
                checkTypeAssignableTo(
                  typeWithThis,
                  getTypeWithThisArgument(baseType, `type`.thisType),
                  node.name,
    ...
    def computeEnumMemberValues(node: EnumDeclaration) = {
      ...
      if ((!((nodeLinks.flags & NodeCheckFlags.EnumValuesComputed)))) {
        ...
        (node.members).foreach { fresh134 =>
          val member = zeroOfMyType = fresh134 {
            if (isComputedNonLiteralName(
                  member.name.asInstanceOf[PropertyName])) {
              error(
                member.name,
              ...
              val text =
                getTextOfPropertyName(member.name.asInstanceOf[PropertyName])
              if ((isNumericLiteralName(text) && (!isInfinityOrNaNString(
                    text)))) {
                error(
                  member.name,
            ...
            if (initializer) {
              (autoValue = computeConstantValueForEnumMemberInitializer(
                initializer,
                enumType,
                enumIsConst,
                ambient))
      ...
      def computeConstantValueForEnumMemberInitializer(
        ...
        def evalConstant(e: Node): Int = {
          e.kind match {
            case SyntaxKind.PrefixUnaryExpression =>
              val value = evalConstant(
                (e.asInstanceOf[PrefixUnaryExpression]).operand)
              if ((value === undefined)) {
                return undefined
            ...
            case SyntaxKind.BinaryExpression =>
              ...
              val right = evalConstant(
                (e.asInstanceOf[BinaryExpression]).right)
              if ((right === undefined)) {
                return undefined
            ...
            case SyntaxKind.ParenthesizedExpression =>
              return evalConstant(
                (e.asInstanceOf[ParenthesizedExpression]).expression)
            case SyntaxKind.Identifier | SyntaxKind.ElementAccessExpression |
                SyntaxKind.PropertyAccessExpression =>
              ...
              if ((e.kind === SyntaxKind.Identifier)) {
                ...
                if ((e.kind === SyntaxKind.ElementAccessExpression)) {
                  if ((((e
                        .asInstanceOf[ElementAccessExpression])
                        .argumentExpression === undefined) || ((e
                        .asInstanceOf[ElementAccessExpression])
                        .argumentExpression
                  ...
                  (expression =
                    (e.asInstanceOf[ElementAccessExpression]).expression)
                  (propertyName = ((e
                    .asInstanceOf[ElementAccessExpression])
                    .argumentExpression
                    .asInstanceOf[LiteralExpression])
                  ...
                  (expression =
                    (e.asInstanceOf[PropertyAccessExpression]).expression)
                  (propertyName =
                    (e.asInstanceOf[PropertyAccessExpression]).name.text)
    ...
    def checkEnumDeclaration(node: EnumDeclaration) = {
      ...
      if (((compilerOptions.isolatedModules && enumIsConst) && isInAmbientContext(
        ...
        error(
          node.name,
          Diagnostics.Ambient_const_enums_are_not_allowed_when_the_isolatedModules_flag_is_provided)
    ...
    def getFirstNonAmbientClassOrFunctionDeclaration(
        symbol: Symbol): Declaration = {
      val declarations = symbol.declarations
      (declarations).foreach { fresh135 =>
        val declaration = zeroOfMyType = fresh135 {
          if (((((declaration.kind === SyntaxKind.ClassDeclaration) || (((declaration.kind === SyntaxKind.FunctionDeclaration) && nodeIsPresent(
                (declaration
                  .asInstanceOf[FunctionLikeDeclaration])
                  .body))))) && (!isInAmbientContext(declaration)))) {
            return declaration
    ...
    def checkModuleDeclaration(node: ModuleDeclaration) = {
      if (produceDiagnostics) {
        ...
        if (((!checkGrammarDecorators(node)) && (!checkGrammarModifiers(
              node)))) {
        ...
        checkExportsOnMergedDeclarations(node)
        val symbol = getSymbolOfNode(node)
        if (((((symbol.flags & SymbolFlags.ValueModule) && (symbol.declarations.length > 1)) && (!inAmbientContext)) && isInstantiatedModule(
              node,
              (compilerOptions.preserveConstEnums || compilerOptions.isolatedModules)))) {
          val firstNonAmbientClassOrFunc =
            getFirstNonAmbientClassOrFunctionDeclaration(symbol)
          if (firstNonAmbientClassOrFunc) {
            if ((getSourceFileOfNode(node) !== getSourceFileOfNode(
          ...
          val mergedClass =
            getDeclarationOfKind(symbol, SyntaxKind.ClassDeclaration)
        ...
        if (isAmbientExternalModule) {
          if (isExternalModuleAugmentation(node)) {
            ...
            if ((checkBody && node.body)) {
              ((node.body.asInstanceOf[ModuleBlock]).statements).foreach {
                fresh136 =>
                  val statement = zeroOfMyType = fresh136 {
    ...
    def checkModuleAugmentationElement(node: Node,
                                       isGlobalAugmentation: Boolean): Unit = {
      node.kind match {
        case SyntaxKind.VariableStatement =>
          ((node
            ...
            .declarations)
            .foreach { fresh137 =>
              val decl = zeroOfMyType = fresh137 {
        ...
        case SyntaxKind.BindingElement | SyntaxKind.VariableDeclaration =>
          val name =
            (node.asInstanceOf[(VariableDeclaration | BindingElement)]).name
          if (isBindingPattern(name)) {
            (name.elements).foreach { fresh138 =>
              val el = zeroOfMyType = fresh138 {
        ...
        case SyntaxKind.ClassDeclaration | SyntaxKind.EnumDeclaration |
            SyntaxKind.FunctionDeclaration | SyntaxKind.InterfaceDeclaration |
            SyntaxKind.ModuleDeclaration | SyntaxKind.TypeAliasDeclaration =>
          if (isGlobalAugmentation) {
            return
          ...
          if (symbol) {
            ...
            if ((!reportError)) {
              (reportError = isExternalModuleAugmentation(
    ...
    def getFirstIdentifier(
        node: EntityNameOrEntityNameExpression): Identifier = {
      node.kind match {
        case SyntaxKind.Identifier =>
    ...
    def checkExternalImportOrExportDeclaration(
      ...
      val inAmbientExternalModule = ((node.parent.kind === SyntaxKind.ModuleBlock) && isAmbientModule(
          node.parent.parent.asInstanceOf[ModuleDeclaration]))
      ...
      if ((inAmbientExternalModule && isExternalModuleNameRelative(
            (moduleName.asInstanceOf[LiteralExpression]).text))) {
    ...
    def checkAliasSymbol(
        node: (ImportEqualsDeclaration | ImportClause | NamespaceImport | ImportSpecifier | ExportSpecifier)) = {
      val symbol = getSymbolOfNode(node)
      val target = resolveAlias(symbol)
      if ((target !== unknownSymbol)) {
        val excludedMeanings = ((((if ((symbol.flags & ((SymbolFlags.Value | SymbolFlags.ExportValue))))
                                     SymbolFlags.Value
                                   else 0)
            ) | ((if ((symbol.flags & SymbolFlags.Type)) SymbolFlags.Type
                  else 0) )) | ((if ((symbol.flags & SymbolFlags.Namespace))
                                   SymbolFlags.Namespace
                                 else 0) ))
    ...
    def checkImportBinding(
        node: (ImportEqualsDeclaration | ImportClause | NamespaceImport | ImportSpecifier)) = {
      checkCollisionWithCapturedThisVariable(node, node.name)
      checkCollisionWithRequireExportsInGeneratedCode(node, node.name)
      checkCollisionWithGlobalPromiseInGeneratedCode(node, node.name)
    ...
    def checkImportDeclaration(node: ImportDeclaration) = {
      ...
      if ((((!checkGrammarDecorators(node)) && (!checkGrammarModifiers(node))) && (getModifierFlags(
            node) !== 0))) {
        grammarErrorOnFirstToken(
          node,
        ...
        if (importClause) {
          ...
          if (importClause.namedBindings) {
            if ((importClause.namedBindings.kind === SyntaxKind.NamespaceImport)) {
              checkImportBinding(
                importClause.namedBindings.asInstanceOf[NamespaceImport])
    ...
    def checkImportEqualsDeclaration(node: ImportEqualsDeclaration) = {
      ...
      (checkGrammarDecorators(node) || checkGrammarModifiers(node))
      if ((isInternalModuleImportEqualsDeclaration(node) || checkExternalImportOrExportDeclaration(
            node))) {
        ...
        if (isInternalModuleImportEqualsDeclaration(node)) {
          ...
          if ((target !== unknownSymbol)) {
            if ((target.flags & SymbolFlags.Value)) {
              val moduleName = getFirstIdentifier(
                node.moduleReference.asInstanceOf[EntityName])
              if ((!((resolveEntityName(
                    moduleName,
                    (SymbolFlags.Value | SymbolFlags.Namespace)).flags & SymbolFlags.Namespace)))) {
                error(
                  moduleName,
          ...
          if (((modulekind === ModuleKind.ES2015) && (!isInAmbientContext(
                node)))) {
            grammarErrorOnNode(
              node,
              Diagnostics.Import_assignment_cannot_be_used_when_targeting_ECMAScript_2015_modules_Consider_using_import_Asterisk_as_ns_from_mod_import_a_from_mod_import_d_from_mod_or_another_module_format_instead)
    ...
    def checkExportDeclaration(node: ExportDeclaration) = {
      ...
      if (((!node.moduleSpecifier) || checkExternalImportOrExportDeclaration(
            node))) {
    ...
    def checkGrammarModuleElementContext(
        node: Statement,
        errorMessage: DiagnosticMessage): Boolean = {
    ...
    def checkExportSpecifier(node: ExportSpecifier) = {
      ...
      if ((!(node.parent.parent
        ...
        val symbol = resolveName(
          ...
          undefined,
          undefined)
        if ((symbol && (((symbol === undefinedSymbol) || isGlobalSourceFile(
    ...
    def checkExportAssignment(node: ExportAssignment) = {
      ...
      val container =
        (if ((node.parent.kind === SyntaxKind.SourceFile))
           node.parent.asInstanceOf[SourceFile]
         else node.parent.parent.asInstanceOf[ModuleDeclaration])
      if (((container.kind === SyntaxKind.ModuleDeclaration) && (!isAmbientModule(
            container)))) {
        error(
          node,
      ...
      if ((node.isExportEquals && (!isInAmbientContext(node)))) {
        if ((modulekind === ModuleKind.ES2015)) {
          grammarErrorOnNode(
            node,
            Diagnostics.Export_assignment_cannot_be_used_when_targeting_ECMAScript_2015_modules_Consider_using_export_default_or_another_module_format_instead)
    ...
    def checkExternalModuleExports(node: (SourceFile | ModuleDeclaration)) = {
      ...
      if ((!links.exportsChecked)) {
        ...
        (exports).keys.foreach { fresh140 =>
          val id = zeroOfMyType = fresh140 {
            ...
            val exportedDeclarationsCount =
              countWhere(declarations, isNotOverload)
            ...
            if ((exportedDeclarationsCount > 1)) {
              (declarations).foreach { fresh142 =>
                val declaration = zeroOfMyType = fresh142 {
      ...
      def isNotOverload(declaration: Declaration): Boolean = {
        return ((((declaration.kind !== SyntaxKind.FunctionDeclaration) && (declaration.kind !== SyntaxKind.MethodDeclaration))) || (!(!(declaration
          .asInstanceOf[FunctionDeclaration])
          .body)))
    ...
    def checkSourceElement(node: Node): Unit = {
      ...
      kind match {
        ...
        case SyntaxKind.PropertyDeclaration | SyntaxKind.PropertySignature =>
          return checkPropertyDeclaration(
            node.asInstanceOf[PropertyDeclaration])
        case SyntaxKind.FunctionType | SyntaxKind.ConstructorType |
            SyntaxKind.CallSignature | SyntaxKind.ConstructSignature =>
          return checkSignatureDeclaration(
            node.asInstanceOf[SignatureDeclaration])
        ...
        case SyntaxKind.ContinueStatement | SyntaxKind.BreakStatement =>
          return checkBreakOrContinueStatement(
        ...
        case SyntaxKind.ImportEqualsDeclaration =>
          return checkImportEqualsDeclaration(
            node.asInstanceOf[ImportEqualsDeclaration])
    ...
    def checkDeferredNodes() = {
      (deferredNodes).foreach { fresh143 =>
        val node = zeroOfMyType = fresh143 {
          node.kind match {
            case SyntaxKind.FunctionExpression | SyntaxKind.ArrowFunction |
                SyntaxKind.MethodDeclaration | SyntaxKind.MethodSignature =>
    ...
    def checkSourceFileWorker(node: SourceFile) = {
      ...
      if ((!((links.flags & NodeCheckFlags.TypeChecked)))) {
        ...
        (deferredUnusedIdentifierNodes =
          (if ((produceDiagnostics && noUnusedIdentifiers)) Array()
           else undefined))
        forEach(node.statements, checkSourceElement)
        checkDeferredNodes()
    ...
    def getDiagnostics(sourceFile: SourceFile,
                       ct: CancellationToken): Array[Diagnostic] = {
      try {
    ...
    def isInsideWithStatementBody(node: Node): Boolean = {
      if (node) {
        while (node.parent) {
          {
            if (((node.parent.kind === SyntaxKind.WithStatement) && ((node.parent
    ...
    def getSymbolsInScope(location: Node,
                          meaning: SymbolFlags): Array[Symbol] = {
    ...
    def isTypeDeclaration(node: Node): Boolean = {
      node.kind match {
        case SyntaxKind.TypeParameter | SyntaxKind.ClassDeclaration |
    ...
    def getLeftSideOfImportEqualsOrExportAssignment(
      ...
      if ((nodeOnRightSide.parent.kind === SyntaxKind.ImportEqualsDeclaration)) {
        return (((nodeOnRightSide.parent
          .asInstanceOf[ImportEqualsDeclaration])
          .moduleReference === nodeOnRightSide) && nodeOnRightSide.parent
          .asInstanceOf[ImportEqualsDeclaration])
      ...
      if ((nodeOnRightSide.parent.kind === SyntaxKind.ExportAssignment)) {
        return (((nodeOnRightSide.parent
          ...
          .expression === nodeOnRightSide
          .asInstanceOf[Node]) && nodeOnRightSide.parent
          .asInstanceOf[ExportAssignment])
    ...
    def getSymbolOfEntityNameOrPropertyAccessExpression(
      ...
      if ((isInJavaScriptFile(entityName) && (entityName.parent.kind === SyntaxKind.PropertyAccessExpression))) {
        val specialPropertyAssignmentKind = getSpecialPropertyAssignmentKind(
          entityName.parent.parent)
        specialPropertyAssignmentKind match {
          case SpecialPropertyAssignmentKind.ExportsProperty |
              SpecialPropertyAssignmentKind.PrototypeProperty =>
            return getSymbolOfNode(entityName.parent)
          case SpecialPropertyAssignmentKind.ThisProperty |
              SpecialPropertyAssignmentKind.ModuleExports =>
            return getSymbolOfNode(entityName.parent.parent)
          case _ =>
        }

      }
      if (((entityName.parent.kind === SyntaxKind.ExportAssignment) && isEntityNameExpression(
            entityName
              .asInstanceOf[(Identifier | PropertyAccessExpression)]))) {
        return resolveEntityName(
          entityName.asInstanceOf[EntityNameExpression],
          (((SymbolFlags.Value | SymbolFlags.Type) | SymbolFlags.Namespace) | SymbolFlags.Alias))

      }
      if (((entityName.kind !== SyntaxKind.PropertyAccessExpression) && isInRightSideOfImportOrExportAssignment(
            entityName.asInstanceOf[EntityName]))) {
        val importEqualsDeclaration =
          getAncestor(entityName, SyntaxKind.ImportEqualsDeclaration)
            .asInstanceOf[ImportEqualsDeclaration]
      ...
      if (isRightSideOfQualifiedNameOrPropertyAccess(entityName)) {
        (entityName = entityName.parent
          .asInstanceOf[(QualifiedName | PropertyAccessEntityNameExpression)])
      ...
      if (isHeritageClauseElementIdentifier(
        ...
        if ((entityName.kind === SyntaxKind.Identifier)) {
          if ((isJSXTagName(entityName) && isJsxIntrinsicIdentifier(
                entityName.asInstanceOf[Identifier]))) {
            return getIntrinsicTagSymbol(
              entityName.parent.asInstanceOf[JsxOpeningLikeElement])
          ...
          if ((!symbol)) {
            checkPropertyAccessExpression(
              entityName.asInstanceOf[PropertyAccessExpression])
          ...
          return getNodeLinks(entityName).resolvedSymbol
                   ...
                   entityName.asInstanceOf[EntityName])) {
        val meaning =
          (if ((((entityName.parent.kind === SyntaxKind.TypeReference) || (entityName.parent.kind === SyntaxKind.JSDocTypeReference))))
             SymbolFlags.Type
           else SymbolFlags.Namespace)
        return resolveEntityName(
          entityName.asInstanceOf[EntityName],
        ...
        return getJsxAttributePropertySymbol(
          entityName.parent.asInstanceOf[JsxAttribute])
    ...
    def getSymbolAtLocation(node: Node) = {
      if ((node.kind === SyntaxKind.SourceFile)) {
        return (if (isExternalModule(node.asInstanceOf[SourceFile]))
      ...
      if ((node.kind === SyntaxKind.Identifier)) {
        ...
        } else if ((((node.parent.kind === SyntaxKind.BindingElement) && (node.parent.parent.kind === SyntaxKind.ObjectBindingPattern)) && (node === (node.parent
                     .asInstanceOf[BindingElement])
                     .propertyName))) {
          val typeOfPattern = getTypeOfNode(node.parent.parent)
          val propertyDeclaration = (typeOfPattern && getPropertyOfType(
              typeOfPattern,
              (node.asInstanceOf[Identifier]).text))
      ...
      node.kind match {
        case SyntaxKind.Identifier | SyntaxKind.PropertyAccessExpression |
          ...
          return getSymbolOfEntityNameOrPropertyAccessExpression(
            node.asInstanceOf[(EntityName | PropertyAccessExpression)])
        case SyntaxKind.ThisKeyword =>
          val container = getThisContainer(node, false)
        ...
        case SyntaxKind.SuperKeyword =>
          val `type` =
            (if (isPartOfExpression(node))
               checkExpression(node.asInstanceOf[Expression])
             else getTypeFromTypeNode(node.asInstanceOf[TypeNode]))
          return `type`.symbol
        ...
        case SyntaxKind.ConstructorKeyword =>
          ...
          return undefined
        case SyntaxKind.StringLiteral =>
          if ((((isExternalModuleImportEqualsDeclaration(node.parent.parent) && (getExternalModuleImportEqualsDeclarationExpression(
                node.parent.parent) === node))) || (((((node.parent.kind === SyntaxKind.ImportDeclaration) || (node.parent.kind === SyntaxKind.ExportDeclaration))) && ((node.parent
                .asInstanceOf[ImportDeclaration])
                .moduleSpecifier === node))))) {
            return resolveExternalModuleName(
              node,
              node.asInstanceOf[LiteralExpression])
        ...
        case SyntaxKind.NumericLiteral =>
          if (((node.parent.kind === SyntaxKind.ElementAccessExpression) && ((node.parent
                .asInstanceOf[ElementAccessExpression])
                .argumentExpression === node))) {
            val objectType = checkExpression(
              (node.parent.asInstanceOf[ElementAccessExpression]).expression)
            ...
            return getPropertyOfType(
              apparentType,
              (node.asInstanceOf[NumericLiteral]).text)
    ...
    def getShorthandAssignmentValueSymbol(location: Node): Symbol = {
      if ((location && (location.kind === SyntaxKind.ShorthandPropertyAssignment))) {
        return resolveEntityName(
          (location.asInstanceOf[ShorthandPropertyAssignment]).name,
          (SymbolFlags.Value | SymbolFlags.Alias))
    ...
    def getTypeOfNode(node: Node): Type = {
      ...
      if (isExpressionWithTypeArgumentsInClassExtendsClause(node)) {
        return getBaseTypes(
          getDeclaredTypeOfSymbol(getSymbolOfNode(node.parent.parent))
            .asInstanceOf[InterfaceType])(0)
      ...
      if (isBindingPattern(node)) {
        return getTypeForVariableLikeDeclaration(
          node.parent.asInstanceOf[VariableLikeDeclaration],
          true)
      ...
      if (isInRightSideOfImportOrExportAssignment(
        ...
        return (if ((declaredType !== unknownType)) declaredType
    ...
    def getTypeOfArrayLiteralOrObjectLiteralDestructuringAssignment(
      ...
      if ((expr.parent.kind === SyntaxKind.ForOfStatement)) {
        val iteratedType = checkRightHandSideOfForOf(
          (expr.parent.asInstanceOf[ForOfStatement]).expression)
        return checkDestructuringAssignment(
          expr,
          (iteratedType || unknownType))

      }
      if ((expr.parent.kind === SyntaxKind.BinaryExpression)) {
        val iteratedType = checkExpression(
          (expr.parent.asInstanceOf[BinaryExpression]).right)
        return checkDestructuringAssignment(
          expr,
          (iteratedType || unknownType))
      ...
      if ((expr.parent.kind === SyntaxKind.PropertyAssignment)) {
        val typeOfParentObjectLiteral =
          getTypeOfArrayLiteralOrObjectLiteralDestructuringAssignment(
            expr.parent.parent.asInstanceOf[Expression])
        return checkObjectLiteralDestructuringPropertyAssignment(
          (typeOfParentObjectLiteral || unknownType),
          expr.parent.asInstanceOf[ObjectLiteralElementLike])

      }
      Debug.assert((expr.parent.kind === SyntaxKind.ArrayLiteralExpression))
      val typeOfArrayLiteral =
        getTypeOfArrayLiteralOrObjectLiteralDestructuringAssignment(
          expr.parent.asInstanceOf[Expression])
      val elementType = (checkIteratedTypeOrElementType(
          (typeOfArrayLiteral || unknownType),
          expr.parent,
          false) || unknownType)
      return checkArrayLiteralDestructuringElementAssignment(
        expr.parent.asInstanceOf[ArrayLiteralExpression],
        typeOfArrayLiteral,
        indexOf(
          (expr.parent.asInstanceOf[ArrayLiteralExpression]).elements,
          expr),
        (elementType || unknownType))

    }
    def getPropertySymbolOfDestructuringAssignment(location: Identifier) = {
      val typeOfObjectLiteral =
        getTypeOfArrayLiteralOrObjectLiteralDestructuringAssignment(
          location.parent.parent.asInstanceOf[Expression])
    ...
    def getParentTypeOfClassElement(node: ClassElement) = {
      val classSymbol = getSymbolOfNode(node.parent)
      return (if ((getModifierFlags(node) & ModifierFlags.Static))
                getTypeOfSymbol(classSymbol)
              else getDeclaredTypeOfSymbol(classSymbol))

    }
    def getAugmentedPropertiesOfType(`type`: Type): Array[Symbol] = {
      (`type` = getApparentType(`type`))
      val propsByName = createSymbolTable(getPropertiesOfType(`type`))
      if ((getSignaturesOfType(`type`, SignatureKind.Call).length || getSignaturesOfType(
            `type`,
            SignatureKind.Construct).length)) {
        forEach(
          getPropertiesOfType(globalFunctionType),
          (p => {
    ...
    def moduleExportsSomeValue(
      ...
      if ((symbolLinks.exportsSomeValue === undefined)) {
        (symbolLinks.exportsSomeValue =
          (if (hasExportAssignment)
             (!(!((moduleSymbol.flags & SymbolFlags.Value))))
           else forEachProperty(getExportsOfModule(moduleSymbol), isValue)))
    ...
    def getReferencedExportContainer(node: Identifier, prefixLocals: Boolean)
      ...
      if (node) {
        var symbol =
          getReferencedValueSymbol(node, isNameOfModuleOrEnumDeclaration(node))
        if (symbol) {
          ...
          if (parentSymbol) {
            if (((parentSymbol.flags & SymbolFlags.ValueModule) && (parentSymbol.valueDeclaration.kind === SyntaxKind.SourceFile))) {
              val symbolFile =
                parentSymbol.valueDeclaration.asInstanceOf[SourceFile]
              ...
              while (n) {
                {
                  if ((isModuleOrEnumDeclaration(n) && (getSymbolOfNode(n) === parentSymbol))) {
                    return n
                      .asInstanceOf[(ModuleDeclaration | EnumDeclaration)]
    ...
    def isSymbolOfDeclarationWithCollidingName(symbol: Symbol): Boolean = {
      if ((symbol.flags & SymbolFlags.BlockScoped)) {
        ...
        if ((links.isDeclarationWithCollidingName === undefined)) {
          val container = getEnclosingBlockScopeContainer(
            symbol.valueDeclaration)
          if (isStatementWithLocals(container)) {
            ...
            if ((!(!resolveName(
              ...
              val inLoopBodyBlock = ((container.kind === SyntaxKind.Block) && isIterationStatement(
                  container.parent,
                  false))
    ...
    def isValueAliasDeclaration(node: Node): Boolean = {
      ...
      node.kind match {
        case SyntaxKind.ImportEqualsDeclaration | SyntaxKind.ImportClause |
        ...
        case SyntaxKind.ExportDeclaration =>
          val exportClause =
            (node.asInstanceOf[ExportDeclaration]).exportClause
          return (exportClause && forEach(
    ...
    def isTopLevelValueImportEqualsWithEntityName(
      ...
      if ((((node === undefined) || (node.parent.kind !== SyntaxKind.SourceFile)) || (!isInternalModuleImportEqualsDeclaration(
            node)))) {
        return false
    ...
    def isAliasResolvedToValue(symbol: Symbol): Boolean = {
      ...
      return ((target.flags & SymbolFlags.Value) && ((compilerOptions.preserveConstEnums || (!isConstEnumOrConstEnumOnlyModule(
        target)))))
    ...
    def isImplementationOfOverload(node: FunctionLikeDeclaration) = {
      if (nodeIsPresent(node.body)) {
        ...
        return ((signaturesOfSymbol.length > 1) || (((signaturesOfSymbol.length === 1) && (signaturesOfSymbol(
          0).declaration !== node))))
    ...
    def getTypeReferenceSerializationKind(
      ...
      val valueSymbol =
        resolveEntityName(typeName, SymbolFlags.Value, true, false, location)
      ...
      val constructorType =
        (if (valueSymbol) getTypeOfSymbol(valueSymbol) else undefined)
      if ((constructorType && isConstructorType(constructorType))) {
        return TypeReferenceSerializationKind.TypeWithConstructSignatureAndValue

      }
      val typeSymbol =
        resolveEntityName(typeName, SymbolFlags.Type, true, false, location)
      if ((!typeSymbol)) {
    ...
    def writeReturnTypeOfSignatureDeclaration(
        ...
        writer: SymbolWriter) = {
      val signature = getSignatureFromDeclaration(signatureDeclaration)
      getSymbolDisplayBuilder().buildTypeDisplay(
        getReturnTypeOfSignature(signature),
        writer,
        enclosingDeclaration,
        flags)
    ...
    def writeBaseConstructorTypeOfClass(node: ClassLikeDeclaration,
                                        ...
                                        flags: TypeFormatFlags,
                                        writer: SymbolWriter) = {
      val classType = getDeclaredTypeOfSymbol(getSymbolOfNode(node))
        .asInstanceOf[InterfaceType]
      resolveBaseTypesOfClass(classType)
      val baseType =
        (if (classType.resolvedBaseTypes.length) classType.resolvedBaseTypes(0)
         else unknownType)
      getSymbolDisplayBuilder()
        .buildTypeDisplay(baseType, writer, enclosingDeclaration, flags)
    ...
    def getReferencedValueSymbol(
        reference: Identifier,
        startInDeclarationContainer: Boolean): Symbol = {
    ...
    def createResolver(): EmitResolver = {
      val resolvedTypeReferenceDirectives =
        host.getResolvedTypeReferenceDirectives()
      ...
      def getTypeReferenceDirectivesForEntityName(
          node: EntityNameOrEntityNameExpression): Array[String] = {
        if ((!fileToDirective)) {
          return undefined

        }
        val meaning =
          (if ((((node.kind === SyntaxKind.PropertyAccessExpression)) || (((node.kind === SyntaxKind.Identifier) && isInTypeQuery(
                 node))))) (SymbolFlags.Value | SymbolFlags.ExportValue)
           else (SymbolFlags.Type | SymbolFlags.Namespace))
        val symbol = resolveEntityName(node, meaning, true)
        return (if ((symbol && (symbol !== unknownSymbol)))
      ...
      def isSymbolFromTypeDeclarationFile(symbol: Symbol): Boolean = {
        ...
        (symbol.declarations).foreach { fresh147 =>
          val decl = zeroOfMyType = fresh147 {
            val file = getSourceFileOfNode(decl)
    ...
    def getExternalModuleFileFromDeclaration(
      ...
      val moduleSymbol =
        resolveExternalModuleNameWorker(specifier, specifier, undefined)
      if ((!moduleSymbol)) {
        return undefined
    ...
    def initializeTypeChecker() = {
      (host.getSourceFiles()).foreach { fresh148 =>
        val file = zeroOfMyType = fresh148 {
      ...
      (host.getSourceFiles()).foreach { fresh149 =>
        val file = zeroOfMyType = fresh149 {
          ...
          if ((file.patternAmbientModules && file.patternAmbientModules.length)) {
            (patternAmbientModules =
              concatenate(patternAmbientModules, file.patternAmbientModules))

          }
          if (file.moduleAugmentations.length) {
            ((augmentations || ((augmentations = Array()))))
      ...
      if (augmentations) {
        (augmentations).foreach { fresh151 =>
          val list = zeroOfMyType = fresh151 {
            (list).foreach { fresh152 =>
              val augmentation = zeroOfMyType = fresh152 {
      ...
      (getGlobalClassDecoratorType = memoize(
        (() => getGlobalType("ClassDecorator"))))
      (getGlobalPropertyDecoratorType = memoize(
        (() => getGlobalType("PropertyDecorator"))))
      (getGlobalMethodDecoratorType = memoize(
        (() => getGlobalType("MethodDecorator"))))
      (getGlobalParameterDecoratorType = memoize(
        (() => getGlobalType("ParameterDecorator"))))
      (getGlobalTypedPropertyDescriptorType = memoize(
        (() => getGlobalType("TypedPropertyDescriptor", 1))))
      (getGlobalESSymbolConstructorSymbol = memoize(
        (() => getGlobalValueSymbol("Symbol"))))
      (getGlobalPromiseType = memoize((() => getGlobalType("Promise", 1))))
      (tryGetGlobalPromiseType = memoize((() =>
                                            (getGlobalSymbol(
                                              "Promise",
                                              SymbolFlags.Type,
                                              undefined) && getGlobalPromiseType()))))
      (getGlobalPromiseLikeType = memoize(
        (() => getGlobalType("PromiseLike", 1))))
      (getInstantiatedGlobalPromiseLikeType = memoize(
        createInstantiatedPromiseLikeType))
      (getGlobalPromiseConstructorSymbol = memoize(
        (() => getGlobalValueSymbol("Promise"))))
      (tryGetGlobalPromiseConstructorSymbol = memoize(
        (() =>
           (getGlobalSymbol("Promise", SymbolFlags.Value, undefined) && getGlobalPromiseConstructorSymbol()))))
      (getGlobalPromiseConstructorLikeType = memoize(
        (() => getGlobalType("PromiseConstructorLike"))))
      (getGlobalThenableType = memoize(createThenableType))
      (getGlobalTemplateStringsArrayType = memoize(
        (() => getGlobalType("TemplateStringsArray"))))
      if ((languageVersion >= ScriptTarget.ES2015)) {
        (getGlobalESSymbolType = memoize((() => getGlobalType("Symbol"))))
        (getGlobalIterableType = memoize(
          (() => getGlobalType("Iterable", 1).asInstanceOf[GenericType])))
        (getGlobalIteratorType = memoize(
          (() => getGlobalType("Iterator", 1).asInstanceOf[GenericType])))
        (getGlobalIterableIteratorType = memoize(
          (() =>
             getGlobalType("IterableIterator", 1).asInstanceOf[GenericType])))
      ...
      val symbol =
        getGlobalSymbol("ReadonlyArray", SymbolFlags.Type, undefined)
      ...
      if ((compilerOptions.importHelpers && firstFileRequestingExternalHelpers)) {
        val helpersModule = resolveExternalModule(
    ...
    def verifyHelperSymbol(symbols: SymbolTable,
                           name: String,
                           meaning: SymbolFlags) = {
    ...
    def createInstantiatedPromiseLikeType(): ObjectType = {
      ...
      if ((promiseLikeType !== emptyGenericType)) {
        return createTypeReference(
          promiseLikeType.asInstanceOf[GenericType],
          Array(anyType))
    ...
    def createThenableType() = {
      val thenPropertySymbol =
        createSymbol((SymbolFlags.Transient | SymbolFlags.Property), "then")
      (getSymbolLinks(thenPropertySymbol).`type` = globalFunctionType)
      val thenableType = createObjectType(ObjectFlags.Anonymous)
        .asInstanceOf[ResolvedType]
    ...
    def checkGrammarDecorators(node: Node): Boolean = {
      ...
      if ((!nodeCanBeDecorated(node))) {
        if (((node.kind === SyntaxKind.MethodDeclaration) && (!ts
              .nodeIsPresent((node.asInstanceOf[MethodDeclaration]).body)))) {
          return grammarErrorOnFirstToken(
            node,
    ...
    def checkGrammarModifiers(node: Node): Boolean = {
      ...
      (node.modifiers).foreach { fresh153 =>
        val modifier = zeroOfMyType = fresh153 {
          ...
          modifier.kind match {
            ...
            case SyntaxKind.PublicKeyword | SyntaxKind.ProtectedKeyword |
            ...
            case SyntaxKind.AbstractKeyword =>
              ...
              if ((node.kind !== SyntaxKind.ClassDeclaration)) {
                ...
                if ((!(((node.parent.kind === SyntaxKind.ClassDeclaration) && (getModifierFlags(
                      node.parent) & ModifierFlags.Abstract))))) {
                  return grammarErrorOnNode(
                    modifier,
            ...
            case SyntaxKind.AsyncKeyword =>
              ...
              } else if (((flags & ModifierFlags.Ambient) || isInAmbientContext(
                           node.parent))) {
                return grammarErrorOnNode(
                  modifier,
                  Diagnostics._0_modifier_cannot_be_used_in_an_ambient_context,
      ...
      } else if ((((node.kind === SyntaxKind.Parameter) && ((flags & ModifierFlags.ParameterPropertyModifier))) && isBindingPattern(
                   (node.asInstanceOf[ParameterDeclaration]).name))) {
        return grammarErrorOnNode(
          node,
          Diagnostics.A_parameter_property_may_not_be_declared_using_a_binding_pattern)

      } else if ((((node.kind === SyntaxKind.Parameter) && ((flags & ModifierFlags.ParameterPropertyModifier))) && (node
                   .asInstanceOf[ParameterDeclaration])
                   .dotDotDotToken)) {
        return grammarErrorOnNode(
          node,
    ...
    def reportObviousModifierErrors(node: Node): (Boolean | undefined) = {
      return (if ((!node.modifiers)) false
              else
                (if (shouldReportBadModifier(node))
                   grammarErrorOnFirstToken(
                     node,
                     Diagnostics.Modifiers_cannot_appear_here)
                 else undefined))
    ...
    def shouldReportBadModifier(node: Node): Boolean = {
      node.kind match {
        case SyntaxKind.GetAccessor | SyntaxKind.SetAccessor |
            ...
            SyntaxKind.PropertySignature | SyntaxKind.MethodDeclaration |
            SyntaxKind.MethodSignature | SyntaxKind.IndexSignature |
            SyntaxKind.ModuleDeclaration | SyntaxKind.ImportDeclaration |
            SyntaxKind.ImportEqualsDeclaration | SyntaxKind.ExportDeclaration |
            SyntaxKind.ExportAssignment | SyntaxKind.FunctionExpression |
            SyntaxKind.ArrowFunction | SyntaxKind.Parameter =>
          return false
        case _ =>
    ...
    def nodeHasAnyModifiersExcept(node: Node,
                                  allowedModifier: SyntaxKind): Boolean = {
      return ((node.modifiers.length > 1) || (node
        .modifiers(0)
        .kind !== allowedModifier))

    }
    def checkGrammarAsyncModifier(node: Node, asyncModifier: Node): Boolean = {
      node.kind match {
        case SyntaxKind.MethodDeclaration | SyntaxKind.FunctionDeclaration |
            SyntaxKind.FunctionExpression | SyntaxKind.ArrowFunction =>
    ...
    def checkGrammarForDisallowedTrailingComma(
        list: NodeArray[Node]): Boolean = {
    ...
    def checkGrammarTypeParameterList(
    ...
    def checkGrammarFunctionLikeDeclaration(
        node: FunctionLikeDeclaration): Boolean = {
      val file = getSourceFileOfNode(node)
      return ((((checkGrammarDecorators(node) || checkGrammarModifiers(node)) || checkGrammarTypeParameterList(
        node.typeParameters,
        file)) || checkGrammarParameterList(node.parameters)) || checkGrammarArrowFunction(
        node,
        file))

    }
    def checkGrammarArrowFunction(node: FunctionLikeDeclaration,
                                  file: SourceFile): Boolean = {
      if ((node.kind === SyntaxKind.ArrowFunction)) {
        val arrowFunction = node.asInstanceOf[ArrowFunction]
        val startLine = getLineAndCharacterOfPosition(
          file,
          arrowFunction.equalsGreaterThanToken.pos).line
        val endLine = getLineAndCharacterOfPosition(
          file,
          arrowFunction.equalsGreaterThanToken.end).line
    ...
    def checkGrammarIndexSignatureParameters(
        node: SignatureDeclaration): Boolean = {
      val parameter = node.parameters(0)
      if ((node.parameters.length !== 1)) {
        if (parameter) {
          return grammarErrorOnNode(
    ...
    def checkGrammarIndexSignature(node: SignatureDeclaration) = {
      return ((checkGrammarDecorators(node) || checkGrammarModifiers(node)) || checkGrammarIndexSignatureParameters(
        node))

    }
    def checkGrammarForAtLeastOneTypeArgument(
        node: Node,
        typeArguments: NodeArray[TypeNode]): Boolean = {
    ...
    def checkGrammarForOmittedArgument(
        node: (CallExpression | NewExpression),
        args: NodeArray[Expression]): Boolean = {
      if (args) {
        val sourceFile = getSourceFileOfNode(node)
        (args).foreach { fresh154 =>
          val arg = zeroOfMyType = fresh154 {
    ...
    def checkGrammarArguments(node: (CallExpression | NewExpression),
                              args: NodeArray[Expression]): Boolean = {
    ...
    def checkGrammarClassDeclarationHeritageClauses(
        node: ClassLikeDeclaration) = {
      var seenExtendsClause = false
      var seenImplementsClause = false
      if ((((!checkGrammarDecorators(node)) && (!checkGrammarModifiers(node))) && node.heritageClauses)) {
        (node.heritageClauses).foreach { fresh155 =>
          val heritageClause = zeroOfMyType = fresh155 {
    ...
    def checkGrammarInterfaceDeclaration(node: InterfaceDeclaration) = {
      ...
      if (node.heritageClauses) {
        (node.heritageClauses).foreach { fresh156 =>
          val heritageClause = zeroOfMyType = fresh156 {
            if ((heritageClause.token === SyntaxKind.ExtendsKeyword)) {
              if (seenExtendsClause) {
                return grammarErrorOnFirstToken(
                  heritageClause,
    ...
    def checkGrammarComputedPropertyName(node: Node): Boolean = {
      ...
      if (((computedPropertyName.expression.kind === SyntaxKind.BinaryExpression) && ((computedPropertyName.expression
            .asInstanceOf[BinaryExpression])
            .operatorToken
    ...
    def checkGrammarForGenerator(node: FunctionLikeDeclaration) = {
      if (node.asteriskToken) {
        ...
        if ((languageVersion < ScriptTarget.ES2015)) {
          return grammarErrorOnNode(
            node.asteriskToken,
            Diagnostics.Generators_are_only_available_when_targeting_ECMAScript_2015_or_higher)
    ...
    def checkGrammarForInvalidQuestionMark(
        questionToken: Node,
    ...
    def checkGrammarObjectLiteralExpression(node: ObjectLiteralExpression,
      ...
      (node.properties).foreach { fresh157 =>
        val prop = zeroOfMyType = fresh157 {
          val name = prop.name
          if ((name.kind === SyntaxKind.ComputedPropertyName)) {
            checkGrammarComputedPropertyName(
              name.asInstanceOf[ComputedPropertyName])

          }
          if ((((prop.kind === SyntaxKind.ShorthandPropertyAssignment) && (!inDestructuring)) && (prop
                .asInstanceOf[ShorthandPropertyAssignment])
                .objectAssignmentInitializer)) {
            return grammarErrorOnNode(
              (prop.asInstanceOf[ShorthandPropertyAssignment]).equalsToken,
              Diagnostics.can_only_be_used_in_an_object_literal_property_inside_a_destructuring_assignment)

          }
          if (prop.modifiers) {
            (prop.modifiers).foreach { fresh158 =>
              val mod = zeroOfMyType = fresh158 {
          ...
          if (((prop.kind === SyntaxKind.PropertyAssignment) || (prop.kind === SyntaxKind.ShorthandPropertyAssignment))) {
            checkGrammarForInvalidQuestionMark(
              (prop.asInstanceOf[PropertyAssignment]).questionToken,
            ...
            Debug.fail(
              ("Unexpected syntax kind:" + (prop.asInstanceOf[Node]).kind))
    ...
    def checkGrammarJsxElement(node: JsxOpeningLikeElement) = {
      ...
      (node.attributes).foreach { fresh159 =>
        val attr = zeroOfMyType = fresh159 {
          ...
          if (((initializer && (initializer.kind === SyntaxKind.JsxExpression)) && (!(initializer
    ...
    def checkGrammarForInOrForOfStatement(
        forInOrOfStatement: (ForInStatement | ForOfStatement)): Boolean = {
      ...
      if ((forInOrOfStatement.initializer.kind === SyntaxKind.VariableDeclarationList)) {
        val variableList =
          forInOrOfStatement.initializer.asInstanceOf[VariableDeclarationList]
        if ((!checkGrammarVariableDeclarationList(variableList))) {
          ...
          if ((declarations.length > 1)) {
            val diagnostic =
              (if ((forInOrOfStatement.kind === SyntaxKind.ForInStatement))
    ...
    def checkGrammarAccessor(accessor: AccessorDeclaration): Boolean = {
      ...
      if ((languageVersion < ScriptTarget.ES5)) {
        ...
        return grammarErrorOnNode(
          accessor.name,
          Diagnostics.An_accessor_cannot_be_declared_in_an_ambient_context)

      } else if (((accessor.body === undefined) && (!((getModifierFlags(
                   accessor) & ModifierFlags.Abstract))))) {
        return grammarErrorAtPos(
          getSourceFileOfNode(accessor),
          (accessor.end - 1),
          ";".length,
        ...
        return grammarErrorOnNode(
          ...
          (if ((kind === SyntaxKind.GetAccessor))
           ...
           else Diagnostics.A_set_accessor_must_have_exactly_one_parameter))
    ...
    def doesAccessorHaveCorrectParameterCount(accessor: AccessorDeclaration) = {
      return (getAccessorThisParameter(accessor) || (accessor.parameters.length === ((if ((accessor.kind === SyntaxKind.GetAccessor))
                                                                                        0
                                                                                      else 1)
        )))

    }
    def getAccessorThisParameter(
        accessor: AccessorDeclaration): ParameterDeclaration = {
      if ((accessor.parameters.length === ((if ((accessor.kind === SyntaxKind.GetAccessor))
                                              1
                                            else 2) ))) {
    ...
    def checkGrammarForNonSymbolComputedProperty(
    ...
    def checkGrammarMethod(node: MethodDeclaration) = {
      if (((checkGrammarDisallowedModifiersOnObjectLiteralExpressionMethod(
            node) || checkGrammarFunctionLikeDeclaration(node)) || checkGrammarForGenerator(
            node))) {
        return true

      }
      if ((node.parent.kind === SyntaxKind.ObjectLiteralExpression)) {
        if (checkGrammarForInvalidQuestionMark(
              node.questionToken,
              Diagnostics.An_object_member_cannot_be_declared_optional)) {
          return true
    ...
    def checkGrammarBreakOrContinueStatement(
        node: BreakOrContinueStatement): Boolean = {
      var current: Node = node
      while (current) {
        {
          if (isFunctionLike(current)) {
          ...
          current.kind match {
            case SyntaxKind.LabeledStatement =>
              if ((node.label && ((current
                    ...
                    .label
                    .text === node.label.text))) {
                val isMisplacedContinueLabel = ((node.kind === SyntaxKind.ContinueStatement) && (!isIterationStatement(
                    (current.asInstanceOf[LabeledStatement]).statement,
                    true)))
    ...
    def isStringOrNumberLiteralExpression(expr: Expression) = {
      return (((expr.kind === SyntaxKind.StringLiteral) || (expr.kind === SyntaxKind.NumericLiteral)) || (((expr.kind === SyntaxKind.PrefixUnaryExpression) && ((expr
        .asInstanceOf[PrefixUnaryExpression])
        .operator === SyntaxKind.MinusToken)) && ((expr
        .asInstanceOf[PrefixUnaryExpression])
        .operand
    ...
    def checkGrammarVariableDeclaration(node: VariableDeclaration) = {
      if (((node.parent.parent.kind !== SyntaxKind.ForInStatement) && (node.parent.parent.kind !== SyntaxKind.ForOfStatement))) {
        if (isInAmbientContext(node)) {
          if (node.initializer) {
            if ((isConst(node) && (!node.`type`))) {
              if ((!isStringOrNumberLiteralExpression(node.initializer))) {
                return grammarErrorOnNode(
                  node.initializer,
                  Diagnostics.A_const_initializer_in_an_ambient_context_must_be_a_string_or_numeric_literal)
              ...
              return grammarErrorAtPos(
                getSourceFileOfNode(node),
                (node.initializer.pos - equalsTokenLength),
                equalsTokenLength,
          ...
          if ((isBindingPattern(node.name) && (!isBindingPattern(
                node.parent)))) {
            return grammarErrorOnNode(
              node,
    ...
    def checkGrammarNameInLetOrConstDeclarations(
        name: (Identifier | BindingPattern)): Boolean = {
      if ((name.kind === SyntaxKind.Identifier)) {
        if (((name
              .asInstanceOf[Identifier])
              .originalKeywordKind === SyntaxKind.LetKeyword)) {
          return grammarErrorOnNode(
            name,
        ...
        (elements).foreach { fresh160 =>
          val element = zeroOfMyType = fresh160 {
            if ((!isOmittedExpression(element))) {
    ...
    def checkGrammarVariableDeclarationList(
        declarationList: VariableDeclarationList): Boolean = {
      ...
      if ((!declarationList.declarations.length)) {
        return grammarErrorAtPos(
          getSourceFileOfNode(declarationList),
          declarations.pos,
    ...
    def allowLetAndConstDeclarations(parent: Node): Boolean = {
      parent.kind match {
        case SyntaxKind.IfStatement | SyntaxKind.DoStatement |
            SyntaxKind.WhileStatement | SyntaxKind.WithStatement |
            SyntaxKind.ForStatement | SyntaxKind.ForInStatement |
            SyntaxKind.ForOfStatement =>
          return false
        case SyntaxKind.LabeledStatement =>
    ...
    def grammarErrorOnFirstToken(node: Node,
                                 message: DiagnosticMessage,
                                 arg0: Any,
                                 arg1: Any,
                                 arg2: Any): Boolean = {
    ...
    def grammarErrorAtPos(sourceFile: SourceFile,
                          start: Int,
                          length: Int,
                          message: DiagnosticMessage,
                          arg0: Any,
    ...
    def grammarErrorOnNode(node: Node,
                           message: DiagnosticMessage,
                           arg0: Any,
                           arg1: Any,
                           arg2: Any): Boolean = {
    ...
    def checkGrammarProperty(node: PropertyDeclaration) = {
      if (isClassLike(node.parent)) {
        if (checkGrammarForNonSymbolComputedProperty(
              node.name,
              Diagnostics.A_computed_property_name_in_a_class_property_declaration_must_directly_refer_to_a_built_in_symbol)) {
          return true
    ...
    def checkGrammarTopLevelElementForRequiredDeclareModifier(
        node: Node): Boolean = {
      if (((((((((node.kind === SyntaxKind.InterfaceDeclaration) || (node.kind === SyntaxKind.TypeAliasDeclaration)) || (node.kind === SyntaxKind.ImportDeclaration)) || (node.kind === SyntaxKind.ImportEqualsDeclaration)) || (node.kind === SyntaxKind.ExportDeclaration)) || (node.kind === SyntaxKind.ExportAssignment)) || (node.kind === SyntaxKind.NamespaceExportDeclaration)) || (getModifierFlags(
            node) & (((ModifierFlags.Ambient | ModifierFlags.Export) | ModifierFlags.Default))))) {
        return false

      }
      return grammarErrorOnFirstToken(
        node,
        Diagnostics.A_declare_modifier_is_required_for_a_top_level_declaration_in_a_d_ts_file)

    }
    def checkGrammarTopLevelElementsForRequiredDeclareModifier(
        file: SourceFile): Boolean = {
      (file.statements).foreach { fresh161 =>
        val decl = zeroOfMyType = fresh161 {
    ...
    def checkGrammarSourceFile(node: SourceFile): Boolean = {
      return (isInAmbientContext(node) && checkGrammarTopLevelElementsForRequiredDeclareModifier(
        node))
    ...
    def checkGrammarStatementInAmbientContext(node: Node): Boolean = {
      if (isInAmbientContext(node)) {
        ...
        val links = getNodeLinks(node)
        if (((!links.hasReportedStatementInAmbientContext) && isFunctionLike(
              node.parent))) {
          return (getNodeLinks(node).hasReportedStatementInAmbientContext =
            grammarErrorOnFirstToken(
              node,
    ...
    def checkGrammarNumericLiteral(node: NumericLiteral): Boolean = {
      if ((node.isOctalLiteral && (languageVersion >= ScriptTarget.ES5))) {
        return grammarErrorOnNode(
          node,
          Diagnostics.Octal_literals_are_not_available_when_targeting_ECMAScript_5_and_higher)

      }

    }
    def grammarErrorAfterFirstToken(node: Node,
                                    message: DiagnosticMessage,
                                    arg0: Any,
                                    arg1: Any,
                                    arg2: Any): Boolean = {
