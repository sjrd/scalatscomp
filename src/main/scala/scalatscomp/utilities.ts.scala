...
object Utilities {
  ...
  def getDeclarationOfKind(symbol: Symbol, kind: SyntaxKind): Declaration = {
    ...
    if (declarations) {
      (declarations).foreach { fresh1 =>
        val declaration = zeroOfMyType = fresh1 {
  ...
  def hasResolvedModule(sourceFile: SourceFile,
                        moduleNameText: String): Boolean = {
    return (!(!(((sourceFile && sourceFile.resolvedModules) && sourceFile
      .resolvedModules(moduleNameText)))))

  }
  def getResolvedModule(sourceFile: SourceFile,
                        moduleNameText: String): ResolvedModule = {
    return (if (hasResolvedModule(sourceFile, moduleNameText))
              sourceFile.resolvedModules(moduleNameText)
            else undefined)

  }
  def setResolvedModule(sourceFile: SourceFile,
                        moduleNameText: String,
                        resolvedModule: ResolvedModule): Unit = {
  ...
  def setResolvedTypeReferenceDirective(
      sourceFile: SourceFile,
      typeReferenceDirectiveName: String,
      resolvedTypeReferenceDirective: ResolvedTypeReferenceDirective): Unit = {
    if ((!sourceFile.resolvedTypeReferenceDirectiveNames)) {
      (sourceFile.resolvedTypeReferenceDirectiveNames =
        createMap[ResolvedTypeReferenceDirective]())

    }
    (sourceFile.resolvedTypeReferenceDirectiveNames(typeReferenceDirectiveName) =
      resolvedTypeReferenceDirective)

  }
  def moduleResolutionIsEqualTo(oldResolution: ResolvedModule,
                                newResolution: ResolvedModule): Boolean = {
  ...
  def typeDirectiveIsEqualTo(
      oldResolution: ResolvedTypeReferenceDirective,
      newResolution: ResolvedTypeReferenceDirective): Boolean = {
    return ((oldResolution.resolvedFileName === newResolution.resolvedFileName) && (oldResolution.primary === newResolution.primary))

  }
  def hasChangesInResolutions[T](names: Array[String],
                                 newResolutions: Array[T],
                                 oldResolutions: Map[T],
                                 comparer: ((T, T) => Boolean)): Boolean = {
  ...
  def aggregateChildData(node: Node): Unit = {
    if ((!((node.flags & NodeFlags.HasAggregatedChildData)))) {
      val thisNodeOrAnySubNodesHasError = (((((node.flags & NodeFlags.ThisNodeHasError)) !== 0)) || forEachChild(
          node,
          containsParseError))
  ...
  def isStatementWithLocals(node: Node) = {
    node.kind match {
      case SyntaxKind.Block | SyntaxKind.CaseBlock | SyntaxKind.ForStatement |
  ...
  def getTokenPosOfNode(node: Node,
                        sourceFile: SourceFile,
                        includeJsDocComment: Boolean): Int = {
    ...
    if (((node.kind === SyntaxKind.SyntaxList) && ((node
          .asInstanceOf[SyntaxList])
          ._children
          .length > 0))) {
      return getTokenPosOfNode(
        (node.asInstanceOf[SyntaxList])._children(0),
        sourceFile,
        includeJsDocComment)
  ...
  def getSourceTextOfNodeFromSourceFile(
      sourceFile: SourceFile,
      node: Node,
      includeTrivia: Nothing = false): String = {
    if (nodeIsMissing(node)) {
      return ""
    ...
    return text.substring(
      (if (includeTrivia) node.pos else skipTrivia(text, node.pos)),
      node.end)
  ...
  def getLiteralText(node: LiteralLikeNode,
                     sourceFile: SourceFile,
                     languageVersion: ScriptTarget) = {
    if (((languageVersion < ScriptTarget.ES2015) && ((isTemplateLiteralKind(
          node.kind) || node.hasExtendedUnicodeEscape)))) {
      return getQuotedEscapedLiteralText("\"", node.text, "\"")

    }
    if (((!nodeIsSynthesized(node)) && node.parent)) {
      val text = getSourceTextOfNodeFromSourceFile(sourceFile, node)
      if (((languageVersion < ScriptTarget.ES2015) && isBinaryOrOctalIntegerLiteral(
            node,
            text))) {
        return node.text
  ...
  def isBinaryOrOctalIntegerLiteral(node: LiteralLikeNode, text: String) = {
    if (((node.kind === SyntaxKind.NumericLiteral) && (text.length > 1))) {
      text.charCodeAt(1) match {
        case CharacterCodes.b | CharacterCodes.B | CharacterCodes.o |
            CharacterCodes.O =>
          return true
        case _ =>
      }
  ...
  def escapeIdentifier(identifier: String): String = {
    return (if ((((identifier.length >= 2) && (identifier.charCodeAt(0) === CharacterCodes._underscore_)) && (identifier
                  .charCodeAt(1) === CharacterCodes._underscore_)))
              ("_" + identifier)
            else identifier)

  }
  def unescapeIdentifier(identifier: String): String = {
    return (if (((((identifier.length >= 3) && (identifier.charCodeAt(0) === CharacterCodes._underscore_)) && (identifier
                  .charCodeAt(1) === CharacterCodes._underscore_)) && (identifier
                  .charCodeAt(2) === CharacterCodes._underscore_)))
              identifier.substr(1)
            else identifier)
  ...
  def makeIdentifierFromModuleName(moduleName: String): String = {
    return getBaseFileName(moduleName)
      .replace(java.util.regex.Pattern.compile(raw"""^(\d)"""), "_$1")
      .replace(java.util.regex.Pattern.compile(raw"""\W""", "g"), "_")

  }
  def isBlockOrCatchScoped(declaration: Declaration) = {
    return ((((getCombinedNodeFlags(declaration) & NodeFlags.BlockScoped)) !== 0) || isCatchClauseVariableDeclarationOrBindingElement(
      declaration))

  }
  def isCatchClauseVariableDeclarationOrBindingElement(
  ...
  def isAmbientModule(node: Node): Boolean = {
    return ((node && (node.kind === SyntaxKind.ModuleDeclaration)) && ((((node
      .asInstanceOf[ModuleDeclaration])
      .name
      .kind === SyntaxKind.StringLiteral) || isGlobalScopeAugmentation(
  ...
  def isShorthandAmbientModule(node: Node): Boolean = {
    return ((node.kind === SyntaxKind.ModuleDeclaration) && ((!(node
      .asInstanceOf[ModuleDeclaration])
      .body)))

  }
  def isBlockScopedContainerTopLevel(node: Node): Boolean = {
    return (((node.kind === SyntaxKind.SourceFile) || (node.kind === SyntaxKind.ModuleDeclaration)) || isFunctionLike(
      node))
  ...
  def isExternalModuleAugmentation(node: Node): Boolean = {
    ...
    node.parent.kind match {
      ...
      case SyntaxKind.ModuleBlock =>
        return (isAmbientModule(node.parent.parent) && (!isExternalModule(
          node.parent.parent.parent.asInstanceOf[SourceFile])))
      case _ =>
    }
    return false
  ...
  def isBlockScope(node: Node, parentNode: Node) = {
    node.kind match {
      case SyntaxKind.SourceFile | SyntaxKind.CaseBlock |
          SyntaxKind.CatchClause | SyntaxKind.ModuleDeclaration |
          SyntaxKind.ForStatement | SyntaxKind.ForInStatement |
          SyntaxKind.ForOfStatement | SyntaxKind.Constructor |
  ...
  def declarationNameToString(name: DeclarationName) = {
    return (if ((getFullWidth(name) === 0)) "(Missing)"
            else getTextOfNode(name))

  }
  def createDiagnosticForNode(node: Node,
                              message: DiagnosticMessage,
                              arg0: (String | Int),
                              arg1: (String | Int),
                              arg2: (String | Int)): Diagnostic = {
  ...
  def createDiagnosticForNodeFromMessageChain(
      node: Node,
    ...
    return Map(
      ...
      "messageText" -> (if (messageChain.next) messageChain
                        else messageChain.messageText))
  ...
  def getErrorSpanForArrowFunction(sourceFile: SourceFile,
                                   node: ArrowFunction): TextSpan = {
  ...
  def getErrorSpanForNode(sourceFile: SourceFile, node: Node): TextSpan = {
    ...
    node.kind match {
      ...
      case SyntaxKind.VariableDeclaration | SyntaxKind.BindingElement |
          ...
          SyntaxKind.InterfaceDeclaration | SyntaxKind.ModuleDeclaration |
          SyntaxKind.EnumDeclaration | SyntaxKind.EnumMember |
          SyntaxKind.FunctionDeclaration | SyntaxKind.FunctionExpression |
          SyntaxKind.MethodDeclaration | SyntaxKind.GetAccessor |
          SyntaxKind.SetAccessor | SyntaxKind.TypeAliasDeclaration =>
    ...
    val pos =
      (if (nodeIsMissing(errorNode)) errorNode.pos
       else skipTrivia(sourceFile.text, errorNode.pos))
  ...
  def isConst(node: Node): Boolean = {
    return ((!(!((getCombinedNodeFlags(node) & NodeFlags.Const)))) || (!(!((getCombinedModifierFlags(
      node) & ModifierFlags.Const)))))
  ...
  def isSuperCall(n: Node): Boolean = {
    return ((n.kind === SyntaxKind.CallExpression) && ((n
  ...
  def isPrologueDirective(node: Node): Boolean = {
    return ((node.kind === SyntaxKind.ExpressionStatement) && ((node
  ...
  def getJsDocCommentsFromText(node: Node, text: String) = {
    val commentRanges =
      (if ((((((node.kind === SyntaxKind.Parameter) || (node.kind === SyntaxKind.TypeParameter)) || (node.kind === SyntaxKind.FunctionExpression)) || (node.kind === SyntaxKind.ArrowFunction))))
         concatenate(
           getTrailingCommentRanges(text, node.pos),
           getLeadingCommentRanges(text, node.pos))
       else getLeadingCommentRangesOfNodeFromText(node, text))
    return filter(commentRanges, isJsDocComment)
    def isJsDocComment(comment: CommentRange) = {
      return (((text.charCodeAt((comment.pos + 1)) === CharacterCodes.asterisk) && (text
        .charCodeAt((comment.pos + 2)) === CharacterCodes.asterisk)) && (text
        .charCodeAt((comment.pos + 3)) !== CharacterCodes.slash))

    }

  }
  var fullTripleSlashReferencePathRegEx = java.util.regex.Pattern
    .compile(raw"""^(\/\/\/\s*<reference\s+path\s*=\s*)('|")(.+?)\2.*?\/>""")
  var fullTripleSlashReferenceTypeReferenceDirectiveRegEx =
    java.util.regex.Pattern.compile(
      raw"""^(\/\/\/\s*<reference\s+types\s*=\s*)('|")(.+?)\2.*?\/>""")
  var fullTripleSlashAMDReferencePathRegEx = java.util.regex.Pattern.compile(
    raw"""^(\/\/\/\s*<amd-dependency\s+path\s*=\s*)('|")(.+?)\2.*?\/>""")
  def isPartOfTypeNode(node: Node): Boolean = {
    ...
    node.kind match {
      case SyntaxKind.AnyKeyword | SyntaxKind.NumberKeyword |
          SyntaxKind.StringKeyword | SyntaxKind.BooleanKeyword |
      ...
      case SyntaxKind.Identifier =>
        if (((node.parent.kind === SyntaxKind.QualifiedName) && ((node.parent
              .asInstanceOf[QualifiedName])
              .right === node))) {
          (node = node.parent)

        } else if (((node.parent.kind === SyntaxKind.PropertyAccessExpression) && ((node.parent
      ...
      case SyntaxKind.QualifiedName | SyntaxKind.PropertyAccessExpression |
        ...
        parent.kind match {
          ...
          case SyntaxKind.PropertyDeclaration | SyntaxKind.PropertySignature |
            ...
            return (node === (parent
              ...
              .`type`)
          case SyntaxKind.FunctionDeclaration | SyntaxKind.FunctionExpression |
              SyntaxKind.ArrowFunction | SyntaxKind.Constructor |
          ...
          case SyntaxKind.CallExpression | SyntaxKind.NewExpression =>
            return ((parent
              .asInstanceOf[CallExpression])
              .typeArguments && (indexOf(
              (parent.asInstanceOf[CallExpression]).typeArguments,
              node) >= 0))
  ...
  def forEachReturnStatement[T](body: Block,
                                visitor: ((ReturnStatement) => T)): T = {
    return traverse(body)
    def traverse(node: Node): T = {
      node.kind match {
        case SyntaxKind.ReturnStatement =>
          return visitor(node.asInstanceOf[ReturnStatement])
        case SyntaxKind.CaseBlock | SyntaxKind.Block | SyntaxKind.IfStatement |
            SyntaxKind.DoStatement | SyntaxKind.WhileStatement |
            SyntaxKind.ForStatement | SyntaxKind.ForInStatement |
            SyntaxKind.ForOfStatement | SyntaxKind.WithStatement |
            SyntaxKind.SwitchStatement | SyntaxKind.CaseClause |
            SyntaxKind.DefaultClause | SyntaxKind.LabeledStatement |
  ...
  def forEachYieldExpression(body: Block,
                             visitor: ((YieldExpression) => Unit)): Unit = {
    return traverse(body)
    def traverse(node: Node): Unit = {
      node.kind match {
        ...
        case SyntaxKind.EnumDeclaration | SyntaxKind.InterfaceDeclaration |
  ...
  def isVariableLike(node: Node): Boolean = {
    if (node) {
      node.kind match {
        case SyntaxKind.BindingElement | SyntaxKind.EnumMember |
  ...
  def isFunctionLikeKind(kind: SyntaxKind): Boolean = {
    kind match {
      case SyntaxKind.Constructor | SyntaxKind.FunctionExpression |
          ...
          SyntaxKind.IndexSignature | SyntaxKind.FunctionType |
  ...
  def isIterationStatement(node: Node,
                           lookInLabeledStatements: Boolean): Boolean = {
    node.kind match {
      case SyntaxKind.ForStatement | SyntaxKind.ForInStatement |
          SyntaxKind.ForOfStatement | SyntaxKind.DoStatement |
          SyntaxKind.WhileStatement =>
        return true
      case SyntaxKind.LabeledStatement =>
        return (lookInLabeledStatements && isIterationStatement(
    ...
    while (true) {
      {
        ...
        node.kind match {
          ...
          case SyntaxKind.Decorator =>
            if (((node.parent.kind === SyntaxKind.Parameter) && isClassElement(
                  node.parent.parent))) {
          ...
          case SyntaxKind.FunctionDeclaration | SyntaxKind.FunctionExpression |
              SyntaxKind.ModuleDeclaration | SyntaxKind.PropertyDeclaration |
              SyntaxKind.PropertySignature | SyntaxKind.MethodDeclaration |
              SyntaxKind.MethodSignature | SyntaxKind.Constructor |
              SyntaxKind.GetAccessor | SyntaxKind.SetAccessor |
              SyntaxKind.CallSignature | SyntaxKind.ConstructSignature |
              SyntaxKind.IndexSignature | SyntaxKind.EnumDeclaration |
  ...
  def getImmediatelyInvokedFunctionExpression(func: Node): CallExpression = {
    if (((func.kind === SyntaxKind.FunctionExpression) || (func.kind === SyntaxKind.ArrowFunction))) {
      ...
      if (((parent.kind === SyntaxKind.CallExpression) && ((parent
            .asInstanceOf[CallExpression])
            .expression === prev))) {
  ...
  def isSuperProperty(node: Node): Boolean = {
    val kind = node.kind
    return ((((kind === SyntaxKind.PropertyAccessExpression) || (kind === SyntaxKind.ElementAccessExpression))) && ((node
      .asInstanceOf[(PropertyAccessExpression | ElementAccessExpression)])
      .expression
      .kind === SyntaxKind.SuperKeyword))

  }
  def getEntityNameFromTypeNode(
      node: TypeNode): EntityNameOrEntityNameExpression = {
    if (node) {
      node.kind match {
  ...
  def isCallLikeExpression(node: Node): Boolean = {
    node.kind match {
      case SyntaxKind.CallExpression | SyntaxKind.NewExpression |
          SyntaxKind.TaggedTemplateExpression | SyntaxKind.Decorator =>
        return true
      case _ =>
        return false
    }
  ...
  def nodeCanBeDecorated(node: Node): Boolean = {
    node.kind match {
      ...
      case SyntaxKind.GetAccessor | SyntaxKind.SetAccessor |
        ...
        return (((node
          .asInstanceOf[FunctionLikeDeclaration])
          .body !== undefined) && (node.parent.kind === SyntaxKind.ClassDeclaration))
      case SyntaxKind.Parameter =>
        return ((((node.parent
          .asInstanceOf[FunctionLikeDeclaration])
          .body !== undefined) && ((((node.parent.kind === SyntaxKind.Constructor) || (node.parent.kind === SyntaxKind.MethodDeclaration)) || (node.parent.kind === SyntaxKind.SetAccessor)))) && (node.parent.parent.kind === SyntaxKind.ClassDeclaration))
      case _ =>
    }
    return false
  ...
  def childIsDecorated(node: Node): Boolean = {
    node.kind match {
      case SyntaxKind.ClassDeclaration =>
        return forEach(
          (node.asInstanceOf[ClassDeclaration]).members,
          nodeOrChildIsDecorated)
      case SyntaxKind.MethodDeclaration | SyntaxKind.SetAccessor =>
        return forEach(
          (node.asInstanceOf[FunctionLikeDeclaration]).parameters,
          nodeIsDecorated)
      case _ =>
    }
  ...
  def isPartOfExpression(node: Node): Boolean = {
    node.kind match {
      case SyntaxKind.ThisKeyword | SyntaxKind.SuperKeyword |
          ...
          SyntaxKind.FalseKeyword | SyntaxKind.RegularExpressionLiteral |
          SyntaxKind.ArrayLiteralExpression |
          SyntaxKind.ObjectLiteralExpression |
          SyntaxKind.PropertyAccessExpression |
          SyntaxKind.ElementAccessExpression | SyntaxKind.CallExpression |
          SyntaxKind.NewExpression | SyntaxKind.TaggedTemplateExpression |
          ...
          SyntaxKind.ArrowFunction | SyntaxKind.VoidExpression |
          SyntaxKind.DeleteExpression | SyntaxKind.TypeOfExpression |
          SyntaxKind.PrefixUnaryExpression |
          SyntaxKind.PostfixUnaryExpression | SyntaxKind.BinaryExpression |
          SyntaxKind.ConditionalExpression |
          SyntaxKind.SpreadElementExpression | SyntaxKind.TemplateExpression |
          SyntaxKind.NoSubstitutionTemplateLiteral |
          SyntaxKind.OmittedExpression | SyntaxKind.JsxElement |
          SyntaxKind.JsxSelfClosingElement | SyntaxKind.YieldExpression |
      ...
      case SyntaxKind.QualifiedName =>
        ...
        return ((node.parent.kind === SyntaxKind.TypeQuery) || isJSXTagName(
          node))
      case SyntaxKind.Identifier =>
        if (((node.parent.kind === SyntaxKind.TypeQuery) || isJSXTagName(
              node))) {
          return true
      ...
      case SyntaxKind.NumericLiteral | SyntaxKind.StringLiteral |
        ...
        parent.kind match {
          case SyntaxKind.VariableDeclaration | SyntaxKind.Parameter |
              SyntaxKind.PropertyDeclaration | SyntaxKind.PropertySignature |
              SyntaxKind.EnumMember | SyntaxKind.PropertyAssignment |
              SyntaxKind.BindingElement =>
            return ((parent
              .asInstanceOf[VariableLikeDeclaration])
              .initializer === node)
          case SyntaxKind.ExpressionStatement | SyntaxKind.IfStatement |
              SyntaxKind.DoStatement | SyntaxKind.WhileStatement |
              SyntaxKind.ReturnStatement | SyntaxKind.WithStatement |
              SyntaxKind.SwitchStatement | SyntaxKind.CaseClause |
              SyntaxKind.ThrowStatement | SyntaxKind.SwitchStatement =>
            return ((parent
              .asInstanceOf[ExpressionStatement])
              .expression === node)
          ...
          case SyntaxKind.ForInStatement | SyntaxKind.ForOfStatement =>
            var forInStatement =
              parent.asInstanceOf[(ForInStatement | ForOfStatement)]
          ...
          case SyntaxKind.TypeAssertionExpression | SyntaxKind.AsExpression =>
            return (node === (parent
              .asInstanceOf[AssertionExpression])
              .expression)
          ...
          case SyntaxKind.ComputedPropertyName =>
            return (node === (parent
              ...
              .expression)
          case SyntaxKind.Decorator | SyntaxKind.JsxExpression |
              SyntaxKind.JsxSpreadAttribute =>
            return true
          case SyntaxKind.ExpressionWithTypeArguments =>
            return (((parent
              .asInstanceOf[ExpressionWithTypeArguments])
              .expression === node) && isExpressionWithTypeArgumentsInClassExtendsClause(
              parent))
          case _ =>
  ...
  def isInstantiatedModule(node: ModuleDeclaration,
  ...
  def isExternalModuleImportEqualsDeclaration(node: Node) = {
    return ((node.kind === SyntaxKind.ImportEqualsDeclaration) && ((node
  ...
  def isRequireCall(expression: Node,
                    checkArgumentIsStringLiteral: Boolean): Boolean = {
    val isRequire = ((((expression.kind === SyntaxKind.CallExpression) && ((expression
        .asInstanceOf[CallExpression])
        .expression
        .kind === SyntaxKind.Identifier)) && (((expression
        .asInstanceOf[CallExpression])
        .expression
        .asInstanceOf[Identifier])
        .text === "require")) && ((expression
        .asInstanceOf[CallExpression])
        .arguments
        .length === 1))
    return (isRequire && (((!checkArgumentIsStringLiteral) || ((expression
      .asInstanceOf[CallExpression])
      .arguments(0)
      .kind === SyntaxKind.StringLiteral))))
  ...
  def getSpecialPropertyAssignmentKind(
    ...
    if ((lhs.expression.kind === SyntaxKind.Identifier)) {
      ...
      val innerPropertyAccess =
        lhs.expression.asInstanceOf[PropertyAccessExpression]
      if ((innerPropertyAccess.expression.kind === SyntaxKind.Identifier)) {
        val innerPropertyAccessIdentifier =
          innerPropertyAccess.expression.asInstanceOf[Identifier]
  ...
  def getExternalModuleName(node: Node): Expression = {
    ...
    if (((node.kind === SyntaxKind.ModuleDeclaration) && ((node
          .asInstanceOf[ModuleDeclaration])
          .name
          .kind === SyntaxKind.StringLiteral))) {
  ...
  def getNamespaceDeclarationNode(
      node: (ImportDeclaration | ImportEqualsDeclaration | ExportDeclaration))
    : (ImportEqualsDeclaration | NamespaceImport) = {
    if ((node.kind === SyntaxKind.ImportEqualsDeclaration)) {
  ...
  def isDefaultImport(
      node: (ImportDeclaration | ImportEqualsDeclaration | ExportDeclaration)) = {
    return (((node.kind === SyntaxKind.ImportDeclaration) && (node
  ...
  def hasQuestionToken(node: Node) = {
    if (node) {
      node.kind match {
        case SyntaxKind.Parameter | SyntaxKind.MethodDeclaration |
          ...
          return ((node
            .asInstanceOf[
              (ParameterDeclaration | MethodDeclaration | PropertyDeclaration)])
  ...
  def isJSDocConstructSignature(node: Node) = {
    return (((node.kind === SyntaxKind.JSDocFunctionType) && ((node
  ...
  def getJSDocTag(node: Node,
                  kind: SyntaxKind,
                  checkParentVariableStatement: Boolean): JSDocTag = {
    if ((!node)) {
      return undefined
  ...
  def append[T](previous: (Array[T] | undefined),
                additional: (Array[T] | undefined)): (Array[T] | undefined) = {
    if (additional) {
      if ((!previous)) {
        (previous = Array())
  ...
  def getJSDocComments(
      node: Node,
      checkParentVariableStatement: Boolean): Array[String] = {
    return getJSDocs(
      node,
      checkParentVariableStatement,
      (docs => map(docs, (doc => doc.comment))),
      (tags => map(tags, (tag => tag.comment))))

  }
  def getJSDocTags(node: Node,
                   checkParentVariableStatement: Boolean): Array[JSDocTag] = {
    return getJSDocs(
      node,
      checkParentVariableStatement,
      (docs => {
  ...
  def getJSDocs[T](node: Node,
                   checkParentVariableStatement: Boolean,
                   getDocs: ((Array[JSDoc]) => Array[T]),
                   getTags: ((Array[JSDocTag]) => Array[T])): Array[T] = {
    var result: Array[T] = undefined
    if (checkParentVariableStatement) {
      val isInitializerOfVariableDeclarationInStatement = ((isVariableLike(
          node.parent) && ((node.parent).initializer === node)) && (node.parent.parent.parent.kind === SyntaxKind.VariableStatement))
      val isVariableOfVariableDeclarationStatement = (isVariableLike(node) && (node.parent.parent.kind === SyntaxKind.VariableStatement))
      val variableStatementNode =
        (if (isInitializerOfVariableDeclarationInStatement)
           node.parent.parent.parent
         else (if (isVariableOfVariableDeclarationStatement) node.parent.parent
               else undefined))
      ...
      val parent = node.parent
      val isSourceOfAssignmentExpressionStatement = ((((parent && parent.parent) && (parent.kind === SyntaxKind.BinaryExpression)) && ((parent
          .asInstanceOf[BinaryExpression])
          .operatorToken
          .kind === SyntaxKind.EqualsToken)) && (parent.parent.kind === SyntaxKind.ExpressionStatement))
      ...
      if (isPropertyAssignmentExpression) {
        (result = append(
          result,
          getJSDocs(parent, checkParentVariableStatement, getDocs, getTags)))

      }
      if ((node.kind === SyntaxKind.Parameter)) {
        val paramTags = getJSDocParameterTag(
    ...
    if ((isVariableLike(node) && node.initializer)) {
      (result =
        append(result, getJSDocs(node.initializer, false, getDocs, getTags)))
  ...
  def getJSDocParameterTag(
    ...
    if ((!param.name)) {
      ...
      val paramTags =
        filter(tags, (tag => (tag.kind === SyntaxKind.JSDocParameterTag)))
      ...
      val paramTags = filter(
        tags,
        (tag =>
           ((tag.kind === SyntaxKind.JSDocParameterTag) && ((tag
  ...
  def getJSDocTypeTag(node: Node): JSDocTypeTag = {
    return getJSDocTag(node, SyntaxKind.JSDocTypeTag, false)
      .asInstanceOf[JSDocTypeTag]

  }
  def getJSDocReturnTag(node: Node): JSDocReturnTag = {
    return getJSDocTag(node, SyntaxKind.JSDocReturnTag, true)
      .asInstanceOf[JSDocReturnTag]

  }
  def getJSDocTemplateTag(node: Node): JSDocTemplateTag = {
    return getJSDocTag(node, SyntaxKind.JSDocTemplateTag, false)
      .asInstanceOf[JSDocTemplateTag]

  }
  def getCorrespondingJSDocParameterTag(
    ...
    while (true) {
      {
        ...
        return ((((parent.kind === SyntaxKind.BinaryExpression) && isAssignmentOperator(
          (parent
            .asInstanceOf[BinaryExpression])
            .operatorToken
            .kind)) && ((parent
          .asInstanceOf[BinaryExpression])
          .left === node)) || ((((parent.kind === SyntaxKind.ForInStatement) || (parent.kind === SyntaxKind.ForOfStatement))) && ((parent
  ...
  def isInAmbientContext(node: Node): Boolean = {
    while (node) {
      {
        if ((hasModifier(node, ModifierFlags.Ambient) || (((node.kind === SyntaxKind.SourceFile) && (node
  ...
  def isLiteralComputedPropertyDeclarationName(node: Node) = {
    return (((((node.kind === SyntaxKind.StringLiteral) || (node.kind === SyntaxKind.NumericLiteral))) && (node.parent.kind === SyntaxKind.ComputedPropertyName)) && isDeclaration(
      node.parent.parent))
  ...
  def isIdentifierName(node: Identifier): Boolean = {
    ...
    parent.kind match {
      case SyntaxKind.PropertyDeclaration | SyntaxKind.PropertySignature |
          ...
          SyntaxKind.PropertyAccessExpression =>
        return ((parent
          .asInstanceOf[(Declaration | PropertyAccessExpression)])
          .name === node)
      case SyntaxKind.QualifiedName =>
      ...
      case SyntaxKind.BindingElement | SyntaxKind.ImportSpecifier =>
        return ((parent
          .asInstanceOf[(BindingElement | ImportSpecifier)])
          .propertyName === node)
  ...
  def isAliasSymbolDeclaration(node: Node): Boolean = {
    return (((((((node.kind === SyntaxKind.ImportEqualsDeclaration) || (node.kind === SyntaxKind.NamespaceExportDeclaration)) || ((node.kind === SyntaxKind.ImportClause) && (!(!(node
      .asInstanceOf[ImportClause])
      .name)))) || (node.kind === SyntaxKind.NamespaceImport)) || (node.kind === SyntaxKind.ImportSpecifier)) || (node.kind === SyntaxKind.ExportSpecifier)) || ((node.kind === SyntaxKind.ExportAssignment) && exportAssignmentIsAlias(
  ...
  def getClassExtendsHeritageClauseElement(
      node: (ClassLikeDeclaration | InterfaceDeclaration)) = {
    val heritageClause =
      getHeritageClause(node.heritageClauses, SyntaxKind.ExtendsKeyword)
    return (if ((heritageClause && (heritageClause.types.length > 0)))
  ...
  def getClassImplementsHeritageClauseElements(node: ClassLikeDeclaration) = {
    val heritageClause =
      getHeritageClause(node.heritageClauses, SyntaxKind.ImplementsKeyword)
  ...
  def tryResolveScriptReference(host: ScriptReferenceHost,
                                sourceFile: SourceFile,
                                reference: FileReference) = {
    if ((!host.getCompilerOptions().noResolve)) {
      val referenceFileName =
        (if (isRootedDiskPath(reference.fileName)) reference.fileName
         else
           combinePaths(
  ...
  def getFileReferenceFromReferencePath(
      comment: String,
      commentRange: CommentRange): ReferencePathMatchResult = {
    val simpleReferenceRegEx =
      java.util.regex.Pattern.compile(raw"""^\/\/\/\s*<reference\s+""", "gim")
    val isNoDefaultLibRegEx = java.util.regex.Pattern.compile(
      raw"""^(\/\/\/\s*<reference\s+no-default-lib\s*=\s*)('|")(.+?)\2\s*\/>""",
      "gim")
    if (simpleReferenceRegEx.test(comment)) {
      if (isNoDefaultLibRegEx.test(comment)) {
        ...
        val refLibResult = ((!refMatchResult) && fullTripleSlashReferenceTypeReferenceDirectiveRegEx
            .exec(comment))
  ...
  def isAsyncFunctionLike(node: Node): Boolean = {
    return ((isFunctionLike(node) && hasModifier(node, ModifierFlags.Async)) && (!isAccessor(
      node)))
  ...
  def isDynamicName(name: DeclarationName): Boolean = {
    return (((name.kind === SyntaxKind.ComputedPropertyName) && (!isStringOrNumericLiteral(
      (name
        .asInstanceOf[ComputedPropertyName])
        .expression
        .kind))) && (!isWellKnownSymbolSyntactically(
      (name.asInstanceOf[ComputedPropertyName]).expression)))

  }
  def isWellKnownSymbolSyntactically(node: Expression): Boolean = {
    return (isPropertyAccessExpression(node) && isESSymbolIdentifier(
      node.expression))
  ...
  def getOriginalSourceFiles(sourceFiles: Array[SourceFile]) = {
    ...
    (sourceFiles).foreach { fresh9 =>
      val sourceFile = zeroOfMyType = fresh9 {
  ...
  def getExpressionAssociativity(expression: Expression) = {
    ...
    val hasArguments = ((expression.kind === SyntaxKind.NewExpression) && ((expression
        .asInstanceOf[NewExpression])
        .arguments !== undefined))
    return getOperatorAssociativity(expression.kind, operator, hasArguments)

  }
  def getOperatorAssociativity(kind: SyntaxKind,
                               operator: SyntaxKind,
                               hasArguments: Boolean) = {
    kind match {
      case SyntaxKind.NewExpression =>
        return (if (hasArguments) Associativity.Left else Associativity.Right)
      case SyntaxKind.PrefixUnaryExpression | SyntaxKind.TypeOfExpression |
          SyntaxKind.VoidExpression | SyntaxKind.DeleteExpression |
          SyntaxKind.AwaitExpression | SyntaxKind.ConditionalExpression |
          SyntaxKind.YieldExpression =>
        return Associativity.Right
      case SyntaxKind.BinaryExpression =>
        operator match {
          case SyntaxKind.AsteriskAsteriskToken | SyntaxKind.EqualsToken |
              ...
              SyntaxKind.AmpersandEqualsToken | SyntaxKind.CaretEqualsToken |
  ...
  def getOperatorPrecedence(nodeKind: SyntaxKind,
    ...
    nodeKind match {
      case SyntaxKind.ThisKeyword | SyntaxKind.SuperKeyword |
          ...
          SyntaxKind.ArrowFunction | SyntaxKind.ClassExpression |
          SyntaxKind.JsxElement | SyntaxKind.JsxSelfClosingElement |
          SyntaxKind.RegularExpressionLiteral |
          SyntaxKind.NoSubstitutionTemplateLiteral |
          SyntaxKind.TemplateExpression | SyntaxKind.ParenthesizedExpression |
      ...
      case SyntaxKind.BinaryExpression =>
        operatorKind match {
          case SyntaxKind.ExclamationToken | SyntaxKind.TildeToken =>
            return 15
          case SyntaxKind.AsteriskAsteriskToken | SyntaxKind.AsteriskToken |
          ...
          case SyntaxKind.LessThanLessThanToken |
            ...
            return 12
          case SyntaxKind.LessThanToken | SyntaxKind.LessThanEqualsToken |
          ...
          case SyntaxKind.BarBarToken =>
            return 5
          case SyntaxKind.EqualsToken | SyntaxKind.PlusEqualsToken |
  ...
  def createDiagnosticCollection(): DiagnosticCollection = {
    ...
    def reattachFileDiagnostics(newFile: SourceFile): Unit = {
      ...
      (fileDiagnostics(newFile.fileName)).foreach { fresh10 =>
        val diagnostic = zeroOfMyType = fresh10 {
    ...
    def sortAndDeduplicate() = {
      ...
      (fileDiagnostics).keys.foreach { fresh12 =>
        val key = zeroOfMyType = fresh12 {
          (fileDiagnostics(key) =
            sortAndDeduplicateDiagnostics(fileDiagnostics(key)))
  ...
  val escapedCharsRegExp = java.util.regex.Pattern
    .compile(raw"""[\\\"\u0000-\u001f\t\v\f\b\r\n\u2028\u2029\u0085]""", "g")
  val escapedCharsMap = createMap(
    Map(
      "\0" -> "\\0",
      "\t" -> "\\t",
      "\u000b" -> "\\u000b",
  ...
  def escapeString(s: String): String = {
    (s =
      (if (escapedCharsRegExp.test(s))
         s.replace(escapedCharsRegExp, getReplacement)
       else s))
    return s
    def getReplacement(c: String) = {
      return (escapedCharsMap(c) || get16BitUnicodeEscapeSequence(
        c.charCodeAt(0)))
  ...
  val nonAsciiCharacters =
    java.util.regex.Pattern.compile(raw"""[^\u0000-\u007F]""", "g")
  def escapeNonAsciiCharacters(s: String): String = {
    return (if (nonAsciiCharacters.test(s))
              s.replace(
                nonAsciiCharacters,
                (c => get16BitUnicodeEscapeSequence(c.charCodeAt(0))))
            else s)
  ...
  def getIndentString(level: Int) = {
    if ((indentStrings(level) === undefined)) {
      (indentStrings(level) = (getIndentString((level - 1)) + indentStrings(
          1)))
  ...
  def createTextWriter(newLine: String): EmitTextWriter = {
    ...
    def writeLiteral(s: String) = {
      if ((s && s.length)) {
        ...
        if ((lineStartsOfS.length > 1)) {
          ...
          (linePos = ((output.length - s.length) + lastOrUndefined(
              lineStartsOfS)))
    ...
    return Map(
      ...
      "getLine" -> (() => (lineCount + 1)),
      "getColumn" -> (() =>
                        (if (lineStart) ((indent * getIndentSize()) + 1)
                         else ((output.length - linePos) + 1))),
      "getText" -> (() => output),
  ...
  def getResolvedExternalModuleName(host: EmitHost, file: SourceFile): String = {
    return (file.moduleName || getExternalModuleNameFromPath(
      host,
      file.fileName))

  }
  def getExternalModuleNameFromDeclaration(
      host: EmitHost,
  ...
  def getExternalModuleNameFromPath(host: EmitHost, fileName: String): String = {
    ...
    val filePath =
      getNormalizedAbsolutePath(fileName, host.getCurrentDirectory())
    val relativePath = getRelativePathToDirectoryOrUrl(
      dir,
      filePath,
      dir,
      getCanonicalFileName,
  ...
  def getOwnEmitOutputFilePath(sourceFile: SourceFile,
                               host: EmitHost,
                               extension: String) = {
    ...
    if (compilerOptions.outDir) {
      (emitOutputFilePathWithoutExtension = removeFileExtension(
        getSourceFilePathInNewDir(sourceFile, host, compilerOptions.outDir)))
  ...
  def getDeclarationEmitOutputFilePath(sourceFile: SourceFile,
                                       host: EmitHost) = {
    val options = host.getCompilerOptions()
    val outputDir = (options.declarationDir || options.outDir)
    val path =
      (if (outputDir) getSourceFilePathInNewDir(sourceFile, host, outputDir)
  ...
  def getSourceFilesToEmit(host: EmitHost, targetSourceFile: SourceFile) = {
    ...
    if ((options.outFile || options.out)) {
      ...
      return filter(
        sourceFiles,
        (if (moduleEmitEnabled) isNonDeclarationFile
         else isBundleEmitNonExternalModule))

    } else {
      val sourceFiles =
        (if ((targetSourceFile === undefined)) host.getSourceFiles()
         else Array(targetSourceFile))
  ...
  def isBundleEmitNonExternalModule(sourceFile: SourceFile) = {
    return ((!isDeclarationFile(sourceFile)) && (!isExternalModule(
      sourceFile)))

  }
  def forEachTransformedEmitFile(
      host: EmitHost,
      sourceFiles: Array[SourceFile],
      action: ((String, String, String, Array[SourceFile], Boolean) => Unit),
    ...
    if ((options.outFile || options.out)) {
      ...
      (sourceFiles).foreach { fresh13 =>
        val sourceFile = zeroOfMyType = fresh13 {
          if (((!isDeclarationFile(sourceFile)) && (!host
    ...
    def onSingleFileEmit(host: EmitHost, sourceFile: SourceFile) = {
      ...
      val declarationFilePath =
        (if (((!isSourceFileJavaScript(sourceFile)) && ((options.declaration || emitOnlyDtsFiles))))
           getDeclarationEmitOutputFilePath(sourceFile, host)
         else undefined)
      action(
        jsFilePath,
        sourceMapFilePath,
    ...
    def onBundledEmit(sourceFiles: Array[SourceFile]) = {
      if (sourceFiles.length) {
        ...
        val sourceMapFilePath = getSourceMapFilePath(jsFilePath, options)
        val declarationFilePath =
          (if (options.declaration) (removeFileExtension(jsFilePath) + ".d.ts")
           else undefined)
        action(
          jsFilePath,
          sourceMapFilePath,
  ...
  def forEachExpectedEmitFile(host: EmitHost,
                              action: ((EmitFileNames, Array[SourceFile],
    ...
    if ((options.outFile || options.out)) {
      ...
      (sourceFiles).foreach { fresh14 =>
        val sourceFile = zeroOfMyType = fresh14 {
          if (((!isDeclarationFile(sourceFile)) && (!host
                .isSourceFileFromExternalLibrary(sourceFile)))) {
            onSingleFileEmit(host, sourceFile)
    ...
    def onSingleFileEmit(host: EmitHost, sourceFile: SourceFile) = {
      ...
      val jsFilePath = getOwnEmitOutputFilePath(sourceFile, host, extension)
      val declarationFilePath =
        (if (((!isSourceFileJavaScript(sourceFile)) && ((emitOnlyDtsFiles || options.declaration))))
           getDeclarationEmitOutputFilePath(sourceFile, host)
         else undefined)
      val emitFileNames: EmitFileNames = Map(
    ...
    def onBundledEmit(host: EmitHost) = {
      val bundledSources = filter(
        host.getSourceFiles(),
        (sourceFile =>
           (((!isDeclarationFile(sourceFile)) && (!host
             .isSourceFileFromExternalLibrary(sourceFile))) && (((!isExternalModule(
             sourceFile)) || (!(!getEmitModuleKind(options))))))))
      if (bundledSources.length) {
        val jsFilePath = (options.outFile || options.out)
        val emitFileNames: EmitFileNames = Map(
          "jsFilePath" -> jsFilePath,
          "sourceMapFilePath" -> getSourceMapFilePath(jsFilePath, options),
          "declarationFilePath" -> (if (options.declaration)
                                      (removeFileExtension(jsFilePath) + ".d.ts")
                                    else undefined))
  ...
  def getSourceFilePathInNewDir(sourceFile: SourceFile,
                                host: EmitHost,
                                newDirPath: String) = {
    var sourceFilePath = getNormalizedAbsolutePath(
      sourceFile.fileName,
      host.getCurrentDirectory())
    val commonSourceDirectory = host.getCommonSourceDirectory()
    val isSourceFileInCommonSourceDirectory = (host
        .getCanonicalFileName(sourceFilePath)
        .indexOf(host.getCanonicalFileName(commonSourceDirectory)) === 0)
    (sourceFilePath =
  ...
  def getFirstConstructorWithBody(
  ...
  def getSetAccessorTypeAnnotationNode(
      accessor: SetAccessorDeclaration): TypeNode = {
    if ((accessor && (accessor.parameters.length > 0))) {
      val hasThis = ((accessor.parameters.length === 2) && parameterIsThisKeyword(
  ...
  def getThisParameter(
      signature: SignatureDeclaration): (ParameterDeclaration | undefined) = {
  ...
  def isThisIdentifier(node: (Node | undefined)): Boolean = {
    return ((node && (node.kind === SyntaxKind.Identifier)) && identifierIsThisKeyword(
  ...
  def getAllAccessorDeclarations(
      declarations: NodeArray[Declaration],
      accessor: AccessorDeclaration): AllAccessorDeclarations = {
    ...
    if (hasDynamicName(accessor)) {
      ...
      forEach(
        declarations,
        ((member: Declaration) => {
           if (((((member.kind === SyntaxKind.GetAccessor) || (member.kind === SyntaxKind.SetAccessor))) && (hasModifier(
                 member,
                 ModifierFlags.Static) === hasModifier(
                 accessor,
                 ModifierFlags.Static)))) {
             val memberName = getPropertyNameForPropertyNameNode(member.name)
             val accessorName =
               getPropertyNameForPropertyNameNode(accessor.name)
  ...
  def emitNewLineBeforeLeadingComments(
      ...
      node: TextRange,
      leadingComments: Array[CommentRange]) = {
  ...
  def emitNewLineBeforeLeadingCommentsOfPosition(
      lineMap: Array[Int],
      writer: EmitTextWriter,
      pos: Int,
      leadingComments: Array[CommentRange]) = {
    if ((((leadingComments && leadingComments.length) && (pos !== leadingComments(
          0).pos)) && (getLineOfLocalPositionFromLineMap(lineMap, pos) !== getLineOfLocalPositionFromLineMap(
          lineMap,
  ...
  def emitNewLineBeforeLeadingCommentOfPosition(lineMap: Array[Int],
                                                writer: EmitTextWriter,
                                                pos: Int,
                                                commentPos: Int) = {
    if (((pos !== commentPos) && (getLineOfLocalPositionFromLineMap(
          lineMap,
          pos) !== getLineOfLocalPositionFromLineMap(lineMap, commentPos)))) {
      writer.writeLine()

    }

  }
  def emitComments(text: String,
                   lineMap: Array[Int],
                   writer: EmitTextWriter,
                   ...
                   newLine: String,
                   writeComment: ((String, Array[Int], EmitTextWriter, Int,
                                   Int, String) => Unit)) = {
    if ((comments && (comments.length > 0))) {
      ...
      (comments).foreach { fresh15 =>
        val comment = zeroOfMyType = fresh15 {
  ...
  def emitDetachedComments(text: String,
                           lineMap: Array[Int],
                           writer: EmitTextWriter,
                           writeComment: ((String, Array[Int], EmitTextWriter,
                                           Int, Int, String) => Unit),
                           node: TextRange,
                           newLine: String,
    ...
    if (removeComments) {
      if ((node.pos === 0)) {
        (leadingComments =
          filter(getLeadingCommentRanges(text, node.pos), isPinnedComment))
    ...
    if (leadingComments) {
      ...
      (leadingComments).foreach { fresh16 =>
        val comment = zeroOfMyType = fresh16 {
          if (lastComment) {
            val lastCommentLine =
              getLineOfLocalPositionFromLineMap(lineMap, lastComment.end)
            val commentLine =
              getLineOfLocalPositionFromLineMap(lineMap, comment.pos)
      ...
      if (detachedComments.length) {
        val lastCommentLine = getLineOfLocalPositionFromLineMap(
          lineMap,
          lastOrUndefined(detachedComments).end)
        val nodeLine = getLineOfLocalPositionFromLineMap(
          lineMap,
        ...
        if ((nodeLine >= (lastCommentLine + 2))) {
          ...
          emitComments(
            ...
            newLine,
            writeComment)
          (currentDetachedCommentInfo = Map(
            "nodePos" -> node.pos,
    ...
    def isPinnedComment(comment: CommentRange) = {
      return ((text.charCodeAt((comment.pos + 1)) === CharacterCodes.asterisk) && (text
        .charCodeAt((comment.pos + 2)) === CharacterCodes.exclamation))

    }

  }
  def writeCommentRange(text: String,
                        lineMap: Array[Int],
                        writer: EmitTextWriter,
                        commentPos: Int,
                        commentEnd: Int,
    ...
    if ((text.charCodeAt((commentPos + 1)) === CharacterCodes.asterisk)) {
      val firstCommentLineAndCharacter =
        computeLineAndCharacterOfPosition(lineMap, commentPos)
      ...
      var firstCommentLineIndent: Int = zeroOfMyType {
        ...
        while ((pos < commentEnd)) {
          {
            val nextLineStart =
              (if ((((currentLine + 1)) === lineCount)) (text.length + 1)
               else lineMap((currentLine + 1)))
            if ((pos !== commentPos)) {
              ...
              val currentWriterIndentSpacing = (writer
                  .getIndent() * getIndentSize())
              val spacesToEmit = ((currentWriterIndentSpacing - firstCommentLineIndent) + calculateIndent(
                  text,
                  pos,
                  nextLineStart))
              if ((spacesToEmit > 0)) {
                var numberOfSingleSpacesToEmit = (spacesToEmit % getIndentSize())
                val indentSizeSpaceString = getIndentString(
  ...
  def writeTrimmedCurrentLine(text: String,
                              commentEnd: Int,
                              writer: EmitTextWriter,
                              newLine: String,
                              pos: Int,
    ...
    val currentLineText = text
      .substring(pos, end)
      .replace(java.util.regex.Pattern.compile(raw"""^\s+|\s+$$""", "g"), "")
    if (currentLineText) {
  ...
  def getModifierFlags(node: Node): ModifierFlags = {
    ...
    if (node.modifiers) {
      (node.modifiers).foreach { fresh17 =>
        val modifier = zeroOfMyType = fresh17 {
    ...
    if (((node.flags & NodeFlags.NestedNamespace) || (((node.kind === SyntaxKind.Identifier) && (node
          .asInstanceOf[Identifier])
          .isInJSDocNamespace)))) {
  ...
  def tryGetClassExtendingExpressionWithTypeArguments(
      node: Node): (ClassLikeDeclaration | undefined) = {
    if ((((node.kind === SyntaxKind.ExpressionWithTypeArguments) && ((node.parent
          .asInstanceOf[HeritageClause])
          .token === SyntaxKind.ExtendsKeyword)) && isClassLike(
          node.parent.parent))) {
  ...
  def isSupportedExpressionWithTypeArguments(
      node: ExpressionWithTypeArguments): Boolean = {
  ...
  def isEntityNameExpression(node: Expression): Boolean = {
    return ((node.kind === SyntaxKind.Identifier) || ((node.kind === SyntaxKind.PropertyAccessExpression) && isEntityNameExpression(
      (node.asInstanceOf[PropertyAccessExpression]).expression)))

  }
  def isRightSideOfQualifiedNameOrPropertyAccess(node: Node) = {
    return ((((node.parent.kind === SyntaxKind.QualifiedName) && ((node.parent
      .asInstanceOf[QualifiedName])
      .right === node))) || (((node.parent.kind === SyntaxKind.PropertyAccessExpression) && ((node.parent
      .asInstanceOf[PropertyAccessExpression])
      .name === node))))
  ...
  def getLocalSymbolForExportDefault(symbol: Symbol) = {
    return (if (((symbol && symbol.valueDeclaration) && hasModifier(
                  symbol.valueDeclaration,
                  ModifierFlags.Default))) symbol.valueDeclaration.localSymbol
            else undefined)
  ...
  def stringifyValue(value: Any): String = {
    return (if ((typeof(value) === "string")) s"""\"${escapeString(value)}\""""
            else
              (if ((typeof(value) === "number"))
                 (if (isFinite(value)) String(value) else "null")
               else
                 (if ((typeof(value) === "boolean"))
                    (if (value) "true" else "false")
                  else
                    (if (((typeof(value) === "object") && value))
                       (if (isArray(value)) cycleCheck(stringifyArray, value)
                        else cycleCheck(stringifyObject, value))
                     else "null"))))

  }
  def cycleCheck(cb: ((Any) => String), value: Any) = {
    Debug.assert(
      (!value.`hasOwnProperty`("__cycle")),
  ...
  def stringifyProperty(memo: String, value: Any, key: String) = {
    return (if ((((value === undefined) || (typeof(value) === "function")) || (key === "__cycle")))
              memo
            else
              (((if (memo) (memo + ",") else memo) ) + s"""\"${escapeString(
    ...
    while ((i < length)) {
      {
        ...
        (result += (((base64Digits.charAt(byte1) + base64Digits.charAt(byte2)) + base64Digits
          .charAt(byte3)) + base64Digits.charAt(byte4)))
        (i += 3)
  ...
  def isSimpleExpressionWorker(node: Expression, depth: Int): Boolean = {
    if ((depth <= 5)) {
      ...
      if (((((((((((kind === SyntaxKind.StringLiteral) || (kind === SyntaxKind.NumericLiteral)) || (kind === SyntaxKind.RegularExpressionLiteral)) || (kind === SyntaxKind.NoSubstitutionTemplateLiteral)) || (kind === SyntaxKind.Identifier)) || (kind === SyntaxKind.ThisKeyword)) || (kind === SyntaxKind.SuperKeyword)) || (kind === SyntaxKind.TrueKeyword)) || (kind === SyntaxKind.FalseKeyword)) || (kind === SyntaxKind.NullKeyword))) {
        ...
        return isSimpleExpressionWorker(
          (node.asInstanceOf[PropertyAccessExpression]).expression,
          (depth + 1))

      } else if ((kind === SyntaxKind.ElementAccessExpression)) {
        return (isSimpleExpressionWorker(
          (node.asInstanceOf[ElementAccessExpression]).expression,
          (depth + 1)) && isSimpleExpressionWorker(
        ...
        return ((((node
          .asInstanceOf[BinaryExpression])
          .operatorToken
          .kind !== SyntaxKind.AsteriskAsteriskToken) && isSimpleExpressionWorker(
        ...
        return ((isSimpleExpressionWorker(
          (node.asInstanceOf[ConditionalExpression]).condition,
          (depth + 1)) && isSimpleExpressionWorker(
          (node.asInstanceOf[ConditionalExpression]).whenTrue,
          (depth + 1))) && isSimpleExpressionWorker(
        ...
        return isSimpleExpressionWorker(
          (node
            .asInstanceOf[
              (VoidExpression | TypeOfExpression | DeleteExpression)])
            .expression,
          (depth + 1))
        ...
        if ((!isSimpleExpressionWorker(
              (node.asInstanceOf[CallExpression]).expression,
              (depth + 1)))) {
          return false

        }
        ((node.asInstanceOf[CallExpression]).arguments).foreach { fresh18 =>
          val argument = zeroOfMyType = fresh18 {
  ...
  def formatSyntaxKind(kind: SyntaxKind): String = {
    ...
    if (syntaxKindEnum) {
      ...
      (syntaxKindEnum).keys.foreach { fresh19 =>
        val name = zeroOfMyType = fresh19 {
          if ((syntaxKindEnum(name) === kind)) {
            return (syntaxKindCache(kind) = (((kind
                .`toString`() + " (") + name) + ")"))
  ...
  def moveRangePastDecorators(node: Node): TextRange = {
    return (if ((node.decorators && (node.decorators.length > 0)))
              moveRangePos(node, node.decorators.end)
            else node)

  }
  def moveRangePastModifiers(node: Node): TextRange = {
    return (if ((node.modifiers && (node.modifiers.length > 0)))
              moveRangePos(node, node.modifiers.end)
            else moveRangePastDecorators(node))
  ...
  def collapseRangeToStart(range: TextRange): TextRange = {
    return (if (isCollapsedRange(range)) range
            else moveRangeEnd(range, range.pos))

  }
  def collapseRangeToEnd(range: TextRange): TextRange = {
    return (if (isCollapsedRange(range)) range
            else moveRangePos(range, range.end))
  ...
  def rangeStartPositionsAreOnSameLine(range1: TextRange,
                                       range2: TextRange,
                                       sourceFile: SourceFile) = {
    return positionsAreOnSameLine(
      getStartPositionOfRange(range1, sourceFile),
      getStartPositionOfRange(range2, sourceFile),
      sourceFile)

  }
  def rangeEndPositionsAreOnSameLine(range1: TextRange,
                                     range2: TextRange,
                                     sourceFile: SourceFile) = {
    return positionsAreOnSameLine(range1.end, range2.end, sourceFile)

  }
  def rangeStartIsOnSameLineAsRangeEnd(range1: TextRange,
                                       range2: TextRange,
                                       sourceFile: SourceFile) = {
    return positionsAreOnSameLine(
      getStartPositionOfRange(range1, sourceFile),
  ...
  def rangeEndIsOnSameLineAsRangeStart(range1: TextRange,
                                       range2: TextRange,
                                       sourceFile: SourceFile) = {
    return positionsAreOnSameLine(
      range1.end,
  ...
  def positionsAreOnSameLine(pos1: Int, pos2: Int, sourceFile: SourceFile) = {
    return ((pos1 === pos2) || (getLineOfLocalPosition(sourceFile, pos1) === getLineOfLocalPosition(
      sourceFile,
      pos2)))

  }
  def getStartPositionOfRange(range: TextRange, sourceFile: SourceFile) = {
    return (if (positionIsSynthesized(range.pos)) (-1)
            else skipTrivia(sourceFile.text, range.pos))

  }
  def collectExternalModuleInfo(sourceFile: SourceFile) = {
    val externalImports: Array[
          ...
          case SyntaxKind.ExportDeclaration =>
            if ((node.asInstanceOf[ExportDeclaration]).moduleSpecifier) {
              ...
              ((node
                ...
                .elements)
                .foreach { fresh21 =>
                  val specifier = zeroOfMyType = fresh21 {
                    val name =
                      ((specifier.propertyName || specifier.name)).text
                    ((exportSpecifiers(name) || ((
                      exportSpecifiers(name) = Array())))).push(specifier)
  ...
  def isMergedWithClass(node: Node) = {
    if (node.symbol) {
      (node.symbol.declarations).foreach { fresh22 =>
        val declaration = zeroOfMyType = fresh22 {
  ...
  def isUnaryExpressionKind(kind: SyntaxKind): Boolean = {
    return ((((((((kind === SyntaxKind.PrefixUnaryExpression) || (kind === SyntaxKind.PostfixUnaryExpression)) || (kind === SyntaxKind.DeleteExpression)) || (kind === SyntaxKind.TypeOfExpression)) || (kind === SyntaxKind.VoidExpression)) || (kind === SyntaxKind.AwaitExpression)) || (kind === SyntaxKind.TypeAssertionExpression)) || isLeftHandSideExpressionKind(
      kind))
  ...
  def isExpressionKind(kind: SyntaxKind) = {
    return ((((((((kind === SyntaxKind.ConditionalExpression) || (kind === SyntaxKind.YieldExpression)) || (kind === SyntaxKind.ArrowFunction)) || (kind === SyntaxKind.BinaryExpression)) || (kind === SyntaxKind.SpreadElementExpression)) || (kind === SyntaxKind.AsExpression)) || (kind === SyntaxKind.OmittedExpression)) || isUnaryExpressionKind(
      kind))
  ...
  def isStatement(node: Node): Boolean = {
    val kind = node.kind
    return ((isStatementKindButNotDeclarationKind(kind) || isDeclarationStatementKind(
      kind)) || (kind === SyntaxKind.Block))
...
object ts {
  ...
  def textSpanContainsTextSpan(span: TextSpan, other: TextSpan) = {
    return ((other.start >= span.start) && (textSpanEnd(other) <= textSpanEnd(
      span)))
  ...
  def decodedTextSpanIntersectsWith(start1: Int,
                                    length1: Int,
                                    start2: Int,
                                    length2: Int) = {
  ...
  def createTextSpan(start: Int, length: Int): TextSpan = {
    ...
    return Map("start" -> start, "length" -> length)
  ...
  def createTextChangeRange(span: TextSpan, newLength: Int): TextChangeRange = {
    ...
    return Map("span" -> span, "newLength" -> newLength)

  }
  var unchangedTextChangeRange = createTextChangeRange(createTextSpan(0, 0), 0)
  def collapseTextChangeRangesAcrossMultipleVersions(
      changes: Array[TextChangeRange]): TextChangeRange = {
    if ((changes.length === 0)) {
  ...
  def isParameterPropertyDeclaration(node: ParameterDeclaration): Boolean = {
    return ((hasModifier(node, ModifierFlags.ParameterPropertyModifier) && (node.parent.kind === SyntaxKind.Constructor)) && isClassLike(
      node.parent.parent))

  }
  def walkUpBindingElementsAndPatterns(node: Node): Node = {
    while ((node && (((node.kind === SyntaxKind.BindingElement) || isBindingPattern(
             node))))) {
      {
        (node = node.parent)
