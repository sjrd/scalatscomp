package scalatscomp
object Performance {
val performance: ( {   def now(): Int
 } | undefined ) = zeroOfMyType
val timestamp = (if (((typeof(performance)!=="undefined")&&performance.now)) (() =>  performance.now()) else (if (Date.now) Date.now else (() =>  (+(new Date())))))
}
object ts {
object performance {
val onProfilerEvent: {   def apply(markName: String): Unit
  var profiler: Boolean
 } = zeroOfMyType
val profilerEvent = (if (((typeof(onProfilerEvent)==="function")&&(onProfilerEvent.profiler===true))) onProfilerEvent else (( _markName: String ) =>  {
...
def measure(measureName: String, startMarkName: String, endMarkName: String) = {
 if (enabled) {
