...
object JSX {
  ...
  def transformJsx(context: TransformationContext) = {
    ...
    def visitJsxElement(node: JsxElement, isChild: Boolean) = {
      return visitJsxOpeningLikeElement(
        node.openingElement,
        node.children,
        isChild,
        node)
    ...
    def visitJsxSelfClosingElement(node: JsxSelfClosingElement,
                                   isChild: Boolean) = {
      return visitJsxOpeningLikeElement(node, undefined, isChild, node)

    }
    def visitJsxOpeningLikeElement(node: JsxOpeningLikeElement,
      ...
      if ((attrs.length === 0)) {
        ...
        (objectProperties = (singleOrUndefined(segments) || createAssignHelper(
            currentSourceFile.externalHelpersModuleName,
            segments)))

      }
      val element = createReactCreateElement(
    ...
    def transformJsxAttributeInitializer(
        node: (StringLiteral | JsxExpression)) = {
      if ((node === undefined)) {
        return createLiteral(true)

      } else if ((node.kind === SyntaxKind.StringLiteral)) {
        val decoded = tryDecodeEntities(
          (node.asInstanceOf[StringLiteral]).text)
