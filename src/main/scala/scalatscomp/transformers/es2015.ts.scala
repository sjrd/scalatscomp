...
var enclosingNonAsyncFunctionBody: ( FunctionLikeDeclaration | ClassElement ) = zeroOfMyType
...
def dispatcher(node: Node): VisitResult[Node] = {
 return (if (convertedLoopState) visitorForConvertedLoopWorker( node ) else visitorWorker( node ))
...
val savedEnclosingBlockScopeContainerParent = enclosingBlockScopeContainerParent
...
val savedIsInConstructorWithCapturedSuper = isInConstructorWithCapturedSuper
...
def isReturnVoidStatementInConstructorWithCapturedSuper(node: Node): Boolean = {
 return ((isInConstructorWithCapturedSuper&&(node.kind===SyntaxKind.ReturnStatement))&&(!(node.asInstanceOf[ReturnStatement]).expression))

}
def shouldCheckNode(node: Node): Boolean = {
 return (((((node.transformFlags&TransformFlags.ES2015))!==0)||(node.kind===SyntaxKind.LabeledStatement))||((isIterationStatement( node, false )&&shouldConvertIterationStatementBody( node ))))
...
(node=(if (isReturnVoidStatementInConstructorWithCapturedSuper( node )) returnCapturedThis( node ) else node))
...
return visitBreakOrContinueStatement( node.asInstanceOf[BreakOrContinueStatement] )
...
return visitVariableDeclaration( node.asInstanceOf[VariableDeclaration] )
  case  SyntaxKind.Identifier  =>
return visitIdentifier( node.asInstanceOf[Identifier] )
  case  SyntaxKind.VariableDeclarationList  =>
return visitVariableDeclarationList( node.asInstanceOf[VariableDeclarationList] )
...
return visitObjectLiteralExpression( node.asInstanceOf[ObjectLiteralExpression] )
  case  SyntaxKind.CatchClause  =>
return visitCatchClause( node.asInstanceOf[CatchClause] )
  case  SyntaxKind.ShorthandPropertyAssignment  =>
return visitShorthandPropertyAssignment( node.asInstanceOf[ShorthandPropertyAssignment] )
  case  SyntaxKind.ArrayLiteralExpression  =>
return visitArrayLiteralExpression( node.asInstanceOf[ArrayLiteralExpression] )
...
return visitParenthesizedExpression( node.asInstanceOf[ParenthesizedExpression], true )
  case  SyntaxKind.BinaryExpression  =>
return visitBinaryExpression( node.asInstanceOf[BinaryExpression], true )
  case  SyntaxKind.NoSubstitutionTemplateLiteral | SyntaxKind.TemplateHead | SyntaxKind.TemplateMiddle | SyntaxKind.TemplateTail  =>
return visitTemplateLiteral( node.asInstanceOf[LiteralExpression] )
  case  SyntaxKind.TaggedTemplateExpression  =>
return visitTaggedTemplateExpression( node.asInstanceOf[TaggedTemplateExpression] )
...
currentNode.kind match {
  case  SyntaxKind.VariableStatement  =>
(enclosingVariableStatement=currentNode.asInstanceOf[VariableStatement])
  case  SyntaxKind.VariableDeclarationList | SyntaxKind.VariableDeclaration | SyntaxKind.BindingElement | SyntaxKind.ObjectBindingPattern | SyntaxKind.ArrayBindingPattern  =>
  case _ =>
...
val savedAllowedNonLabeledJumps = convertedLoopState.allowedNonLabeledJumps
...
return createReturn( createObjectLiteral( Array( createPropertyAssignment( createIdentifier( "value" ), (if (node.expression) visitNode( node.expression, visitor, isExpression ) else createVoidZero()) ) ) ) )
...
def visitBreakOrContinueStatement(node: BreakOrContinueStatement): Statement = {
 if (convertedLoopState) {
 val jump = (if ((node.kind===SyntaxKind.BreakStatement)) Jump.Break else Jump.Continue)
val canUseBreakOrContinue = ((((node.label&&convertedLoopState.labels)&&convertedLoopState.labels(node.label.text)))||(((!node.label)&&((convertedLoopState.allowedNonLabeledJumps&jump)))))
...
while( (i<outParams.length)) {
 {
 val copyExpr = copyOutParameter( outParams(i), CopyDirection.ToOutParameter )
if ((i===0)) {
 (expr=copyExpr)
...
val modifiers = (if ((isExported&&(!isDefault))) filter( node.modifiers, isExportModifier ) else undefined)
val statement = createVariableStatement( modifiers, createVariableDeclarationList( Array( createVariableDeclaration( getDeclarationName( node, true ), undefined, transformClassLikeDeclarationToExpression( node ) ) ) ), node )
setOriginalNode( statement, node )
startOnNewLine( statement )
if ((isExported&&isDefault)) {
 val statements: Array[Statement] = Array( statement )
statements.push( createExportAssignment( undefined, undefined, false, getDeclarationName( node, false ) ) )
return statements
...
def transformClassLikeDeclarationToExpression(node: ( ClassExpression | ClassDeclaration )): Expression = {
 if (node.name) {
 enableSubstitutionsForBlockScopedBindings()

}
val extendsClauseElement = getClassExtendsHeritageClauseElement( node )
val classFunction = createFunctionExpression( undefined, undefined, undefined, undefined, (if (extendsClauseElement) Array( createParameter( "_super" ) ) else Array()), undefined, transformClassBody( node, extendsClauseElement ) )
...
return createParen( createCall( outer, undefined, (if (extendsClauseElement) Array( visitNode( extendsClauseElement.expression, visitor, isExpression ) ) else Array()) ) )

}
def transformClassBody(node: ( ClassExpression | ClassDeclaration ), extendsClauseElement: ExpressionWithTypeArguments): Block = {
 val statements: Array[Statement] = Array()
startLexicalEnvironment()
addExtendsHelperIfNeeded( statements, node, extendsClauseElement )
addConstructor( statements, node, extendsClauseElement )
addClassMembers( statements, node )
val closingBraceLocation = createTokenRange( skipTrivia( currentText, node.members.end ), SyntaxKind.CloseBraceToken )
...
val block = createBlock( createNodeArray( statements, node.members ), undefined, true )
setEmitFlags( block, EmitFlags.NoComments )
return block

}
def addExtendsHelperIfNeeded(statements: Array[Statement], node: ( ClassExpression | ClassDeclaration ), extendsClauseElement: ExpressionWithTypeArguments): Unit = {
 if (extendsClauseElement) {
 statements.push( createStatement( createExtendsHelper( currentSourceFile.externalHelpersModuleName, getDeclarationName( node ) ), extendsClauseElement ) )

}

}
def addConstructor(statements: Array[Statement], node: ( ClassExpression | ClassDeclaration ), extendsClauseElement: ExpressionWithTypeArguments): Unit = {
 val `constructor` = getFirstConstructorWithBody( node )
val hasSynthesizedSuper = hasSynthesizedDefaultSuperCall( `constructor`, (extendsClauseElement!==undefined) )
val constructorFunction = createFunctionDeclaration( undefined, undefined, undefined, getDeclarationName( node ), undefined, transformConstructorParameters( `constructor`, hasSynthesizedSuper ), undefined, transformConstructorBody( `constructor`, node, extendsClauseElement, hasSynthesizedSuper ), (`constructor`||node) )
if (extendsClauseElement) {
...
def transformConstructorParameters(`constructor`: ConstructorDeclaration, hasSynthesizedSuper: Boolean): Array[ParameterDeclaration] = {
...
def transformConstructorBody(`constructor`: ( ConstructorDeclaration | undefined ), node: ( ClassDeclaration | ClassExpression ), extendsClauseElement: ExpressionWithTypeArguments, hasSynthesizedSuper: Boolean) = {
 val statements: Array[Statement] = Array()
startLexicalEnvironment()
...
if (hasSynthesizedSuper) {
 ...
 (statementOffset=addPrologueDirectives( statements, `constructor`.body.statements, false, visitor ))
...
val superCaptureStatus = declareOrCaptureOrReturnThisForConstructorIfNeeded( statements, `constructor`, (!(!extendsClauseElement)), hasSynthesizedSuper, statementOffset )
if (((superCaptureStatus===SuperCaptureResult.ReplaceSuperCapture)||(superCaptureStatus===SuperCaptureResult.ReplaceWithReturn))) {
 (statementOffset+= 1)

}
if (`constructor`) {
 val body = saveStateAndInvoke( `constructor`, (`constructor` =>  {
 (isInConstructorWithCapturedSuper=(superCaptureStatus===SuperCaptureResult.ReplaceSuperCapture))
 return visitNodes( `constructor`.body.statements, visitor, isStatement, statementOffset )

}) )
addRange( statements, body )

}
if (((extendsClauseElement&&(superCaptureStatus!==SuperCaptureResult.ReplaceWithReturn))&&(!((`constructor`&&isSufficientlyCoveredByReturnStatements( `constructor`.body )))))) {
 statements.push( createReturn( createIdentifier( "_this" ) ) )

}
addRange( statements, endLexicalEnvironment() )
val block = createBlock( createNodeArray( statements, (if (`constructor`) `constructor`.body.statements else node.members) ), (if (`constructor`) `constructor`.body else node), true )
if ((!`constructor`)) {
 setEmitFlags( block, EmitFlags.NoComments )
...
def isSufficientlyCoveredByReturnStatements(statement: Statement): Boolean = {
...
if (ifStatement.elseStatement) {
 ...
 val lastStatement = lastOrUndefined( (statement.asInstanceOf[Block]).statements )
...
def declareOrCaptureOrReturnThisForConstructorIfNeeded(statements: Array[Statement], ctor: ( ConstructorDeclaration | undefined ), hasExtendsClause: Boolean, hasSynthesizedSuper: Boolean, statementOffset: Int) = {
 if ((!hasExtendsClause)) {
 if (ctor) {
...
if (((firstStatement.kind===SyntaxKind.ExpressionStatement)&&isSuperCall( (firstStatement.asInstanceOf[ExpressionStatement]).expression ))) {
 val superCall = (firstStatement.asInstanceOf[ExpressionStatement]).expression.asInstanceOf[CallExpression]
(superCallExpression=setOriginalNode( saveStateAndInvoke( superCall, visitImmediateSuperCallInBody ), superCall ))
...
val superCall = createFunctionApply( createIdentifier( "_super" ), actualThis, createIdentifier( "arguments" ) )
...
def visitParameter(node: ParameterDeclaration): ParameterDeclaration = {
 ...
 return setOriginalNode( createParameter( getGeneratedNameForNode( node ), undefined, node ), node )

}
else if (node.initializer) {
 return setOriginalNode( createParameter( node.name, undefined, node ), node )
...
def shouldAddDefaultValueAssignments(node: FunctionLikeDeclaration): Boolean = {
 return (((node.transformFlags&TransformFlags.ContainsDefaultValueAssignments))!==0)

}
def addDefaultValueAssignmentsIfNeeded(statements: Array[Statement], node: FunctionLikeDeclaration): Unit = {
...
def addDefaultValueAssignmentForBindingPattern(statements: Array[Statement], parameter: ParameterDeclaration, name: BindingPattern, initializer: Expression): Unit = {
 val temp = getGeneratedNameForNode( parameter )
if ((name.elements.length>0)) {
 statements.push( setEmitFlags( createVariableStatement( undefined, createVariableDeclarationList( flattenParameterDestructuring( parameter, temp, visitor ) ) ), EmitFlags.CustomPrologue ) )

}
else if (initializer) {
 statements.push( setEmitFlags( createStatement( createAssignment( temp, visitNode( initializer, visitor, isExpression ) ) ), EmitFlags.CustomPrologue ) )

}

}
def addDefaultValueAssignmentForInitializer(statements: Array[Statement], parameter: ParameterDeclaration, name: Identifier, initializer: Expression): Unit = {
 (initializer=visitNode( initializer, visitor, isExpression ))
val statement = createIf( createStrictEquality( getSynthesizedClone( name ), createVoidZero() ), setEmitFlags( createBlock( Array( createStatement( createAssignment( setEmitFlags( getMutableClone( name ), EmitFlags.NoSourceMap ), setEmitFlags( initializer, (EmitFlags.NoSourceMap|getEmitFlags( initializer )) ), parameter ) ) ), parameter ), ((EmitFlags.SingleLine|EmitFlags.NoTrailingSourceMap)|EmitFlags.NoTokenSourceMaps) ), undefined, parameter )
(statement.startsOnNewLine=true)
setEmitFlags( statement, ((EmitFlags.NoTokenSourceMaps|EmitFlags.NoTrailingSourceMap)|EmitFlags.CustomPrologue) )
statements.push( statement )

}
def shouldAddRestParameter(node: ParameterDeclaration, inConstructorWithSynthesizedSuper: Boolean) = {
 return (((node&&node.dotDotDotToken)&&(node.name.kind===SyntaxKind.Identifier))&&(!inConstructorWithSynthesizedSuper))

}
def addRestParameterIfNeeded(statements: Array[Statement], node: FunctionLikeDeclaration, inConstructorWithSynthesizedSuper: Boolean): Unit = {
 val parameter = lastOrUndefined( node.parameters )
if ((!shouldAddRestParameter( parameter, inConstructorWithSynthesizedSuper ))) {
 return

}
val declarationName = getMutableClone( parameter.name.asInstanceOf[Identifier] )
setEmitFlags( declarationName, EmitFlags.NoSourceMap )
val expressionName = getSynthesizedClone( parameter.name.asInstanceOf[Identifier] )
val restIndex = (node.parameters.length-1)
val temp = createLoopVariable()
statements.push( setEmitFlags( createVariableStatement( undefined, createVariableDeclarationList( Array( createVariableDeclaration( declarationName, undefined, createArrayLiteral( Array() ) ) ) ), parameter ), EmitFlags.CustomPrologue ) )
val forStatement = createFor( createVariableDeclarationList( Array( createVariableDeclaration( temp, undefined, createLiteral( restIndex ) ) ), parameter ), createLessThan( temp, createPropertyAccess( createIdentifier( "arguments" ), "length" ), parameter ), createPostfixIncrement( temp, parameter ), createBlock( Array( startOnNewLine( createStatement( createAssignment( createElementAccess( expressionName, createSubtract( temp, createLiteral( restIndex ) ) ), createElementAccess( createIdentifier( "arguments" ), temp ) ), parameter ) ) ) ) )
setEmitFlags( forStatement, EmitFlags.CustomPrologue )
startOnNewLine( forStatement )
statements.push( forStatement )

}
def addCaptureThisForNodeIfNeeded(statements: Array[Statement], node: Node): Unit = {
...
def captureThisForNode(statements: Array[Statement], node: Node, initializer: ( Expression | undefined ), originalStatement: Statement): Unit = {
 enableSubstitutionsForCapturedThis()
val captureThisStatement = createVariableStatement( undefined, createVariableDeclarationList( Array( createVariableDeclaration( "_this", undefined, initializer ) ) ), originalStatement )
setEmitFlags( captureThisStatement, (EmitFlags.NoComments|EmitFlags.CustomPrologue) )
setSourceMapRange( captureThisStatement, node )
statements.push( captureThisStatement )

}
def addClassMembers(statements: Array[Statement], node: ( ClassExpression | ClassDeclaration )): Unit = {
...
statements.push( transformSemicolonClassElementToStatement( member.asInstanceOf[SemicolonClassElement] ) )
  case  SyntaxKind.MethodDeclaration  =>
statements.push( transformClassMethodDeclarationToStatement( getClassMemberPrefix( node, member ), member.asInstanceOf[MethodDeclaration] ) )
  case  SyntaxKind.GetAccessor | SyntaxKind.SetAccessor  =>
val accessors = getAllAccessorDeclarations( node.members, member.asInstanceOf[AccessorDeclaration] )
...
def transformSemicolonClassElementToStatement(member: SemicolonClassElement) = {
 return createEmptyStatement( member )

}
def transformClassMethodDeclarationToStatement(receiver: LeftHandSideExpression, member: MethodDeclaration) = {
...
val statement = createStatement( createAssignment( createMemberAccessForPropertyName( receiver, visitNode( member.name, visitor, isPropertyName ), member.name ), func ), member )
...
def transformAccessorsToStatement(receiver: LeftHandSideExpression, accessors: AllAccessorDeclarations): Statement = {
 val statement = createStatement( transformAccessorsToExpression( receiver, accessors, false ), getSourceMapRange( accessors.firstAccessor ) )
setEmitFlags( statement, EmitFlags.NoComments )
return statement

}
def transformAccessorsToExpression(receiver: LeftHandSideExpression, triple: AllAccessorDeclarations, startsOnNewLine: Boolean): Expression = {
 const fresh5 = triple
...
val propertyName = createExpressionForPropertyName( visitNode( firstAccessor.name, visitor, isPropertyName ) )
...
if (getAccessor) {
 val getterFunction = transformFunctionLikeToExpression( getAccessor, undefined, undefined )
...
if (setAccessor) {
 val setterFunction = transformFunctionLikeToExpression( setAccessor, undefined, undefined )
...
properties.push( createPropertyAssignment( "enumerable", createLiteral( true ) ), createPropertyAssignment( "configurable", createLiteral( true ) ) )
val call = createCall( createPropertyAccess( createIdentifier( "Object" ), "defineProperty" ), undefined, Array( target, propertyName, createObjectLiteral( properties, undefined, true ) ) )
if (startsOnNewLine) {
...
def visitFunctionDeclaration(node: FunctionDeclaration): FunctionDeclaration = {
 return setOriginalNode( createFunctionDeclaration( undefined, node.modifiers, node.asteriskToken, node.name, undefined, visitNodes( node.parameters, visitor, isParameter ), undefined, transformFunctionBody( node ), node ), node )

}
def transformFunctionLikeToExpression(node: FunctionLikeDeclaration, location: TextRange, name: Identifier): FunctionExpression = {
 val savedContainingNonArrowFunction = enclosingNonArrowFunction
if ((node.kind!==SyntaxKind.ArrowFunction)) {
 (enclosingNonArrowFunction=node)

}
val expression = setOriginalNode( createFunctionExpression( undefined, node.asteriskToken, name, undefined, visitNodes( node.parameters, visitor, isParameter ), undefined, saveStateAndInvoke( node, transformFunctionBody ), location ), node )
...
val equalsGreaterThanToken = (node.asInstanceOf[ArrowFunction]).equalsGreaterThanToken
...
val block = createBlock( createNodeArray( statements, statementsLocation ), node.body, multiLine )
...
def visitExpressionStatement(node: ExpressionStatement): ExpressionStatement = {
 node.expression.kind match {
  case  SyntaxKind.ParenthesizedExpression  =>
return updateStatement( node, visitParenthesizedExpression( node.expression.asInstanceOf[ParenthesizedExpression], false ) )
  case  SyntaxKind.BinaryExpression  =>
return updateStatement( node, visitBinaryExpression( node.expression.asInstanceOf[BinaryExpression], false ) )
  case _ =>
}
return visitEachChild( node, visitor, context )

}
def visitParenthesizedExpression(node: ParenthesizedExpression, needsDestructuringValue: Boolean): ParenthesizedExpression = {
 if (needsDestructuringValue) {
 node.expression.kind match {
  case  SyntaxKind.ParenthesizedExpression  =>
return createParen( visitParenthesizedExpression( node.expression.asInstanceOf[ParenthesizedExpression], true ), node )
  case  SyntaxKind.BinaryExpression  =>
return createParen( visitBinaryExpression( node.expression.asInstanceOf[BinaryExpression], true ), node )
  case _ =>
}
...
def visitBinaryExpression(node: BinaryExpression, needsDestructuringValue: Boolean): Expression = {
...
if (isBindingPattern( decl.name )) {
 (assignment=flattenVariableDestructuringToExpression( decl, hoistVariableDeclaration, undefined, visitor ))

}
else {
 (assignment=createBinary( decl.name.asInstanceOf[Identifier], SyntaxKind.EqualsToken, visitNode( decl.initializer, visitor, isExpression ) ))
...
def visitVariableDeclarationList(node: VariableDeclarationList): VariableDeclarationList = {
 if ((node.flags&NodeFlags.BlockScoped)) {
 enableSubstitutionsForBlockScopedBindings()

}
val declarations = flatten( map( node.declarations, (if ((node.flags&NodeFlags.Let)) visitVariableDeclarationInLetDeclarationList else visitVariableDeclaration) ) )
val declarationList = createVariableDeclarationList( declarations, node )
setOriginalNode( declarationList, node )
setCommentRange( declarationList, node )
if (((node.transformFlags&TransformFlags.ContainsBindingPattern)&&((isBindingPattern( node.declarations(0).name )||isBindingPattern( lastOrUndefined( node.declarations ).name ))))) {
...
def shouldEmitExplicitInitializerForLetDeclaration(node: VariableDeclaration) = {
...
val emittedAsTopLevel = (isBlockScopedContainerTopLevel( enclosingBlockScopeContainer )||((((isCapturedInFunction&&isDeclaredInLoop)&&isBlock( enclosingBlockScopeContainer ))&&isIterationStatement( enclosingBlockScopeContainerParent, false ))))
val emitExplicitInitializer = ((((!emittedAsTopLevel)&&(enclosingBlockScopeContainer.kind!==SyntaxKind.ForInStatement))&&(enclosingBlockScopeContainer.kind!==SyntaxKind.ForOfStatement))&&(((!resolver.isDeclarationWithCollidingName( node ))||(((isDeclaredInLoop&&(!isCapturedInFunction))&&(!isIterationStatement( enclosingBlockScopeContainer, false )))))))
return emitExplicitInitializer

}
def visitVariableDeclarationInLetDeclarationList(node: VariableDeclaration) = {
 val name = node.name
if (isBindingPattern( name )) {
...
def visitVariableDeclaration(node: VariableDeclaration): VisitResult[VariableDeclaration] = {
 if (isBindingPattern( node.name )) {
 val recordTempVariablesInLine = ((!enclosingVariableStatement)||(!hasModifier( enclosingVariableStatement, ModifierFlags.Export )))
return flattenVariableDestructuring( node, undefined, visitor, (if (recordTempVariablesInLine) undefined else hoistVariableDeclaration) )
...
if ((isIterationStatement( node.statement, false )&&shouldConvertIterationStatementBody( node.statement.asInstanceOf[IterationStatement] ))) {
 (result=visitNodes( createNodeArray( Array( node.statement ) ), visitor, isStatement ))
...
def convertForOfToFor(node: ForOfStatement, convertedLoopBodyStatements: Array[Statement]): ForStatement = {
...
val rhsReference = (if ((expression.kind===SyntaxKind.Identifier)) createUniqueName( (expression.asInstanceOf[Identifier]).text ) else createTempVariable( undefined ))
if (isVariableDeclarationList( initializer )) {
 if ((initializer.flags&NodeFlags.BlockScoped)) {
 enableSubstitutionsForBlockScopedBindings()

}
val firstOriginalDeclaration = firstOrUndefined( initializer.declarations )
if ((firstOriginalDeclaration&&isBindingPattern( firstOriginalDeclaration.name ))) {
 val declarations = flattenVariableDestructuring( firstOriginalDeclaration, createElementAccess( rhsReference, counter ), visitor )
val declarationList = createVariableDeclarationList( declarations, initializer )
...
statements.push( createVariableStatement( undefined, declarationList ) )
 ...
 statements.push( createVariableStatement( undefined, createVariableDeclarationList( Array( createVariableDeclaration( (if (firstOriginalDeclaration) firstOriginalDeclaration.name else createTempVariable( undefined )), undefined, createElementAccess( rhsReference, counter ) ) ), moveRangePos( initializer, (-1) ) ), moveRangeEnd( initializer, (-1) ) ) )

}

}
else {
 val assignment = createAssignment( initializer, createElementAccess( rhsReference, counter ) )
if (isDestructuringAssignment( assignment )) {
 statements.push( createStatement( flattenDestructuringAssignment( context, assignment, false, hoistVariableDeclaration, visitor ) ) )

}
else {
 ((assignment.asInstanceOf[BinaryExpression]).end=initializer.end)
statements.push( createStatement( assignment, moveRangeEnd( initializer, (-1) ) ) )
...
val body = createBlock( createNodeArray( statements, statementsLocation ), bodyLocation )
setEmitFlags( body, (EmitFlags.NoSourceMap|EmitFlags.NoTokenSourceMaps) )
val forStatement = createFor( createVariableDeclarationList( Array( createVariableDeclaration( counter, undefined, createLiteral( 0 ), moveRangePos( node.expression, (-1) ) ), createVariableDeclaration( rhsReference, undefined, expression, node.expression ) ), node.expression ), createLessThan( counter, createPropertyAccess( rhsReference, "length" ), node.expression ), createPostfixIncrement( counter, node.expression ), body, node )
setEmitFlags( forStatement, EmitFlags.NoTokenTrailingSourceMaps )
return forStatement

}
def visitObjectLiteralExpression(node: ObjectLiteralExpression): Expression = {
...
val assignment = createAssignment( temp, setEmitFlags( createObjectLiteral( visitNodes( properties, visitor, isObjectLiteralElementLike, 0, numInitialProperties ), undefined, node.multiLine ), EmitFlags.Indented ) )
if (node.multiLine) {
...
expressions.push( (if (node.multiLine) startOnNewLine( getMutableClone( temp ) ) else temp) )
return inlineExpressions( expressions )

}
def shouldConvertIterationStatementBody(node: IterationStatement): Boolean = {
 return (((resolver.getNodeCheckFlags( node )&NodeCheckFlags.LoopWithCapturedBlockScopedBinding))!==0)

}
def hoistVariableDeclarationDeclaredInConvertedLoop(state: ConvertedLoopState, node: VariableDeclaration): Unit = {
...
def convertIterationStatementBodyIfNecessary(node: IterationStatement, convert: ((IterationStatement, Array[Statement]) => IterationStatement)): VisitResult[Statement] = {
...
val result = (if (convert) convert( node, undefined ) else visitEachChild( node, visitor, context ))
if (convertedLoopState) {
...
node.kind match {
  case  SyntaxKind.ForStatement | SyntaxKind.ForInStatement | SyntaxKind.ForOfStatement  =>
val initializer = (node.asInstanceOf[( ForStatement | ForInStatement | ForOfStatement )]).initializer
if ((initializer&&(initializer.kind===SyntaxKind.VariableDeclarationList))) {
 (loopInitializer=initializer.asInstanceOf[VariableDeclarationList])
...
if (loopOutParameters.length) {
 val statements = (if (isBlock( loopBody )) (loopBody.asInstanceOf[Block]).statements.slice() else Array( loopBody ))
...
val isAsyncBlockContainingAwait = ((enclosingNonArrowFunction&&(((getEmitFlags( enclosingNonArrowFunction )&EmitFlags.AsyncFunctionBody))!==0))&&(((node.statement.transformFlags&TransformFlags.ContainsYield))!==0))
...
val convertedLoopVariable = createVariableStatement( undefined, createVariableDeclarationList( Array( createVariableDeclaration( functionName, undefined, setEmitFlags( createFunctionExpression( undefined, (if (isAsyncBlockContainingAwait) createToken( SyntaxKind.AsteriskToken ) else undefined), undefined, undefined, loopParameters, undefined, loopBody.asInstanceOf[Block] ), loopBodyFlags ) ) ) ) )
...
if (currentState.argumentsName) {
 ...
 ((extraVariableDeclarations||((extraVariableDeclarations=Array())))).push( createVariableDeclaration( currentState.argumentsName, undefined, createIdentifier( "arguments" ) ) )
...
if (currentState.thisName) {
 ...
 ((extraVariableDeclarations||((extraVariableDeclarations=Array())))).push( createVariableDeclaration( currentState.thisName, undefined, createIdentifier( "this" ) ) )
...
val convertedLoopBodyStatements = generateCallToConvertedLoop( functionName, loopParameters, currentState, isAsyncBlockContainingAwait )
...
(loop.statement=createBlock( convertedLoopBodyStatements, undefined, true ))
(loop.transformFlags=0)
aggregateTransformFlags( loop )

}
statements.push( (if ((currentParent.kind===SyntaxKind.LabeledStatement)) createLabel( (currentParent.asInstanceOf[LabeledStatement]).label, loop ) else loop) )
return statements

}
def copyOutParameter(outParam: LoopOutParameter, copyDirection: CopyDirection): BinaryExpression = {
 val source = (if ((copyDirection===CopyDirection.ToOriginal)) outParam.outParamName else outParam.originalName)
val target = (if ((copyDirection===CopyDirection.ToOriginal)) outParam.originalName else outParam.outParamName)
return createBinary( target, SyntaxKind.EqualsToken, source )

}
def copyOutParameters(outParams: Array[LoopOutParameter], copyDirection: CopyDirection, statements: Array[Statement]): Unit = {
...
def generateCallToConvertedLoop(loopFunctionExpressionName: Identifier, parameters: Array[ParameterDeclaration], state: ConvertedLoopState, isAsyncBlockContainingAwait: Boolean): Array[Statement] = {
 val outerConvertedLoopState = convertedLoopState
val statements: Array[Statement] = Array()
val isSimpleLoop = (((!((state.nonLocalJumps&(~Jump.Continue))))&&(!state.labeledNonLocalBreaks))&&(!state.labeledNonLocalContinues))
val call = createCall( loopFunctionExpressionName, undefined, map( parameters, (p =>  p.name.asInstanceOf[Identifier]) ) )
val callResult = (if (isAsyncBlockContainingAwait) createYield( createToken( SyntaxKind.AsteriskToken ), call ) else call)
if (isSimpleLoop) {
...
val stateVariable = createVariableStatement( undefined, createVariableDeclarationList( Array( createVariableDeclaration( loopResultName, undefined, callResult ) ) ) )
...
def processLabeledJumps(table: Map[String], isBreak: Boolean, loopResultName: Identifier, outerLoop: ConvertedLoopState, caseClauses: Array[CaseClause]): Unit = {
 if ((!table)) {
 return

}
(table).keys.foreach { fresh12 =>
val labelText = zeroOfMyType
 = fresh12
 {
...
statements.push( (if (isBreak) createBreak( label ) else createContinue( label )) )
...
def processLoopVariableDeclaration(decl: ( VariableDeclaration | BindingElement ), loopParameters: Array[ParameterDeclaration], loopOutParameters: Array[LoopOutParameter]) = {
 val name = decl.name
if (isBindingPattern( name )) {
...
loopOutParameters.push( Map( "originalName" -> name,
...
def addObjectLiteralMembers(expressions: Array[Expression], node: ObjectLiteralExpression, receiver: Identifier, start: Int) = {
 val properties = node.properties
val numProperties = properties.length
{
var i = start
...
property.kind match {
  case  SyntaxKind.GetAccessor | SyntaxKind.SetAccessor  =>
val accessors = getAllAccessorDeclarations( node.properties, property.asInstanceOf[AccessorDeclaration] )
if ((property===accessors.firstAccessor)) {
 expressions.push( transformAccessorsToExpression( receiver, accessors, node.multiLine ) )

}
  case  SyntaxKind.PropertyAssignment  =>
expressions.push( transformPropertyAssignmentToExpression( property.asInstanceOf[PropertyAssignment], receiver, node.multiLine ) )
  case  SyntaxKind.ShorthandPropertyAssignment  =>
expressions.push( transformShorthandPropertyAssignmentToExpression( property.asInstanceOf[ShorthandPropertyAssignment], receiver, node.multiLine ) )
  case  SyntaxKind.MethodDeclaration  =>
expressions.push( transformObjectLiteralMethodDeclarationToExpression( property.asInstanceOf[MethodDeclaration], receiver, node.multiLine ) )
  case _ =>
Debug.failBadSyntaxKind( node )
}
...
def transformPropertyAssignmentToExpression(property: PropertyAssignment, receiver: Expression, startsOnNewLine: Boolean) = {
 val expression = createAssignment( createMemberAccessForPropertyName( receiver, visitNode( property.name, visitor, isPropertyName ) ), visitNode( property.initializer, visitor, isExpression ), property )
if (startsOnNewLine) {
...
def transformShorthandPropertyAssignmentToExpression(property: ShorthandPropertyAssignment, receiver: Expression, startsOnNewLine: Boolean) = {
 val expression = createAssignment( createMemberAccessForPropertyName( receiver, visitNode( property.name, visitor, isPropertyName ) ), getSynthesizedClone( property.name ), property )
if (startsOnNewLine) {
 (expression.startsOnNewLine=true)
...
def transformObjectLiteralMethodDeclarationToExpression(method: MethodDeclaration, receiver: Expression, startsOnNewLine: Boolean) = {
 val expression = createAssignment( createMemberAccessForPropertyName( receiver, visitNode( method.name, visitor, isPropertyName ) ), transformFunctionLikeToExpression( method, method, undefined ), method )
if (startsOnNewLine) {
 (expression.startsOnNewLine=true)
...
val newVariableDeclaration = createVariableDeclaration( temp, undefined, undefined, node.variableDeclaration )
val vars = flattenVariableDestructuring( node.variableDeclaration, temp, visitor )
val list = createVariableDeclarationList( vars, node.variableDeclaration, node.variableDeclaration.flags )
...
def addStatementToStartOfBlock(block: Block, statement: Statement): Block = {
 val transformedStatements = visitNodes( block.statements, visitor, isStatement )
return updateBlock( block, Array( statement ).concat( transformedStatements ) )

}
def visitMethodDeclaration(node: MethodDeclaration): ObjectLiteralElementLike = {
 Debug.assert( (!isComputedPropertyName( node.name )) )
val functionExpression = transformFunctionLikeToExpression( node, moveRangePos( node, (-1) ), undefined )
...
def visitShorthandPropertyAssignment(node: ShorthandPropertyAssignment): ObjectLiteralElementLike = {
 return createPropertyAssignment( node.name, getSynthesizedClone( node.name ), node )
...
def visitCallExpressionWithPotentialCapturedThisAssignment(node: CallExpression, assignToCapturedThis: Boolean): ( CallExpression | BinaryExpression ) = {
 const fresh14 = createCallBinding( node.expression, hoistVariableDeclaration )
...
if ((node.transformFlags&TransformFlags.ContainsSpreadElementExpression)) {
 (resultingCall=createFunctionApply( visitNode( target, visitor, isExpression ), visitNode( thisArg, visitor, isExpression ), transformAndSpreadElements( node.arguments, false, false, false ) ))

}
else {
 (resultingCall=createFunctionCall( visitNode( target, visitor, isExpression ), visitNode( thisArg, visitor, isExpression ), visitNodes( node.arguments, visitor, isExpression ), node ))
...
return (if (assignToCapturedThis) createAssignment( createIdentifier( "_this" ), initializer ) else initializer)
...
const fresh15 = createCallBinding( createPropertyAccess( node.expression, "bind" ), hoistVariableDeclaration )
val target = fresh15.target
val thisArg = fresh15.thisArg
return createNew( createFunctionApply( visitNode( target, visitor, isExpression ), thisArg, transformAndSpreadElements( createNodeArray( Array( createVoidZero(), node.arguments: _* ) ), false, false, false ) ), undefined, Array() )

}
def transformAndSpreadElements(elements: NodeArray[Expression], needsUniqueCopy: Boolean, multiLine: Boolean, hasTrailingComma: Boolean): Expression = {
 val numElements = elements.length
val segments = flatten( spanMap( elements, partitionSpreadElement, (( partition, visitPartition, _start, end ) =>  visitPartition( partition, multiLine, (hasTrailingComma&&(end===numElements)) )) ) )
if ((segments.length===1)) {
 val firstElement = elements(0)
return (if (((needsUniqueCopy&&isSpreadElementExpression( firstElement ))&&(firstElement.expression.kind!==SyntaxKind.ArrayLiteralExpression))) createArraySlice( segments(0) ) else segments(0))
...
def partitionSpreadElement(node: Expression) = {
 return (if (isSpreadElementExpression( node )) visitSpanOfSpreadElements else visitSpanOfNonSpreadElements)

}
def visitSpanOfSpreadElements(chunk: Array[Expression]): VisitResult[Expression] = {
 return map( chunk, visitExpressionOfSpreadElement )

}
def visitSpanOfNonSpreadElements(chunk: Array[Expression], multiLine: Boolean, hasTrailingComma: Boolean): VisitResult[Expression] = {
 return createArrayLiteral( visitNodes( createNodeArray( chunk, undefined, hasTrailingComma ), visitor, isExpression ), undefined, multiLine )
...
return createParen( inlineExpressions( Array( createAssignment( temp, createArrayLiteral( cookedStrings ) ), createAssignment( createPropertyAccess( temp, "raw" ), createArrayLiteral( rawStrings ) ), createCall( tag, undefined, templateArguments ) ) ) )
...
(text=text.replace( java.util.regex.Pattern.compile(raw"""\r\n?""", "g"), "\n" ))
...
def addTemplateHead(expressions: Array[Expression], node: TemplateExpression): Unit = {
...
def addTemplateSpans(expressions: Array[Expression], node: TemplateExpression): Unit = {
 (node.templateSpans).foreach { fresh17 =>
val span = zeroOfMyType
 = fresh17
 {
...
def visitSuperKeyword(): LeftHandSideExpression = {
 return (if ((((enclosingNonAsyncFunctionBody&&isClassElement( enclosingNonAsyncFunctionBody ))&&(!hasModifier( enclosingNonAsyncFunctionBody, ModifierFlags.Static )))&&(currentParent.kind!==SyntaxKind.CallExpression))) createPropertyAccess( createIdentifier( "_super" ), "prototype" ) else createIdentifier( "_super" ))
...
def onEmitNode(emitContext: EmitContext, node: Node, emitCallback: ((EmitContext, Node) => Unit)) = {
 val savedEnclosingFunction = enclosingFunction
...
parent.kind match {
  case  SyntaxKind.BindingElement | SyntaxKind.ClassDeclaration | SyntaxKind.EnumDeclaration | SyntaxKind.VariableDeclaration  =>
return (((parent.asInstanceOf[Declaration]).name===node)&&resolver.isDeclarationWithCollidingName( parent.asInstanceOf[Declaration] ))
  case _ =>
}
return false
...
def substituteThisKeyword(node: PrimaryExpression): PrimaryExpression = {
 if ((((enabledSubstitutions&ES2015SubstitutionFlags.CapturedThis)&&enclosingFunction)&&(getEmitFlags( enclosingFunction )&EmitFlags.CapturesThis))) {
...
def getLocalName(node: ( ClassDeclaration | ClassExpression | FunctionDeclaration ), allowComments: Boolean, allowSourceMaps: Boolean) = {
 return getDeclarationName( node, allowComments, allowSourceMaps, EmitFlags.LocalName )

}
def getDeclarationName(node: ( ClassDeclaration | ClassExpression | FunctionDeclaration ), allowComments: Boolean, allowSourceMaps: Boolean, emitFlags: EmitFlags) = {
...
def getClassMemberPrefix(node: ( ClassExpression | ClassDeclaration ), member: ClassElement) = {
 val expression = getLocalName( node )
return (if (hasModifier( member, ModifierFlags.Static )) expression else createPropertyAccess( expression, "prototype" ))

}
def hasSynthesizedDefaultSuperCall(`constructor`: ConstructorDeclaration, hasExtendsClause: Boolean) = {
...
val statementExpression = (statement.asInstanceOf[ExpressionStatement]).expression
if (((!nodeIsSynthesized( statementExpression ))||(statementExpression.kind!==SyntaxKind.CallExpression))) {
 return false

}
val callTarget = (statementExpression.asInstanceOf[CallExpression]).expression
if (((!nodeIsSynthesized( callTarget ))||(callTarget.kind!==SyntaxKind.SuperKeyword))) {
 return false

}
val callArgument = singleOrUndefined( (statementExpression.asInstanceOf[CallExpression]).arguments )
if ((((!callArgument)||(!nodeIsSynthesized( callArgument )))||(callArgument.kind!==SyntaxKind.SpreadElementExpression))) {
 return false

}
val expression = (callArgument.asInstanceOf[SpreadElementExpression]).expression
