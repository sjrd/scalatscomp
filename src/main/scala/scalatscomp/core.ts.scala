...
val collator: {   def compare(a: String, b: String): Int
 } = (if (((typeof(Intl)==="object")&&(typeof(Intl.Collator)==="function"))) new Intl.Collator() else undefined)
...
def toPath(fileName: String, basePath: String, getCanonicalFileName: ((String) => String)): Path = {
 val nonCanonicalizedPath = (if (isRootedDiskPath( fileName )) normalizePath( fileName ) else getNormalizedAbsolutePath( fileName, basePath ))
...
def forEach[T, U](array: ( Array[T] | undefined ), callback: ((T, Int) => ( U | undefined ))): ( U | undefined ) = {
 if (array) {
 {
var i = 0
...
def find[T](array: Array[T], predicate: ((T, Int) => Boolean)): ( T | undefined ) = {
 {
var i = 0
var len = array.length
while( (i<len)) {
...
def findMap[T, U](array: Array[T], callback: ((T, Int) => ( U | undefined ))): U = {
 {
var i = 0
var len = array.length
while( (i<len)) {
...
def indexOfAnyCharCode(text: String, charCodes: Array[Int], start: Int): Int = {
 {
var i = (start||0)
var len = text.length
while( (i<len)) {
...
def flatMap[T, U](array: Array[T], mapfn: ((T, Int) => ( U | Array[U] ))): Array[U] = {
 var result: Array[U] = zeroOfMyType
if (array) {
 (result=Array())
{
...
def span[T](array: Array[T], f: ((T, Int) => Boolean)): (Array[T], Array[T]) = {
 if (array) {
 {
var i = 0
while( (i<array.length)) {
...
def spanMap[T, K, U](array: Array[T], keyfn: ((T, Int) => K), mapfn: ((Array[T], K, Int, Int) => U)): Array[U] = {
 var result: Array[U] = zeroOfMyType
if (array) {
 (result=Array())
val len = array.length
...
def mapObject[T, U](`object`: MapLike[T], f: ((String, T) => (String, U))): MapLike[U] = {
...
val v = zeroOfMyType
 = fresh8
 {
 val keyvalue: (String, U) = (f( v, `object`(v) )||Array( undefined, undefined ))
val key = keyvalue(0)
...
def deduplicate[T](array: Array[T], areEqual: ((T, T) => Boolean)): Array[T] = {
 var result: Array[T] = zeroOfMyType
if (array) {
 (result=Array())
...
def arrayIsEqualTo[T](array1: ReadonlyArray[T], array2: ReadonlyArray[T], equaler: ((T, T) => Boolean)): Boolean = {
 if (((!array1)||(!array2))) {
...
while( (i<array1.length)) {
 {
 val equals = (if (equaler) equaler( array1(i), array2(i) ) else (array1(i)===array2(i)))
if ((!equals)) {
 return false
...
def changesAffectModuleResolution(oldOptions: CompilerOptions, newOptions: CompilerOptions): Boolean = {
...
def lastOrUndefined[T](array: Array[T]): T = {
 return (if ((array&&(array.length>0))) array((array.length-1)) else undefined)
...
def binarySearch[T](array: Array[T], value: T, comparer: ((T, T) => Int)): Int = {
...
(comparer=(if ((comparer!==undefined)) comparer else (( v1, v2 ) =>  ((if ((v1<v2)) (-1) else ((if ((v1>v2)) 1 else 0)))))))
while ((low<=high)) {
{
...
def reduceLeft[T, U](array: Array[T], f: ((U, T, Int) => U), initial: U, start: Int, count: Int): U
def reduceLeft[T](array: Array[T], f: ((T, T, Int) => T)): T
def reduceLeft[T](array: Array[T], f: ((T, T, Int) => T), initial: T, start: Int, count: Int): T = {
 if ((array&&(array.length>0))) {
 val size = array.length
if ((size>0)) {
 var pos = (if (((start===undefined)||(start<0))) 0 else start)
val end = (if (((count===undefined)||((pos+count)>(size-1)))) (size-1) else (pos+count))
...
def reduceRight[T, U](array: Array[T], f: ((U, T, Int) => U), initial: U, start: Int, count: Int): U
def reduceRight[T](array: Array[T], f: ((T, T, Int) => T)): T
def reduceRight[T](array: Array[T], f: ((T, T, Int) => T), initial: T, start: Int, count: Int): T = {
 if (array) {
 val size = array.length
if ((size>0)) {
 var pos = (if (((start===undefined)||(start>(size-1)))) (size-1) else start)
val end = (if (((count===undefined)||((pos-count)<0))) 0 else (pos-count))
var result: T = zeroOfMyType
if ((arguments.length<=2)) {
 (result=array(pos))
(pos-= 1)
...
def assign[T1 <: MapLike[{
}], T2, T3](t: T1, arg1: T2, arg2: T3): ( T1 with T2 with T3 )
def assign[T1 <: MapLike[{
}], T2](t: T1, arg1: T2): ( T1 with T2 )
def assign[T1 <: MapLike[{
}]](t: T1, args: Array[Any]): Any
def assign[T1 <: MapLike[{
}]](t: T1, args: Array[Any]) = {
...
def reduceProperties[T, U](map: Map[T], callback: ((U, T, String) => U), initial: U): U = {
 var result = initial
...
def reduceOwnProperties[T, U](map: MapLike[T], callback: ((U, T, String) => U), initial: U): U = {
 var result = initial
...
def equalOwnProperties[T](left: MapLike[T], right: MapLike[T], equalityComparer: ((T, T) => Boolean)) = {
 if ((left===right))
return true
if (((!left)||(!right)))
return false
...
return false
if ((if (equalityComparer) (!equalityComparer( left(key), right(key) )) else (left(key)!==right(key))))
return false
...
def arrayToMap[T, U](array: Array[T], makeKey: ((T) => String), makeValue: ((T) => U)): Map[U]
def arrayToMap[T, U](array: Array[T], makeKey: ((T) => String), makeValue: ((T) => U)): Map[( T | U )] = {
 val result = createMap[ ( T | U ) ]()
(array).foreach { fresh24 =>
val value = zeroOfMyType
 = fresh24
 {
...
def isArray(value: Any): Boolean = {
 return (if (Array.isArray) Array.isArray( value ) else (valueinstanceofArray))
...
def chain[T, U](a: ((T) => ((U) => U)), b: ((T) => ((U) => U)), c: ((T) => ((U) => U)), d: ((T) => ((U) => U)), e: ((T) => ((U) => U))): ((T) => ((U) => U)) = {
 if (e) {
...
def compose[T](a: ((T) => T), b: ((T) => T), c: ((T) => T), d: ((T) => T), e: ((T) => T)): ((T) => T) = {
 if (e) {
...
def formatStringFromArgs(text: String, args: {   def apply(index: Int): String
  /* def update() -- if you need it */
 }, baseIndex: Int): String = {
 (baseIndex=(baseIndex||0))
return text.replace( java.util.regex.Pattern.compile(raw"""{(\d+)}""", "g"), (( _match, index ) =>  args(((+index)+baseIndex))) )
...
def createFileDiagnostic(file: SourceFile, start: Int, length: Int, message: DiagnosticMessage, args: Array[( String | Int )]): Diagnostic
...
if (file) {
 Debug.assert( (start<=file.text.length), s"""start must be within the bounds of the file. ${ start} > ${ file.text.length}"""  )
Debug.assert( (end<=file.text.length), s"""end must be the bounds of the file. ${ end} > ${ file.text.length}"""  )
...
def createCompilerDiagnostic(message: DiagnosticMessage, args: Array[( String | Int )]): Diagnostic
...
def createCompilerDiagnosticFromMessageChain(chain: DiagnosticMessageChain): Diagnostic = {
 return Map( "file" -> undefined,
"start" -> undefined,
"length" -> undefined,
"code" -> chain.code,
...
if (ignoreCase) {
 ...
 val result = a.localeCompare( b, undefined, Map( "usage" -> "sort",
"sensitivity" -> "accent" ) )
return (if ((result<0)) Comparison.LessThan else (if ((result>0)) Comparison.GreaterThan else Comparison.EqualTo))
...
def compareDiagnostics(d1: Diagnostic, d2: Diagnostic): Comparison = {
 return (((((compareValues( getDiagnosticFileName( d1 ), getDiagnosticFileName( d2 ) )||compareValues( d1.start, d2.start ))||compareValues( d1.length, d2.length ))||compareValues( d1.code, d2.code ))||compareMessageText( d1.messageText, d2.messageText ))||Comparison.EqualTo)

}
def compareMessageText(text1: ( String | DiagnosticMessageChain ), text2: ( String | DiagnosticMessageChain )): Comparison = {
 while ((text1&&text2)) {
{
 val string1 = (if ((typeof(text1)==="string")) text1 else text1.messageText)
val string2 = (if ((typeof(text2)==="string")) text2 else text2.messageText)
...
def getNormalizedParts(normalizedSlashedPath: String, rootLength: Int): Array[String] = {
...
return (if (pathEndsWithDirectorySeparator( path )) (joinedParts+directorySeparator) else joinedParts)
...
def isExternalModuleNameRelative(moduleName: String): Boolean = {
 return java.util.regex.Pattern.compile(raw"""^\.\.?($$|[\\/])""").test( moduleName )
...
def getEmitModuleKind(compilerOptions: CompilerOptions) = {
 return (if ((typeof(compilerOptions.module)==="number")) compilerOptions.module else (if ((getEmitScriptTarget( compilerOptions )>=ScriptTarget.ES2015)) ModuleKind.ES2015 else ModuleKind.CommonJS))
...
def convertToRelativePath(absoluteOrRelativePath: String, basePath: String, getCanonicalFileName: ((String) => String)): String = {
 return (if ((!isRootedDiskPath( absoluteOrRelativePath ))) absoluteOrRelativePath else getRelativePathToDirectoryOrUrl( basePath, absoluteOrRelativePath, basePath, getCanonicalFileName, false ))
...
def getRelativePathToDirectoryOrUrl(directoryPathOrUrl: String, relativeOrAbsolutePath: String, currentDirectory: String, getCanonicalFileName: ((String) => String), isAbsolutePathAnUrl: Boolean) = {
...
def comparePaths(a: String, b: String, currentDirectory: String, ignoreCase: Boolean) = {
 if ((a===b))
return Comparison.EqualTo
if ((a===undefined))
return Comparison.LessThan
...
while( (i<sharedLength)) {
 {
 val result = compareStrings( aComponents(i), bComponents(i), ignoreCase )
...
def containsPath(parent: String, child: String, currentDirectory: String, ignoreCase: Boolean) = {
...
if ((parent===child))
return true
val parentComponents = getNormalizedPathComponents( parent, currentDirectory )
...
while( (i<parentComponents.length)) {
 {
 val result = compareStrings( parentComponents(i), childComponents(i), ignoreCase )
if ((result!==Comparison.EqualTo)) {
 return false
...
val reservedCharacterPattern = java.util.regex.Pattern.compile(raw"""[^\w\s\/]""", "g")
val wildcardCharCodes = Array( CharacterCodes.asterisk, CharacterCodes.question )
val singleAsteriskRegexFragmentFiles = "([^./]|(\\.(?!min\\.js$))?)*"
val singleAsteriskRegexFragmentOther = "[^/]*"
def getRegularExpressionForWildcard(specs: Array[String], basePath: String, usage: ( `"files"` | `"directories"` | `"exclude"` )) = {
 if (((specs===undefined)||(specs.length===0))) {
 return undefined

}
val replaceWildcardCharacter = (if ((usage==="files")) replaceWildCardCharacterFiles else replaceWildCardCharacterOther)
val singleAsteriskRegexFragment = (if ((usage==="files")) singleAsteriskRegexFragmentFiles else singleAsteriskRegexFragmentOther)
val doubleAsteriskRegexFragment = (if ((usage==="exclude")) "(/.+?)?" else "(/[^/.][^/]*)*?")
var pattern = ""
...
return (("^("+pattern)+((if ((usage==="exclude")) ")($|/)" else ")$")))
...
def replaceWildcardCharacter(`match`: String, singleAsteriskRegexFragment: String) = {
 return (if ((`match`==="*")) singleAsteriskRegexFragment else (if ((`match`==="?")) "[^/]" else ("\\"+`match`)))
...
def getFileMatcherPatterns(path: String, excludes: Array[String], includes: Array[String], useCaseSensitiveFileNames: Boolean, currentDirectory: String): FileMatcherPatterns = {
 (path=normalizePath( path ))
(currentDirectory=normalizePath( currentDirectory ))
val absolutePath = combinePaths( currentDirectory, path )
return Map( "includeFilePattern" -> getRegularExpressionForWildcard( includes, absolutePath, "files" ),
"includeDirectoryPattern" -> getRegularExpressionForWildcard( includes, absolutePath, "directories" ),
"excludePattern" -> getRegularExpressionForWildcard( excludes, absolutePath, "exclude" ),
"basePaths" -> getBasePaths( path, includes, useCaseSensitiveFileNames ) )

}
def matchFiles(path: String, extensions: Array[String], excludes: Array[String], includes: Array[String], useCaseSensitiveFileNames: Boolean, currentDirectory: String, getFileSystemEntries: ((String) => FileSystemEntries)): Array[String] = {
 (path=normalizePath( path ))
(currentDirectory=normalizePath( currentDirectory ))
val patterns = getFileMatcherPatterns( path, excludes, includes, useCaseSensitiveFileNames, currentDirectory )
val regexFlag = (if (useCaseSensitiveFileNames) "" else "i")
val includeFileRegex = (patterns.includeFilePattern&&new RegExp( patterns.includeFilePattern, regexFlag ))
val includeDirectoryRegex = (patterns.includeDirectoryPattern&&new RegExp( patterns.includeDirectoryPattern, regexFlag ))
val excludeRegex = (patterns.excludePattern&&new RegExp( patterns.excludePattern, regexFlag ))
val result: Array[String] = Array()
(patterns.basePaths).foreach { fresh33 =>
val basePath = zeroOfMyType
 = fresh33
 {
...
if ((((((!extensions)||fileExtensionIsAny( name, extensions )))&&(((!includeFileRegex)||includeFileRegex.test( absoluteName ))))&&(((!excludeRegex)||(!excludeRegex.test( absoluteName )))))) {
 result.push( name )
...
def getBasePaths(path: String, includes: Array[String], useCaseSensitiveFileNames: Boolean) = {
 val basePaths: Array[String] = Array( path )
if (includes) {
 val includeBasePaths: Array[String] = Array()
(includes).foreach { fresh37 =>
val include = zeroOfMyType
 = fresh37
 {
 val absolute: String = (if (isRootedDiskPath( include )) include else normalizePath( combinePaths( path, include ) ))
val wildcardOffset = indexOfAnyCharCode( absolute, wildcardCharCodes )
val includeBasePath = (if ((wildcardOffset<0)) removeTrailingDirectorySeparator( getDirectoryPath( absolute ) ) else absolute.substring( 0, absolute.lastIndexOf( directorySeparator, wildcardOffset ) ))
includeBasePaths.push( includeBasePath )

}
}
includeBasePaths.sort( (if (useCaseSensitiveFileNames) compareStrings else compareStringsCaseInsensitive) )
...
val supportedTypescriptExtensionsForExtractExtension = Array( ".d.ts", ".ts", ".tsx" )
...
def getSupportedExtensions(options: CompilerOptions): Array[String] = {
 return (if ((options&&options.allowJs)) allSupportedExtensions else supportedTypeScriptExtensions)
...
def getExtensionPriority(path: String, supportedExtensions: Array[String]): ExtensionPriority = {
 {
...
def adjustExtensionPriority(extensionPriority: ExtensionPriority): ExtensionPriority = {
...
def getNextLowestExtensionPriority(extensionPriority: ExtensionPriority): ExtensionPriority = {
 if ((extensionPriority<ExtensionPriority.DeclarationAndJavaScriptFiles)) {
 return ExtensionPriority.DeclarationAndJavaScriptFiles
...
def tryRemoveExtension(path: String, extension: String): ( String | undefined ) = {
 return (if (fileExtensionIs( path, extension )) removeExtension( path, extension ) else undefined)
...
trait ObjectAllocator {
  ...
  def getIdentifierConstructor(): ((SyntaxKind.Identifier, Int, Int) => Identifier)
  def getSourceFileConstructor(): ((SyntaxKind.SourceFile, Int, Int) => SourceFile)
...
var objectAllocator: ObjectAllocator = Map( "getNodeConstructor" -> (() =>  Node.asInstanceOf[Any]),
...
def unorderedRemoveFirstItemWhere[T](array: Array[T], predicate: ((T) => Boolean)): Unit = {
 {
var i = 0
while( (i<array.length)) {
 {
...
def createGetCanonicalFileName(useCaseSensitiveFileNames: Boolean): ((String) => String) = {
 return (if (useCaseSensitiveFileNames) ((fileName =>  fileName)) else ((fileName =>  fileName.toLowerCase())))

}
def matchPatternOrExact(patternStrings: Array[String], candidate: String): ( String | Pattern | undefined ) = {
 val patterns: Array[Pattern] = Array()
(patternStrings).foreach { fresh40 =>
val patternString = zeroOfMyType
 = fresh40
 {
...
return findBestPatternMatch( patterns, (_underscore_ =>  _underscore_), candidate )
...
def findBestPatternMatch[T](values: Array[T], getPattern: ((T) => Pattern), candidate: String): ( T | undefined ) = {
...
return (if ((indexOfStar===(-1))) undefined else Map( "prefix" -> pattern.substr( 0, indexOfStar ),
