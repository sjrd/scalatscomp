...
trait CommentWriter {
  ...
  def emitNodeWithComments(emitContext: EmitContext, node: Node, emitCallback: ((EmitContext, Node) => Unit)): Unit
  def emitBodyWithDetachedComments(node: Node, detachedRange: TextRange, emitCallback: ((Node) => Unit)): Unit
  def emitTrailingCommentsOfPosition(pos: Int): Unit
}
def createCommentWriter(host: EmitHost, writer: EmitTextWriter, sourceMap: SourceMapWriter): CommentWriter = {
...
var detachedCommentsInfo: Array[{   var nodePos: Int
...
def emitNodeWithComments(emitContext: EmitContext, node: Node, emitCallback: ((EmitContext, Node) => Unit)) = {
 if (disabled) {
...
def emitBodyWithDetachedComments(node: Node, detachedRange: TextRange, emitCallback: ((Node) => Unit)) = {
 if (extendedDiagnostics) {
...
def emitTripleSlashLeadingComment(commentPos: Int, commentEnd: Int, kind: SyntaxKind, hasTrailingNewLine: Boolean, rangePos: Int) = {
...
def emitLeadingComment(commentPos: Int, commentEnd: Int, _kind: SyntaxKind, hasTrailingNewLine: Boolean, rangePos: Int) = {
...
def emitTrailingComment(commentPos: Int, commentEnd: Int, _kind: SyntaxKind, hasTrailingNewLine: Boolean) = {
...
def emitTrailingCommentOfPosition(commentPos: Int, commentEnd: Int, _kind: SyntaxKind, hasTrailingNewLine: Boolean) = {
 emitPos( commentPos )
writeCommentRange( currentText, currentLineMap, writer, commentPos, commentEnd, newLine )
emitPos( commentEnd )
if (hasTrailingNewLine) {
...
def forEachLeadingCommentToEmit(pos: Int, cb: ((Int, Int, SyntaxKind, Boolean, Int) => Unit)) = {
...
def forEachTrailingCommentToEmit(end: Int, cb: ((Int, Int, SyntaxKind, Boolean) => Unit)) = {
...
def forEachLeadingCommentWithoutDetachedComments(cb: ((Int, Int, SyntaxKind, Boolean, Int) => Unit)) = {
...
def emitDetachedCommentsAndUpdateCommentsInfo(range: TextRange) = {
 val currentDetachedCommentInfo = emitDetachedComments( currentText, currentLineMap, writer, writeComment, range, newLine, disabled )
...
def writeComment(text: String, lineMap: Array[Int], writer: EmitTextWriter, commentPos: Int, commentEnd: Int, newLine: String) = {
 emitPos( commentPos )
...
def isTripleSlashComment(commentPos: Int, commentEnd: Int) = {
 if ((((currentText.charCodeAt( (commentPos+1) )===CharacterCodes.slash)&&((commentPos+2)<commentEnd))&&(currentText.charCodeAt( (commentPos+2) )===CharacterCodes.slash))) {
 val textSubStr = currentText.substring( commentPos, commentEnd )
return (if ((textSubStr.`match`( fullTripleSlashReferencePathRegEx )||textSubStr.`match`( fullTripleSlashAMDReferencePathRegEx ))) true else false)
