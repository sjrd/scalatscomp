package scalatscomp
object Transformer {
val moduleTransformerMap = createMap[ Transformer ]( Map( ModuleKind.ES2015 -> transformES2015Module,
...
trait TransformationResult {
  ...
  def emitNodeWithSubstitution(emitContext: EmitContext, node: Node, emitCallback: ((EmitContext, Node) => Unit)): Unit
  def emitNodeWithNotification(emitContext: EmitContext, node: Node, emitCallback: ((EmitContext, Node) => Unit)): Unit
}
trait TransformationContext extends LexicalEnvironment {
  ...
  var onEmitNode: ((EmitContext, Node, ((EmitContext, Node) => Unit)) => Unit)
}
...
def transformFiles(resolver: EmitResolver, host: EmitHost, sourceFiles: Array[SourceFile], transformers: Array[Transformer]): TransformationResult = {
 val lexicalEnvironmentVariableDeclarationsStack: Array[Array[VariableDeclaration]] = Array()
val lexicalEnvironmentFunctionDeclarationsStack: Array[Array[FunctionDeclaration]] = Array()
val enabledSyntaxKindFeatures = new Array[ SyntaxKindFeatureFlags ]( SyntaxKind.Count )
...
val context: TransformationContext = Map( "getCompilerOptions" -> (() =>  host.getCompilerOptions()),
...
return Map( "transformed" -> transformed,
...
def emitNodeWithSubstitution(emitContext: EmitContext, node: Node, emitCallback: ((EmitContext, Node) => Unit)) = {
 if (node) {
...
def emitNodeWithNotification(emitContext: EmitContext, node: Node, emitCallback: ((EmitContext, Node) => Unit)) = {
 if (node) {
...
if (hoistedVariableDeclarations) {
 val statement = createVariableStatement( undefined, createVariableDeclarationList( hoistedVariableDeclarations ) )
if ((!statements)) {
