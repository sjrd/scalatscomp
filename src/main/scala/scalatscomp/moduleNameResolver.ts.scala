...
def createResolvedModule(resolvedFileName: String, isExternalLibraryImport: Boolean, failedLookupLocations: Array[String]): ResolvedModuleWithFailedLookupLocations = {
 return Map( "resolvedModule" -> (if (resolvedFileName) Map( "resolvedFileName" -> resolvedFileName,
"isExternalLibraryImport" -> isExternalLibraryImport ) else undefined),
...
def tryReadTypesSection(packageJsonPath: String, baseDirectory: String, state: ModuleResolutionState): String = {
...
if ((typeof(typesFile)==="string")) {
 val typesFilePath = normalizePath( combinePaths( baseDirectory, typesFile ) )
...
val typesFilePath = (tryReadFromField( "typings" )||tryReadFromField( "types" ))
if (typesFilePath) {
 return typesFilePath
...
val mainFilePath = normalizePath( combinePaths( baseDirectory, jsonContent.main ) )
return mainFilePath
...
def readJson(path: String, host: ModuleResolutionHost): {   var typings: String
  var types: String
  var main: String
 } = {
 try {
 val jsonText = host.readFile( path )
return (if (jsonText) JSON.parse( jsonText ) else Map(
))
...
def getEffectiveTypeRoots(options: CompilerOptions, host: {   var directoryExists: ((String) => Boolean)
...
def getDefaultTypeRoots(currentDirectory: String, host: {   var directoryExists: ((String) => Boolean)
 }): ( Array[String] | undefined ) = {
...
def resolveTypeReferenceDirective(typeReferenceDirectiveName: String, containingFile: String, options: CompilerOptions, host: ModuleResolutionHost): ResolvedTypeReferenceDirectiveWithFailedLookupLocations = {
 val traceEnabled = isTraceEnabled( options, host )
val moduleResolutionState: ModuleResolutionState = Map( "compilerOptions" -> options,
"host" -> host,
"skipTsx" -> true,
...
val typeRoot = zeroOfMyType
 = fresh1
 {
 val candidate = combinePaths( typeRoot, typeReferenceDirectiveName )
val candidateDirectory = getDirectoryPath( candidate )
val resolvedFile = loadNodeModuleFromDirectory( typeReferenceExtensions, candidate, failedLookupLocations, (!directoryProbablyExists( candidateDirectory, host )), moduleResolutionState )
if (resolvedFile) {
 if (traceEnabled) {
...
return Map( "resolvedTypeReferenceDirective" -> Map( "primary" -> true,
...
return Map( "resolvedTypeReferenceDirective" -> (if (resolvedFile) Map( "primary" -> false,
"resolvedFileName" -> resolvedFile ) else undefined),
"failedLookupLocations" -> failedLookupLocations )

}
def getAutomaticTypeDirectiveNames(options: CompilerOptions, host: ModuleResolutionHost): Array[String] = {
 if (options.types) {
...
val root = zeroOfMyType
 = fresh2
 {
 if (host.directoryExists( root )) {
 (host.getDirectories( root )).foreach { fresh3 =>
val typeDirectivePath = zeroOfMyType
 = fresh3
 {
 val normalized = normalizePath( typeDirectivePath )
val packageJsonPath = pathToPackageJson( combinePaths( root, normalized ) )
val isNotNeededPackage = (host.fileExists( packageJsonPath )&&(readJson( packageJsonPath, host ).typings===null))
...
if ((moduleResolution===undefined)) {
 (moduleResolution=(if ((getEmitModuleKind( compilerOptions )===ModuleKind.CommonJS)) ModuleResolutionKind.NodeJs else ModuleResolutionKind.Classic))
if (traceEnabled) {
...
type ResolutionKindSpecificLoader = ((String, Array[String], Array[String], Boolean, ModuleResolutionState) => String)
def tryLoadModuleUsingOptionalResolutionSettings(moduleName: String, containingDirectory: String, loader: ResolutionKindSpecificLoader, failedLookupLocations: Array[String], supportedExtensions: Array[String], state: ModuleResolutionState): String = {
...
def tryLoadModuleUsingRootDirs(moduleName: String, containingDirectory: String, loader: ResolutionKindSpecificLoader, failedLookupLocations: Array[String], supportedExtensions: Array[String], state: ModuleResolutionState): String = {
...
def tryLoadModuleUsingBaseUrl(moduleName: String, loader: ResolutionKindSpecificLoader, failedLookupLocations: Array[String], supportedExtensions: Array[String], state: ModuleResolutionState): String = {
...
if (matchedPattern) {
 val matchedStar = (if ((typeof(matchedPattern)==="string")) undefined else matchedText( matchedPattern, moduleName ))
val matchedPatternText = (if ((typeof(matchedPattern)==="string")) matchedPattern else patternText( matchedPattern ))
if (state.traceEnabled) {
...
val subst = zeroOfMyType
 = fresh6
 {
 val path = (if (matchedStar) subst.replace( "*", matchedStar ) else subst)
...
val traceEnabled = isTraceEnabled( compilerOptions, host )
val failedLookupLocations: Array[String] = Array()
val state = Map( "compilerOptions" -> compilerOptions,
"host" -> host,
"traceEnabled" -> traceEnabled,
"skipTsx" -> false )
...
def nodeLoadModuleByRelativeName(candidate: String, supportedExtensions: Array[String], failedLookupLocations: Array[String], onlyRecordFailures: Boolean, state: ModuleResolutionState): String = {
 if (state.traceEnabled) {
 trace( state.host, Diagnostics.Loading_module_as_file_Slash_folder_candidate_module_location_0, candidate )

}
val resolvedFileName = ((!pathEndsWithDirectorySeparator( candidate ))&&loadModuleFromFile( candidate, supportedExtensions, failedLookupLocations, onlyRecordFailures, state ))
...
def directoryProbablyExists(directoryName: String, host: {   var directoryExists: ((String) => Boolean)
 }): Boolean = {
 return ((!host.directoryExists)||host.directoryExists( directoryName ))

}
def loadModuleFromFile(candidate: String, extensions: Array[String], failedLookupLocation: Array[String], onlyRecordFailures: Boolean, state: ModuleResolutionState): ( String | undefined ) = {
 val resolvedByAddingExtension = tryAddingExtensions( candidate, extensions, failedLookupLocation, onlyRecordFailures, state )
...
def tryAddingExtensions(candidate: String, extensions: Array[String], failedLookupLocation: Array[String], onlyRecordFailures: Boolean, state: ModuleResolutionState): ( String | undefined ) = {
...
return forEach( extensions, (ext =>  ((!((state.skipTsx&&isJsxOrTsxExtension( ext ))))&&tryFile( (candidate+ext), failedLookupLocation, onlyRecordFailures, state ))) )

}
def tryFile(fileName: String, failedLookupLocation: Array[String], onlyRecordFailures: Boolean, state: ModuleResolutionState): ( String | undefined ) = {
...
if (typesFile) {
 val onlyRecordFailures = (!directoryProbablyExists( getDirectoryPath( typesFile ), state.host ))
val result = (tryFile( typesFile, failedLookupLocation, onlyRecordFailures, state )||tryAddingExtensions( typesFile, extensions, failedLookupLocation, onlyRecordFailures, state ))
if (result) {
 return result
...
def loadModuleFromNodeModulesFolder(moduleName: String, directory: String, failedLookupLocations: Array[String], state: ModuleResolutionState): String = {
...
def loadModuleFromNodeModules(moduleName: String, directory: String, failedLookupLocations: Array[String], state: ModuleResolutionState, checkOneLevel: Boolean): String = {
 return loadModuleFromNodeModulesWorker( moduleName, directory, failedLookupLocations, state, checkOneLevel, false )

}
def loadModuleFromNodeModulesAtTypes(moduleName: String, directory: String, failedLookupLocations: Array[String], state: ModuleResolutionState): String = {
 return loadModuleFromNodeModulesWorker( moduleName, directory, failedLookupLocations, state, false, true )

}
def loadModuleFromNodeModulesWorker(moduleName: String, directory: String, failedLookupLocations: Array[String], state: ModuleResolutionState, checkOneLevel: Boolean, typesOnly: Boolean): String = {
...
val typesResult = loadModuleFromNodeModulesFolder( combinePaths( "@types", moduleName ), directory, failedLookupLocations, state )
...
return (if (referencedSourceFile) Map( "resolvedModule" -> Map( "resolvedFileName" -> referencedSourceFile ),
"failedLookupLocations" -> failedLookupLocations ) else Map( "resolvedModule" -> undefined,
"failedLookupLocations" -> failedLookupLocations ))

}
def loadModuleFromAncestorDirectories(moduleName: String, containingDirectory: String, supportedExtensions: Array[String], failedLookupLocations: Array[String], state: ModuleResolutionState): ( String | undefined ) = {
 while (true) {
{
 val searchName = normalizePath( combinePaths( containingDirectory, moduleName ) )
val referencedSourceFile = loadModuleFromFile( searchName, supportedExtensions, failedLookupLocations, false, state )
